...
class _modulebrowser:
	...
	def __init__(self):
		...
		self.window.browsebutton = W.Button((100, 8, 80, 16), "Browseä", self.browsebuttonhit)
		self.window.reloadbutton = W.Button((10, 32, 80, 16), "Reload", self.reloadbuttonhit)
		self.window.openotherbutton = W.Button((100, 32, 80, 16), "Open otherä", self.openother)
		...
		for i in sel:
			...
			try:
				...
				try:
					...
					W.SetCursor("arrow")
					W.Message("Canπt find file for module ≥%s≤." 
							% modname)
				else:
	...
	def openscript(self, path, modname):
		if path[-3:] == '.py':
			...
			W.Message("Canπt edit ≥%s≤; it might be a shared library or a .pyc file." 
					% modname)
		...
		if modname:
			try:
				...
				if modname in sys.builtin_module_names:
					alerttext = "≥%s≤ is a builtin module, which you canπt edit." % modname
				else:
