...
of positional parameters (including positional-only parameters). The value of
*co_posonlyargcount* can be used to distinguish which parameters are
positional-only, and the difference (*co_argcount* - *co_posonlyargcount*)
is the number of positional-or-keyword parameters. Patch by Pablo Galindo.
...
PDB command ``args`` now also displays values of positional-only parameters. Patch contributed
by Rémi Lapeyre.
...
PDB command ``args`` now also displays values of keyword-only parameters. Patch contributed by
Rémi Lapeyre.
