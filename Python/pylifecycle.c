...
static void emit_stderr_warning_for_legacy_locale(_PyRuntimeState *runtime) {
...
int _Py_IsLocaleCoercionTarget(const char *ctype_loc) {
...
int _Py_CoerceLegacyLocale(int warn) {
  ...
  if (locale_override == NULL || *locale_override == '\0') {
    ...
    for (target = _TARGET_LOCALES; target->locale_name; target++) {
      ...
      if (new_locale != NULL) {
#if !defined(_Py_FORCE_UTF8_LOCALE) && defined(HAVE_LANGINFO_H) &&             \
    defined(CODESET)
...
static int interpreter_update_config(PyThreadState *tstate,
                                     int only_update_path_config) {
...
static PyStatus pyinit_core_reconfigure(_PyRuntimeState *runtime,
...
#if defined(PYMALLOC_USE_HUGEPAGES) && defined(MS_WINDOWS)
static PyStatus get_huge_pages_privilege(void) {
  HANDLE hToken;
  ...
  if (!LookupPrivilegeValue(NULL, SE_LOCK_MEMORY_NAME,
    ...
    return _PyStatus_ERR(
        "failed to lookup SeLockMemoryPrivilege for huge pages");
  }
  tp.PrivilegeCount = 1;
  ...
  if (!status || (error != ERROR_SUCCESS)) {
    CloseHandle(hToken);
    return _PyStatus_ERR("SeLockMemoryPrivilege not held; "
...
static PyStatus pycore_init_runtime(_PyRuntimeState *runtime,
                                    const PyConfig *config) {
...
static PyStatus init_interp_settings(PyInterpreterState *interp,
...
static void init_interp_create_gil(PyThreadState *tstate, int gil) {
...
static int builtins_dict_watcher(PyDict_WatchEvent event, PyObject *dict,
...
static PyStatus pycore_create_interpreter(_PyRuntimeState *runtime,
  ...
  PyThreadState *tstate =
      _PyThreadState_New(interp, _PyThreadState_WHENCE_INIT);
  if (tstate == NULL) {
...
static PyStatus pycore_init_global_objects(PyInterpreterState *interp) {
  PyStatus status;
...
static PyStatus pycore_init_builtins(PyThreadState *tstate) {
  PyInterpreterState *interp = tstate->interp;
  ...
  PyObject *isinstance =
      PyDict_GetItemWithError(builtins_dict, &_Py_ID(isinstance));
  if (!isinstance) {
    goto error;
  }
...
PyStatus Py_PreInitializeFromBytesArgs(const PyPreConfig *src_config,
                                       Py_ssize_t argc, char **argv) {
  _PyArgv args = {.use_bytes_argv = 1, .argc = argc, .bytes_argv = argv};
  return _Py_PreInitializeFromPyArgv(src_config, &args);
}

PyStatus Py_PreInitializeFromArgs(const PyPreConfig *src_config,
...
PyStatus _Py_PreInitializeFromConfig(const PyConfig *config,
                                     const _PyArgv *args) {
  assert(config != NULL);
  ...
  if (!config->parse_argv) {
    ...
    _PyArgv config_args = {.use_bytes_argv = 0,
...
/* Py_Initialize() has already been called: update the main interpreter
   ...
   Py_Initialize(). */
static PyStatus pyinit_main_reconfigure(PyThreadState *tstate) {
...
static PyObject *presite_resolve_name(PyObject *obj, PyObject *attr) {
  obj = Py_NewRef(obj);
  attr = Py_NewRef(attr);
  PyObject *res;
...
static void run_presite(PyThreadState *tstate) {
  PyInterpreterState *interp = tstate->interp;
  const PyConfig *config = _PyInterpreterState_GetConfig(interp);
...
static PyStatus init_interp_main(PyThreadState *tstate) {
  assert(!_PyErr_Occurred(tstate));
    ...
    if (enabled) {
      ...
      if (config->perf_profiling > 0) {
        (void)PyErr_WarnEx(
            ...
            "JIT deactivated as perf profiling support is active", 0);
      } else
#endif
      {
        interp->jit = true;
 ...
 */
static PyStatus pyinit_main(PyThreadState *tstate) {
  PyInterpreterState *interp = tstate->interp;
...
static void finalize_modules_delete_special(PyThreadState *tstate,
                                            int verbose) {
  // List of names to clear in sys
  static const char *const sys_deletes[] = {
      "path", "argv", "ps1", "ps2", "last_exc", "last_type", "last_value",
      "last_traceback", "__interactivehook__",
  ...
  static const char *const sys_files[] = {
      "stdin",  "__stdin__",  "stdout", "__stdout__",
      "stderr", "__stderr__", NULL};
...
static PyObject *finalize_remove_modules(PyObject *modules, int verbose) {
...
static void finalize_clear_modules_dict(PyObject *modules) {
  if (PyDict_CheckExact(modules)) {
...
static void finalize_restore_builtins(PyThreadState *tstate) {
  PyInterpreterState *interp = tstate->interp;
...
static void finalize_modules_clear_weaklist(PyInterpreterState *interp,
...
static void finalize_clear_sys_builtins_dict(PyInterpreterState *interp,
                                             int verbose) {
  // Clear sys dict
  if (verbose) {
...
static void finalize_modules(PyThreadState *tstate) {
  PyInterpreterState *interp = tstate->interp;
...
static int file_is_closed(PyObject *fobj) {
  int r;
...
static int flush_std_files(void) {
  PyObject *file;
  int status = 0;
...
static void finalize_interp_types(PyInterpreterState *interp) {
...
static void finalize_interp_clear(PyThreadState *tstate) {
  int is_main_interp = _Py_IsMainInterpreter(tstate->interp);
...
static void finalize_interp_delete(PyInterpreterState *interp) {
...
static PyThreadState *resolve_final_tstate(_PyRuntimeState *runtime) {
...
static int interp_has_threads(PyInterpreterState *interp) {
...
static int interp_has_pending_calls(PyInterpreterState *interp) {
  assert(interp != NULL);
  ASSERT_WORLD_STOPPED(interp);
  return interp->ceval.pending.npending != 0;
}

static int interp_has_atexit_callbacks(PyInterpreterState *interp) {
  assert(interp != NULL);
...
static int runtime_has_subinterpreters(_PyRuntimeState *runtime) {
  assert(runtime != NULL);
  HEAD_LOCK(runtime);
...
static void make_pre_finalization_calls(PyThreadState *tstate,
                                        int subinterpreters) {
  assert(tstate != NULL);
  PyInterpreterState *interp = tstate->interp;
  ...
  for (;;) {
     ...
     */
    if (_Py_atomic_load_ssize_relaxed(&interp->finalization_guards.countdown) >
        0) {
      for (;;) {
        PyTime_t wait_ns = 1000 * 1000; // 1ms
        if (PyEvent_WaitTimed(&interp->finalization_guards.done, wait_ns,
                              /*detach=*/1)) {
          break;
        }
        ...
        if (PyErr_CheckSignals()) {
          PyErr_FormatUnraisable(
              "Exception ignored while waiting on finalization guards");
    ...
    int has_subinterpreters =
        subinterpreters ? runtime_has_subinterpreters(interp->runtime) : 0;
    int should_continue =
        (interp_has_threads(interp) || interp_has_atexit_callbacks(interp) ||
         interp_has_pending_calls(interp) || has_subinterpreters ||
         _Py_atomic_load_ssize_acquire(&interp->finalization_guards.countdown) >
             0);
    if (!should_continue) {
      break;
    }
...
static int _Py_Finalize(_PyRuntimeState *runtime) {
  int status = 0;
...
int Py_FinalizeEx(void) { return _Py_Finalize(&_PyRuntime); }

void Py_Finalize(void) { (void)_Py_Finalize(&_PyRuntime); }
...
static PyStatus new_interpreter(PyThreadState **tstate_p,
                                const PyInterpreterConfig *config,
                                long whence) {
  PyStatus status;
...
PyStatus Py_NewInterpreterFromConfig(PyThreadState **tstate_p,
                                     const PyInterpreterConfig *config) {
...
static PyObject *create_stdio(const PyConfig *config, PyObject *io, int fd,
                              int write_mode, const char *name,
  ...
  */
  ...
  if (write_mode)
    ...
    mode = "rb";
  buf = _PyObject_CallMethod(io, &_Py_ID(open), "isiOOOO", fd, mode, buffering,
  ...
  stream = _PyObject_CallMethod(io, &_Py_ID(TextIOWrapper), "OOOsOO", buf,
                                encoding_str, errors_str, newline,
...
static PyStatus init_sys_streams(PyThreadState *tstate) {
   ...
   */
  std = create_stdio(config, iomod, fd, 0, "<stdin>", config->stdio_encoding,
                     config->stdio_errors);
  if (std == NULL)
    goto error;
  ...
  /* Set sys.stdout */
  fd = fileno(stdout);
  std = create_stdio(config, iomod, fd, 1, "<stdout>", config->stdio_encoding,
                     config->stdio_errors);
  if (std == NULL)
    goto error;
  ...
  fd = fileno(stderr);
  std = create_stdio(config, iomod, fd, 1, "<stderr>", config->stdio_encoding,
                     L"backslashreplace");
  if (std == NULL)
    goto error;
...
static PyObject *android_log_write_impl(PyObject *self, PyObject *args) {
  int prio = 0;
  const char *tag = NULL;
...
static PyStatus init_android_streams(PyThreadState *tstate) {
  ...
  result = PyObject_CallMethod(_android_support, "init_streams", "Oii",
                               android_log_write, ANDROID_LOG_INFO,
                               ANDROID_LOG_WARN);
  if (result == NULL) {
    goto error;
  }
...
static PyObject *apple_log_write_impl(PyObject *self, PyObject *args) {
  int logtype = 0;
  const char *text = NULL;
...
static PyStatus init_apple_streams(PyThreadState *tstate) {
  ...
  result = PyObject_CallMethod(_apple_support, "init_streams", "Oii",
                               apple_log_write, OS_LOG_TYPE_DEFAULT,
                               OS_LOG_TYPE_ERROR);
  if (result == NULL) {
    goto error;
  }
...
static void _Py_FatalError_DumpTracebacks(int fd, PyInterpreterState *interp,
                                          PyThreadState *tstate) {
  PUTS(fd, "\n");
...
static int _Py_FatalError_PrintExc(PyThreadState *tstate) {
...
#ifdef MS_WINDOWS
static void fatal_output_debug(const char *msg) {
...
static void fatal_error_dump_runtime(int fd, _PyRuntimeState *runtime) {
...
static inline void _Py_NO_RETURN fatal_error_exit(int status) {
  if (status < 0) {
...
static inline int acquire_dict_lock_for_dump(PyObject *obj) {
#ifdef Py_GIL_DISABLED
...
static inline void release_dict_lock_for_dump(PyObject *obj) {
#ifdef Py_GIL_DISABLED
  PyMutex *mutex = &obj->ob_mutex;
...
static void _Py_NO_RETURN fatal_error(int fd, int header, const char *prefix,
...
void _Py_NO_RETURN _Py_FatalErrorFormat(const char *func, const char *format,
                                        ...) {
  static int reentrant = 0;
  if (reentrant) {
...
void _Py_NO_RETURN _Py_FatalRefcountErrorFunc(const char *func,
                                              const char *msg) {
...
void _Py_NO_RETURN Py_ExitStatusException(PyStatus status) {
...
static void handle_thread_shutdown_exception(PyThreadState *tstate) {
  assert(tstate != NULL);
  assert(_PyErr_Occurred(tstate));
  PyInterpreterState *interp = tstate->interp;
  assert(interp->threads.head != NULL);
...
void _Py_NO_RETURN Py_Exit(int sts) {
  PyThreadState *tstate = _PyThreadState_GET();
...
int Py_FdIsInteractive(FILE *fp, const char *filename) {
  ...
  return ((filename == NULL) || (strcmp(filename, "<stdin>") == 0) ||
          (strcmp(filename, "???") == 0));
}
