...
// Do not edit!
        ...
        /* _CHECK_PERIODIC_AT_END_PENDING is not a viable micro-op for tier 2 */
        ...
        case _POP_TOP_OPARG: {
            ...
            for (int i = CALL_ARGCOUNT(oparg)-1; i >= 0; i--) {
                optimize_pop_top(ctx, this_instr, args[i]);
            }
            CHECK_STACK_BOUNDS(-(CALL_ARGCOUNT(oparg)));
        ...
        case _PY_FRAME_GENERAL: {
            ...
            CHECK_STACK_BOUNDS(-1 - (CALL_ARGCOUNT(oparg)));
            stack_pointer[-2 - (CALL_ARGCOUNT(oparg))] = new_frame;
        ...
        case _CHECK_FUNCTION_EXACT_ARGS: {
            ...
            if (sym_is_const(ctx, callable)) {
                if (sym_is_null(self_or_null) || sym_is_not_null(self_or_null)) {
                    ...
                    PyCodeObject *co = (PyCodeObject *)func->func_code;
                    if (co->co_argcount == CALL_ARGCOUNT(oparg) + sym_is_not_null(self_or_null)) {
                        ADD_OP(_NOP, 0 ,0);
                    }
                }
            }
        ...
        case _INIT_CALL_PY_EXACT_ARGS: {
            ...
            self_or_null = stack_pointer[-1 - (CALL_ARGCOUNT(oparg))];
            callable = stack_pointer[-2 - (CALL_ARGCOUNT(oparg))];
            int argcount = CALL_ARGCOUNT(oparg);
        ...
        case _CREATE_INIT_FRAME: {
            ...
            args = &stack_pointer[-(CALL_ARGCOUNT(oparg))];
            self = stack_pointer[-1 - (CALL_ARGCOUNT(oparg))];
            init = stack_pointer[-2 - (CALL_ARGCOUNT(oparg))];
            ctx->frame->stack_pointer = stack_pointer - CALL_ARGCOUNT(oparg) - 2;
            ...
            init_frame = PyJitRef_WrapInvalid(frame_new_from_symbol(ctx, init, args-1, CALL_ARGCOUNT(oparg)+1));
            CHECK_STACK_BOUNDS(-1 - (CALL_ARGCOUNT(oparg)));
            stack_pointer[-2 - (CALL_ARGCOUNT(oparg))] = init_frame;
            stack_pointer += -1 - (CALL_ARGCOUNT(oparg));
            ASSERT_WITHIN_STACK_BOUNDS(__FILE__, __LINE__);
            break;
        }
        ...
        case _CALL_METHOD_DESCRIPTOR_O: {
            ...
            if (callable_o && Py_IS_TYPE(callable_o, &PyMethodDescr_Type)
                ...
                ADD_OP(_CALL_METHOD_DESCRIPTOR_O_INLINE, CALL_OPARG(CALL_ARGCOUNT(oparg) + 1, 0), (uintptr_t)cfunc);
            }
            res = sym_new_not_null(ctx);
            c = callable;
            if (sym_is_not_null(self_or_null)) {
            ...
            CHECK_STACK_BOUNDS(2 - (CALL_ARGCOUNT(oparg)));
            stack_pointer[-2 - (CALL_ARGCOUNT(oparg))] = res;
            stack_pointer[-1 - (CALL_ARGCOUNT(oparg))] = c;
            stack_pointer[-(CALL_ARGCOUNT(oparg))] = s;
            stack_pointer[1 - (CALL_ARGCOUNT(oparg))] = a;
        ...
        case _CALL_METHOD_DESCRIPTOR_O_INLINE: {
            ...
            CHECK_STACK_BOUNDS(3 - (CALL_ARGCOUNT(oparg)));
            stack_pointer[-1 - (CALL_ARGCOUNT(oparg))] = res;
            stack_pointer[-(CALL_ARGCOUNT(oparg))] = c;
            stack_pointer[1 - (CALL_ARGCOUNT(oparg))] = s;
            stack_pointer[2 - (CALL_ARGCOUNT(oparg))] = a;
            stack_pointer += 3 - (CALL_ARGCOUNT(oparg));
            ASSERT_WITHIN_STACK_BOUNDS(__FILE__, __LINE__);
            break;
        }
        ...
        case _CALL_METHOD_DESCRIPTOR_NOARGS: {
            ...
            if (callable_o && Py_IS_TYPE(callable_o, &PyMethodDescr_Type)
                ...
                PyMethodDescrObject *method = (PyMethodDescrObject *)callable_o;
                PyCFunction cfunc = method->d_method->ml_meth;
                ADD_OP(_CALL_METHOD_DESCRIPTOR_NOARGS_INLINE, CALL_OPARG(CALL_ARGCOUNT(oparg) + 1, 0), (uintptr_t)cfunc);
            }
            res = sym_new_not_null(ctx);
            c = callable;
            if (sym_is_not_null(self_or_null)) {
        ...
        case _RECORD_CALLABLE: {
            JitOptRef func;
            func = stack_pointer[-2 - (CALL_ARGCOUNT(oparg))];
