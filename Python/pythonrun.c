...
_PyRun_InteractiveLoopObject(FILE *fp, PyObject *filename, PyCompilerFlags *flags)
{
    ...
    PyObject *v;
    if (PySys_GetAttr(&_Py_ID(ps1), &v) < 0) {
        PyErr_Print();
        return -1;
    }
    if (v == NULL) {
    ...
    Py_XDECREF(v);
    if (PySys_GetAttr(&_Py_ID(ps2), &v) < 0) {
        PyErr_Print();
        return -1;
    }
    if (v == NULL) {
                    ...
                    mod_ty *pmod, PyObject** interactive_src)
{
    ...
    if (fp == stdin) {
        if (PySys_GetAttr(&_Py_ID(stdin), &attr) < 0) {
            PyErr_Clear();
        }
    ...
    const char *ps1 = "";
    if (PySys_GetAttr(&_Py_ID(ps1), &attr) < 0) {
        PyErr_Clear();
    }
    else if (attr != NULL) {
    ...
    const char *ps2 = "";
    if (PySys_GetAttr(&_Py_ID(ps2), &attr) < 0) {
        PyErr_Clear();
    }
    else if (attr != NULL) {
...
_Py_HandleSystemExitAndKeyboardInterrupt(int *exitcode_p)
{
    ...
    PyObject *sys_stderr;
    if (PySys_GetAttr(&_Py_ID(stderr), &sys_stderr) < 0) {
        PyErr_Clear();
    }
...
_PyErr_PrintEx(PyThreadState *tstate, int set_sys_last_vars)
{
    ...
    if (PySys_GetAttr(&_Py_ID(excepthook), &hook) < 0) {
        PyErr_Clear();
    }
