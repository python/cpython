...
PyThreadState_Clear(PyThreadState *tstate)
{
    ...
    if (verbose && tstate->cframe->frame != NULL) {
...
PyThreadState_GetFrame(PyThreadState *tstate)
{
    ...
    PyFrameObject *frame = _PyFrame_GetFrameObject(tstate->cframe->frame);
    if (frame == NULL) {
        PyErr_Clear();
    }
    Py_XINCREF(frame);
...
_PyThread_CurrentFrames(void)
{
    ...
    for (i = runtime->interpreters.head; i != NULL; i = i->next) {
        ...
        for (t = i->tstate_head; t != NULL; t = t->next) {
            InterpreterFrame *frame = t->cframe->frame;
            if (frame == NULL) {
                continue;
            }
