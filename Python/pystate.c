...
#ifdef HAVE_THREAD_LOCAL
_Py_thread_local static const struct _Py_freelist _Py_tss_tstates_freelist = { 0 };
#define _PyThreadState_FREELIST() ((struct _Py_freelist *) &_Py_tss_tstates_freelist)
#endif
...
{
#ifdef HAVE_THREAD_LOCAL
    _PyThreadStateImpl *impl = _PyFreeList_PopMem(_PyThreadState_FREELIST());
    if (impl == NULL)
    {
...
free_threadstate(_PyThreadStateImpl *tstate)
{
    ...
    if (tstate == &tstate->base.interp->_initial_thread) {
        ...
        _PyFreeList_Free(_PyThreadState_FREELIST(), tstate, 100, PyMem_RawFree);
#else
