...
{
    const node * const n = is_async ? CHILD(n0, 1) : n0;
    int i, n_items, paren_surrounded;
    asdl_seq *items, *body;

    REQ(n, with_stmt);

    paren_surrounded = TYPE(CHILD(n, 1)) == LPAR ? 1 : 0;
    n_items = (NCH(n) - 2 - 2 * paren_surrounded) / 2;
    items = _Py_asdl_seq_new(n_items, c->c_arena);

    if (!items)
        return NULL;

    /* We are jumping by two elements (the item and the comma) and therefore
     * the limit of i < NCH(n) - 2 - paren_surrounded will handle also the case
     * when there is a trailing comma (it will not iterate over the comma). */
    for (i = 1 + paren_surrounded; i < NCH(n) - 2 - paren_surrounded; i += 2) {
