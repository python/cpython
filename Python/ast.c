...
static int validate_stmts(struct validator *, asdl_seq *);
static int validate_exprs(struct validator *, asdl_seq *, expr_context_ty, int);
...
static int
validate_comprehension(struct validator *state, asdl_seq *gens)
{
    Py_ssize_t i;
...
static int
validate_keywords(struct validator *state, asdl_seq *keywords)
{
    Py_ssize_t i;
    for (i = 0; i < asdl_seq_LEN(keywords); i++)
        if (!validate_expr(state, ((keyword_ty)asdl_seq_GET(keywords, i))->value, Load))
            return 0;
    return 1;
}

static int
validate_args(struct validator *state, asdl_seq *args)
{
    Py_ssize_t i;
...
static int
validate_assignlist(struct validator *state, asdl_seq *targets, expr_context_ty ctx)
{
...
static int
validate_body(struct validator *state, asdl_seq *body, const char *owner)
{
...
static int
validate_stmts(struct validator *state, asdl_seq *seq)
{
    Py_ssize_t i;
...
static int
validate_exprs(struct validator *state, asdl_seq *exprs, expr_context_ty ctx, int null_ok)
{
    Py_ssize_t i;
