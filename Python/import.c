...
_PyImportZip_Init(PyThreadState *tstate)
{
    ...
    if (zipimport == NULL) {
        ...
        PyObject *zipimporter = PyObject_GetAttr(zipimport,
                                                 _Py_ID(zipimporter));
        Py_DECREF(zipimport);
        if (zipimporter == NULL) {
...
module_dict_for_exec(PyThreadState *tstate, PyObject *name)
{
    ...
    if (r == 0) {
        r = PyDict_SetItem(d, _Py_ID(__builtins__),
                           PyEval_GetBuiltins());
    }
    if (r < 0) {
                              ...
                              PyObject *cpathname)
{
    ...
    res = PyObject_CallMethodObjArgs(external,
...
import_find_and_load(PyThreadState *tstate, PyObject *abs_name)
{
    ...
    mod = PyObject_CallMethodObjArgs(interp->importlib,
