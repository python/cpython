...
gc_select_generation(GCState *gcstate)
{
    for (int i = NUM_GENERATIONS-1; i >= 0; i--) {
        if (GEN(gcstate, i)->count > GEN(gcstate, i)->threshold) {
...
static Py_ssize_t
gc_collect_generational(PyThreadState *tstate, int generation, struct gc_collection_stats *stats)
{
    int i;
                  ...
                  struct gc_collection_stats *stats)
{
    ...
    untrack_tuples(from);
    if (from != to) {
        if (from == GEN_HEAD(gcstate, NUM_GENERATIONS-2)) {
...
_PyGC_Collect(PyThreadState *tstate, int generation, _PyGC_Reason reason)
{
    ...
    if (gc_collect_generational(tstate, generation, &stats) < 0) {
        if (_PyErr_Occurred(tstate)) {
            if (reason == _Py_GC_REASON_SHUTDOWN) {
                _PyErr_Clear(tstate);
            }
...
_PyGC_Fini(PyInterpreterState *interp)
{
     ...
     */
    for (int i = 0; i < NUM_GENERATIONS; i++) {
        finalize_unlink_gc_head(GEN_HEAD(gcstate, i));
    }
