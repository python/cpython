...
compiler_body(struct compiler *c, location loc, asdl_stmt_seq *stmts)
{
    ...
    if (c->u->u_scope_type == COMPILER_SCOPE_MODULE && asdl_seq_LEN(stmts)) {
        loc = LOC((stmt_ty)asdl_seq_GET(stmts, 0));
    }
...
{
    ADDOP_LOAD_CONST_NEW(c,
                         LOC(annotation),
                         _PyAST_ExprAsUnicode(annotation));
    return 1;
}
...
compiler_function(struct compiler *c, stmt_ty s, int is_async)
{
    ...
    funcflags = compiler_default_arguments(c, LOC(s), args);
    if (funcflags == -1) {
        return 0;
    }
    annotations = compiler_visit_annotations(c, LOC(s), args, returns);
    if (annotations == 0) {
        return 0;
    }
    ...
    if (!compiler_make_closure(c, LOC(s), co, funcflags, qualname)) {
        Py_DECREF(qualname);
        Py_DECREF(co);
        return 0;
    }
...
compiler_class(struct compiler *c, stmt_ty s)
{
    ...
    */
    ...
    ADDOP(c, LOC(s), LOAD_BUILD_CLASS);

    /* 3. load a function (or closure) made from the code object */
    if (!compiler_make_closure(c, LOC(s), co, 0, NULL)) {
        Py_DECREF(co);
        return 0;
    }
    Py_DECREF(co);
    ...
    /* 4. load class name */
    ADDOP_LOAD_CONST(c, LOC(s), s->v.ClassDef.name);

    /* 5. generate the rest of the code for the call */
    if (!compiler_call_helper(c, LOC(s), 2,
                              s->v.ClassDef.bases,
    ...
    /* 7. store into <name> */
    if (!compiler_nameop(c, LOC(s), s->v.ClassDef.name, Store))
        return 0;
    return 1;
}
...
compiler_lambda(struct compiler *c, expr_ty e)
{
    ...
    funcflags = compiler_default_arguments(c, LOC(e), args);
    if (funcflags == -1) {
        return 0;
    }
    ...
    if (!compiler_make_closure(c, LOC(e), co, funcflags, qualname)) {
        Py_DECREF(qualname);
        Py_DECREF(co);
        return 0;
    }
...
compiler_for(struct compiler *c, stmt_ty s)
{
    ...
    if (!compiler_push_fblock(c, LOC(s), FOR_LOOP, start, end, NULL)) {
        return 0;
    }
    VISIT(c, expr, s->v.For.iter);
    ADDOP(c, LOC(s), GET_ITER);

    USE_LABEL(c, start);
    ADDOP_JUMP(c, LOC(s), FOR_ITER, cleanup);

    USE_LABEL(c, body);
    VISIT(c, expr, s->v.For.target);
    VISIT_SEQ(c, stmt, s->v.For.body);
    ADDOP_JUMP(c, NO_LOCATION, JUMP, start);  /* artificial */
...
compiler_async_for(struct compiler *c, stmt_ty s)
{
    if (IS_TOP_LEVEL_AWAIT(c)) {
        ...
        return compiler_error(c, LOC(s), "'async for' outside async function");
    }
    ...
    ADDOP_JUMP(c, LOC(s), SETUP_FINALLY, except);
    ADDOP(c, LOC(s), GET_ANEXT);
    ADDOP_LOAD_CONST(c, LOC(s), Py_None);
    ADD_YIELD_FROM(c, LOC(s), 1);
    ADDOP(c, LOC(s), POP_BLOCK);  /* for SETUP_FINALLY */
    ...
    ADDOP(c, LOC(s->v.AsyncFor.iter), END_ASYNC_FOR);
...
compiler_try_finally(struct compiler *c, stmt_ty s)
{
    ...
    /* `try` block */
    ADDOP_JUMP(c, LOC(s), SETUP_FINALLY, end);

    USE_LABEL(c, body);
    if (!compiler_push_fblock(c, LOC(s), FINALLY_TRY, body, end, s->v.Try.finalbody))
        return 0;
    ...
    if (!compiler_push_fblock(c, LOC(s), FINALLY_END, end, NO_LABEL, NULL))
        return 0;
    VISIT_SEQ(c, stmt, s->v.Try.finalbody);
    location loc = location_of_last_executing_statement(s->v.Try.finalbody);
...
compiler_try_star_finally(struct compiler *c, stmt_ty s)
{
    ...
    USE_LABEL(c, body);
    if (!compiler_push_fblock(c, LOC(s), FINALLY_TRY, body, end, s->v.TryStar.finalbody)) {
        return 0;
    }
    ...
    ADDOP_JUMP(c, NO_LOCATION, SETUP_CLEANUP, cleanup);
    ADDOP(c, NO_LOCATION, PUSH_EXC_INFO);
    if (!compiler_push_fblock(c, NO_LOCATION, FINALLY_END, end, NO_LABEL, NULL)) {
        return 0;
    }
    VISIT_SEQ(c, stmt, s->v.TryStar.finalbody);
...
compiler_try_except(struct compiler *c, stmt_ty s)
{
    ...
    USE_LABEL(c, body);
    if (!compiler_push_fblock(c, LOC(s), TRY_EXCEPT, body, NO_LABEL, NULL)) {
        return 0;
    }
    VISIT_SEQ(c, stmt, s->v.Try.body);
    ...
    if (!compiler_push_fblock(c, LOC(s), EXCEPTION_HANDLER, NO_LABEL,
                              NO_LABEL, NULL)) {
        return 0;
    }
    for (Py_ssize_t i = 0; i < n; i++) {
        ...
        if (!handler->v.ExceptHandler.type && i < n-1) {
            return compiler_error(c, LOC(handler),
        ...
        if (handler->v.ExceptHandler.type) {
            ...
            ADDOP(c, LOC(handler), CHECK_EXC_MATCH);
            ADDOP_JUMP(c, LOC(handler), POP_JUMP_IF_FALSE, except);
        }
        if (handler->v.ExceptHandler.name) {
            NEW_JUMP_TARGET_LABEL(c, cleanup_end);
            NEW_JUMP_TARGET_LABEL(c, cleanup_body);

            compiler_nameop(c, LOC(handler),
            ...
            */
            ...
            /* second try: */
            ADDOP_JUMP(c, LOC(handler), SETUP_CLEANUP, cleanup_end);

            USE_LABEL(c, cleanup_body);
            if (!compiler_push_fblock(c, LOC(handler), HANDLER_CLEANUP,
            ...
            ADDOP(c, LOC(handler), POP_TOP); /* exc_value */
...
compiler_try_star_except(struct compiler *c, stmt_ty s)
{
    ...
    ADDOP(c, NO_LOCATION, PUSH_EXC_INFO);
    /* Runtime will push a block here, so we need to account for that */
    if (!compiler_push_fblock(c, LOC(s), EXCEPTION_GROUP_HANDLER,
    ...
    for (Py_ssize_t i = 0; i < n; i++) {
        ...
        if (i == 0) {
            ...
            */
            ...
            /*
               ...
               [orig, [], exc]
            */
            ADDOP_I(c, LOC(handler), BUILD_LIST, 0);
            ADDOP_I(c, LOC(handler), SWAP, 2);
        }
        if (handler->v.ExceptHandler.type) {
            VISIT(c, expr, handler->v.ExceptHandler.type);
            ADDOP(c, LOC(handler), CHECK_EG_MATCH);
            ADDOP_I(c, LOC(handler), COPY, 1);
            ADDOP_JUMP(c, LOC(handler), POP_JUMP_IF_NOT_NONE, handle_match);
            ADDOP(c, LOC(handler), POP_TOP);  // match
            ADDOP_JUMP(c, LOC(handler), JUMP, except);
        }
        ...
        if (handler->v.ExceptHandler.name) {
            compiler_nameop(c, LOC(handler), handler->v.ExceptHandler.name, Store);
        }
        else {
            ADDOP(c, LOC(handler), POP_TOP);  // match
        }
        ...
        */
        ...
        USE_LABEL(c, cleanup_body);
        if (!compiler_push_fblock(c, LOC(handler), HANDLER_CLEANUP, cleanup_body,
...
compiler_import(struct compiler *c, stmt_ty s)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        ADDOP_LOAD_CONST(c, LOC(s), zero);
        ADDOP_LOAD_CONST(c, LOC(s), Py_None);
        ADDOP_NAME(c, LOC(s), IMPORT_NAME, alias->name, names);

        if (alias->asname) {
            r = compiler_import_as(c, LOC(s), alias->name, alias->asname);
            if (!r)
                return r;
        }
        else {
            ...
            r = compiler_nameop(c, LOC(s), tmp, Store);
            if (dot != -1) {
                Py_DECREF(tmp);
            }
            if (!r)
...
compiler_list(struct compiler *c, expr_ty e)
{
    ...
    if (e->v.List.ctx == Store) {
        return assignment_helper(c, LOC(e), elts);
    }
    else if (e->v.List.ctx == Load) {
        return starunpack_helper(c, LOC(e), elts, 0,
...
{
    return starunpack_helper(c, LOC(e), e->v.Set.elts, 0,
...
compiler_subdict(struct compiler *c, expr_ty e, Py_ssize_t begin, Py_ssize_t end)
{
    ...
    if (n > 1 && !big && are_all_items_const(e->v.Dict.keys, begin, end)) {
        ...
        ADDOP_LOAD_CONST_NEW(c, LOC(e), keys);
        ADDOP_I(c, LOC(e), BUILD_CONST_KEY_MAP, n);
        return 1;
    }
    if (big) {
        ADDOP_I(c, LOC(e), BUILD_MAP, 0);
    }
    for (i = begin; i < end; i++) {
        VISIT(c, expr, (expr_ty)asdl_seq_GET(e->v.Dict.keys, i));
        VISIT(c, expr, (expr_ty)asdl_seq_GET(e->v.Dict.values, i));
        if (big) {
            ADDOP_I(c, LOC(e), MAP_ADD, 1);
        }
    }
    if (!big) {
        ADDOP_I(c, LOC(e), BUILD_MAP, n);
    }
    return 1;
}
...
compiler_compare(struct compiler *c, expr_ty e)
{
    ...
    if (n == 0) {
        ...
        ADDOP_COMPARE(c, LOC(e), asdl_seq_GET(e->v.Compare.ops, 0));
    }
    else {
        NEW_JUMP_TARGET_LABEL(c, cleanup);
        for (i = 0; i < n; i++) {
            ...
            ADDOP_JUMP(c, LOC(e), JUMP_IF_FALSE_OR_POP, cleanup);
        }
        VISIT(c, expr, (expr_ty)asdl_seq_GET(e->v.Compare.comparators, n));
...
check_caller(struct compiler *c, expr_ty e)
{
    ...
    case JoinedStr_kind:
    case FormattedValue_kind: {
        return compiler_warn(c, LOC(e), "'%.200s' object is not callable; "
...
check_subscripter(struct compiler *c, expr_ty e)
{
    ...
    case GeneratorExp_kind:
    case Lambda_kind: {
        return compiler_warn(c, LOC(e), "'%.200s' object is not subscriptable; "
                                        "perhaps you missed a comma?",
                                        infer_type(e)->tp_name);
    }
    default:
...
check_index(struct compiler *c, expr_ty e, expr_ty s)
{
    ...
    case JoinedStr_kind:
    case FormattedValue_kind: {
        return compiler_warn(c, LOC(e), "%.200s indices must be integers "
                                        "or slices, not %.200s; "
                                        "perhaps you missed a comma?",
                                        infer_type(e)->tp_name,
                                        index_type->tp_name);
...
maybe_optimize_method_call(struct compiler *c, expr_ty e)
{
    ...
    location loc = update_start_location_to_match_attr(c, LOC(meth), meth);
...
validate_keywords(struct compiler *c, asdl_keyword_seq *keywords)
{
    ...
    for (Py_ssize_t i = 0; i < nkeywords; i++) {
        ...
        if (forbidden_name(c, LOC(key), key->arg, Store)) {
            return -1;
        }
...
compiler_call(struct compiler *c, expr_ty e)
{
    ...
    ADDOP(c, LOC(e->v.Call.func), PUSH_NULL);
    VISIT(c, expr, e->v.Call.func);
    return compiler_call_helper(c, LOC(e), 0,
                                e->v.Call.args,
                                e->v.Call.keywords);
}
...
compiler_joined_str(struct compiler *c, expr_ty e)
{
    ...
    if (value_count > STACK_USE_GUIDELINE) {
        _Py_DECLARE_STR(empty, "");
        ADDOP_LOAD_CONST_NEW(c, LOC(e), Py_NewRef(&_Py_STR(empty)));
        ADDOP_NAME(c, LOC(e), LOAD_METHOD, &_Py_ID(join), names);
        ADDOP_I(c, LOC(e), BUILD_LIST, 0);
        for (Py_ssize_t i = 0; i < asdl_seq_LEN(e->v.JoinedStr.values); i++) {
            VISIT(c, expr, asdl_seq_GET(e->v.JoinedStr.values, i));
            ADDOP_I(c, LOC(e), LIST_APPEND, 1);
        }
        ADDOP_I(c, LOC(e), CALL, 1);
    }
    else {
        VISIT_SEQ(c, expr, e->v.JoinedStr.values);
        if (asdl_seq_LEN(e->v.JoinedStr.values) != 1) {
            ADDOP_I(c, LOC(e), BUILD_STRING, asdl_seq_LEN(e->v.JoinedStr.values));
        }
    }
    return 1;
}
...
compiler_formatted_value(struct compiler *c, expr_ty e)
{
    ...
    */
    ...
    ADDOP_I(c, LOC(e), FORMAT_VALUE, oparg);
                       ...
                       expr_ty val)
{
    ...
    if (is_async_generator && type != COMP_GENEXP &&
        ...
        !is_top_level_await)
    {
        compiler_error(c, LOC(e), "asynchronous comprehension outside of "
    ...
    if (!compiler_make_closure(c, LOC(e), co, 0, qualname)) {
        goto error;
    }
    Py_DECREF(qualname);
    Py_DECREF(co);
    ...
    if (outermost->is_async) {
        ADDOP(c, LOC(e), GET_AITER);
    }
    else {
        ADDOP(c, LOC(e), GET_ITER);
    }

    ADDOP_I(c, LOC(e), CALL, 0);

    if (is_async_generator && type != COMP_GENEXP) {
        ADDOP_I(c, LOC(e), GET_AWAITABLE, 0);
        ADDOP_LOAD_CONST(c, LOC(e), Py_None);
        ADD_YIELD_FROM(c, LOC(e), 1);
    }
...
compiler_async_with(struct compiler *c, stmt_ty s, int pos)
{
    ...
    if (IS_TOP_LEVEL_AWAIT(c)){
        ...
        return compiler_error(c, LOC(s), "'async with' outside async function");
    }
    ...
    ADDOP(c, LOC(s), BEFORE_ASYNC_WITH);
    ADDOP_I(c, LOC(s), GET_AWAITABLE, 1);
    ADDOP_LOAD_CONST(c, LOC(s), Py_None);
    ADD_YIELD_FROM(c, LOC(s), 1);

    ADDOP_JUMP(c, LOC(s), SETUP_WITH, final);

    /* SETUP_WITH pushes a finally block. */
    USE_LABEL(c, block);
    if (!compiler_push_fblock(c, LOC(s), ASYNC_WITH, block, final, s)) {
        return 0;
    }
    ...
    ADDOP(c, LOC(s), POP_BLOCK);
    /* End of body; start the cleanup */

    /* For successful outcome:
     * call __exit__(None, None, None)
     */
    if(!compiler_call_exit_with_nones(c, LOC(s)))
        return 0;
    ADDOP_I(c, LOC(s), GET_AWAITABLE, 2);
    ADDOP_LOAD_CONST(c, LOC(s), Py_None);
    ADD_YIELD_FROM(c, LOC(s), 1);

    ADDOP(c, LOC(s), POP_TOP);

    ADDOP_JUMP(c, LOC(s), JUMP, exit);

    /* For exceptional outcome: */
    USE_LABEL(c, final);

    ADDOP_JUMP(c, LOC(s), SETUP_CLEANUP, cleanup);
    ADDOP(c, LOC(s), PUSH_EXC_INFO);
    ADDOP(c, LOC(s), WITH_EXCEPT_START);
    ADDOP_I(c, LOC(s), GET_AWAITABLE, 2);
    ADDOP_LOAD_CONST(c, LOC(s), Py_None);
    ADD_YIELD_FROM(c, LOC(s), 1);
...
compiler_with(struct compiler *c, stmt_ty s, int pos)
{
    ...
    ADDOP(c, LOC(s), BEFORE_WITH);
    ADDOP_JUMP(c, LOC(s), SETUP_WITH, final);

    /* SETUP_WITH pushes a finally block. */
    USE_LABEL(c, block);
    if (!compiler_push_fblock(c, LOC(s), WITH, block, final, s)) {
        return 0;
    }
    ...
    ADDOP(c, NO_LOCATION, POP_BLOCK);  /* artificial */
...
compiler_visit_expr1(struct compiler *c, expr_ty e)
{
    ...
    case BinOp_kind:
        ...
        ADDOP_BINARY(c, LOC(e), e->v.BinOp.op);
        break;
    case UnaryOp_kind:
        VISIT(c, expr, e->v.UnaryOp.operand);
        ADDOP(c, LOC(e), unaryop(e->v.UnaryOp.op));
        break;
    case Lambda_kind:
    ...
    case Yield_kind:
        if (c->u->u_ste->ste_type != FunctionBlock)
            return compiler_error(c, LOC(e), "'yield' outside function");
        if (e->v.Yield.value) {
    ...
    case YieldFrom_kind:
        ...
        if (c->u->u_scope_type == COMPILER_SCOPE_ASYNC_FUNCTION)
            return compiler_error(c, LOC(e), "'yield from' inside async function");

        VISIT(c, expr, e->v.YieldFrom.value);
        ADDOP(c, LOC(e), GET_YIELD_FROM_ITER);
        ADDOP_LOAD_CONST(c, LOC(e), Py_None);
        ADD_YIELD_FROM(c, LOC(e), 0);
        break;
    case Await_kind:
        if (!IS_TOP_LEVEL_AWAIT(c)){
            if (c->u->u_ste->ste_type != FunctionBlock){
                return compiler_error(c, LOC(e), "'await' outside function");
            }

            if (c->u->u_scope_type != COMPILER_SCOPE_ASYNC_FUNCTION &&
                    c->u->u_scope_type != COMPILER_SCOPE_COMPREHENSION){
                return compiler_error(c, LOC(e), "'await' outside async function");
            }
        }
    ...
    case Constant_kind:
        ADDOP_LOAD_CONST(c, LOC(e), e->v.Constant.value);
        break;
    case JoinedStr_kind:
    ...
    case Attribute_kind:
    {
        ...
        location loc = update_start_location_to_match_attr(c, LOC(e), e);
    ...
    case Slice_kind:
    {
        ...
        ADDOP_I(c, LOC(e), BUILD_SLICE, n);
        break;
    }
    case Name_kind:
        return compiler_nameop(c, LOC(e), e->v.Name.id, e->v.Name.ctx);
...
compiler_augassign(struct compiler *c, stmt_ty s)
{
    ...
    case Subscript_kind:
        ...
        if (is_two_element_slice(e->v.Subscript.slice)) {
            ...
            ADDOP_I(c, LOC(e), COPY, 3);
            ADDOP_I(c, LOC(e), COPY, 3);
            ADDOP_I(c, LOC(e), COPY, 3);
            ADDOP(c, LOC(e), BINARY_SLICE);
        }
        else {
            VISIT(c, expr, e->v.Subscript.slice);
            ADDOP_I(c, LOC(e), COPY, 2);
            ADDOP_I(c, LOC(e), COPY, 2);
            ADDOP(c, LOC(e), BINARY_SUBSCR);
        }
        break;
    case Name_kind:
        if (!compiler_nameop(c, LOC(e), e->v.Name.id, Load)) {
            return 0;
        }
        break;
    default:
    ...
    ADDOP_INPLACE(c, LOC(s), s->v.AugAssign.op);
    ...
    case Subscript_kind:
        if (is_two_element_slice(e->v.Subscript.slice)) {
            ADDOP_I(c, LOC(e), SWAP, 4);
            ADDOP_I(c, LOC(e), SWAP, 3);
            ADDOP_I(c, LOC(e), SWAP, 2);
            ADDOP(c, LOC(e), STORE_SLICE);
        }
        else {
            ADDOP_I(c, LOC(e), SWAP, 3);
            ADDOP_I(c, LOC(e), SWAP, 2);
            ADDOP(c, LOC(e), STORE_SUBSCR);
        }
        break;
    case Name_kind:
        return compiler_nameop(c, LOC(e), e->v.Name.id, Store);
    default:
        Py_UNREACHABLE();
    }
    return 1;
...
compiler_annassign(struct compiler *c, stmt_ty s)
{
    ...
    switch (targ->kind) {
    case Name_kind:
        if (forbidden_name(c, LOC(s), targ->v.Name.id, Store))
            return 0;
        ...
        if (s->v.AnnAssign.simple &&
            ...
            ADDOP_NAME(c, LOC(s), LOAD_NAME, &_Py_ID(__annotations__), names);
            mangled = _Py_Mangle(c->u->u_private, targ->v.Name.id);
            ADDOP_LOAD_CONST_NEW(c, LOC(s), mangled);
            ADDOP(c, LOC(s), STORE_SUBSCR);
        }
        break;
    case Attribute_kind:
        if (forbidden_name(c, LOC(s), targ->v.Attribute.attr, Store))
            return 0;
        if (!s->v.AnnAssign.value &&
