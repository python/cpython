...
codegen_call_exit_with_nones(compiler *c, location loc)
{
    ...
    ADDOP_LOAD_CONST(c, loc, Py_None);
    ADDOP_LOAD_CONST(c, loc, Py_None);
    ADDOP_I(c, loc, CALL, CALL_OPARG(3, 0));
    return SUCCESS;
}
...
codegen_apply_decorators(compiler *c, asdl_expr_seq* decos)
{
    ...
    for (Py_ssize_t i = asdl_seq_LEN(decos) - 1; i > -1; i--) {
        ...
        ADDOP_I(c, loc, CALL, CALL_OPARG(0, 0));
    }
    return SUCCESS;
}
...
codegen_function(compiler *c, stmt_ty s, int is_async)
{
    ...
    if (is_generic) {
        ...
        if (num_typeparam_args > 0) {
            ...
            ADDOP_I(c, loc, CALL, CALL_OPARG(num_typeparam_args - 1, 0));
        }
        else {
...
codegen_assert(compiler *c, stmt_ty s)
{
    ...
    if (s->v.Assert.msg) {
        ...
        ADDOP_I(c, LOC(s), CALL, CALL_OPARG(0, 0));
    }
...
maybe_optimize_method_call(compiler *c, expr_ty e)
{
    ...
    if (kwdsl) {
        ...
        loc = update_start_location_to_match_attr(c, LOC(e), meth);
        ADDOP_I(c, loc, CALL, CALL_OPARG(argsl, 0));
    }
    return 1;
}
...
codegen_joined_str(compiler *c, expr_ty e)
{
    ...
    if (value_count > _PY_STACK_USE_GUIDELINE) {
        ...
        ADDOP_I(c, loc, CALL, CALL_OPARG(1, 0));
    }
    else {
                         ...
                         asdl_keyword_seq *keywords)
{
    ...
    if (nkwelts) {
        ...
        ADDOP_I(c, loc, CALL, CALL_OPARG(n + nelts, 0));
    }
    return SUCCESS;
...
codegen_async_with_inner(compiler *c, stmt_ty s, int pos)
{
    ...
    ADDOP_I(c, loc, CALL, CALL_OPARG(0, CALL_OPARG_SKIP_PENDING_MASK));
