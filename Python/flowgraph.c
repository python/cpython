...
typedef struct _PyCfgBasicblock {
    ...
    unsigned b_warm : 1;
    /* Used by the borrow-safe analysis to mark visited block states. */
    int b_dfs_visited_gen;
    Py_ssize_t b_dfs_visited_depth;
} basicblock;
...
typedef enum {
    ...
    REF_UNCONSUMED  = 4,
    // The loaded reference is duplicated on the stack
    REF_COPIED      = 8,
} LoadFastInstrFlag;
...
is_borrow_safe(
    ...
    cfg_builder *g,
    Py_ssize_t stack_depth_on_entry,
    int dfs_generation);

static bool
is_exception_path_borrow_safe(
    cfg_instr *instr,
    Py_ssize_t current_target_depth,
    Py_ssize_t current_stack_depth,
    int original_local_idx,
    cfg_builder *g,
    int dfs_generation)
{
    basicblock *handler = instr->i_except;
    if (handler == NULL) {
        return true;
    }

    Py_ssize_t unwind_depth = handler->b_startdepth - 1;
    ...
    Py_ssize_t value_stack_index = current_stack_depth - current_target_depth - 1;
    if (value_stack_index < 0) {
        return false;
    }
    if (value_stack_index >= unwind_depth) {
        return true;
    }

    Py_ssize_t handler_entry_depth = handler->b_startdepth - value_stack_index - 1;
    return is_borrow_safe(
        handler, handler_entry_depth, original_local_idx, g,
        handler->b_startdepth, dfs_generation);
}
 ...
 *
 * The function uses the current DFS generation in conjunction with
 * 'current_block->b_dfs_visited_gen' and 'b_dfs_visited_depth' to detect
 * cycles within a specific DFS traversal. Revisiting the same block with the
 * same tracked-value depth is safe because the current traversal already
 * checked the loop body for local kills.
 *
 ...
 * - 'stack_depth_on_entry': The total operand stack depth upon entering
 *      'current_block'. This is used to determine whether exception unwinding
 *      pops the tracked item or leaves it live in the exception handler.
 *
 */
static bool
is_borrow_safe(
    ...
    int dfs_generation)
{
    ...
    for (int i = 0; i < current_block->b_iused; i++) {
        ...
        if (!is_exception_path_borrow_safe(
                instr, current_target_depth, current_stack_depth,
                original_local_idx, g, dfs_generation))
        {
            return false;
        }
        ...
        if (opcode == FOR_ITER && current_target_depth < num_popped) {
            if (current_block->b_next && BB_HAS_FALLTHROUGH(current_block)) {
                Py_ssize_t fallthrough_depth = current_target_depth -
                                               num_popped + num_pushed;
                Py_ssize_t fallthrough_stack_depth = current_stack_depth -
                                                     num_popped + num_pushed;
                return is_borrow_safe(
                    current_block->b_next, fallthrough_depth, original_local_idx,
                    g, fallthrough_stack_depth, dfs_generation);
            }
            return true;
        }
        ...
        current_stack_depth = current_stack_depth - num_popped + num_pushed;
        ...
        if (HAS_TARGET(opcode)) {
            ...
            Py_ssize_t stack_depth_before_jump_op_effect = stack_depth_on_entry;
            for(int k=0; k < i; ++k) {
                ...
                stack_depth_before_jump_op_effect = stack_depth_before_jump_op_effect - prev_popped_val +
                                                    _PyOpcode_num_pushed(prev_instr_in_block->i_opcode, prev_instr_in_block->i_oparg);
            }
            ...
            if (target_entry_depth < jump_popped_val) {
                ...
                Py_ssize_t target_entry_stack_depth = stack_depth_before_jump_op_effect -
                                                      jump_popped_val +
                                                      _PyOpcode_num_pushed(opcode, oparg);
                if (!is_borrow_safe(
                        instr->i_target, target_entry_depth, original_local_idx, g,
                        target_entry_stack_depth, dfs_generation))
                {
                    safe_on_all_branches = false;
                }
            }

            // Analyze fallthrough path, if the jump path was safe.
            if (safe_on_all_branches && current_block->b_next &&
                BB_HAS_FALLTHROUGH(current_block))
            {
                if (!is_borrow_safe(
                        current_block->b_next, current_target_depth, original_local_idx, g,
                        current_stack_depth, dfs_generation))
                {
                    safe_on_all_branches = false;
                }
            }
 ...
 *
 * A new DFS traversal is started by incrementing '*dfs_generation_counter' to
 * ensure that 'is_borrow_safe()' uses a fresh set of visited markers
...
check_borrow_safety_globally(
    ...
    int original_local_idx,
    Py_ssize_t stack_depth_at_producer_end,
    cfg_builder *g,
    int *dfs_generation_counter)
{
    int dfs_generation = ++*dfs_generation_counter;
    ...
    if (last_instr && HAS_TARGET(last_instr->i_opcode)) {
        stack_effects effects_noj;
        if (get_stack_effects(last_instr->i_opcode, last_instr->i_oparg, 0, &effects_noj) < 0) {
            return false;
        }
        int fall_popped = _PyOpcode_num_popped(last_instr->i_opcode, last_instr->i_oparg);
        int fall_pushed = _PyOpcode_num_pushed(last_instr->i_opcode, last_instr->i_oparg);
        Py_ssize_t depth_before_jump = depth_of_value_at_producer_end + fall_popped - fall_pushed;
        Py_ssize_t stack_depth_before_jump = stack_depth_at_producer_end + fall_popped - fall_pushed;

        stack_effects effects_jump;
        if (get_stack_effects(last_instr->i_opcode, last_instr->i_oparg, 1, &effects_jump) < 0) {
            return false;
        }
        int jump_popped = _PyOpcode_num_popped(last_instr->i_opcode, last_instr->i_oparg);
        Py_ssize_t entry_depth_for_target = depth_before_jump;
        ...
        if (entry_depth_for_target < jump_popped) {
            ...
            Py_ssize_t entry_stack_depth_for_target = stack_depth_before_jump -
                                                      jump_popped +
                                                      _PyOpcode_num_pushed(last_instr->i_opcode, last_instr->i_oparg);
            if (!is_borrow_safe(
                    last_instr->i_target, entry_depth_for_target, original_local_idx, g,
                    entry_stack_depth_for_target, dfs_generation))
            {
                overall_safety = false;
            }
        }
        ...
        // Analyze fallthrough path, if the jump path was safe.
        if (overall_safety && producer_block->b_next &&
            BB_HAS_FALLTHROUGH(producer_block))
        {
            if (!is_borrow_safe(
                    producer_block->b_next, depth_of_value_at_producer_end, original_local_idx, g,
                    stack_depth_at_producer_end, dfs_generation))
            {
                overall_safety = false;
            }
        }
...
static bool
has_nonterminal_target_after(basicblock *block, int instr_idx)
{
    for (int i = instr_idx + 1; i < block->b_iused - 1; i++) {
        if (HAS_TARGET(block->b_instr[i].i_opcode)) {
            return true;
        }
    }
    return false;
    ...
    while (sp != blocks) {
        ...
        for (int i = 0; i < block->b_iused; i++) {
            ...
            switch (opcode) {
                ...
                case COPY: {
                    ...
                    if (r.instr != DUMMY_INSTR) {
                        instr_flags[r.instr] |= REF_COPIED;
                    }
                    break;
                }
        ...
        for (int i = 0; i < block->b_iused; i++) {
            ...
            if (is_load_fast_type) {
                ...
                if (!unconsumed_in_block) {
                ...
                } else if (has_nonterminal_target_after(block, i)) {
                    can_borrow = false;
                } else {
                    if (instr->i_opcode == LOAD_FAST) {
                        ...
                        if (depth_from_tos_at_block_end != -1) {
                            can_borrow = check_borrow_safety_globally(
                                block, depth_from_tos_at_block_end, local_idx,
                        ...
                        if (depth1 != -1) {
                            if (!check_borrow_safety_globally(
                                    block, depth1, local_idx1, refs.size, g,
                                    &dfs_generation_counter))
                            {
                                can_borrow = false;
                            }
                        }
                        if (can_borrow && depth2 != -1) {
                            if (!check_borrow_safety_globally(
                                    block, depth2, local_idx2, refs.size, g,
                                    &dfs_generation_counter))
                            {
                                can_borrow = false;
                            }
