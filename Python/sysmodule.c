...
static PyObject *
sys_get_object(PyThreadState *tstate, PyObject *key)
{
    ...
    PyObject *value = PyDict_GetItemWithError(sd, key);
...
PyObject *
_PySys_GetObject2(PyObject *key)
{
    PyThreadState *tstate = _PyThreadState_GET();
    return sys_get_object(tstate, key);
}
...
int
_PySys_SetObject2(PyObject *key, PyObject *v)
{
...
/*[clinic end generated code: output=347477d006df92ed input=08ba730166d7ef72]*/
{
    ...
    if (PyObject_SetAttr(builtins, _Py_ID(_), Py_None) != 0)
        return NULL;
    outf = sys_get_object(tstate, _Py_ID(stdout));
...
{
    PyObject *warnoptions = sys_get_object(tstate, _Py_ID(warnoptions));
...
{
    PyObject *xoptions = sys_get_object(tstate, _Py_ID(_xoptions));
...
PySys_SetArgvEx(int argc, wchar_t **argv, int updatepath)
{
    ...
    if (updatepath) {
        ...
        if (_PyPathConfig_ComputeSysPath0(&argv_list, &path0)) {
            ...
            PyObject *sys_path = sys_get_object(tstate, _Py_ID(path));
            if (sys_path != NULL) {
...
sys_write(PyObject *key, FILE *fp, const char *format, va_list va)
{
    ...
    file = sys_get_object(tstate, key);
