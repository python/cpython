...
_PyCode_Quicken(PyCodeObject *code)
{
    ...
    for (int i = 0; i < Py_SIZE(code); i += OPSIZE) {
        ...
        if (caches) {
            instructions[i + OPSIZE] = adaptive_counter_warmup();
            previous_opcode = 0;
            i += caches;
            continue;
        }
        switch (previous_opcode << 8 | opcode) {
            case LOAD_CONST << 8 | LOAD_FAST:
                _Py_SET_OPCODE(instructions[i - OPSIZE], LOAD_CONST__LOAD_FAST);
                break;
            case LOAD_FAST << 8 | LOAD_CONST:
                _Py_SET_OPCODE(instructions[i - OPSIZE], LOAD_FAST__LOAD_CONST);
                break;
            case LOAD_FAST << 8 | LOAD_FAST:
                _Py_SET_OPCODE(instructions[i - OPSIZE], LOAD_FAST__LOAD_FAST);
                break;
            case STORE_FAST << 8 | LOAD_FAST:
                _Py_SET_OPCODE(instructions[i - OPSIZE], STORE_FAST__LOAD_FAST);
                break;
            case STORE_FAST << 8 | STORE_FAST:
                _Py_SET_OPCODE(instructions[i - OPSIZE], STORE_FAST__STORE_FAST);
                break;
        }
