...
_PyCode_Quicken(PyCodeObject *code)
{
    ...
    for (int i = 0; i < Py_SIZE(code); i += 2) {
        int opcode = _PyOpcode_Deopt[_Py_OPCODE(instructions[i])];
        int caches = _PyOpcode_Caches[opcode];
        if (caches) {
            instructions[i + 2] = adaptive_counter_warmup();
            previous_opcode = 0;
            i += caches;
            continue;
        }
        switch (previous_opcode << 8 | opcode) {
            case LOAD_CONST << 8 | LOAD_FAST:
                _Py_SET_OPCODE(instructions[i - 2], LOAD_CONST__LOAD_FAST);
                break;
            case LOAD_FAST << 8 | LOAD_CONST:
                _Py_SET_OPCODE(instructions[i - 2], LOAD_FAST__LOAD_CONST);
                break;
            case LOAD_FAST << 8 | LOAD_FAST:
                _Py_SET_OPCODE(instructions[i - 2], LOAD_FAST__LOAD_FAST);
                break;
            case STORE_FAST << 8 | LOAD_FAST:
                _Py_SET_OPCODE(instructions[i - 2], STORE_FAST__LOAD_FAST);
                break;
            case STORE_FAST << 8 | STORE_FAST:
                _Py_SET_OPCODE(instructions[i - 2], STORE_FAST__STORE_FAST);
                break;
        }
...
{
    _PyAttrCache *cache = (_PyAttrCache *)(instr + 2);
...
_Py_Specialize_LoadAttr(PyObject *owner, _Py_CODEUNIT *instr, PyObject *name)
{
    ...
    switch(kind) {
        ...
        case PROPERTY:
        {
            _PyLoadMethodCache *lm_cache = (_PyLoadMethodCache *)(instr + 2);
...
specialize_class_load_attr(PyObject *owner, _Py_CODEUNIT *instr,
                             PyObject *name)
{
    _PyLoadMethodCache *cache = (_PyLoadMethodCache *)(instr + 2);
    ...
    _Py_CODEUNIT *instr, PyObject *name)
{
    ...
    /* Use inline cache */
    _PyLoadGlobalCache *cache = (_PyLoadGlobalCache *)(instr + 2);
    assert(PyUnicode_CheckExact(name));
     ...
     PyObject *container, PyObject *sub, _Py_CODEUNIT *instr)
{
    ...
    _PyBinarySubscrCache *cache = (_PyBinarySubscrCache *)(instr + 2);
...
{
    _PyStoreSubscrCache *cache = (_PyStoreSubscrCache *)(instr + 2);
    PyTypeObject *container_type = Py_TYPE(container);
    if (container_type == &PyList_Type) {
        if (PyLong_CheckExact(sub)) {
...
{
    _PyCallCache *cache = (_PyCallCache *)(instr + 2);
    PyCodeObject *code = (PyCodeObject *)func->func_code;
    int kind = function_kind(code);
                        ...
                        int oparg, PyObject **locals)
{
    ...
    _PyBinaryOpCache *cache = (_PyBinaryOpCache *)(instr + 2);
    switch (oparg) {
        case NB_ADD:
        case NB_INPLACE_ADD:
            if (!Py_IS_TYPE(lhs, Py_TYPE(rhs))) {
                         ...
                         int oparg)
{
    ...
    _PyCompareOpCache *cache = (_PyCompareOpCache *)(instr + 2);
...
_Py_Specialize_UnpackSequence(PyObject *seq, _Py_CODEUNIT *instr, int oparg)
{
    ...
    _PyUnpackSequenceCache *cache = (_PyUnpackSequenceCache *)(instr + 2);
...
_Py_Specialize_ForIter(PyObject *iter, _Py_CODEUNIT *instr, int oparg)
{
    ...
    _PyForIterCache *cache = (_PyForIterCache *)(instr + 2);
    PyTypeObject *tp = Py_TYPE(iter);
    _Py_CODEUNIT next = instr[2+INLINE_CACHE_ENTRIES_FOR_ITER];
    ...
    if (tp == &PyListIter_Type) {
        ...
        assert(_Py_OPCODE(instr[oparg + INLINE_CACHE_ENTRIES_FOR_ITER + 2]) == END_FOR);
