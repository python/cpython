...
_imp__fix_co_filename(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (!PyObject_TypeCheck(args[0], &PyCode_Type)) {
        _PyArg_BadArgument("_fix_co_filename", 1, NULL, (&PyCode_Type)->tp_name, args[0]);
        goto exit;
    }
    code = (PyCodeObject *)args[0];
    if (!PyUnicode_Check(args[1])) {
        _PyArg_BadArgument("_fix_co_filename", 2, NULL, "str", args[1]);
        goto exit;
    }
...
_imp_init_frozen(PyObject *module, PyObject *arg)
{
    ...
    if (!PyUnicode_Check(arg)) {
        _PyArg_BadArgument("init_frozen", 0, NULL, "str", arg);
        goto exit;
    }
...
_imp_get_frozen_object(PyObject *module, PyObject *arg)
{
    ...
    if (!PyUnicode_Check(arg)) {
        _PyArg_BadArgument("get_frozen_object", 0, NULL, "str", arg);
        goto exit;
    }
    if (PyUnicode_READY(arg) == -1) {
        goto exit;
...
_imp_is_frozen_package(PyObject *module, PyObject *arg)
{
    ...
    if (!PyUnicode_Check(arg)) {
        _PyArg_BadArgument("is_frozen_package", 0, NULL, "str", arg);
        goto exit;
    }
    if (PyUnicode_READY(arg) == -1) {
        goto exit;
...
_imp_is_builtin(PyObject *module, PyObject *arg)
{
    ...
    if (!PyUnicode_Check(arg)) {
        _PyArg_BadArgument("is_builtin", 0, NULL, "str", arg);
        goto exit;
    }
    if (PyUnicode_READY(arg) == -1) {
        goto exit;
...
_imp_is_frozen(PyObject *module, PyObject *arg)
{
    ...
    if (!PyUnicode_Check(arg)) {
        _PyArg_BadArgument("is_frozen", 0, NULL, "str", arg);
        goto exit;
    }
    if (PyUnicode_READY(arg) == -1) {
        goto exit;
...
_imp_source_hash(PyObject *module, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    if (!PyBuffer_IsContiguous(&source, 'C')) {
        _PyArg_BadArgument("source_hash", 2, "source", "contiguous buffer", args[1]);
        goto exit;
    }
...
/*[clinic end generated code: output=d1baf0a8f43bf0c2 input=a9049054013a1b77]*/
