...
obj2ast_mod(PyObject* obj, mod_ty* out, PyArena* arena)
{
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Module field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Interactive field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Suite field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
...
obj2ast_stmt(PyObject* obj, stmt_ty* out, PyArena* arena)
{
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "FunctionDef field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "FunctionDef field \"decorator_list\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "AsyncFunctionDef field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "AsyncFunctionDef field \"decorator_list\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "ClassDef field \"bases\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "ClassDef field \"keywords\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "ClassDef field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "ClassDef field \"decorator_list\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Delete field \"targets\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Assign field \"targets\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "For field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "For field \"orelse\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "AsyncFor field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "AsyncFor field \"orelse\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "While field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "While field \"orelse\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "If field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "If field \"orelse\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "With field \"items\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "With field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "AsyncWith field \"items\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "AsyncWith field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Try field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Try field \"handlers\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Try field \"orelse\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Try field \"finalbody\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Import field \"names\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "ImportFrom field \"names\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Global field \"names\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Nonlocal field \"names\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
...
obj2ast_expr(PyObject* obj, expr_ty* out, PyArena* arena)
{
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "BoolOp field \"values\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Dict field \"keys\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Dict field \"values\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Set field \"elts\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "ListComp field \"generators\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "SetComp field \"generators\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "DictComp field \"generators\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "GeneratorExp field \"generators\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Compare field \"ops\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Compare field \"comparators\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Call field \"args\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Call field \"keywords\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "JoinedStr field \"values\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "List field \"elts\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "Tuple field \"elts\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
...
obj2ast_slice(PyObject* obj, slice_ty* out, PyArena* arena)
{
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "ExtSlice field \"dims\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
...
obj2ast_comprehension(PyObject* obj, comprehension_ty* out, PyArena* arena)
{
    ...
    if (tmp == NULL) {
        ...
        Py_ssize_t i;
        if (!PyList_Check(tmp)) {
            PyErr_Format(PyExc_TypeError, "comprehension field \"ifs\" must be a list, not a %.200s",
                         Py_TYPE(tmp)->tp_name);
            goto failed;
        }
        len = PyList_GET_SIZE(tmp);
...
obj2ast_excepthandler(PyObject* obj, excepthandler_ty* out, PyArena* arena)
{
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            Py_ssize_t i;
            if (!PyList_Check(tmp)) {
                PyErr_Format(PyExc_TypeError, "ExceptHandler field \"body\" must be a list, not a %.200s",
                             Py_TYPE(tmp)->tp_name);
                goto failed;
            }
            len = PyList_GET_SIZE(tmp);
...
obj2ast_arguments(PyObject* obj, arguments_ty* out, PyArena* arena)
{
    ...
    if (tmp == NULL) {
        ...
        Py_ssize_t i;
        if (!PyList_Check(tmp)) {
            PyErr_Format(PyExc_TypeError, "arguments field \"args\" must be a list, not a %.200s",
                         Py_TYPE(tmp)->tp_name);
            goto failed;
        }
        len = PyList_GET_SIZE(tmp);
    ...
    if (tmp == NULL) {
        ...
        Py_ssize_t i;
        if (!PyList_Check(tmp)) {
            PyErr_Format(PyExc_TypeError, "arguments field \"kwonlyargs\" must be a list, not a %.200s",
                         Py_TYPE(tmp)->tp_name);
            goto failed;
        }
        len = PyList_GET_SIZE(tmp);
    ...
    if (tmp == NULL) {
        ...
        Py_ssize_t i;
        if (!PyList_Check(tmp)) {
            PyErr_Format(PyExc_TypeError, "arguments field \"kw_defaults\" must be a list, not a %.200s",
                         Py_TYPE(tmp)->tp_name);
            goto failed;
        }
        len = PyList_GET_SIZE(tmp);
    ...
    if (tmp == NULL) {
        ...
        Py_ssize_t i;
        if (!PyList_Check(tmp)) {
            PyErr_Format(PyExc_TypeError, "arguments field \"defaults\" must be a list, not a %.200s",
                         Py_TYPE(tmp)->tp_name);
            goto failed;
        }
        len = PyList_GET_SIZE(tmp);
