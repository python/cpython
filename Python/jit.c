...
static size_t
calculate_jump_abs_offset(_PyUOpInstruction *trace, _PyUOpInstruction *jump_absolute)
{
...
_PyJIT_Compile(_PyExecutorObject *executor, _PyUOpInstruction *trace, size_t length)
{
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        if (instruction->opcode == _JUMP_ABSOLUTE) {
            assert(i + 1 == length);
            patches[HoleValue_CONTINUE] = (uint64_t)memory + calculate_jump_abs_offset(trace, instruction);
        }
        else {
