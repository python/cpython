    ...
    if (*arg && *arg == L'-' && *++arg) {
        ...
        if (argLen > 0) {
            if (STARTSWITH(L"2") || STARTSWITH(L"3")) {
                ...
                // (If the tag ends with -32, it will filter fine, so no more is needed)
                if (argLen > 3 && 0 == _compareArgument(&arg[argLen - 3], 3, L"-64", 3)) {
                    search->tagLength -= 3;
...
_selectEnvironment(const SearchInfo *search, EnvironmentInfo *env, EnvironmentInfo **best)
{
    ...
    while (env) {
        ...
        if (!search->oldStyleTag) {
            ...
            if (0 == _compare(env->company, -1, L"PythonCore", -1) &&
                _startsWith(env->tag, -1, search->tag, search->tagLength)) {
