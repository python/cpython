...
- The ``Android/makesetup`` script creates a Makefile and accepts the same
  ...
  may run ``makesetup`` for multiple (api, architecture) combinations in the
  same directory.

  The Makefile is used to build a Python distribution for an Android device when
  the environment variable ``DEVICE_PREFIXES`` is set at the time the Makefile
  was generated, or to build Python and run it on the Android emulator
  otherwise.

  Depending on whether ``DEVICE_PREFIXES`` is set at the time the Makefile is
...
=====================
Both ``configure-android`` and ``Android/makesetup`` scripts use the following
environment variables:
...
The Makefile generated by these two scripts is run according to the values of
these environment variables at the time its has been generated.
