...
static int
shift(stack *s, int type, char *str, int newstate, int lineno, int col_offset, int end_col_offset)
{
    int err;
    assert(!s_empty(s));
    err = PyNode_AddChild(s->s_top->s_parent, type, str, lineno, col_offset, end_col_offset);
    if (err)
        return err;
...
static int
push(stack *s, int type, dfa *d, int newstate, int lineno, int col_offset, int end_col_offset)
{
    int err;
    node *n;
    n = s->s_top->s_parent;
    assert(!s_empty(s));
    err = PyNode_AddChild(n, type, (char *)NULL, lineno, col_offset, end_col_offset);
    if (err)
        return err;
    s->s_top->s_state = newstate;
            ...
            if (x != -1) {
                if (x & (1<<7)) {
                    ...
                    if ((err = push(&ps->p_stack, nt, d1,
                        arrow, lineno, col_offset, end_col_offset)) > 0) {
                ...
                if ((err = shift(&ps->p_stack, type, str,
                                x, lineno, col_offset, end_col_offset)) > 0) {
