...
#define _tmp_90_type 1250
#define _loop0_92_type 1251
#define _gather_91_type 1252
#define _tmp_93_type 1253
#define _tmp_94_type 1254
#define _tmp_95_type 1255
#define _tmp_96_type 1256
#define _loop1_97_type 1257
#define _tmp_98_type 1258
#define _tmp_99_type 1259
#define _loop0_101_type 1260
#define _gather_100_type 1261
#define _loop1_102_type 1262
#define _loop0_103_type 1263
#define _loop0_104_type 1264
#define _loop0_106_type 1265
#define _gather_105_type 1266
#define _tmp_107_type 1267
...
#define _loop1_149_type 1309
#define _loop1_150_type 1310
...
static expr_ty star_element_rule(Parser *p);
...
// star_element: '*' bitwise_or | '(' '*' bitwise_or ')'
static expr_ty
star_element_rule(Parser *p)
{
    D(p->level++);
    if (p->error_indicator) {
        D(p->level--);
    ...
    { // '*' bitwise_or
        ...
        D(fprintf(stderr, "%*c> star_element[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "'*' bitwise_or"));
        Token * _literal;
        expr_ty a;
        if (
            (_literal = _PyPegen_expect_token(p, 16))  // token='*'
        ...
        {
            D(fprintf(stderr, "%*c+ star_element[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "'*' bitwise_or"));
            Token *_token = _PyPegen_get_last_nonnwhitespace_token(p);
            if (_token == NULL) {
                D(p->level--);
                return NULL;
        ...
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s star_element[%d-%d]: %s failed!\n", p->level, ' ',
...
// star_expression: star_element | expression
static expr_ty
star_expression_rule(Parser *p)
{
    ...
    { // star_element
        ...
        D(fprintf(stderr, "%*c> star_expression[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "star_element"));
        expr_ty star_element_var;
        if (
            (star_element_var = star_element_rule(p))  // star_element
        )
        {
            D(fprintf(stderr, "%*c+ star_expression[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "star_element"));
            _res = star_element_var;
            goto done;
        }
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s star_expression[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "star_element"));
    }
    { // expression
        if (p->error_indicator) {
            D(p->level--);
...
// star_named_expression: star_element | named_expression
static expr_ty
...
slices_rule(Parser *p)
{
    ...
    { // ','.slice+ ','?
        ...
        asdl_expr_seq* a;
        if (
            (a = (asdl_expr_seq*)_gather_91_rule(p))  // ','.slice+
            &&
            (_opt_var = _PyPegen_expect_token(p, 12), 1)  // ','?
        )
        {
...
for_if_clauses_rule(Parser *p)
{
    ...
    { // for_if_clause+
        ...
        if (
            (a = (asdl_comprehension_seq*)_loop1_102_rule(p))  // for_if_clause+
        )
        {
...
args_rule(Parser *p)
{
    ...
    { // ','.(starred_expression | named_expression !'=')+ [',' kwargs]
        ...
        void *b;
        if (
            (a = (asdl_expr_seq*)_gather_105_rule(p))  // ','.(starred_expression | named_expression !'=')+
            &&
...
star_targets_seq_rule(Parser *p)
{
    ...
    { // ','.star_target+ ','?
        ...
        asdl_expr_seq* a;
        if (
            (a = (asdl_expr_seq*)_gather_117_rule(p))  // ','.star_target+
            &&
            (_opt_var = _PyPegen_expect_token(p, 12), 1)  // ','?
        )
        {
...
del_targets_rule(Parser *p)
{
    ...
    { // ','.del_target+ ','?
        ...
        asdl_expr_seq* a;
        if (
            (a = (asdl_expr_seq*)_gather_120_rule(p))  // ','.del_target+
            &&
            (_opt_var = _PyPegen_expect_token(p, 12), 1)  // ','?
        )
        {
...
targets_rule(Parser *p)
{
    ...
    { // ','.target+ ','?
        ...
        asdl_expr_seq* a;
        if (
            (a = (asdl_expr_seq*)_gather_122_rule(p))  // ','.target+
            &&
            (_opt_var = _PyPegen_expect_token(p, 12), 1)  // ','?
        )
        {
...
invalid_assignment_rule(Parser *p)
{
    ...
    { // star_named_expression ',' star_named_expressions* ':' expression
        ...
        if (
            ...
            (_literal = _PyPegen_expect_token(p, 12))  // token=','
            &&
            (_loop0_125_var = _loop0_125_rule(p))  // star_named_expressions*
            &&
    ...
    { // ((star_targets '='))* star_expressions '='
        ...
        expr_ty a;
        if (
            (_loop0_126_var = _loop0_126_rule(p))  // ((star_targets '='))*
            &&
            (a = star_expressions_rule(p))  // star_expressions
            &&
            (_literal = _PyPegen_expect_token(p, 22))  // token='='
...
invalid_lambda_parameters_rule(Parser *p)
{
    ...
    { // lambda_param_no_default* (lambda_slash_with_default | lambda_param_with_default+) lambda_param_no_default
        ...
        if (
            (_loop0_132_var = _loop0_132_rule(p))  // lambda_param_no_default*
            &&
            (_tmp_133_var = _tmp_133_rule(p))  // lambda_slash_with_default | lambda_param_with_default+
            &&
...
_loop0_106_rule(Parser *p)
{
    ...
    { // ',' (starred_expression | named_expression !'=')
        ...
        while (
            (_literal = _PyPegen_expect_token(p, 12))  // token=','
            &&
            (elem = _tmp_145_rule(p))  // starred_expression | named_expression !'='
        )
        {
            _res = elem;
            if (_res == NULL && PyErr_Occurred()) {
...
_gather_120_rule(Parser *p)
{
    ...
    asdl_seq * _res = NULL;
    int _mark = p->mark;
    { // del_target _loop0_121
        if (p->error_indicator) {
            D(p->level--);
            return NULL;
        }
        D(fprintf(stderr, "%*c> _gather_120[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "del_target _loop0_121"));
        expr_ty elem;
        asdl_seq * seq;
        if (
            (elem = del_target_rule(p))  // del_target
        ...
        D(fprintf(stderr, "%*c%s _gather_120[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "del_target _loop0_121"));
    }
    _res = NULL;
  done:
    D(p->level--);
...
_gather_122_rule(Parser *p)
{
    ...
    { // target _loop0_123
        ...
        D(fprintf(stderr, "%*c> _gather_122[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "target _loop0_123"));
        expr_ty elem;
        asdl_seq * seq;
        if (
            (elem = target_rule(p))  // target
        ...
        D(fprintf(stderr, "%*c%s _gather_122[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "target _loop0_123"));
    }
    _res = NULL;
  done:
    D(p->level--);
...
_loop0_126_rule(Parser *p)
{
    ...
    { // (star_targets '=')
        ...
        void *_tmp_147_var;
        while (
            (_tmp_147_var = _tmp_147_rule(p))  // star_targets '='
        )
        {
            _res = _tmp_147_var;
            if (_n == _children_capacity) {
...
_tmp_133_rule(Parser *p)
{
    ...
    { // lambda_param_with_default+
        ...
        if (
            (_loop1_150_var = _loop1_150_rule(p))  // lambda_param_with_default+
        )
        {
            D(fprintf(stderr, "%*c+ _tmp_133[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "lambda_param_with_default+"));
            _res = _loop1_150_var;
...
// _tmp_145: starred_expression | named_expression !'='
static void *
