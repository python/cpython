...
#define _tmp_129_type 1280
#define _tmp_130_type 1281
#define _tmp_131_type 1282
#define _loop0_132_type 1283
#define _tmp_133_type 1284
...
// bitwise_or: factor ((operator factor))+ | factor
static expr_ty
bitwise_or_rule(Parser *p)
{
    ...
    { // factor ((operator factor))+
        ...
        if (
            ...
            &&
            (b = _loop1_93_rule(p))  // ((operator factor))+
        )
        {
        ...
        D(fprintf(stderr, "%*c%s bitwise_or[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "factor ((operator factor))+"));
    }
    { // factor
        if (p->error_indicator) {
            D(p->level--);
...
primary_rule(Parser *p)
{
    ...
    int _resmark = p->mark;
    while (1) {
        int tmpvar_1 = _PyPegen_update_memo(p, _mark, primary_type, _res);
        if (tmpvar_1) {
            D(p->level--);
            return _res;
        }
...
t_primary_rule(Parser *p)
{
    ...
    int _resmark = p->mark;
    while (1) {
        int tmpvar_2 = _PyPegen_update_memo(p, _mark, t_primary_type, _res);
        if (tmpvar_2) {
            D(p->level--);
            return _res;
        }
...
_loop1_93_rule(Parser *p)
{
    ...
    { // (operator factor)
        ...
        void *_tmp_144_var;
        while (
            (_tmp_144_var = _tmp_144_rule(p))  // operator factor
        )
        {
            _res = _tmp_144_var;
            if (_n == _children_capacity) {
        ...
        D(fprintf(stderr, "%*c%s _loop1_93[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "(operator factor)"));
    }
    if (_n == 0 || p->error_indicator) {
        PyMem_Free(_children);
        D(p->level--);
...
_tmp_144_rule(Parser *p)
{
    ...
    { // operator factor
        ...
        void *o;
        if (
            (o = operator_rule(p))  // operator
            &&
            (f = factor_rule(p))  // factor
        )
        {
