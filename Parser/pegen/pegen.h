...
RAISE_ERROR_KNOWN_LOCATION(Parser *p, PyObject *errtype, int with_col_number,
                            int lineno, int col_offset, const char *errmsg, ...)
{
    va_list va;
    va_start(va, errmsg);
    _PyPegen_raise_error_known_location(p, errtype, with_col_number,
                                        lineno, col_offset + 1, errmsg, va);
    va_end(va);
    return NULL;
}
...
#define RAISE_SYNTAX_ERROR_KNOWN_LOCATION(a, msg, ...) \
    RAISE_ERROR_KNOWN_LOCATION(p, PyExc_SyntaxError, 0, a->lineno, a->col_offset, msg, ##__VA_ARGS__)
