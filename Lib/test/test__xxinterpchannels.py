...
def _run_action(cid, action, end, state):
    if action == 'use':
        if end == 'send':
            channels.send(cid, b'spam', wait=False)
            return state.incr()
        elif end == 'recv':
...
class ChannelIDTests(TestBase):
    ...
    def test_shareable(self):
        ...
        obj = channels.create()
        channels.send(chan, obj, wait=False)
...
class ChannelTests(TestBase):
    ...
    def test_channel_list_interpreters_basic(self):
        ...
        cid = channels.create()
        channels.send(cid, "send", wait=False)
    ...
    def test_channel_list_interpreters_multiple(self):
        ...
        _run_output(interp1, dedent(f"""
            import _xxinterpchannels as _channels
            _channels.send({cid}, "send", wait=False)
            """))
    ...
    def test_channel_list_interpreters_released(self):
        ...
        interp2 = interpreters.create()
        cid = channels.create()
        channels.send(cid, "data", wait=False)
        _run_output(interp1, dedent(f"""
            import _xxinterpchannels as _channels
            obj = _channels.recv({cid})
            """))
    ...
    def test_send_recv_main(self):
        cid = channels.create()
        orig = b'spam'
        channels.send(cid, orig, wait=False)
    ...
    def test_send_recv_same_interpreter(self):
        ...
        out = _run_output(id1, dedent("""
            ...
            cid = _channels.create()
            orig = b'spam'
            _channels.send(cid, orig, wait=False)
            obj = _channels.recv(cid)
    ...
    def test_send_recv_different_interpreters(self):
        ...
        out = _run_output(id1, dedent(f"""
            import _xxinterpchannels as _channels
            _channels.send({cid}, b'spam', wait=False)
            """))
        obj = channels.recv(cid)
    ...
    def test_recv_default(self):
        ...
        channels.send(cid, None, wait=False)
        channels.send(cid, 1, wait=False)
        channels.send(cid, b'spam', wait=False)
        channels.send(cid, b'eggs', wait=False)
    ...
    def test_recv_sending_interp_destroyed(self):
        with self.subTest('closed'):
            ...
            interpreters.run_string(interp, dedent(f"""
                import _xxinterpchannels as _channels
                _channels.send({cid1}, b'spam', wait=False)
                """))
        ...
        with self.subTest('still open'):
            ...
            interpreters.run_string(interp, dedent(f"""
                import _xxinterpchannels as _channels
                _channels.send({cid2}, b'spam', wait=False)
                """))
            channels.send(cid2, b'eggs', wait=False)
            interpreters.destroy(interp)
    ...
    def test_send_buffer(self):
        ...
        cid = channels.create()
        channels.send_buffer(cid, buf, wait=False)
        obj = channels.recv(cid)
    ...
    def test_allowed_types(self):
        ...
        for obj in objects:
            with self.subTest(obj):
                channels.send(cid, obj, wait=False)
    ...
    def test_run_string_arg_resolved(self):
        ...
        out = _run_output(interp, dedent("""
            import _xxinterpchannels as _channels
            print(chan.id.end)
            _channels.send(chan.id, b'spam', wait=False)
            """),
            dict(chan=cid.send))
        obj = channels.recv(cid)
    ...
    def test_close_defaults_with_unused_items(self):
        cid = channels.create()
        channels.send(cid, b'spam', wait=False)
        channels.send(cid, b'ham', wait=False)
...
class ChannelReleaseTests(TestBase):
    ...
    def test_close_if_unassociated(self):
        ...
        interpreters.run_string(interp, dedent(f"""
            import _xxinterpchannels as _channels
            obj = _channels.send({cid}, b'spam', wait=False)
            _channels.release({cid})
            """))
    ...
    def _new_channel(self, creator):
        if creator.name == 'main':
            ...
            run_interp(creator.id, f"""
                ...
                _xxsubchannels.send({ch}, int(cid), wait=False)
    ...
    def run_action(self, fix, action, *, hideclosed=True):
        ...
        if interp.name == 'main':
            ...
            run_interp(interp.id, f"""
                ...
                channels.send({_cid}, result.pending.to_bytes(1, 'little'), wait=False)
                channels.send({_cid}, b'X' if result.closed else b'', wait=False)
                """)
            result = ChannelState(
