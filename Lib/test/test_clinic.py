...
class LimitedCAPIOutputTests(unittest.TestCase):
    ...
    def test_limited_capi_required_keyword_only(self):
        ...
        """)
        generated = self.clinic.parse(block)
        # "|" after "$" ends the required keyword-only parameters.
    ...
    def test_limited_capi_no_optional_keyword_only(self):
        block = self.wrap_clinic_input("""
            func
                a: object
                b: object = None
        ...
        """)
        generated = self.clinic.parse(block)
        # A trailing "|" is needed to end the required keyword-only parameters.
        self.assertIn('"O|O$O|:func"', generated)

    def test_limited_capi_keyword_only(self):
        # A lone "$" is enough if it can express the signature.
        block = self.wrap_clinic_input("""
            func
                a: object
                *
