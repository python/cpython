...
class TestMarkingVariablesAsKnown(BytecodeTestCase):
    ...
    def test_load_fast_known_simple(self):
        def f():
            x = 1
            y = x + x
        self.assertInBytecode(f, 'LOAD_FAST_KNOWN')
    ...
    def test_load_fast_known_because_parameter(self):
        def f1(x):
            print(x)
        self.assertInBytecode(f1, 'LOAD_FAST_KNOWN')
        self.assertNotInBytecode(f1, 'LOAD_FAST')

        def f2(*, x):
            print(x)
        self.assertInBytecode(f2, 'LOAD_FAST_KNOWN')
        self.assertNotInBytecode(f2, 'LOAD_FAST')

        def f3(*args):
            print(args)
        self.assertInBytecode(f3, 'LOAD_FAST_KNOWN')
        self.assertNotInBytecode(f3, 'LOAD_FAST')

        def f4(**kwargs):
            print(kwargs)
        self.assertInBytecode(f4, 'LOAD_FAST_KNOWN')
        self.assertNotInBytecode(f4, 'LOAD_FAST')

        def f5(x=0):
            print(x)
        self.assertInBytecode(f5, 'LOAD_FAST_KNOWN')
        self.assertNotInBytecode(f5, 'LOAD_FAST')
    ...
    def test_load_fast_unknown_after_error(self):
        def f():
            try:
                ...
                pass
            return res
        # LOAD_FAST_KNOWN actually does occur: in the no-exception branch
        # Assert that it doesn't occur in the LOAD_FAST branch
        self.assertInBytecode(f, 'LOAD_FAST')
