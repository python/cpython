...
class RFCTestCasesMixin(TestVectorsMixin):
    ...
    def test_md5(self):
        def md5test(key, msg, digest):
            self.assert_hmac(key, msg, digest, self.md5, "md5", 16, 64)
    ...
    def test_sha1(self):
        def shatest(key, msg, digest):
            self.assert_hmac(key, msg, digest, self.sha1, "sha1", 20, 64)
