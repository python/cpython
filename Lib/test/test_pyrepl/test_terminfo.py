...
class TestCursesCompatibility(unittest.TestCase):
    ...
    def test_setupterm_basic(self):
        ...
        for term in test_terms:
            with self.subTest(term=term):
                ...
                result = subprocess.run([sys.executable, '-c', ncurses_code],
                ...
                if std_success:
                    self.assertTrue(pyrepl_success,
    ...
    def test_tigetstr_common_capabilities(self):
        ...
        all_caps = []
        try:
            result = subprocess.run(['infocmp', '-1', term],
        ...
        result = subprocess.run([sys.executable, '-c', ncurses_code],
                              capture_output=True, text=True)
        self.assertEqual(result.returncode, 0, f"Failed to run ncurses: {result.stderr}")
        ...
        for cap in all_caps:
            ...
            with self.subTest(capability=cap):
                ...
                self.assertEqual(pyrepl_val, ncurses_val,
    ...
    def test_tigetstr_input_types(self):
        ...
        self.assertEqual(pyrepl_accepts_str, ncurses_data["accepts_str"],
                        "PyREPL and standard curses should have same string handling")
        self.assertTrue(pyrepl_accepts_str, "PyREPL should accept string input")
        ...
        if cup and cup != -1:
            ...
            for row, col in test_cases:
                with self.subTest(row=row, col=col):
                    ...
                    key = f"{row},{col}"
                    if isinstance(ncurses_data[key], dict) and "error" in ncurses_data[key]:
                        self.fail(f"ncurses tparm failed: {ncurses_data[key]['error']}")
                    ...
                    self.assertEqual(pyrepl_result, std_result,
                                   f"tparm(cup, {row}, {col}): "
                                   f"std={repr(std_result)}, pyrepl={repr(pyrepl_result)}")
        else:
            raise unittest.SkipTest("test_tparm_basic() requires the `cup` capability")
    ...
    def test_special_terminals(self):
        ...
        try:
            ...
            result = subprocess.run(['infocmp', '-1', 'xterm'],
                                  capture_output=True, text=True, check=True)
            for line in result.stdout.splitlines():
                line = line.strip()
                if '=' in line:
            ...
            all_caps = ['cup', 'clear', 'el', 'cub', 'cuf', 'cud', 'cuu',
                       'cub1', 'cuf1', 'cud1', 'cuu1', 'home', 'bel',
                       'ind', 'ri', 'nel', 'cr', 'ht', 'hpa', 'vpa',
                       'dch', 'dch1', 'dl', 'dl1', 'ich', 'ich1', 'il', 'il1',
                       'sgr0', 'smso', 'rmso', 'smul', 'rmul', 'bold', 'rev',
                       'blink', 'dim', 'smacs', 'rmacs', 'civis', 'cnorm',
                       'sc', 'rc', 'hts', 'tbc', 'ed', 'kbs', 'kcud1', 'kcub1',
                       'kcuf1', 'kcuu1', 'kdch1', 'khome', 'kend', 'knp', 'kpp',
                       'kich1', 'kf1', 'kf2', 'kf3', 'kf4', 'kf5', 'kf6', 'kf7',
                       'kf8', 'kf9', 'kf10', 'rmkx', 'smkx']
        ...
        for term in special_terms:
            with self.subTest(term=term):
                ...
                if result.returncode != 0:
                    self.fail(f"Failed to get ncurses data for {term}: {result.stderr}")

                try:
                    ncurses_data = json.loads(result.stdout)
                except json.JSONDecodeError:
                    self.fail(f"Failed to parse ncurses output for {term}: {result.stdout}")
    ...
    def test_terminfo_fallback(self):
        ...
        if ncurses_data["success"]:
            ...
            self.skipTest(f"System unexpectedly has terminfo for '{fake_term}'")
        ...
        self.assertTrue(pyrepl_ok, "PyREPL should fall back for unknown terminals")

        # Should still be able to get basic capabilities
        bel = ti.get('bel')
        self.assertIsNotNone(bel, "PyREPL should provide basic capabilities after fallback")
