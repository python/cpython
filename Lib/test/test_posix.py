...
class PosixTester(unittest.TestCase):
    ...
    def test_mknod(self):
        ...
        os_helper.unlink(os_helper.TESTFN)
        try:
            posix.mknod(path=os_helper.TESTFN, mode=, device=0, dir_fd=None)
        except OSError as e:
            self.assertIn(e.errno, (errno.EPERM, errno.EINVAL, errno.EACCES))
...
class TestPosixDirFd(unittest.TestCase):
    ...
    def test_access_dir_fd(self):
        with self.prepare_file() as (dir_fd, name, fullname):
            self.assertTrue(posix.access(name, os.R_OK, dir_fd=))
    ...
    def test_chmod_dir_fd(self):
        with self.prepare_file() as (dir_fd, name, fullname):
            ...
            posix.chmod(name, stat.S_IRUSR | stat.S_IWUSR, dir_fd=)
    ...
    def test_chown_dir_fd(self):
        with self.prepare_file() as (dir_fd, name, fullname):
            posix.chown(name, os.getuid(), os.getgid(), dir_fd=)
    ...
    def test_utime_dir_fd(self):
        with self.prepare_file() as (dir_fd, name, fullname):
            now = time.time()
            posix.utime(name, None, dir_fd=)
            posix.utime(name, dir_fd=)
            self.assertRaises(TypeError, posix.utime, name, now, dir_fd=)
            ...
            self.assertRaises(TypeError, posix.utime, name,
                              (now, "x"), dir_fd=)
            posix.utime(name, (int(now), int(now)), dir_fd=)
            posix.utime(name, (now, now), dir_fd=)
            posix.utime(name, (int(now), int((now - int(now)) * 1e9)), dir_fd=)
            posix.utime(name, dir_fd=,
            ...
            if os.utime in os.supports_follow_symlinks:
                try:
                    posix.utime(name, follow_symlinks=False, dir_fd=)
                except ValueError:
    ...
    def test_mknod_dir_fd(self):
        ...
        with self.prepare() as (dir_fd, name, fullname):
            mode = stat.S_IFIFO | stat.S_IRUSR | stat.S_IWUSR
            try:
                posix.mknod(name, mode, 0, dir_fd=)
            except OSError as e:
                # Some old systems don't allow unprivileged users to use
                # mknod(), or only support creating device nodes.
                self.assertIn(e.errno, (errno.EPERM, errno.EINVAL, errno.EACCES))
    ...
    def test_open_dir_fd(self):
        with self.prepare() as (dir_fd, name, fullname):
            ...
            self.addCleanup(posix.unlink, fullname)
            fd = posix.open(name, posix.O_RDONLY, dir_fd=)
            try:
                res = posix.read(fd, 9)
    ...
    def test_readlink_dir_fd(self):
        with self.prepare() as (dir_fd, name, fullname):
            ...
            self.addCleanup(posix.unlink, fullname)
            self.assertEqual(posix.readlink(name, dir_fd=), 'symlink')
    ...
    def test_symlink_dir_fd(self):
        with self.prepare() as (dir_fd, name, fullname):
            posix.symlink('symlink', name, dir_fd=)
            self.addCleanup(posix.unlink, fullname)
    ...
    def test_mkfifo_dir_fd(self):
        with self.prepare() as (dir_fd, name, fullname):
            try:
                posix.mkfifo(name, stat.S_IRUSR | stat.S_IWUSR, dir_fd=)
            except PermissionError as e:
                self.skipTest('posix.mkfifo(): %s' % e)
            self.addCleanup(posix.unlink, fullname)
            self.assertTrue(stat.S_ISFIFO(posix.stat(fullname).st_mode))
...
class _PosixSpawnMixin:
    ...
    def test_open_file(self):
        ...
        args = self.python_args('-c', script)
        pid = self.spawn_func(args[0], args, os.environ, file_actions=)
