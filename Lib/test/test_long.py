...
class LongTest(unittest.TestCase):
    ...
    def test_int64_boundary_sub(self):
        # Verify _PyLong_TryAsInt64Exact handles INT64 extremes for subtraction.
        INT64_MAX = (1 << 63) - 1
        INT64_MIN = -(1 << 63)

        # INT64_MIN: subtraction that stays within range
        self.assertEqual(INT64_MIN - 0, INT64_MIN)
        self.assertEqual(INT64_MIN - (-1), INT64_MIN + 1)

        # INT64_MAX: extraction must succeed and arithmetic must be correct
        self.assertEqual(INT64_MAX - 0, INT64_MAX)
        self.assertEqual(INT64_MAX - 1, INT64_MAX - 1)

        # Subtracting values that overflow int64 falls back to slow path
        self.assertEqual(INT64_MIN - 1, INT64_MIN - 1)
        self.assertEqual(INT64_MAX - (-1), 1 << 63)
