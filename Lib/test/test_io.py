...
class IOTest(unittest.TestCase):
    ...
    def test_invalid_operations(self):
        ...
        with self.open(os_helper.TESTFN, "rb") as fp:
            self.assertRaises(exc, fp.write, b"blah")
            self.assertRaises(exc, fp.writelines, [b"blah\n"])
        with self.open(os_helper.TESTFN, "r", encoding="ascii") as fp:
    ...
    def test_open_handles_NUL_chars(self):
        ...
        self.assertRaises(ValueError, self.open, fn_with_NUL, 'w', encoding="ascii")

        bytes_fn = bytes(fn_with_NUL, 'ascii')
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", DeprecationWarning)
            self.assertRaises(ValueError, self.open, bytes_fn, 'w', encoding="ascii")
    ...
    def test_readline(self):
        ...
        with self.open(os_helper.TESTFN, "r", encoding="ascii") as f:
            self.assertRaises(TypeError, f.readline, 5.3)
    ...
    def test_append_mode_tell(self):
        ...
        with self.open(os_helper.TESTFN, "ab") as f:
            self.assertEqual(f.tell(), 3)
        with self.open(os_helper.TESTFN, "a", encoding="ascii") as f:
    ...
    def test_closefd(self):
        self.assertRaises(ValueError, self.open, os_helper.TESTFN, 'w',
                          encoding="ascii", closefd=False)

    def test_read_closed(self):
        with self.open(os_helper.TESTFN, "w", encoding="ascii") as f:
            f.write("egg\n")
        with self.open(os_helper.TESTFN, "r", encoding="ascii") as f:
            file = self.open(f.fileno(), "r", encoding="ascii", closefd=False)
    ...
    def test_flush_error_on_close(self):
        ...
        os.close(fd)
        # text io
        self.check_flush_error_on_close(os_helper.TESTFN, 'w', encoding="ascii")
        fd = os.open(os_helper.TESTFN, os.O_WRONLY|os.O_CREAT)
        self.check_flush_error_on_close(fd, 'w', encoding="ascii")
        fd = os.open(os_helper.TESTFN, os.O_WRONLY|os.O_CREAT)
        self.check_flush_error_on_close(fd, 'w', encoding="ascii", closefd=False)
        os.close(fd)
    ...
    def test_opener(self):
        ...
        def opener(path, flags):
            return fd
        with self.open("non-existent", "r", encoding="ascii", opener=opener) as f:
    ...
    def test_nonbuffered_textio(self):
        with warnings_helper.check_no_resource_warning(self):
            with self.assertRaises(ValueError):
                self.open(os_helper.TESTFN, 'w', encoding="ascii", buffering=0)

    def test_invalid_newline(self):
        with warnings_helper.check_no_resource_warning(self):
            with self.assertRaises(ValueError):
                self.open(os_helper.TESTFN, 'w', encoding="ascii", newline='invalid')
    ...
    def test_fspath_support(self):
        def check_path_succeeds(path):
            with self.open(path, "w", encoding="ascii") as f:
                f.write("egg\n")

            with self.open(path, "r", encoding="ascii") as f:
                self.assertEqual(f.read(), "egg\n")
        ...
        with self.open(os_helper.TESTFN, "w", encoding="ascii") as f:
            ...
            with self.assertRaises(TypeError):
                self.open(bad_path, 'w', encoding="ascii")
        ...
        with self.assertRaisesRegex(ValueError, 'read/write/append mode'):
            self.open(FakePath(os_helper.TESTFN), 'rwxa', encoding="ascii")
...
class TextIOWrapperTest(unittest.TestCase):
    ...
    def test_uninitialized(self):
        ...
        self.assertRaisesRegex((ValueError, AttributeError),
                               'uninitialized|has no attribute',
                               t.read, 0)
        t.__init__(self.MockRawIO(), encoding="ascii")
    ...
    def test_recursive_repr(self):
        # Issue #25455
        raw = self.BytesIO()
        t = self.TextIOWrapper(raw, encoding="ascii")
    ...
    def test_line_buffering(self):
        ...
        t = self.TextIOWrapper(b, encoding="ascii", newline="\n", line_buffering=True)
        t.write("X")
    ...
    def test_error_through_destructor(self):
        ...
        with support.catch_unraisable_exception() as cm:
            with self.assertRaises(AttributeError):
                self.TextIOWrapper(rawio, encoding="ascii").xyzzy
    ...
    def test_unreadable(self):
        class UnReadable(self.BytesIO):
            def readable(self):
                return False
        txt = self.TextIOWrapper(UnReadable(), encoding="ascii")
    ...
    def test_writelines(self):
        l = ['ab', 'cd', 'ef']
        buf = self.BytesIO()
        txt = self.TextIOWrapper(buf, encoding="ascii")
        txt.writelines(l)
        txt.flush()
    ...
    def test_writelines_error(self):
        txt = self.TextIOWrapper(self.BytesIO(), encoding="ascii")
    ...
    def test_errors_property(self):
        ...
        with self.open(os_helper.TESTFN, "w", encoding="ascii", errors="replace") as f:
    ...
    def test_threads_write(self):
        ...
        with self.open(os_helper.TESTFN, "w", encoding="ascii", buffering=1) as f:
            def run(n):
                text = "Thread%03d\n" % n
                event.wait()
                f.write(text)
            ...
            with threading_helper.start_threads(threads, event.set):
                time.sleep(0.02)
        with self.open(os_helper.TESTFN, encoding="ascii") as f:
            content = f.read()
            for n in range(20):
    ...
    def test_unseekable(self):
        txt = self.TextIOWrapper(self.MockUnseekableIO(self.testdata), encoding="ascii")
    ...
    def test_read_nonbytes(self):
        ...
        t = self.TextIOWrapper(self.StringIO('a'), encoding="ascii")
...
class MiscIOTest(unittest.TestCase):
    ...
    def test_attributes(self):
        ...
        with warnings_helper.check_warnings(('', DeprecationWarning)):
            f = self.open(os_helper.TESTFN, "U", encoding="ascii")
        ...
        f = self.open(os_helper.TESTFN, "w+", encoding="ascii")
    ...
    def test_open_pipe_with_append(self):
        ...
        r, w = os.pipe()
        self.addCleanup(os.close, r)
        f = self.open(w, 'a', encoding="ascii")
    ...
    def test_warn_on_dealloc(self):
        ...
        self._check_warn_on_dealloc(os_helper.TESTFN, "w", encoding="ascii")
    ...
    def test_warn_on_dealloc_fd(self):
        ...
        self._check_warn_on_dealloc_fd("r", encoding="ascii")
    ...
    def test_create_fail(self):
        ...
        with self.open(os_helper.TESTFN, 'w', encoding="ascii"):
            pass
        self.assertRaises(FileExistsError, self.open, os_helper.TESTFN, 'x', encoding="ascii")
    ...
    def test_open_allargs(self):
        ...
        self.assertRaises(ValueError, self.open, os_helper.TESTFN, 'rwax+', encoding="ascii")
