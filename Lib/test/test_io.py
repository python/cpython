...
class IOTest(unittest.TestCase):
    ...
    def test_fspath_support(self):
        ...
        check_path_succeeds(filesystem_helper.FakePath(
                            filesystem_helper.TESTFN))
        check_path_succeeds(filesystem_helper.FakePath(
                            filesystem_helper.TESTFN.encode('utf-8')))

        with self.open(filesystem_helper.TESTFN, "w") as f:
            bad_path = filesystem_helper.FakePath(f.fileno())
            with self.assertRaises(TypeError):
                self.open(bad_path, 'w')

        bad_path = filesystem_helper.FakePath(None)
        with self.assertRaises(TypeError):
            self.open(bad_path, 'w')

        bad_path = filesystem_helper.FakePath(FloatingPointError)
        with self.assertRaises(FloatingPointError):
            self.open(bad_path, 'w')

        # ensure that refcounting is correct with some error conditions
        with self.assertRaisesRegex(ValueError, 'read/write/append mode'):
            self.open(filesystem_helper.FakePath(filesystem_helper.TESTFN), 'rwxa')
...
class BufferedWriterTest(unittest.TestCase, CommonBufferedTests):
    ...
    def test_truncate(self):
        ...
        self.addCleanup(filesystem_helper.unlink, filesystem_helper.TESTFN)
        with self.open(filesystem_helper.TESTFN, self.write_mode, buffering=0) as raw:
            bufio = self.tp(raw, 8)
    ...
    def test_truncate_after_write(self):
        ...
        for buffer_size in buffer_sizes:
            with self.open(filesystem_helper.TESTFN, "r+b", buffering=buffer_size) as f:
...
class MiscIOTest(unittest.TestCase):
    ...
    def test_warn_on_dealloc(self):
        self._check_warn_on_dealloc(filesystem_helper.TESTFN, "wb", buffering=0)
    ...
    def test_create_fail(self):
        ...
        with self.open(filesystem_helper.TESTFN, 'w'):
            pass
        self.assertRaises(FileExistsError, self.open, filesystem_helper.TESTFN, 'x')
