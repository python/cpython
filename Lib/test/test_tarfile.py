...
class ReadTest(TarTest):
    ...
    def setUp(self):
        self.tar = self.tarfile_open(self.tarname, mode=self.mode,
                                encoding="iso8859-1")
...
class SafeTarFileTest(unittest.TestCase, TarFileTest):
...
class UstarReadTest(UstarReadTestBase, unittest.TestCase, TarFileTest):
    pass

class GzipUstarReadTest(GzipTest, UstarReadTestBase, unittest.TestCase, TarFileTest):
    pass

class Bz2UstarReadTest(Bz2Test, UstarReadTestBase, unittest.TestCase, TarFileTest):
    pass

class LzmaUstarReadTest(LzmaTest, UstarReadTestBase, unittest.TestCase, TarFileTest):
    pass
...
class ListTest(ListTestBase, unittest.TestCase, TarFileTest):
    pass

class GzipListTest(GzipTest, ListTestBase, unittest.TestCase, TarFileTest):
    pass

class Bz2ListTest(Bz2Test, ListTestBase, unittest.TestCase, TarFileTest):
    pass

class LzmaListTest(LzmaTest, ListTestBase, unittest.TestCase, TarFileTest):
    pass
...
class CommonReadTest(ReadTest):
    ...
    def test_empty_tarfile(self):
        ...
        # This test checks if self.tarfile_open() is able to open an empty tar
        # archive successfully. Note that an empty tar archive is not the
        # same as an empty file!
        with self.tarfile_open(tmpname, self.mode.replace("r", "w")):
            pass
        try:
            tar = self.tarfile_open(tmpname, self.mode)
            tar.getnames()
        except tarfile.ReadError:
            self.fail("self.tarfile_open() failed on empty archive")
        else:
    ...
    def test_non_existent_tarfile(self):
        ...
        with self.assertRaisesRegex(FileNotFoundError, "xxx"):
            self.tarfile_open("xxx", self.mode)

    def test_null_tarfile(self):
        # Test for issue6123: Allow opening empty archives.
        # This test guarantees that self.tarfile_open() does not treat an empty
        # file as an empty tar archive.
        with open(tmpname, "wb"):
            pass
        self.assertRaises(tarfile.ReadError, self.tarfile_open, tmpname, self.mode)
        self.assertRaises(tarfile.ReadError, self.tarfile_open, tmpname)
    ...
    def test_ignore_zeros(self):
        ...
        for char in (b'\0', b'a'):
            ...
            tar = self.tarfile_open(tmpname, mode="r", ignore_zeros=True)
            try:
    ...
    def test_premature_end_of_archive(self):
        for size in (512, 600, 1024, 1200):
            with self.tarfile_open(tmpname, "w:") as tar:
            ...
            with self.tarfile_open(tmpname) as tar:
...
class MiscReadTestBase(CommonReadTest):
    ...
    def test_no_name_argument(self):
        ...
        with open(self.tarname, "rb") as fobj:
            ...
            with self.tarfile_open(fileobj=fobj, mode=self.mode) as tar:
    ...
    def test_no_name_attribute(self):
        ...
        tar = self.tarfile_open(fileobj=fobj, mode=self.mode)
    ...
    def test_int_name_attribute(self):
        # Issue 21044: self.tarfile_open() should handle fileobj with an integer
        # 'name' attribute.
    ...
    def test_fileobj_with_offset(self):
        ...
        # of the testtar.
        tar = self.tarfile_open(self.tarname, mode=self.mode)
        try:
            tar.next()
            t = tar.next()
            name = t.name
    ...
    def test_fail_comp(self):
        ...
        self.assertRaises(tarfile.ReadError, self.tarfile_open, tarname, self.mode)
        with open(tarname, "rb") as fobj:
            self.assertRaises(tarfile.ReadError, self.tarfile_open,
    ...
    def test_extract_hardlink(self):
        ...
        with self.tarfile_open(tarname, errorlevel=1, encoding="iso8859-1") as tar:
    ...
    def test_extractall(self):
        ...
        tar = self.tarfile_open(tarname, encoding="iso8859-1")
    ...
    def test_extract_directory(self):
        ...
        os.mkdir(DIR)
        try:
            with self.tarfile_open(tarname, encoding="iso8859-1") as tar:
    ...
    def test_extractall_pathlike_name(self):
        ...
        with support.temp_dir(DIR), \
             self.tarfile_open(tarname, encoding="iso8859-1") as tar:
            directories = [t for t in tar if t.isdir()]
            tar.extractall(DIR, directories)
            for tarinfo in directories:
    ...
    def test_parallel_iteration(self):
        ...
        with self.tarfile_open(self.tarname) as tar:
            for m1, m2 in zip(tar, tar):
                self.assertEqual(m1.offset, m2.offset)
                self.assertEqual(m1.get_info(), m2.get_info())

class MiscReadTest(MiscReadTestBase, unittest.TestCase, TarFileTest):
    test_fail_comp = None

class GzipMiscReadTest(GzipTest, MiscReadTestBase, unittest.TestCase, TarFileTest):
    pass

class Bz2MiscReadTest(Bz2Test, MiscReadTestBase, unittest.TestCase, TarFileTest):
    def requires_name_attribute(self):
        self.skipTest("BZ2File have no name attribute")

class LzmaMiscReadTest(LzmaTest, MiscReadTestBase, unittest.TestCase, TarFileTest):
    def requires_name_attribute(self):
...
class StreamReadTestBase(CommonReadTest):
    ...
    def test_compare_members(self):
        tar1 = self.tarfile_open(tarname, encoding="iso8859-1")
        try:
            tar2 = self.tar
...
class StreamReadTest(StreamReadTestBase, unittest.TestCase, TarFileTest):
    pass

class GzipStreamReadTest(GzipTest, StreamReadTestBase, unittest.TestCase, TarFileTest):
    pass

class Bz2StreamReadTest(Bz2Test, StreamReadTestBase, unittest.TestCase, TarFileTest):
    pass

class LzmaStreamReadTest(LzmaTest, StreamReadTestBase, unittest.TestCase, TarFileTest):
    pass
...
class DetectReadTestBase(TarTest):
    def _testfunc_file(self, name, mode):
        try:
            tar = self.tarfile_open(name, mode)
    ...
    def _testfunc_fileobj(self, name, mode):
        try:
            with open(name, "rb") as f:
                tar = self.tarfile_open(name, mode, fileobj=f)
        except tarfile.ReadError as e:
            self.fail()
        else:
            tar.close()
    ...
    def _test_modes(self, testfunc):
        if self.suffix:
            with self.assertRaises(tarfile.ReadError):
                self.tarfile_open(tarname, mode="r:" + self.suffix)
            with self.assertRaises(tarfile.ReadError):
                self.tarfile_open(tarname, mode="r|" + self.suffix)
            with self.assertRaises(tarfile.ReadError):
                self.tarfile_open(self.tarname, mode="r:")
            with self.assertRaises(tarfile.ReadError):
                self.tarfile_open(self.tarname, mode="r|")
...
class DetectReadTest(DetectReadTestBase, unittest.TestCase, TarFileTest):
    pass

class GzipDetectReadTest(GzipTest, DetectReadTestBase, unittest.TestCase, TarFileTest):
    pass

class Bz2DetectReadTest(Bz2Test, DetectReadTestBase, unittest.TestCase, TarFileTest):
...
class LzmaDetectReadTest(LzmaTest, DetectReadTestBase, unittest.TestCase, TarFileTest):
    pass


class MemberReadTest(ReadTest, unittest.TestCase, TarFileTest):
...
class LongnameTest:
    ...
    def test_truncated_longname(self):
        ...
        with self.assertRaises(tarfile.ReadError):
            self.tarfile_open(name="foo.tar", fileobj=fobj)
...
class GNUReadTest(LongnameTest, ReadTest, unittest.TestCase, TarFileTest):
...
class PaxReadTest(LongnameTest, ReadTest, unittest.TestCase, TarFileTest):
...
class WriteTestBaseBase(TarTest):
    ...
    def test_eof_marker(self):
        ...
        with self.tarfile_open(tmpname, self.mode) as tar:
            t = tarfile.TarInfo("foo")
...
class WriteTestBase(WriteTestBaseBase):
    ...
    def test_filter(self):
        ...
        try:
            ...
            tar = self.tarfile_open(tmpname, self.mode, encoding="iso8859-1")
            try:
            ...
            tar = self.tarfile_open(tmpname, "r")
            try:
                for tarinfo in tar:
                    self.assertEqual(tarinfo.uid, 123)
                    self.assertEqual(tarinfo.uname, "foo")
    ...
    def test_extractall_symlinks(self):
        ...
        try:
            ...
            with self.tarfile_open(temparchive, 'w') as tar:
                tar.add(source_file)
                tar.add(target_file)
            # Let's extract it to the location which contains the symlink
            with self.tarfile_open(temparchive) as tar:
    ...
    def test_open_nonwritable_fileobj(self):
        for exctype in OSError, EOFError, RuntimeError:
            ...
            with self.assertRaises(exctype):
                tar = self.tarfile_open(tmpname, self.mode, fileobj=f,
                                   format=tarfile.PAX_FORMAT,
                                   pax_headers={'non': 'empty'})
            self.assertFalse(f.closed)

class WriteTest(WriteTestBase, unittest.TestCase, TarFileTest):
    pass

class GzipWriteTest(GzipTest, WriteTestBase, unittest.TestCase, TarFileTest):
    pass

class Bz2WriteTest(Bz2Test, WriteTestBase, unittest.TestCase, TarFileTest):
    pass

class LzmaWriteTest(LzmaTest, WriteTestBase, unittest.TestCase, TarFileTest):
    pass
...
class StreamWriteTest(StreamWriteTestBase, unittest.TestCase, TarFileTest):
    pass

class GzipStreamWriteTest(GzipTest, StreamWriteTestBase, unittest.TestCase, TarFileTest):
    pass

class Bz2StreamWriteTest(Bz2Test, StreamWriteTestBase, unittest.TestCase, TarFileTest):
    decompressor = bz2.BZ2Decompressor if bz2 else None

class LzmaStreamWriteTest(LzmaTest, StreamWriteTestBase, unittest.TestCase, TarFileTest):
    decompressor = lzma.LZMADecompressor if lzma else None


class GNUWriteTest(unittest.TestCase, TarFileTest):
    ...
    def _test(self, name, link=None):
        ...
        tar = self.tarfile_open(tmpname, "w")
        try:
...
class CreateTestBase(WriteTestBaseBase, unittest.TestCase, TarFileTest):
    ...
    def test_create_pathlike_name(self):
        with self.tarfile_open(pathlib.Path(tmpname), self.mode) as tobj:
...
class CreateTest(CreateTestBase, unittest.TestCase, TarFileTest):
    pass

class GzipCreateTest(GzipTest, CreateTestBase, unittest.TestCase, TarFileTest):
    pass

class Bz2CreateTest(Bz2Test, CreateTestBase, unittest.TestCase, TarFileTest):
    pass

class LzmaCreateTest(LzmaTest, CreateTestBase, unittest.TestCase, TarFileTest):
    pass

class CreateWithXModeTest(CreateTestBase, unittest.TestCase, TarFileTest):
...
class HardlinkTest(unittest.TestCase, TarFileTest):
    ...
    def setUp(self):
        ...
        self.tar = self.tarfile_open(tmpname, "w")
...
class PaxWriteTest(GNUWriteTest):
    ...
    def _test(self, name, link=None):
        ...
        tar = self.tarfile_open(tmpname, "w", format=tarfile.PAX_FORMAT)
        try:
            tar.addfile(tarinfo)
        finally:
            tar.close()
    ...
    def test_pax_global_header(self):
        ...
        tar = self.tarfile_open(tmpname, encoding="iso8859-1")
        try:
...
class UnicodeTest:
    ...
    def _test_unicode_filename(self, encoding):
        tar = self.tarfile_open(tmpname, "w", format=self.format,
        ...
        tar = self.tarfile_open(tmpname, encoding=encoding)
        try:
    ...
    def test_unicode_argument(self):
        tar = self.tarfile_open(tarname, "r",
    ...
    def test_uname_unicode(self):
        ...
        tar = self.tarfile_open(tmpname, mode="w", format=self.format,
                           encoding="iso8859-1")
        try:
            tar.addfile(t)
        finally:
        ...
        try:
            ...
            if self.format != tarfile.PAX_FORMAT:
                tar.close()
                tar = self.tarfile_open(tmpname, encoding="ascii")
                t = tar.getmember("foo")
...
class UstarUnicodeTest(UnicodeTest, unittest.TestCase, TarFileTest):
    ...
    def _test_ustar_name(self, name, exc=None):
        with self.tarfile_open(tmpname, "w", format=self.format, encoding="utf-8") as tar:
            t = tarfile.TarInfo(name)
            if exc is None:
                tar.addfile(t)
            else:
        ...
        if exc is None:
            with self.tarfile_open(tmpname, "r", encoding="utf-8") as tar:
                for t in tar:
...
class GNUUnicodeTest(UnicodeTest, unittest.TestCase, TarFileTest):
    ...
    def test_bad_pax_header(self):
        ...
        for encoding, name in (
            ...
            with self.tarfile_open(tarname, encoding=encoding,
...
class PAXUnicodeTest(UnicodeTest, unittest.TestCase, TarFileTest):
...
class AppendTestBase:
    ...
    def _create_testtar(self, mode="w:"):
        with self.tarfile_open(tarname, encoding="iso8859-1") as src:
            t = src.getmember("ustar/regtype")
            t.name = "foo"
            with src.extractfile(t) as f:
                with self.tarfile_open(self.tarname, mode) as tar:
                    tar.addfile(t, f)

    def test_append_compressed(self):
        self._create_testtar("w:" + self.suffix)
        self.assertRaises(tarfile.ReadError, self.tarfile_open, tmpname, "a")

class AppendTest(AppendTestBase, unittest.TestCase, TarFileTest):
    test_append_compressed = None

    def _add_testfile(self, fileobj=None):
        with self.tarfile_open(self.tarname, "a", fileobj=fileobj) as tar:
            tar.addfile(tarfile.TarInfo("bar"))

    def _test(self, names=["bar"], fileobj=None):
        with self.tarfile_open(self.tarname, fileobj=fileobj) as tar:
    ...
    def test_empty(self):
        self.tarfile_open(self.tarname, "w:").close()
        self._add_testfile()
        self._test()
...
class GzipAppendTest(GzipTest, AppendTestBase, unittest.TestCase, TarFileTest):
    pass

class Bz2AppendTest(Bz2Test, AppendTestBase, unittest.TestCase, TarFileTest):
    pass

class LzmaAppendTest(LzmaTest, AppendTestBase, unittest.TestCase, TarFileTest):
    pass


class LimitsTest(unittest.TestCase, TarFileTest):
...
class MiscTest(unittest.TestCase, TarFileTest):
...
class CommandLineTest(unittest.TestCase, TarFileTest):
    ...
    def make_simple_tarfile(self, tar_name):
        ...
        with self.tarfile_open(tar_name, 'w') as tf:
    ...
    def test_list_command(self):
        for tar_name in testtarnames:
            with support.captured_stdout() as t:
                with self.tarfile_open(tar_name, 'r') as tf:
    ...
    def test_create_command_dotless_filename(self):
        ...
        try:
            ...
            self.assertEqual(out, b'')
            with self.tarfile_open(dotlessname) as tar:
                tar.getmembers()
        finally:
    ...
    def test_create_command_dot_started_filename(self):
        ...
        try:
            ...
            self.assertEqual(out, b'')
            with self.tarfile_open(tar_name) as tar:
                tar.getmembers()
        finally:
...
class ContextManagerTest(unittest.TestCase, TarFileTest):

    def test_basic(self):
        with self.tarfile_open(tarname) as tar:
    ...
    def test_eof(self):
        ...
        with self.tarfile_open(tmpname, "w"):
            pass
    ...
    def test_fileobj(self):
        ...
        with open(tmpname, "wb") as fobj:
            try:
                with self.tarfile_open(fileobj=fobj, mode="w") as tar:
                    raise Exception
            except:
                pass
...
class LinkEmulationTest(ReadTest, unittest.TestCase, TarFileTest):
...
class Bz2PartialReadTest(Bz2Test, unittest.TestCase, TarFileTest):
    ...
    def _test_partial_input(self, mode):
        ...
        for x in range(len(data) + 1):
            try:
                self.tarfile_open(fileobj=MyBytesIO(data[:x]), mode=mode)
            except tarfile.ReadError:
...
class NumericOwnerTest(unittest.TestCase, TarFileTest):
    # mock the following:
    ...
    def _make_test_archive(filename_1, dirname_1, filename_2):
        ...
        with NumericOwnerTest.tarfile_open(tmpname, 'w') as tarfl:
    ...
    def _setup_test(mock_geteuid):
        ...
        with NumericOwnerTest.tarfile_open(tar_filename) as tarfl:
