...
class TarPathTest(unittest.TestCase):
    def setUp(self):
        self.tarpath = support.findfile("testtarpath.tar")
        self.tar = tarfile.TarFile(self.tarpath)
        self.root = tarfile.TarPath(tarfile=self.tar)

    def tearDown(self):
        self.tar.close()

    def test_tarfile(self):
        self.assertIs(self.root.tarfile, self.tar)

    def test_hash(self):
        with tarfile.TarFile(self.tarpath) as tar0:
            with tarfile.TarFile(self.tarpath) as tar1:
                p = tarfile.TarPath('fileA', tarfile=tar0)
                p1 = tarfile.TarPath('fileA', tarfile=tar0)
                p2 = tarfile.TarPath('fileA', tarfile=tar1)
                p3 = tarfile.TarPath('fileB', tarfile=tar0)
                self.assertEqual(hash(p), hash(p1))
                self.assertNotEqual(hash(p), hash(p2))
                self.assertNotEqual(hash(p), hash(p3))
    ...
    def test_samefile(self):
        p = self.root / 'fileA'
        pp = self.root / 'fileA'
        q = self.root / 'dirB' / 'fileB'
        self.assertTrue(p.samefile('fileA'))
        self.assertTrue(p.samefile(pp))
        self.assertFalse(p.samefile('dirB/fileB'))
        self.assertFalse(p.samefile(q))
        # Test the non-existent file case
        r = self.root / 'foo'
        self.assertRaises(FileNotFoundError, p.samefile, r)
        self.assertRaises(FileNotFoundError, p.samefile, 'foo')
        self.assertRaises(FileNotFoundError, r.samefile, p)
        self.assertRaises(FileNotFoundError, r.samefile, 'foo')
    ...
    def test_exists(self):
        ...
        self.assertTrue((p / 'dirA').exists())
        self.assertTrue((p / 'fileA').exists())
        self.assertFalse((p / 'fileA' / 'bah').exists())
        self.assertTrue((p / 'linkA').exists())
        self.assertTrue((p / 'linkB').exists())
        self.assertTrue((p / 'linkB' / 'fileB').exists())
        self.assertFalse((p / 'linkA' / 'bah').exists())
        self.assertFalse((p / 'brokenLink').exists())
        self.assertTrue((p / 'brokenLink').exists(follow_symlinks=False))
        self.assertFalse((p / 'foo').exists())
        self.assertFalse(p.with_segments('/xyzzy').exists())

    def test_open(self):
        with (self.root / 'fileA').open('r') as f:
            self.assertIsInstance(f, io.TextIOBase)
            self.assertEqual(f.read(), "this is file A\n")
        with (self.root / 'fileA').open('rb') as f:
            self.assertIsInstance(f, io.BufferedIOBase)
            self.assertEqual(f.read().strip(), b"this is file A")

    def test_iterdir(self):
        it = self.root.iterdir()
        paths = sorted(it)
        expected = ['brokenLink', 'brokenLinkLoop',
                    'dirA', 'dirB', 'dirC', 'dirE', 'fileA',
                    'linkA', 'linkB']
        self.assertEqual(paths, [ self.root / q for q in expected ])

    def test_iterdir_symlink(self):
        p = self.root / 'linkB'
        paths = sorted(p.iterdir())
        expected = [ p / q for q in ['fileB', 'linkD'] ]
    ...
    def test_glob(self):
        def _check(pattern, expected):
            actual = sorted(self.root.glob(pattern))
            expected = [self.root / q for q in expected]
            self.assertEqual(actual, expected)

        _check("fileA", ["fileA"])
        _check("fileB", [])
        _check("dir*/file*", ["dirB/fileB", "dirC/fileC"])
        _check("*A", ['dirA', 'fileA', 'linkA'])
        _check("*B/*", ['dirB/fileB', 'dirB/linkD', 'linkB/fileB', 'linkB/linkD'])
        _check("*/fileB", ['dirB/fileB', 'linkB/fileB'])
        _check("brokenLink", ['brokenLink'])
        _check("*/", ["dirA", "dirB", "dirC", "dirE", "linkB"])

    def test_glob_case_sensitive(self):
        def _check(pattern, case_sensitive, expected):
            actual = sorted([str(q) for q in self.root.glob(pattern, case_sensitive=case_sensitive)])
            expected = [str(self.root / q) for q in expected]
            self.assertEqual(actual, expected)

        _check("DIRB/FILE*", True, [])
        _check("DIRB/FILE*", False, ["dirB/fileB"])
        _check("dirb/file*", True, [])
        _check("dirb/file*", False, ["dirB/fileB"])

    def test_glob_follow_symlinks(self):
        def _check(pattern, expected):
            actual = sorted([q for q in self.root.glob(pattern, follow_symlinks=True)
                             if "linkD" not in q.parent.parts])
            expected = [self.root / q for q in expected]
            self.assertEqual(actual, expected)
        ...
        _check("*/fileB", ["dirB/fileB", "linkB/fileB"])
        _check("*/", ["dirA", "dirB", "dirC", "dirE", "linkB"])
        _check("dir*/*/..", ["dirA/linkC/..", "dirC/dirD/.."])
        _check("dir*/**/", ["dirA", "dirA/linkC", "dirA/linkC/linkD", "dirB", "dirB/linkD",
                              "dirC", "dirC/dirD", "dirE"])
        _check("dir*/**/..", ["dirA/..", "dirA/linkC/..", "dirB/..",
                              "dirC/..", "dirC/dirD/..", "dirE/.."])
        _check("dir*/*/**/", ["dirA/linkC", "dirA/linkC/linkD", "dirB/linkD", "dirC/dirD"])
        _check("dir*/*/**/..", ["dirA/linkC/..", "dirC/dirD/.."])
        _check("dir*/**/fileC", ["dirC/fileC"])
        _check("dir*/*/../dirD/**/", ["dirC/dirD/../dirD"])
        _check("*/dirD/**/", ["dirC/dirD"])

    def test_glob_no_follow_symlinks(self):
        def _check(pattern, expected):
            actual = sorted(self.root.glob(pattern, follow_symlinks=False))
            expected = [self.root / q for q in expected]
            self.assertEqual(actual, expected)
        ...
        _check("dir*/file*", ["dirB/fileB", "dirC/fileC"])
        _check("*A", ["dirA", "fileA", "linkA"])
        _check("*B/*", ["dirB/fileB", "dirB/linkD"])
        _check("*/fileB", ["dirB/fileB"])
        _check("*/", ["dirA", "dirB", "dirC", "dirE"])
        _check("dir*/*/..", ["dirC/dirD/.."])
        _check("dir*/**/", ["dirA", "dirB", "dirC", "dirC/dirD", "dirE"])
        _check("dir*/**/..", ["dirA/..", "dirB/..", "dirC/..", "dirC/dirD/..", "dirE/.."])
        _check("dir*/*/**/", ["dirC/dirD"])
        _check("dir*/*/**/..", ["dirC/dirD/.."])
        _check("dir*/**/fileC", ["dirC/fileC"])
    ...
    def test_rglob(self):
        def _check(glob, expected):
            self.assertEqual(sorted(glob), sorted(self.root / q for q in expected))
        p = self.root
        _check(p.rglob("fileA"), ["fileA"])
        ...
        q = p / "dirC"
        _check(q.rglob("*"), ["dirC/fileC", "dirC/novel.txt",
                              "dirC/dirD", "dirC/dirD/fileD"])
        _check(q.rglob("file*"), ["dirC/fileC", "dirC/dirD/fileD"])
        _check(q.rglob("**/file*"), ["dirC/fileC", "dirC/dirD/fileD"])
        _check(q.rglob("dir*/**"), ["dirC/dirD"])
        _check(q.rglob("*/*"), ["dirC/dirD/fileD"])
        _check(q.rglob("*/"), ["dirC/dirD"])
        _check(q.rglob(""), ["dirC", "dirC/dirD"])
        _check(q.rglob("**"), ["dirC", "dirC/dirD"])
        _check(q.rglob("*.txt"), ["dirC/novel.txt"])
        _check(q.rglob("*.*"), ["dirC/novel.txt"])

    def test_rglob_follow_symlinks(self):
        def _check(path, pattern, expected):
            actual = sorted([q for q in path.rglob(pattern, follow_symlinks=True)
                             if "linkD" not in q.parent.parts])
            expected = [self.root / q for q in expected]
            self.assertEqual(actual, expected)

        p = self.root
        _check(p, "fileB", ["dirA/linkC/fileB", "dirB/fileB", "linkB/fileB"])
        _check(p, "*/fileA", [])
        _check(p, "*/fileB", ["dirA/linkC/fileB", "dirB/fileB", "linkB/fileB"])
        _check(p, "file*", ["dirA/linkC/fileB", "dirB/fileB",
                            "dirC/dirD/fileD", "dirC/fileC", "fileA", "linkB/fileB"])
        ...
        q = p / "dirC"
        _check(q, "*", ["dirC/dirD", "dirC/dirD/fileD", "dirC/fileC", "dirC/novel.txt"])
        _check(q, "file*", ["dirC/dirD/fileD", "dirC/fileC"])
        _check(q, "*/*", ["dirC/dirD/fileD"])
        _check(q, "*/", ["dirC/dirD"])
        _check(q, "", ["dirC", "dirC/dirD"])
        _check(q, "*.txt", ["dirC/novel.txt"])
        _check(q, "*.*", ["dirC/novel.txt"])

    def test_rglob_no_follow_symlinks(self):
        def _check(path, pattern, expected):
            actual = sorted(path.rglob(pattern, follow_symlinks=False))
            expected = [self.root / q for q in expected]
            self.assertEqual(actual, expected)
        ...
        _check(p, "*/fileA", [])
        _check(p, "*/fileB", ["dirB/fileB"])
        _check(p, "file*", ["dirB/fileB", "dirC/dirD/fileD", "dirC/fileC", "fileA"])
    ...
    def test_rglob_symlink_loop(self):
        ...
        self.assertEqual(given, sorted(self.root / x for x in expect))

    def test_glob_dotdot(self):
        p = self.root
        self.assertEqual(sorted(p.glob("..")), [ p / ".." ])
        self.assertEqual(sorted(p.glob("../..")), [ p / ".." / ".." ])
        self.assertEqual(sorted(p.glob("dirA/..")), [ p / "dirA" / ".." ])
        self.assertEqual(sorted(p.glob("dirA/../file*")), [ p / "dirA/../fileA" ])
        self.assertEqual(sorted(p.glob("dirA/../file*/..")), [])
        self.assertEqual(sorted(p.glob("../xyzzy")), [])
        self.assertEqual(sorted(p.glob("xyzzy/..")), [])
        self.assertEqual(sorted(p.glob("/".join([".."] * 50))), [ p.joinpath(*[".."] * 50)])

    def test_walk(self):
        def _sorted_walk(follow_symlinks):
            results = []
            for dirpath, dirnames, filenames in self.root.walk(follow_symlinks=follow_symlinks):
                if 'linkD' in dirnames:
                    # Treat recursive symlink as file
                    dirnames.remove('linkD')
                    filenames.append('linkD')
                dirnames.sort()
                filenames.sort()
        ...
        p = self.root
        self.assertEqual(_sorted_walk(False), [
            (p,
             ['dirA', 'dirB', 'dirC', 'dirE'],
             ['brokenLink', 'brokenLinkLoop', 'fileA', 'linkA', 'linkB']),
            (p / 'dirA', [], ['linkC']),
            (p / 'dirB', [], ['fileB', 'linkD']),
            (p / 'dirC', ['dirD'], ['fileC', 'novel.txt']),
            (p / 'dirC' / 'dirD', [], ['fileD']),
        ])

        self.assertEqual(_sorted_walk(True), [
            (p,
             ['dirA', 'dirB', 'dirC', 'dirE', 'linkB'],
             ['brokenLink', 'brokenLinkLoop', 'fileA', 'linkA']),
            (p / 'dirA', ['linkC'], []),
            (p / 'dirA' / 'linkC', [], ['fileB', 'linkD']),
            (p / 'dirB', [], ['fileB', 'linkD']),
            (p / 'dirC', ['dirD'], ['fileC', 'novel.txt']),
            (p / 'dirC' / 'dirD', [], ['fileD']),
            (p / 'linkB', [], ['fileB', 'linkD']),
    ...
    def test_readlink(self):
        p = self.root
        self.assertEqual((p / 'linkA').readlink(), p / 'fileA')
        self.assertEqual((p / 'brokenLink').readlink(), p / 'non-existing')
        self.assertEqual((p / 'linkB').readlink(), p / 'dirB')
        with self.assertRaises(OSError):
            (p / 'fileA').readlink()

    def test_resolve(self):
        with self.assertRaises(OSError) as cm:
            self.root.joinpath('foo').resolve(strict=True)
        self.assertEqual(cm.exception.errno, errno.ENOENT)
        def _check(path, expected, strict=True):
            self.assertEqual(self.root.joinpath(path).resolve(strict=strict),
                             self.root.joinpath(expected))
        _check('foo/in/spam', 'foo/in/spam', False)
        _check('../foo/in/spam', '../foo/in/spam', False)
        _check('dirB/fileB', 'dirB/fileB')
        _check('linkA', 'fileA')
        _check('dirA/linkC/fileB', 'dirB/fileB')
        _check('dirB/linkD/fileB', 'dirB/fileB')
        _check('dirA/linkC/fileB/foo/in/spam', 'dirB/fileB/foo/in/spam', False)
        _check('dirA/linkC/../foo/in/spam', 'foo/in/spam', False)

    def test_stat(self):
        statA = self.root.joinpath('fileA').stat()
        statB = self.root.joinpath('dirB', 'fileB').stat()
        statC = self.root.joinpath('dirC').stat()
    ...
    def test_owner(self):
        p = self.root
        self.assertRaises(pathlib.UnsupportedOperation, p.owner)
        self.assertEqual((p / 'fileA').owner(), 'barney')

    def test_group(self):
        p = self.root
        self.assertRaises(pathlib.UnsupportedOperation, p.group)
        self.assertEqual((p / 'fileA').group(), 'barney')

    def test_read_write_bytes(self):
        fileobj = io.BytesIO()
        with tarfile.TarFile(fileobj=fileobj, mode="w") as tar:
            p = tarfile.TarPath('fileA', tarfile=tar)
            p.write_bytes(b'abcdefg')

        fileobj.seek(0)
        with tarfile.TarFile(fileobj=fileobj) as tar:
            p = tarfile.TarPath('fileA', tarfile=tar)
            self.assertEqual(p.read_bytes(), b'abcdefg')
    ...
    def test_read_write_text(self):
        ...
        with tarfile.TarFile(fileobj=fileobj) as tar:
            p = tarfile.TarPath('fileA', tarfile=tar)
            self.assertEqual(p.read_text(encoding='utf-8', errors='ignore'), 'bcdefg')

    def test_mkdir(self):
        fileobj = io.BytesIO()
        with tarfile.TarFile(fileobj=fileobj, mode="w") as tar:
            p = tarfile.TarPath('dirA', tarfile=tar)
            p.mkdir()

        fileobj.seek(0)
        with tarfile.TarFile(fileobj=fileobj) as tar:
            info = tar.getmember('dirA')
            self.assertEqual(info.type, tarfile.DIRTYPE)

    def test_symlink_to(self):
        fileobj = io.BytesIO()
        with tarfile.TarFile(fileobj=fileobj, mode="w") as tar:
            p = tarfile.TarPath(tarfile=tar)
            p.joinpath('linkA').symlink_to('fileA')

        fileobj.seek(0)
        with tarfile.TarFile(fileobj=fileobj) as tar:
            info = tar.getmember('linkA')
            self.assertEqual(info.type, tarfile.SYMTYPE)
            self.assertEqual(info.linkname, 'fileA')

    def test_hardlink_to(self):
        fileobj = io.BytesIO()
        with tarfile.TarFile(fileobj=fileobj, mode="w") as tar:
            p = tarfile.TarPath(tarfile=tar)
            p.joinpath('linkA').hardlink_to('fileA')

        fileobj.seek(0)
        with tarfile.TarFile(fileobj=fileobj) as tar:
            info = tar.getmember('linkA')
            self.assertEqual(info.type, tarfile.LNKTYPE)
            self.assertEqual(info.linkname, 'fileA')


class TarPathFileTypeTest(unittest.TestCase):
    def setUp(self):
        tarpath = support.findfile("testtar.tar")
        self.tar = tarfile.TarFile(tarpath)
        self.root = tarfile.TarPath(tarfile=self.tar)
    ...
    def test_is_dir(self):
        ...
        self.assertTrue((p / 'ustar').is_dir())
        self.assertTrue((p / 'ustar' / 'dirtype').is_dir())
        self.assertFalse((p / 'ustar' / 'regtype').is_dir())
        self.assertFalse((p / 'non-existing').is_dir())
        self.assertFalse((p / 'ustar' / 'regtype' / 'bah').is_dir())
        self.assertFalse((p / 'ustar' / 'symtype').is_dir())
        self.assertFalse((p / 'ustar' / 'lnktype').is_dir())
        self.assertFalse((p / 'ustar' / 'fifotype').is_dir())
        self.assertFalse((p / 'ustar' / 'blktype').is_dir())
        self.assertFalse((p / 'ustar' / 'chrtype').is_dir())

    def test_is_file(self):
        p = self.root
        self.assertFalse(p.is_file())
        self.assertFalse((p / 'ustar').is_file())
        self.assertFalse((p / 'ustar' / 'dirtype').is_file())
        self.assertTrue((p / 'ustar' / 'regtype').is_file())
        self.assertFalse((p / 'non-existing').is_file())
        self.assertFalse((p / 'ustar' / 'regtype' / 'bah').is_file())
        self.assertTrue((p / 'ustar' / 'symtype').is_file())
        self.assertFalse((p / 'ustar' / 'symtype').is_file(follow_symlinks=False))
        self.assertFalse((p / 'ustar' / 'fifotype').is_file())
        self.assertFalse((p / 'ustar' / 'blktype').is_file())
        self.assertFalse((p / 'ustar' / 'chrtype').is_file())

    def test_is_mount(self):
        p = self.root
        self.assertTrue(p.is_mount())
        self.assertFalse((p / 'ustar').is_mount())
        self.assertFalse((p / 'ustar' / 'dirtype').is_mount())
        self.assertFalse((p / 'ustar' / 'regtype').is_mount())
        self.assertFalse((p / 'non-existing').is_mount())
        self.assertFalse((p / 'ustar' / 'regtype' / 'bah').is_mount())
        self.assertFalse((p / 'ustar' / 'symtype').is_mount())
        self.assertFalse((p / 'ustar' / 'fifotype').is_mount())
        self.assertFalse((p / 'ustar' / 'blktype').is_mount())
        self.assertFalse((p / 'ustar' / 'chrtype').is_mount())

    def test_is_symlink(self):
        p = self.root
        self.assertFalse(p.is_symlink())
        self.assertFalse((p / 'ustar').is_symlink())
        self.assertFalse((p / 'ustar' / 'dirtype').is_symlink())
        self.assertFalse((p / 'ustar' / 'regtype').is_symlink())
        self.assertFalse((p / 'non-existing').is_symlink())
        self.assertFalse((p / 'ustar' / 'regtype' / 'bah').is_symlink())
        self.assertTrue((p / 'ustar' / 'symtype').is_symlink())
        self.assertFalse((p / 'ustar' / 'fifotype').is_symlink())
        self.assertFalse((p / 'ustar' / 'blktype').is_symlink())
        self.assertFalse((p / 'ustar' / 'chrtype').is_symlink())

    def test_is_junction(self):
        p = self.root
        self.assertFalse(p.is_junction())
        self.assertFalse((p / 'ustar').is_junction())
        self.assertFalse((p / 'ustar' / 'dirtype').is_junction())
        self.assertFalse((p / 'ustar' / 'regtype').is_junction())
        self.assertFalse((p / 'non-existing').is_junction())
        self.assertFalse((p / 'ustar' / 'regtype' / 'bah').is_junction())
        self.assertFalse((p / 'ustar' / 'symtype').is_junction())
        self.assertFalse((p / 'ustar' / 'fifotype').is_junction())
        self.assertFalse((p / 'ustar' / 'blktype').is_junction())
        self.assertFalse((p / 'ustar' / 'chrtype').is_junction())

    def test_is_fifo(self):
        p = self.root
        self.assertFalse(p.is_fifo())
        self.assertFalse((p / 'ustar').is_fifo())
        self.assertFalse((p / 'ustar' / 'dirtype').is_fifo())
        self.assertFalse((p / 'ustar' / 'regtype').is_fifo())
        self.assertFalse((p / 'non-existing').is_fifo())
        self.assertFalse((p / 'ustar' / 'regtype' / 'bah').is_fifo())
        self.assertFalse((p / 'ustar' / 'symtype').is_fifo())
        self.assertTrue((p / 'ustar' / 'fifotype').is_fifo())
        self.assertFalse((p / 'ustar' / 'blktype').is_fifo())
        self.assertFalse((p / 'ustar' / 'chrtype').is_fifo())

    def test_is_socket(self):
        p = self.root
        self.assertFalse(p.is_socket())
        self.assertFalse((p / 'ustar').is_socket())
        self.assertFalse((p / 'ustar' / 'dirtype').is_socket())
        self.assertFalse((p / 'ustar' / 'regtype').is_socket())
        self.assertFalse((p / 'non-existing').is_socket())
        self.assertFalse((p / 'ustar' / 'regtype' / 'bah').is_socket())
        self.assertFalse((p / 'ustar' / 'symtype').is_socket())
        self.assertFalse((p / 'ustar' / 'fifotype').is_socket())
        self.assertFalse((p / 'ustar' / 'blktype').is_socket())
        self.assertFalse((p / 'ustar' / 'chrtype').is_socket())

    def test_is_block_device(self):
        p = self.root
        self.assertFalse(p.is_block_device())
        self.assertFalse((p / 'ustar').is_block_device())
        self.assertFalse((p / 'ustar' / 'dirtype').is_block_device())
        self.assertFalse((p / 'ustar' / 'regtype').is_block_device())
        self.assertFalse((p / 'non-existing').is_block_device())
        self.assertFalse((p / 'ustar' / 'regtype' / 'bah').is_block_device())
        self.assertFalse((p / 'ustar' / 'symtype').is_block_device())
        self.assertFalse((p / 'ustar' / 'fifotype').is_block_device())
        self.assertTrue((p / 'ustar' / 'blktype').is_block_device())
        self.assertFalse((p / 'ustar' / 'chrtype').is_block_device())

    def test_is_char_device(self):
        p = self.root
        self.assertFalse(p.is_char_device())
        self.assertFalse((p / 'ustar').is_char_device())
        self.assertFalse((p / 'ustar' / 'dirtype').is_char_device())
        self.assertFalse((p / 'ustar' / 'regtype').is_char_device())
        self.assertFalse((p / 'non-existing').is_char_device())
        self.assertFalse((p / 'ustar' / 'regtype' / 'bah').is_char_device())
        self.assertFalse((p / 'ustar' / 'symtype').is_char_device())
        self.assertFalse((p / 'ustar' / 'fifotype').is_char_device())
        self.assertFalse((p / 'ustar' / 'blktype').is_char_device())
        self.assertTrue((p / 'ustar' / 'chrtype').is_char_device())
