...
class AnnotationsFutureTestCase(unittest.TestCase):
    ...
    def test_annotations(self):
        ...
        eq("f'some f-string with {a!f} {few()!f:.2f} {formatted.values!r}'")
        eq('''f"{f'{nested!f} inner'!f} outer"''')
        eq("f'space between opening braces: { {a for a in (1, 2, 3)}!f}'")
        eq("f'{(lambda x: x)!f}'")
        eq("f'{(None if a else lambda x: x)!f}'")
        eq("f'{x}'")
        eq("f'{x!f}'")
        eq("f'{x!r}'")
        eq("f'{x!a}'")
