...
class PurePathTest(unittest.TestCase):
    ...
    def test_match_common(self):
        ...
        self.assertTrue(P('.', '.').match('**'))
        self.assertTrue(P('.', '.').match('**/'))
        # Matching single segment with/without trailing slash
        self.assertTrue(P('foo').match('*'))
        self.assertFalse(P('foo').match('*/'))
        self.assertFalse(P('foo/').match('*'))
        self.assertTrue(P('foo/').match('*/'))
        # Matching any segments with trailing slash
        self.assertFalse(P('/foo').match('**/'))
        self.assertTrue(P('/foo/').match('**/'))
        self.assertFalse(P('foo').match('**/'))
        self.assertTrue(P('foo/').match('**/'))
        self.assertFalse(P('foo/bar').match('**/'))
        self.assertTrue(P('foo/bar/').match('**/'))
...
class PureWindowsPathTest(PurePathTest):
    ...
    def test_parse_path(self):
        ...
        check('Z://b//c/d/',         'Z:', '\\', ['b', 'c', 'd'], True)
        # UNC paths.
        check('//b/c//d',            '\\\\b\\c', '\\', ['d'], False)
        # Extended paths.
        check('//./c:',              '\\\\.\\c:', '', [], False)
        check('//?/c:/',             '\\\\?\\c:', '\\', [], False)
        check('//?/c:/a',            '\\\\?\\c:', '\\', ['a'], False)
        ...
        check('//?/UNC',             '\\\\?\\UNC', '', [], False)
        check('//?/UNC/',            '\\\\?\\UNC\\', '', [], False)
        check('//?/UNC/b',           '\\\\?\\UNC\\b', '', [], False)
        check('//?/UNC/b/',          '\\\\?\\UNC\\b\\', '', [], False)
        check('//?/UNC/b/c',         '\\\\?\\UNC\\b\\c', '\\', [], False)
        check('//?/UNC/b/c/',        '\\\\?\\UNC\\b\\c', '\\', [], False)
        check('//?/UNC/b/c/d',       '\\\\?\\UNC\\b\\c', '\\', ['d'], False)
        # UNC device paths
        check('//./BootPartition/',  '\\\\.\\BootPartition', '\\', [], False)
        check('//?/BootPartition/',  '\\\\?\\BootPartition', '\\', [], False)
        check('//./PhysicalDrive0',  '\\\\.\\PhysicalDrive0', '', [], False)
        check('//?/Volume{}/',       '\\\\?\\Volume{}', '\\', [], False)
        check('//./nul',             '\\\\.\\nul', '', [], False)
        # Paths to files with NTFS alternate data streams
        check('./c:s',               '', '', ['c:s'], False)
        check('cc:s',                '', '', ['cc:s'], False)
        check('C:c:s',               'C:', '', ['c:s'], False)
        check('C:/c:s',              'C:', '\\', ['c:s'], False)
        check('D:a/c:b',             'D:', '', ['a', 'c:b'], False)
        check('D:/a/c:b',            'D:', '\\', ['a', 'c:b'], False)
