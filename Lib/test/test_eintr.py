...
Each test class runs in a fresh subprocess (via @isolation.runInSubprocess()) to ensure
there is no background thread running, so that signals are delivered to the
correct thread.
