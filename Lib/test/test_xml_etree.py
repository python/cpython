...
class ElementTreeTest(unittest.TestCase):
    ...
    def test_interface(self):
        ...
        def check_method(method):
            self.assertHasAttr(method, "__call__", msg="%s not callable" % method)
    ...
    def test_subelement_positional_only_parameter(self):
        ...
        sub2 = ET.SubElement(
            parent, "sub2", attrib={"attrib": "foo"}, tag="bar", parent="baz"
        )
    ...
    def test_processinginstruction(self):
        ...
        self.assertEqual(
            list(ET.ProcessingInstruction("test", "instruction").itertext()),
            ["test instruction"],
        )
...
class IterparseTest(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        self.assertEqual(
            [(action, elem.tag) for action, elem in it],
            [
                ("end", "element"),
                ("end", "empty-element"),
                ("end", "root"),
    ...
    def test_namespace_events(self):
        ...
        it = iterparse(io.BytesIO(rb"<root xmlns=''/>"), events)
    ...
    def test_non_utf8(self):
        ...
        self.assertEqual(
            [(action, elem) for action, elem in it],
            [
...
class XMLPullParserTest(unittest.TestCase):
    ...
    def test_simple_xml(self, chunk_size=None, flush=False):
        ...
        self.assert_event_tags(parser, [])
        self._feed(
            parser, "<root>\n  <element key='value'>text</element", chunk_size, flush
        )
        self.assert_event_tags(parser, [])
        self._feed(parser, ">\n", chunk_size, flush)
    ...
    def test_flush_reparse_deferral_disabled(self):
        ...
        if pyexpat.version_info >= (2, 6, 0):
            if not ET is pyET:
                self.skipTest(
                    f"XMLParser.(Get|Set)ReparseDeferralEnabled "
...
<paragraph id="A-0001" lvl="0">A new cultivar of Begonia plant named &lsquo;BCT9801BEG&rsquo;.</paragraph>
    ...
    def test_issue123213_correct_extend_exception(self):
        ...
        self.assertRaises(
            ZeroDivisionError, ET.Element("tag").extend, (1 / 0 for i in range(2))
        )
        ...
        self.assertRaisesRegex(
            TypeError, "mymessage", ET.Element("tag").extend, (f() for i in range(2))
        )
...
class BasicElementTest(ElementTestCase, unittest.TestCase):
    ...
    def test_remove_errors(self):
        ...
        with self.assertRaisesRegex(
            ValueError, r"<Element 'subtag'.*> not in <Element 'tag'.*>"
        ):
            e.remove(ET.Element("subtag"))
        with self.assertRaisesRegex(TypeError, r".*\bElement, not type"):
            e.remove(ET.Element)
        with self.assertRaisesRegex(TypeError, r".*\bElement, not int"):
            e.remove(1)
...
class BadElementTest(ElementTestCase, unittest.TestCase):
    ...
    def cases_for_remove_missing_with_mutations(self, E, Z):
        ...
        for U, V in [(E, Z), (Z, E), (Z, Z)]:
            description = (
                f"remove missing {E.__name__}() from "
        ...
        for U, V in [(E, E), (E, Z), (Z, E), (Z, Z)]:
            description = (
                f"remove missing {Z.__name__}() from "
                f"[{U.__name__}(), {V.__name__}()]"
            )
    ...
    def cases_for_remove_existing_with_mutations(self, E, Z):
        ...
        for U, R, description in [
            ...
            description = (
                f"remove root[1] (of type {R.__name__}) "
    ...
    def test_deepcopy_grow(self):
        ...
        self.assertEqual([c.tag for c in children[3:]], [a.tag, b.tag, a.tag, b.tag])
...
class KeywordArgsTest(unittest.TestCase):
    ...
    def test_issue14818(self):
        ...
        e3 = ET.SubElement(elements[0], "foobar", attrib=frozendict({"key1": "value1"}))
