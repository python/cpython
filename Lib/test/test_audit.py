...
interpreter.  ``@isolation.runInSubprocess()`` provides one and reports the result, which
...
class _Hook:
    """Collect audited events; for use inside @runInSubprocess() tests.
...
def requires_module(name):
    ...
    the @runInSubprocess() subprocess.
    """
    import functools
...
class AuditTest(unittest.TestCase):
    ...
    def test_excepthook(self):
        ...
        # @runInSubprocess() (whose test body runs under unittest, which catches the
