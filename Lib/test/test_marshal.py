...
class CAPI_TestCase(unittest.TestCase, HelperMixin):
    ...
    cfilename = os.fsencode(filename)

    def test_write_long_to_file(self):
        for v in range(marshal.version + 1):
            _testcapi.pymarshal_write_long_to_file(0x12345678, self.cfilename, v)
    ...
    def test_write_object_to_file(self):
        ...
        for v in range(marshal.version + 1):
            _testcapi.pymarshal_write_object_to_file(obj, self.cfilename, v)
            with open(self.filename, 'rb') as f:
                data = f.read()
            support.unlink(self.filename)
    ...
    def test_read_short_from_file(self):
        ...
        r, p = _testcapi.pymarshal_read_short_from_file(self.cfilename)
        support.unlink(self.filename)
        ...
        with self.assertRaises(EOFError):
            _testcapi.pymarshal_read_short_from_file(self.cfilename)
        support.unlink(self.filename)

    def test_read_long_from_file(self):
        with open(self.filename, 'wb') as f:
            f.write(b'\x78\x56\x34\x12xxxx')
        r, p = _testcapi.pymarshal_read_long_from_file(self.cfilename)
        support.unlink(self.filename)
        ...
        with self.assertRaises(EOFError):
            _testcapi.pymarshal_read_long_from_file(self.cfilename)
        support.unlink(self.filename)
    ...
    def test_read_last_object_from_file(self):
        ...
        for v in range(marshal.version + 1):
            ...
            with open(self.filename, 'wb') as f:
                f.write(data + b'xxxx')
            r, p = _testcapi.pymarshal_read_last_object_from_file(self.cfilename)
            support.unlink(self.filename)
            ...
            with open(self.filename, 'wb') as f:
                f.write(data[:1])
            with self.assertRaises(EOFError):
                _testcapi.pymarshal_read_last_object_from_file(self.cfilename)
            support.unlink(self.filename)
    ...
    def test_read_object_from_file(self):
        ...
        for v in range(marshal.version + 1):
            ...
            with open(self.filename, 'wb') as f:
                f.write(data + b'xxxx')
            r, p = _testcapi.pymarshal_read_object_from_file(self.cfilename)
            support.unlink(self.filename)
            self.assertEqual(r, obj)
            ...
            with open(self.filename, 'wb') as f:
                f.write(data[:1])
            with self.assertRaises(EOFError):
                _testcapi.pymarshal_read_object_from_file(self.cfilename)
            support.unlink(self.filename)
