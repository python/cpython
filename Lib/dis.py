...
def _get_instructions_bytes(code, varname_from_oparg=None,
    ...
    for offset, op, arg, *extra_args in _unpack_opargs(code):
        ...
        if arg is not None:
            ...
            argval = arg
            if deop in hasconst:
                argval, argrepr = _get_const_info(deop, arg, co_consts, consts_idx=consts_idx)
                ...
                argval = offset + (signed_arg + _opsize(deop)) * 2
                if deop == FOR_ITER:
                    argval += 2
                argrepr = "to " + repr(argval)
...
def _disassemble_bytes(code, lasti=-1, varname_from_oparg=None,
    ...
    for instr in _get_instructions_bytes(code, varname_from_oparg, names,
                                         ...
                                         exception_entries=exception_entries,
                                         co_positions=co_positions,
                                         show_caches=show_caches, consts_idx=consts_idx):
        ...
        if arg is not None:
            ...
            if deop in hasjrel:
                if _is_backward_jump(deop):
                    arg = -arg
                label = offset + (arg + _opsize(deop)) * 2
                if deop == FOR_ITER:
                    label += 2
            elif deop in hasjabs:
                label = arg*2
