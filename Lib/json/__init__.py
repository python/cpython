...
def dump(obj, fp, *, skipkeys=False, ensure_ascii=True, check_circular=True,
    ...
    if (not skipkeys and ensure_ascii and
        ...
        cls is None and indent is None and separators is None and
        default is None and not sort_keys and not kw):
        json_string = _default_encoder.encode(obj)  # Encode JSON in one go
    else:
        if cls is None:
            cls = JSONEncoder
        json_string = cls(skipkeys=skipkeys, ensure_ascii=ensure_ascii,
            check_circular=check_circular, allow_nan=allow_nan, indent=indent,
            separators=separators,
            default=default, sort_keys=sort_keys, **kw).encode(obj)

    # Perform a single efficient write operation
