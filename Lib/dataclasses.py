...
def _init_fn(fields, frozen, has_post_init, self_name, globals):
    ...
    arg_fields, kw_arg_fields = _fields_in_init_order(fields)
    _init_params = [_init_param(f) for f in arg_fields]
    if kw_arg_fields:
        # Add the keyword-only args.  Because the * can only be added if
        # there's at least one keyword-only arg, there needs to be a test here
        # (instead of just concatenting the lists together).
        _init_params += ['*']
        _init_params += [_init_param(f) for f in kw_arg_fields]
...
def _process_class(cls, init, repr, eq, order, unsafe_hash, frozen, kw_only):
    ...
    if '__match_args__' not in cls.__dict__:
        args, kw_args = _fields_in_init_order(flds)
        cls.__match_args__ = tuple(f.name for f in args)
...
def dataclass(cls=None, /, *, init=True, repr=True, eq=True, order=False,
    ...
    not be assigned to after instance creation. If kw_only is true,
    then by default all fields are keyword-only.
    """
