...
def _create_fn(name, args, body, *, globals=None, locals=None,
    ...
    if return_type is not MISSING:
        locals['__dataclass_return_type'] = return_type
        return_annotation = '->__dataclass_return_type'
    args = ','.join(args)
...
def _field_init(f, frozen, globals, self_name, slots):
    ...
    default_name = f'__dataclass_dflt_{f.name}'
...
def _init_param(f):
    ...
    if f.default is MISSING and f.default_factory is MISSING:
        ...
        # it up.
        default = f'=__dataclass_dflt_{f.name}'
    elif f.default_factory is not MISSING:
        # There's a factory function.  Set a marker.
        default = '=__dataclass_HAS_DEFAULT_FACTORY'
    return f'{f.name}:__dataclass_type_{f.name}{default}'
...
def _init_fn(fields, std_fields, kw_only_fields, frozen, has_post_init,
    ...
    locals = {f'__dataclass_type_{f.name}': f.type for f in fields}
    locals.update({
        '__dataclass_HAS_DEFAULT_FACTORY': _HAS_DEFAULT_FACTORY,
