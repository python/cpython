...
def _signature_from_callable(obj, *,
    ...
    if isinstance(obj, type):
        ...
        unwrapped_new = _signature_get_user_defined_method(obj, '__new__')
        unwrapped_init = _signature_get_user_defined_method(obj, '__init__')

        # Go through the MRO and see if any class has user-defined
        # pure Python __new__ or __init__ method
        for base in obj.__mro__:
            # Now we check if the 'obj' class has an own '__new__' method
            if unwrapped_new is not None and '__new__' in base.__dict__:
                sig = _get_signature_of(unwrapped_new)
                if skip_bound_arg:
                    sig = _signature_bound_method(sig)
                return sig
            # or an own '__init__' method
            elif unwrapped_init is not None and '__init__' in base.__dict__:
                return _get_signature_of(unwrapped_init)
