...
class zipimporter:
    ...
    def find_loader(self, fullname, path=None):
        ...
        if _is_dir(self, modpath):
            ...
            return None, ['%s%s%s' % (self.archive, path_sep, modpath)]
    ...
    def load_module(self, fullname):
        ...
        try:
            if ispackage:
                ...
                # executed
                path = _get_module_path(self, fullname)
                fullpath = '%s%s%s' % (self.archive, path_sep, path)
...
def _read_directory(archive):
    try:
        ...
        raise ZipImportError("can't open Zip file: %r" % (archive,), path=archive)
    ...
    with fp:
        try:
            ...
            raise ZipImportError("can't read Zip file: %r" % (archive,), path=archive)
        if len(buffer) != 22:
            raise ZipImportError("can't read Zip file: %r" % (archive,), path=archive)
        if buffer[:4] != b'PK\x05\x06':
            # Bad: End of Central Dir signature
            errmsg = 'not a Zip file'
            raise ZipImportError('%s: %r' % (errmsg, archive), path=archive)
        ...
        while True:
            ...
            path = '%s%s%s' % (archive, path_sep, name)
