...
def dump(node, annotate_fields=True, include_attributes=False, *, indent=None):
    ...
    def _format(node, level=0):
        ...
        if isinstance(node, AST):
            ...
            return '%s(%s%s%s)' % (node.__class__.__name__, inprefix, sep.join(args), outprefix), False
        elif isinstance(node, list):
            if not node:
                return '[]', True
            return '[%s%s%s]' % (inprefix, sep.join(_format(x, level)[0] for x in node), outprefix), False
