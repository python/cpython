"""A bytecode browser for IDLE.

The Browse menu's "Bytecode Browser" command (see open() below) shows
...
def open(editwin):
    "Open the bytecode browser for editwin, reusing one already open."
    window = getattr(editwin, "bytecode_browser", None)
    if window is not None and window.winfo_exists():
        window.refresh()
    else:
        editwin.bytecode_browser = BytecodeBrowserWindow(
            editwin.top, editwin.text, editwin)


class BytecodeBrowserWindow(Toplevel):
    "Show the bytecode of a Text widget's content or selection."

    def __init__(self, parent, text, editwin=None, *,
                 _htest=False, _utest=False):
        """Create the bytecode browser.
    ...
    def leave_debug_mode(self, event=None):
        "Return from a stopped frame to the editor's own bytecode."
        if self.debugging:
...
def _bytecode_browser(parent):  # htest #
    "Set up a sample editor Text and open a bytecode browser on it."
    ...
    return BytecodeBrowserWindow(top, text, _htest=True)


if __name__ == "__main__":
    from unittest import main
    main('idlelib.idle_test.test_bytecodebrowser', verbosity=2, exit=False)

    from idlelib.idle_test.htest import run
    run(_bytecode_browser)
