...
Problem with running coverage with repository python is that coverage
uses absolute imports for its submodules, hence needs to be in a
directory in sys.path.  One solution: copy the package to the directory
containing the cpython repository.  Call it 'dev'

Add dev/.coveragerc containing something like the following:
---
...
exclude_lines =
    ...
    if _htest:
---
The 'branch = True' and last three exclude lines are added.  If file
exists already (coverage will create it), add these four lines.

Optionally, add a script like the following:
---
@echo off
...
---
The second parameter was needed for tests of module x not named test_x.
(There were several before modules were renamed, now only one.)