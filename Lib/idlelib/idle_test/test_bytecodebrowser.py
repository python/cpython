"Test bytecodebrowser, coverage 95%."
...
class BytecodeBrowserOpenTest(unittest.TestCase):
    ...
    def test_open_creates_window(self):
        ...
        with mock.patch.object(bytecodebrowser, 'BytecodeBrowserWindow',
                               Func(result='window')) as window:
            bytecodebrowser.open(editwin)
        self.assertEqual(window.args, ('toplevel', 'text', editwin))
        self.assertEqual(editwin.bytecode_browser, 'window')

    def test_open_reuses_window(self):
        editwin = self.make_editwin()
        editwin.bytecode_browser = existing = Func()   # A live window.
...
class GuiTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        root.withdraw()
        cls.text = Text(root)
        cls.window = bytecodebrowser.BytecodeBrowserWindow(root, cls.text, _utest=True)
...
class BytecodeBrowserWindowTest(GuiTest):
    ...
    def test_colorize_groups(self):
        ...
        self.assertEqual(
            ...
            bytecodebrowser.GROUP_COLORS["code"])
    ...
    def test_focused_highlights_and_moves_cursor(self):
        ...
        ranges = [str(i) for i in self.text.tag_ranges(bytecodebrowser.TAG)]
    ...
    def test_not_focused_keeps_editor_clean(self):
        ...
        window.tree.selection_set(self.instr_at(("4.11", "4.12")))
        window.select_instrs()
        self.assertEqual(self.text.tag_ranges(bytecodebrowser.TAG), ())
    ...
    def test_highlight_follows_focus(self):
        ...
        self.assertNotEqual(self.text.tag_ranges(bytecodebrowser.TAG), ())
    ...
    def test_hide(self):
        ...
        window.tree.selection_set(item)
        window.select_instrs()
        self.assertNotEqual(text.tag_ranges(bytecodebrowser.TAG), ())
        window.hide()                  # Double-click (or Escape) hides it.
        self.assertEqual(window.wm_state(), "withdrawn")   # Not destroyed.
        self.assertTrue(window.winfo_exists())
        self.assertEqual(text.tag_ranges(bytecodebrowser.TAG), ())
        window.destroy()
        text.destroy()
...
class BytecodeBrowserDebugTest(GuiTest):
