...
command-line and writes the result to a file::
    ...
    args.log.write(f'{sum(args.integers)}')
    args.log.close()
...
class HelpFormatter(object):
    ...
    def _format_args(self, action, default_metavar):
        ...
        match (action.nargs, metavar):
            case (None, (var,)):
                result = f'{var}'
            case ('?', (var,)):  # OPTIONAL
                result = f'[{var}]'
            case ('*', (var,)):  # ZERO_OR_MORE
                result = f'[{var} ...]'
            case ('*', (var1, var2)):  # ZERO_OR_MORE
                result = f'[{var1} [{var2} ...]]'
            case ('+', _) if len(_metavar := get_metavar(2)) == 2:  # ONE_OR_MORE
                result = f'{_metavar[0]} [{_metavar[1]} ...]'
            case ('...', _):  # REMAINDER
                result = '...'
            case ('A...', (var,)):  # PARSER
                result = f'{var} ...'
            case ('==SUPPRESS==', _):  # SUPPRESS
                result = ''
            case (nargs, _) if nargs in {
                None, OPTIONAL, ZERO_OR_MORE, ONE_OR_MORE, REMAINDER, PARSER, SUPPRESS
            }:
                raise TypeError('nargs and metavar do not match')
            case _:
                if not isinstance(action.nargs, int):
                    raise ValueError('invalid nargs value') from None
                metavar = get_metavar(action.nargs)
                if len(metavar) != action.nargs:
                    raise TypeError('nargs and metavar lengths do not match')
                result = ' '.join([str(var) for var in metavar])
...
class ArgumentError(Exception):
    ...
    def __str__(self):
        if self.argument_name is None:
            ...
            error_msg = _('argument %(argument_name)s: %(message)s')
            return error_msg % dict(message=self.message,
...
class FileType(object):
    ...
    def __repr__(self):
        ...
        args_str = ', '.join([repr(arg) for arg in args if arg != -1] +
                             [f'{kw}={arg!r}' for kw, arg in kwargs
                              if arg is not None])
