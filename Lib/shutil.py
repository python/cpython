...
def copyfile(src, dst, *, follow_symlinks=True, ignore_same_file=False):
    """Copy data from src to dst in the most efficient way possible.

    If follow_symlinks is not set and src is a symbolic link, a new
    symlink will be created instead of copying the file it points to.

    If ignore_same_file is not set and src and dst are the same file, a "SameFileError"
    exception will be raised. If the ignore_same_file is set, the function does noting
    and returns dst.
