...
class GeckoCollector(Collector):
    ...
    def _process_stack(self, thread_data, frames):
        ...
        for frame_tuple in reversed(frames):
            ...
            if stack_key in stack_cache:
                ...
                # Determine category
                is_python = not filename.startswith("<") or filename in [
                    "<stdin>",
                    "<string>",
                ]
                category = 1 if is_python else 2  # Python or Native
                stack_category.append(category)
                stack_subcategory.append(0)  # Other
    ...
    def _get_or_create_resource(self, thread_data, filename):
        ...
        resource_types.append(1)  # Library type
    ...
    def _get_or_create_frame(self, thread_data, func_idx, lineno):
        ...
        category = 1 if is_python else 2

        frame_addresses.append(-1)
        frame_inline_depths.append(0)
        frame_categories.append(category)
        frame_subcategories.append(0)
    ...
    def export(self, filename):
        ...
        print(
            f"Open in Firefox Profiler: https://profiler.firefox.com/from-file/{os.path.abspath(filename)}"
        )
    ...
    def _build_profile(self):
        ...
        profile = {
            "meta": {
                ...
                "symbolicated": True,
                "markerSchema": [],
                "importedFrom": "Python Sampling Profiler",
                "extensions": {
                    "id": [],
                    "name": [],
                    "baseURL": [],
