...
class Maildir(Mailbox):
    ...
    def __setitem__(self, key, message):
        ...
        if self.colon in os.fspath(dominant_subpath):
            suffix = self.colon + os.fspath(dominant_subpath).split(self.colon)[-1]
        else:
            suffix = ''
        self.discard(key)
    ...
    def _refresh(self):
        ...
        for subdir in self._toc_mtimes:
            ...
            for entry in os.listdir(path):
                ...
                self._toc[uniq] = Path(subdir) / Path(entry)
        self._last_read = time.time()

    def _lookup(self, key):
        """Use TOC to return subpath for a given key, or raise a KeyError."""
        try:
    ...
    def __init__(self, path, factory=None, create=True):
        ...
        if not self._path.exists():
            if create:
                ...
                raise NoSuchMailboxError(os.fspath(self._path))
        self._locked = False
