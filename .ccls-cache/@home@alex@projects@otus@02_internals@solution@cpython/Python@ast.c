...
static stmt_ty
ast_for_incr_decr_stmt(struct compiling *c, const node *n, int direction)
{
    node *ch = CHILD(n, 0);
    operator_ty op = direction > 0 ? Add : Sub;
    expr_ty expr1, expr2;
    ...
    return AugAssign(expr1, op, expr2, LINENO(n), n->n_col_offset, c->c_arena);
}
...
ast_for_expr_stmt(struct compiling *c, const node *n)
{
    ...
    if (NCH(n) == 1) {
        ...
        return ast_for_incr_decr_stmt(c, n, TYPE(CHILD(n, 1)) == incr_stmt ? 1 : -1);
    } else {
        int i;
        asdl_seq *targets;
        node *value;
