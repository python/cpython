...
def get_input(fl_state):
    data = sys.stdin.read(1)
    if data == '\x1b':
        make_stdin_non_blocking(fl_state)
        codes = ""
        while True:
            ch = sys.stdin.read(1)
            if ch == '':
                reset_fs_state(fl_state)
                break
            else:
                codes += ch
        data += codes
