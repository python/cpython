...
_PyObject_HasLen(PyObject *o) {
    ...
    return (type->tp_as_sequence->sq_length || type->tp_as_mapping->mp_length);
}
...
PyObject_CheckBuffer(PyObject *obj)
{
    ...
    return (tp_as_buffer->bf_getbuffer != NULL);
}
           ...
           )
{
    ...
    ternaryfunc slotv = NB_TERNOP(mv, op_slot);
...
PyObject_GetAIter(PyObject *o) {
    ...
    if (t->tp_as_async->am_aiter == NULL) {
...
{
    PyTypeObject *tp = Py_TYPE(obj);
    return (tp->tp_as_async->am_anext != NULL &&
...
PyIter_Send(PyObject *iter, PyObject *arg, PyObject **result)
{
    assert(arg != NULL);
    assert(result != NULL);
    if (Py_TYPE(iter)->tp_as_async->am_send) {
