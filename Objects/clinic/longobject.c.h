...
int_to_bytes(PyObject *self, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    if (!PyUnicode_Check(args[1])) {
        _PyArg_BadArgument("to_bytes", 2, "argument 'byteorder'", "str", args[1]);
        goto exit;
    }
...
int_from_bytes(PyTypeObject *type, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    bytes_obj = args[0];
    if (!PyUnicode_Check(args[1])) {
        _PyArg_BadArgument("from_bytes", 2, "argument 'byteorder'", "str", args[1]);
        goto exit;
    }
    if (PyUnicode_READY(args[1]) == -1) {
        goto exit;
...
/*[clinic end generated code: output=05b782a0f235b5d2 input=a9049054013a1b77]*/
