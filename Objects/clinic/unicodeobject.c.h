...
unicode_encode(PyObject *self, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    if (args[0]) {
        if (!PyUnicode_Check(args[0])) {
            _PyArg_BadArgument("encode", 1, "encoding", "str", args[0]);
            goto exit;
        }
    ...
    if (!PyUnicode_Check(args[1])) {
        _PyArg_BadArgument("encode", 2, "errors", "str", args[1]);
        goto exit;
    }
...
unicode_replace(PyObject *self, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (!PyUnicode_Check(args[0])) {
        _PyArg_BadArgument("replace", 1, NULL, "str", args[0]);
        goto exit;
    }
    ...
    old = args[0];
    if (!PyUnicode_Check(args[1])) {
        _PyArg_BadArgument("replace", 2, NULL, "str", args[1]);
        goto exit;
    }
...
unicode_maketrans(void *null, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (!PyUnicode_Check(args[1])) {
        _PyArg_BadArgument("maketrans", 2, NULL, "str", args[1]);
        goto exit;
    }
    if (PyUnicode_READY(args[1]) == -1) {
        goto exit;
    ...
    if (!PyUnicode_Check(args[2])) {
        _PyArg_BadArgument("maketrans", 3, NULL, "str", args[2]);
        goto exit;
    }
...
unicode___format__(PyObject *self, PyObject *arg)
{
    ...
    if (!PyUnicode_Check(arg)) {
        _PyArg_BadArgument("__format__", 0, NULL, "str", arg);
        goto exit;
    }
...
/*[clinic end generated code: output=1614a54b592397f6 input=a9049054013a1b77]*/
