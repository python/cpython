...
"     ntmps, flags, codestring, constants, names, varnames, filename,\n"
"     name, qualname, firstlineno, linetable, exceptiontable,\n"
"     freevars=(), cellvars=(), /)\n"
"--\n"
"\n"
...
code_new(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    ntmps = _PyLong_AsInt(PyTuple_GET_ITEM(args, 5));
    if (ntmps == -1 && PyErr_Occurred()) {
        goto exit;
    }
    flags = _PyLong_AsInt(PyTuple_GET_ITEM(args, 6));
    if (flags == -1 && PyErr_Occurred()) {
        goto exit;
    }
    if (!PyBytes_Check(PyTuple_GET_ITEM(args, 7))) {
        _PyArg_BadArgument("code", "argument 8", "bytes", PyTuple_GET_ITEM(args, 7));
        goto exit;
    }
    code = PyTuple_GET_ITEM(args, 7);
    if (!PyTuple_Check(PyTuple_GET_ITEM(args, 8))) {
        _PyArg_BadArgument("code", "argument 9", "tuple", PyTuple_GET_ITEM(args, 8));
        goto exit;
    }
    consts = PyTuple_GET_ITEM(args, 8);
    if (!PyTuple_Check(PyTuple_GET_ITEM(args, 9))) {
        _PyArg_BadArgument("code", "argument 10", "tuple", PyTuple_GET_ITEM(args, 9));
        goto exit;
    }
    names = PyTuple_GET_ITEM(args, 9);
    if (!PyTuple_Check(PyTuple_GET_ITEM(args, 10))) {
        _PyArg_BadArgument("code", "argument 11", "tuple", PyTuple_GET_ITEM(args, 10));
        goto exit;
    }
    ...
    filename = PyTuple_GET_ITEM(args, 11);
    ...
    name = PyTuple_GET_ITEM(args, 12);
    if (!PyUnicode_Check(PyTuple_GET_ITEM(args, 13))) {
        _PyArg_BadArgument("code", "argument 14", "str", PyTuple_GET_ITEM(args, 13));
        goto exit;
    }
    if (PyUnicode_READY(PyTuple_GET_ITEM(args, 13)) == -1) {
        goto exit;
    }
    qualname = PyTuple_GET_ITEM(args, 13);
    firstlineno = _PyLong_AsInt(PyTuple_GET_ITEM(args, 14));
    ...
    linetable = PyTuple_GET_ITEM(args, 15);
...
skip_optional:
    return_value = code_new_impl(type, argcount, posonlyargcount, kwonlyargcount, nlocals, stacksize, ntmps, flags, code, consts, names, varnames, filename, name, qualname, firstlineno, linetable, exceptiontable, freevars, cellvars);
...
"        co_ntmps=-1, co_flags=-1, co_firstlineno=-1, co_code=None,\n"
...
code_replace(PyCodeObject *self, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    static const char * const _keywords[] = {"co_argcount", "co_posonlyargcount", "co_kwonlyargcount", "co_nlocals", "co_stacksize", "co_ntmps", "co_flags", "co_firstlineno", "co_code", "co_consts", "co_names", "co_varnames", "co_freevars", "co_cellvars", "co_filename", "co_name", "co_qualname", "co_linetable", "co_exceptiontable", NULL};
    ...
    if (args[5]) {
        co_ntmps = _PyLong_AsInt(args[5]);
        if (co_ntmps == -1 && PyErr_Occurred()) {
            goto exit;
        }
        if (!--noptargs) {
            goto skip_optional_kwonly;
    ...
    if (args[7]) {
        co_firstlineno = _PyLong_AsInt(args[7]);
    ...
    if (args[8]) {
        if (!PyBytes_Check(args[8])) {
            _PyArg_BadArgument("replace", "argument 'co_code'", "bytes", args[8]);
            goto exit;
        }
        co_code = (PyBytesObject *)args[8];
        if (!--noptargs) {
            goto skip_optional_kwonly;
        }
    }
    if (args[9]) {
        if (!PyTuple_Check(args[9])) {
            _PyArg_BadArgument("replace", "argument 'co_consts'", "tuple", args[9]);
            goto exit;
        }
        co_consts = args[9];
        if (!--noptargs) {
    ...
    if (args[16]) {
        if (!PyUnicode_Check(args[16])) {
            _PyArg_BadArgument("replace", "argument 'co_qualname'", "str", args[16]);
            goto exit;
        }
        if (PyUnicode_READY(args[16]) == -1) {
            goto exit;
        }
        co_qualname = args[16];
        if (!--noptargs) {
    ...
    if (args[17]) {
        if (!PyBytes_Check(args[17])) {
            _PyArg_BadArgument("replace", "argument 'co_linetable'", "bytes", args[17]);
            goto exit;
        }
        co_linetable = (PyBytesObject *)args[17];
        if (!--noptargs) {
            goto skip_optional_kwonly;
        }
    }
...
skip_optional_kwonly:
    return_value = code_replace_impl(self, co_argcount, co_posonlyargcount, co_kwonlyargcount, co_nlocals, co_stacksize, co_ntmps, co_flags, co_firstlineno, co_code, co_consts, co_names, co_varnames, co_freevars, co_cellvars, co_filename, co_name, co_qualname, co_linetable, co_exceptiontable);
...
/*[clinic end generated code: output=3dc295d844d63af6 input=a9049054013a1b77]*/
