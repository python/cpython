...
bytearray_maketrans(void *null, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (!PyBuffer_IsContiguous(&frm, 'C')) {
        _PyArg_BadArgument("maketrans", 1, NULL, "contiguous buffer", args[0]);
        goto exit;
    }
    ...
    if (!PyBuffer_IsContiguous(&to, 'C')) {
        _PyArg_BadArgument("maketrans", 2, NULL, "contiguous buffer", args[1]);
        goto exit;
    }
...
bytearray_replace(PyByteArrayObject *self, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (!PyBuffer_IsContiguous(&old, 'C')) {
        _PyArg_BadArgument("replace", 1, NULL, "contiguous buffer", args[0]);
        goto exit;
    }
    ...
    if (!PyBuffer_IsContiguous(&new, 'C')) {
        _PyArg_BadArgument("replace", 2, NULL, "contiguous buffer", args[1]);
        goto exit;
    }
    if (nargs < 3) {
        goto skip_optional;
...
bytearray_decode(PyByteArrayObject *self, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    if (args[0]) {
        if (!PyUnicode_Check(args[0])) {
            _PyArg_BadArgument("decode", 1, "encoding", "str", args[0]);
            goto exit;
        }
    ...
    if (!PyUnicode_Check(args[1])) {
        _PyArg_BadArgument("decode", 2, "errors", "str", args[1]);
        goto exit;
    }
...
bytearray_fromhex(PyTypeObject *type, PyObject *arg)
{
    ...
    if (!PyUnicode_Check(arg)) {
        _PyArg_BadArgument("fromhex", 0, NULL, "str", arg);
        goto exit;
    }
...
/*[clinic end generated code: output=05e4e168c88d7565 input=a9049054013a1b77]*/
