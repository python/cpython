...
gen_traverse(PyGenObject *gen, visitproc visit, void *arg)
{
    ...
    if (gen->gi_frame_valid) {
        _Py_framedata *fdata = (_Py_framedata *)(gen->gi_fdata);
        assert(fdata->frame_obj == NULL || fdata->frame_obj->f_owns_frame == 0);
        int err = _PyFrame_Traverse(fdata, visit, arg);
        if (err) {
            return err;
        }
    }
...
_PyGen_Finalize(PyObject *self)
{
    ...
    if (gen->gi_frame_valid == 0 || _PyFrameHasCompleted((_Py_framedata *)gen->gi_fdata)) {
    ...
    if (gen->gi_code != NULL &&
        ...
        ((_Py_framedata *)gen->gi_fdata)->state == FRAME_CREATED)
    {
             ...
             int exc, int closing)
{
    ...
    *presult = NULL;
    if (fdata->state == FRAME_CREATED && arg && arg != Py_None) {
    ...
    if (gen->gi_frame_valid && _PyFrame_IsExecuting(fdata)) {
    ...
    if (gen->gi_frame_valid == 0 || _PyFrameHasCompleted(fdata)) {
    ...
    assert(_PyFrame_IsRunnable(fdata));
    /* Push arg onto the frame's value stack */
    result = arg ? arg : Py_None;
    Py_INCREF(result);
    _PyFrame_StackPush(fdata, result);

    fdata->previous = tstate->cframe->current_frame;
...
_PyGen_yf(PyGenObject *gen)
{
    ...
    if (gen->gi_frame_valid) {
        ...
        if (code[(fdata->lasti-1)*sizeof(_Py_CODEUNIT)] != SEND || fdata->stacktop < 0)
            return NULL;
        yf = _PyFrame_StackPeek(fdata);
        Py_INCREF(yf);
    }
           ...
           PyObject *typ, PyObject *val, PyObject *tb)
{
    ...
    if (yf) {
        ...
        if (PyGen_CheckExact(yf) || PyCoro_CheckExact(yf)) {
            ...
            _Py_framedata *prev = tstate->cframe->current_frame;
            fdata->previous = prev;
            tstate->cframe->current_frame = fdata;
        ...
        if (!ret) {
            ...
            assert(gen->gi_frame_valid);
            ret = _PyFrame_StackPop((_Py_framedata *)gen->gi_fdata);
            assert(ret == yf);
            Py_DECREF(ret);
            ...
            /* Backup to SEND */
            fdata->lasti--;
            assert(code[fdata->lasti*sizeof(_Py_CODEUNIT)] == SEND);
            int jump = code[fdata->lasti*sizeof(_Py_CODEUNIT)+1];
            fdata->lasti += jump;
...
gen_getrunning(PyGenObject *gen, void *Py_UNUSED(ignored))
{
    ...
    return PyBool_FromLong(_PyFrame_IsExecuting((_Py_framedata *)gen->gi_fdata));
}
...
gen_getsuspended(PyGenObject *gen, void *Py_UNUSED(ignored))
{
    ...
    return PyBool_FromLong(((_Py_framedata *)gen->gi_fdata)->state == FRAME_SUSPENDED);
}
...
_gen_getframe(PyGenObject *gen, const char *const name)
{
    ...
    return _Py_XNewRef((PyObject *)_PyFrame_GetFrameObject((_Py_framedata *)gen->gi_fdata));
}
...
{
    Py_ssize_t res;
    res = offsetof(PyGenObject, gi_fdata) + offsetof(_Py_framedata, localsplus);
...
PyTypeObject PyGen_Type = {
    ...
    "generator",                                /* tp_name */
    offsetof(PyGenObject, gi_fdata) +
    offsetof(_Py_framedata, localsplus),       /* tp_basicsize */
...
static PyObject *
compute_cr_origin(int origin_depth, _Py_framedata *current_frame);
                      ...
                      PyObject *name, PyObject *qualname)
{
    ...
    _Py_framedata *fdata = (_Py_framedata *)gen->gi_fdata;
    _PyFrame_Copy((_Py_framedata *)f->_f_owned_fdata, fdata);
    gen->gi_frame_valid = 1;
    assert(fdata->frame_obj == f);
    f->f_owns_frame = 0;
    f->f_fdata = fdata;
    fdata->is_generator = true;
...
cr_getsuspended(PyCoroObject *coro, void *Py_UNUSED(ignored))
{
    ...
    return PyBool_FromLong(((_Py_framedata *)coro->cr_fdata)->state == FRAME_SUSPENDED);
}
...
cr_getrunning(PyCoroObject *coro, void *Py_UNUSED(ignored))
{
    ...
    return PyBool_FromLong(_PyFrame_IsExecuting((_Py_framedata *)coro->cr_fdata));
}
...
PyTypeObject PyCoro_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0)
    "coroutine",                                /* tp_name */
    offsetof(PyCoroObject, cr_fdata) +
    offsetof(_Py_framedata, localsplus),       /* tp_basicsize */
    sizeof(PyObject *),                         /* tp_itemsize */
    /* methods */
    (destructor)gen_dealloc,                    /* tp_dealloc */
...
compute_cr_origin(int origin_depth, _Py_framedata *current_frame)
{
    _Py_framedata *fdata = current_frame;
...
PyTypeObject PyAsyncGen_Type = {
    ...
    offsetof(PyAsyncGenObject, ag_fdata) +
    offsetof(_Py_framedata, localsplus),       /* tp_basicsize */
    sizeof(PyObject *),                         /* tp_itemsize */
    /* methods */
    (destructor)gen_dealloc,                    /* tp_dealloc */
