...
/* Trashcan mechanism, thanks to Christian Tismer for the original.

When deallocating a container object, it's possible to trigger an
unbounded chain of deallocations, as each Py_DECREF in turn drops the
refcount on "the next" object in the chain to 0.  This can easily lead
to stack overflows, especially in threads (which typically have less
stack space to work with).

We avoid this by checking the nesting level of dealloc_gc() calls.

How it works:  Entering dealloc_gc() increments a call-depth counter.
So long as this counter is small, the dealloc method of the type is run
directly without further ado.  But if the counter gets large, it instead
adds the object to a list of objects to be deallocated later and skips
calling the deallocator.  In that case, dealloc_gc() returns without
deallocating anything (and so unbounded call-stack depth is avoided).

When the call stack finishes unwinding again, code generated by the END
macro notices this, and calls another routine to deallocate all the
objects that may have been added to the list of deferred deallocations.
In effect, a chain of N deallocations is broken into
(N-1)/(Py_TRASHCAN_HEADROOM-1) pieces, with the call stack never
exceeding a depth of Py_TRASHCAN_HEADROOM.
*/
...
_Py_Dealloc(PyObject *op)
{
    ...
    if (!_PyType_IS_GC(type)) {
        // definitely non-GC object, just call tp_dealloc
        dealloc_simple(op);
    }
    else if (type->tp_is_gc == NULL || type->tp_is_gc(op)) {
        // supports GC track, gets trashcan protection for tp_dealloc
        dealloc_gc(op);
    }
    else {
        // actually doesn't support GC tracking
        dealloc_simple(op);
    }
