...
mro_hierarchy(PyTypeObject *type, PyObject *temp)
{
    ...
    PyObject *tuple;
    if (old_mro != NULL) {
        tuple = PyTuple_MakeTriplet((PyObject *)type, new_mro, old_mro);
    }
    else {
        tuple = PyTuple_MakePair((PyObject *)type, new_mro);
    }
...
type_new_get_slots(type_new_ctx *ctx, PyObject *dict)
{
    ...
    if (PyUnicode_Check(slots)) {
        new_slots = PyTuple_MakeSingle(slots);
    }
    else {
...
type_new_get_bases(type_new_ctx *ctx, PyObject **type)
{
    ...
    if (nbases == 0) {
        ...
        PyObject *new_bases = PyTuple_MakeSingle((PyObject *)ctx->base);
        if (new_bases == NULL) {
            return -1;
        }
        ctx->bases = new_bases;
...
get_bases_tuple(PyObject *bases_in, PyType_Spec *spec)
{
    if (!bases_in) {
        ...
        if (!bases) {
            return PyTuple_MakeSingle((PyObject *)base);
        }
    ...
    return PyTuple_MakeSingle(bases_in);
}
...
object_getstate_default(PyObject *obj, int required)
{
    ...
    if (slotnames != Py_None && PyList_GET_SIZE(slotnames) > 0) {
        ...
        if (PyDict_GET_SIZE(slots) > 0) {
            ...
            state2 = PyTuple_MakePair(state, slots);
            Py_DECREF(state);
            if (state2 == NULL) {
                Py_DECREF(slotnames);
                Py_DECREF(slots);
...
reduce_newobj(PyObject *obj)
{
    ...
    if (kwargs == NULL || PyDict_GET_SIZE(kwargs) == 0) {
        ...
        newargs = PyTuple_MakeTriplet((PyObject *)Py_TYPE(obj), args, kwargs);
        Py_DECREF(args);
        Py_DECREF(kwargs);
        if (newargs == NULL) {
            Py_DECREF(newobj);
...
type_ready_set_bases(PyTypeObject *type, int initial)
{
    ...
    if (bases == NULL) {
        ...
        if (base == NULL) {
            ...
            bases = PyTuple_MakeSingle((PyObject *)base);
        }
        if (bases == NULL) {
            return -1;
        }
