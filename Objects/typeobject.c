...
type_abstractmethods(PyTypeObject *type, void *context)
{
    ...
    /* type itself has an __abstractmethods__ descriptor (this). Don't return
       that. */
    if (type != &PyType_Type)
        mod = PyDict_GetItemWithError(type->tp_dict, _Py_ID(__abstractmethods__));
    if (!mod) {
        if (!PyErr_Occurred()) {
...
type_get_annotations(PyTypeObject *type, void *context)
{
    ...
    if (PyDict_Contains(type->tp_dict, _Py_ID(__annotations__))) {
        ...
        */
        ...
        if (annotations) {
            int result = PyDict_SetItem(type->tp_dict, _Py_ID(__annotations__), annotations);
            if (result) {
...
type_repr(PyTypeObject *type)
{
    ...
    if (mod != NULL && !_PyUnicode_EQ(mod, _Py_ID(builtins)))
...
static PyObject *
vectorcall_method(PyObject *name,
                  PyObject *const *args, Py_ssize_t nargs)
{
    assert(nargs >= 1);
...
mro_invoke(PyTypeObject *type)
{
    ...
    if (custom) {
        int unbound;
        PyObject *mro_meth = lookup_method((PyObject *)type, _Py_ID(mro),
                                           &unbound);
        if (mro_meth == NULL)
            return NULL;
...
type_new_visit_slots(type_new_ctx *ctx)
{
    ...
    for (Py_ssize_t i = 0; i < nslot; i++) {
        ...
        if (_PyUnicode_EQ(name, _Py_ID(__dict__))) {
        ...
        if (_PyUnicode_EQ(name, _Py_ID(__weakref__))) {
...
type_new_copy_slots(type_new_ctx *ctx, PyObject *dict)
{
    ...
    for (Py_ssize_t i = 0; i < nslot; i++) {
        ...
        if ((ctx->add_dict &&
             _PyUnicode_EQ(slot, _Py_ID(__dict__))) ||
            (ctx->add_weak &&
        ...
        if (r > 0) {
            ...
            if (!_PyUnicode_EQ(slot, _Py_ID(__qualname__)) &&
                !_PyUnicode_EQ(slot, _Py_ID(__classcell__)))
            {
...
{
    PyHeapTypeObject *et = (PyHeapTypeObject *)type;
    PyObject *qualname = PyDict_GetItemWithError(type->tp_dict,
                                                 _Py_ID(__qualname__));
    if (qualname != NULL) {
...
{
    PyObject *cell = PyDict_GetItemWithError(type->tp_dict,
...
import_copyreg(void)
{
    ...
    /* Try to fetch cached copy of copyreg from sys.modules first in an
       ...
       #17408 and #19088). */
    copyreg_module = PyImport_GetModule(_Py_ID(copyreg));
...
_PyType_GetSlotNames(PyTypeObject *cls)
{
    ...
    /* Use _slotnames function from the copyreg module to find the slots
       ...
       in __slotnames__. */
    slotnames = PyObject_CallMethodOneArg(copyreg, _Py_ID(_slotnames),
                                          (PyObject *)cls);
    Py_DECREF(copyreg);
    if (slotnames == NULL)
        return NULL;
...
/*[clinic end generated code: output=2e157766f6b50094 input=f326b43fb8a4c5ff]*/
{
    ...
    if (objreduce == NULL) {
        objreduce = PyDict_GetItemWithError(PyBaseObject_Type.tp_dict,
                                               _Py_ID(__reduce__));
...
{ \
    ...
    return vectorcall_method(_Py_ID(OPSTR), stack, 1); \
}
...
{ \
    ...
    return vectorcall_method(_Py_ID(OPSTR), stack, 2); \
}
...
{ \
    ...
    if (Py_TYPE(self)->tp_as_number != NULL && \
        ...
        if (do_other && PyType_IsSubtype(Py_TYPE(other), Py_TYPE(self))) { \
            int ok = method_is_overloaded(self, other, _Py_ID(ROPSTR)); \
            if (ok < 0) { \
                return NULL; \
            } \
            if (ok) { \
                stack[0] = other; \
                stack[1] = self; \
                r = vectorcall_maybe(tstate, _Py_ID(ROPSTR), stack, 2); \
        ...
        stack[0] = self; \
        stack[1] = other; \
        r = vectorcall_maybe(tstate, _Py_ID(OPSTR), stack, 2); \
        if (r != Py_NotImplemented || \
    ...
    if (do_other) { \
        stack[0] = other; \
        stack[1] = self; \
        return vectorcall_maybe(tstate, _Py_ID(ROPSTR), stack, 2); \
    } \
    Py_RETURN_NOTIMPLEMENTED; \
}
...
super_getattro(PyObject *self, PyObject *name)
{
    ...
    if (PyUnicode_Check(name) &&
        ...
        _PyUnicode_EQ(name, _Py_ID(__class__)))
        goto skip;
                        ...
                        PyTypeObject **type_p, PyObject **obj_p)
{
    ...
    for (; i < co->co_nlocalsplus; i++) {
        ...
        assert(PyUnicode_Check(name));
        if (_PyUnicode_EQ(name, _Py_ID(__class__))) {
