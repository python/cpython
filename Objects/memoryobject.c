...
memory_subscript(PyMemoryViewObject *self, PyObject *key)
{
    ...
    if (PyIndex_Check(key)) {
        ...
        if (PySlice_Unpack((PySliceObject *)key, &start, &stop, &step) < 0) {
            return NULL;
        }
        slicelength = PySlice_AdjustIndices(get_shape0(view), &start, &stop,
                                            step);
...
memory_ass_sub(PyMemoryViewObject *self, PyObject *key, PyObject *value)
{
    ...
    if (PyIndex_Check(key)) {
        ...
        len = PySlice_AdjustIndices(get_shape0(view), &start, &stop, step);
        if (step != 1) {
            PyErr_SetNone(PyExc_NotImplementedError);
            return -1;
        }
