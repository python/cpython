...
PyFrame_FastToLocalsWithError(PyFrameObject *f)
{
    ...
    for (int offset = 0; offset < co->co_nfastlocals; offset++) {
        PyObject *name;
        _PyFastLocalKind kind;
        _PyCode_FastInfoFromOffset(co, offset, &name, &kind);
        ...
        */
        if (kind == CO_FAST_FREE && (co->co_flags & CO_OPTIMIZED) == 0) {
            continue;
        }

        PyObject *value = fast[offset];
...
PyFrame_LocalsToFast(PyFrameObject *f, int clear)
{
    ...
    for (int offset = 0; offset < co->co_nfastlocals; offset++) {
        ...
        /* Same test as in PyFrame_FastToLocalsWithError() above. */
        if (kind == CO_FAST_FREE && (co->co_flags & CO_OPTIMIZED) == 0) {
            continue;
        }
        ...
        if (kind & (CO_FAST_CELL | CO_FAST_FREE)) {
            assert(PyCell_Check(fast[offset]));
            if (PyCell_GET(fast[offset]) != value) {
                if (PyCell_Set(fast[offset], value) < 0) {
                    PyErr_Clear();
                }
            }
        } else if (fast[offset] != value) {
            Py_XINCREF(value);
