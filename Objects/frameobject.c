...
static int64_t *
markblocks(PyCodeObject *co)
{
    const _Py_CODEUNIT *code = (const _Py_CODEUNIT *)PyBytes_AS_STRING(co->co_code);
    int len = PyBytes_Size(co->co_code)/sizeof(_Py_CODEUNIT);
...
const char *explain_incompatible_block_stack(int64_t to_stack) {
    if (to_stack < 0) {
        return "can't jump to unreachable code";
    }
...
static int
first_line_not_before(PyCodeObject *co, int line)
{
    PyLineSpan bounds;
    _PyCode_InitLineSpan(&bounds, co);
    int result = INT_MAX;
    for (int addr = 0; addr < PyBytes_GET_SIZE(co->co_code); addr += sizeof(_Py_CODEUNIT)) {
        int l = _PyCode_CheckLineNumber(co, addr, &bounds);
        if (l == line) {
            return line;
        }
        if (l < result && l > line) {
...
frame_setlineno(PyFrameObject *f, PyObject* p_new_lineno, void *Py_UNUSED(ignored))
{
...

    {
        ...
        new_lineno = first_line_not_before(f->f_code, new_lineno);
        if (new_lineno < 0) {
            PyErr_Format(PyExc_ValueError,
    ...
    unsigned char* codestr = (unsigned char*)PyBytes_AS_STRING(f->f_code->co_code);
    if (codestr[f->f_lasti] == YIELD_VALUE || codestr[f->f_lasti] == YIELD_FROM) {
        PyErr_SetString(PyExc_ValueError,
                "can't jump from a 'yield' statement");
        return -1;
    }

    int64_t *blocks = markblocks(f->f_code);
    if (blocks == NULL) {
        return -1;
    }
    ...
    PyLineSpan bounds;
    _PyCode_InitLineSpan(&bounds, f->f_code);
    int64_t best_block_stack = -1;
    int best_addr = -1;
    const char *error = NULL;
    for (int addr = 0; addr < PyBytes_GET_SIZE(f->f_code->co_code); addr += sizeof(_Py_CODEUNIT)) {
        int line = _PyCode_CheckLineNumber(f->f_code, addr, &bounds);
        if (line < 0) {
            // Artificial code
            continue;
        }
        if (line != last_line) {
            last_line = line;
            if (line == new_lineno) {
                int64_t target_block_stack = blocks[addr/sizeof(_Py_CODEUNIT)];
