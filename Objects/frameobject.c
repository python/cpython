...
frame_getlocals(PyFrameObject *f, void *closure)
{
    if (PyFrame_FastToLocalsWithError(f) < 0)
        return NULL;
    PyObject *locals = f->f_fdata->locals;
    Py_INCREF(locals);
    return locals;
}
...
PyFrame_GetLineNumber(PyFrameObject *f)
{
    ...
    if (f->f_lineno != 0) {
        ...
        return PyCode_Addr2Line(f->f_fdata->code, f->f_fdata->lasti*2);
    }
}
...
{
    if (f->f_fdata->lasti < 0) {
        return PyLong_FromLong(-1);
    }
    return PyLong_FromLong(f->f_fdata->lasti*2);
}

static PyObject *
frame_getglobals(PyFrameObject *f, void *closure)
{
    PyObject *globals = f->f_fdata->globals;
    if (globals == NULL) {
        globals = Py_None;
    }
    Py_INCREF(globals);
...
{
    PyObject *builtins = f->f_fdata->builtins;
    if (builtins == NULL) {
        builtins = Py_None;
    }
    Py_INCREF(builtins);
...
{
    assert(f->f_fdata->stackdepth > 0);
    f->f_fdata->stackdepth--;
    PyObject *v = f->f_fdata->stack[f->f_fdata->stackdepth];
    Py_DECREF(v);
}
...
frame_setlineno(PyFrameObject *f, PyObject* p_new_lineno, void *Py_UNUSED(ignored))
{
    ...
    if (new_lineno < f->f_fdata->code->co_firstlineno) {
        PyErr_Format(PyExc_ValueError,
    ...
    int len = (int)(PyBytes_GET_SIZE(f->f_fdata->code->co_code) / sizeof(_Py_CODEUNIT));
    int *lines = marklines(f->f_fdata->code, len);
    if (lines == NULL) {
        return -1;
    }
    ...
    int64_t *stacks = mark_stacks(f->f_fdata->code, len);
    if (stacks == NULL) {
        PyMem_Free(lines);
        return -1;
    }
    ...
    int best_addr = -1;
    int64_t start_stack = stacks[f->f_fdata->lasti];
    int err = -1;
    ...
    /* Unwind block stack. */
    if (f->f_fdata->state == FRAME_SUSPENDED) {
...
frame_dealloc(PyFrameObject *f)
{
    ...
    if (f->f_own_locals_memory) {
        ...
        _Py_InterpreterFrame *fdata = f->f_fdata;
        /* Don't clear code object until the end */
        co = fdata->code;
        fdata->code = NULL;
        Py_CLEAR(fdata->globals);
        Py_CLEAR(fdata->builtins);
...
frame_traverse(PyFrameObject *f, visitproc visit, void *arg)
{
    ...
    return _Py_InterpreterFrame_Traverse(f->f_fdata, visit, arg);
}
...
frame_tp_clear(PyFrameObject *f)
{
     ...
     */
    f->f_fdata->state = FRAME_CLEARED;

    Py_CLEAR(f->f_trace);

    /* locals */
    PyObject **locals = _Py_InterpreterFrame_GetLocalsArray(f->f_fdata);
    for (int i = 0; i < f->f_fdata->nlocalsplus; i++) {
        Py_CLEAR(locals[i]);
    }

    /* stack */
    for (int i = 0; i < f->f_fdata->stackdepth; i++) {
        Py_CLEAR(f->f_fdata->stack[i]);
    }
    f->f_fdata->stackdepth = 0;
    return 0;
}
...
{
    if (_Py_InterpreterFrame_IsExecuting(f->f_fdata)) {
        PyErr_SetString(PyExc_RuntimeError,
                        "cannot clear an executing frame");
        return NULL;
    }
    if (f->f_fdata->generator) {
        _PyGen_Finalize(f->f_fdata->generator);
        assert(f->f_fdata->generator == NULL);
    }
...
frame_alloc(_Py_InterpreterFrame *fdata, int owns)
{
    PyFrameObject *f;
    struct _Py_frame_state *state = get_frame_state();
    if (state->free_list == NULL)
    {
        ...
        if (f == NULL) {
            if (owns) {
                Py_XDECREF(fdata->code);
                Py_XDECREF(fdata->builtins);
                Py_XDECREF(fdata->globals);
...
_PyFrame_New_NoTrack(_Py_InterpreterFrame *fdata, int owns)
{
            ...
            PyObject *globals, PyObject *locals)
{
    ...
    _Py_InterpreterFrame *fdata = allocate_heap_frame(&desc, locals);
    if (fdata == NULL) {
        return NULL;
    }
    PyFrameObject *f = _PyFrame_New_NoTrack(fdata, 1);
    if (f) {
...
static int
_Py_InterpreterFrame_OpAlreadyRan(_Py_InterpreterFrame *fdata, int opcode, int oparg)
{
    const _Py_CODEUNIT *code =
        (const _Py_CODEUNIT *)PyBytes_AS_STRING(fdata->code->co_code);
    for (int i = 0; i < fdata->lasti; i++) {
...
int
_Py_InterpreterFrame_FastToLocalsWithError(_Py_InterpreterFrame *fdata) {
    /* Merge fast locals into fdata->locals */
    PyObject *locals;
    PyObject **fast;
    PyCodeObject *co;
    locals = fdata->locals;
    ...
    co = fdata->code;
    fast = _Py_InterpreterFrame_GetLocalsArray(fdata);
    for (int i = 0; i < co->co_nlocalsplus; i++) {
            ...
            if (kind & CO_FAST_FREE) {
                ...
                if (value != NULL) {
                    if (PyCell_Check(value) &&
                            _Py_InterpreterFrame_OpAlreadyRan(fdata, MAKE_CELL, i)) {
...
PyFrame_FastToLocalsWithError(PyFrameObject *f)
{
    ...
    return _Py_InterpreterFrame_FastToLocalsWithError(f->f_fdata);
}
...
void
_Py_InterpreterFrame_LocalsToFast(_Py_InterpreterFrame *fdata, int clear)
{
...
{
    if (f == NULL || f->f_fdata->state == FRAME_CLEARED) {
        return;
    }
    _Py_InterpreterFrame_LocalsToFast(f->f_fdata, clear);
}
...
PyFrame_GetBack(PyFrameObject *frame)
{
    ...
    if (back == NULL && frame->f_fdata->previous != NULL) {
        back = _Py_InterpreterFrame_GetFrameObject(frame->f_fdata->previous);
    }
    Py_XINCREF(back);
    return back;
}
