...
# Math library needed to get floor() or sqrt() functions
MATH_LIBS = ['m']
...
class PyBuildExt(build_ext):
    ...
    def build_extension(self, ext):
        ...
        except (CCompilerError, DistutilsError) as exc:
    ...
    def detect_readline_curses(self):
        ...
        if do_readline:
            ...
            self.add_ext( Extension('readline', ['readline.c'],
        ...
        if curses_library.startswith('ncurses'):
            ...
            self.add_ext( Extension('_curses', ['_cursesmodule.c'],
        ...
        if (module_enabled(self.extensions, '_curses') and
            ...
            self.add_ext( Extension('_curses_panel', ['_curses_panel.c'],
                                   include_dirs=curses_includes,
                                   define_macros=curses_defines,
    ...
    def detect_crypt(self):
        ...
        if not VXWORKS:
            self.add_ext( Extension('_crypt', ['_cryptmodule.c'], libraries=libs) )
    ...
    def detect_socket(self):
        # socket(2)
        if not VXWORKS:
            self.add_ext( Extension('_socket', ['socketmodule.c'],
    ...
    def detect_expat_elementtree(self):
        ...
        self.add_ext(Extension('pyexpat',
        ...
        if os.path.isfile(os.path.join(self.srcdir, 'Modules', '_elementtree.c')):
            ...
            self.add_ext(Extension('_elementtree',
                                  define_macros = define_macros,
                                  include_dirs = expat_inc,
                                  libraries = expat_lib,
    ...
    def detect_multibytecodecs(self):
        ...
        self.add_ext(Extension('_multibytecodec',
                              ['cjkcodecs/multibytecodec.c']))
        for loc in ('kr', 'jp', 'cn', 'tw', 'hk', 'iso2022'):
            self.add_ext(Extension('_codecs_%s' % loc,
    ...
    def detect_dbm_gdbm(self):
        ...
        if sqlite_incdir and sqlite_libdir:
            ...
            if sqlite_libdir and sqlite_libdir[0] in self.lib_dirs:
                sqlite_libdir = None
            self.add_ext(Extension('_sqlite3', sqlite_srcs,
        ...
        if ('gdbm' in dbm_order and
            ...
            self.add_ext( Extension('_gdbm', ['_gdbmmodule.c'],
                                   libraries = ['gdbm'] ) )
        else:
    ...
    def detect_compress_exts(self):
        ...
        if zlib_inc is not None:
            ...
            if version >= version_req:
                if (self.compiler.find_library_file(self.lib_dirs, 'z')):
                    ...
                    self.add_ext( Extension('zlib', ['zlibmodule.c'],
                                           libraries = ['z'],
        ...
        if have_zlib:
            ...
            libraries = []
            extra_link_args = []
        self.add_ext( Extension('binascii', ['binascii.c'],
                               extra_compile_args = extra_compile_args,
                               libraries = libraries,
        ...
        if (self.compiler.find_library_file(self.lib_dirs, 'bz2')):
            if MACOS:
                ...
                bz2_extra_link_args = ()
            self.add_ext( Extension('_bz2', ['_bz2module.c'],
                                   libraries = ['bz2'],
        ...
        if self.compiler.find_library_file(self.lib_dirs, 'lzma'):
            self.add_ext( Extension('_lzma', ['_lzmamodule.c'],
                                   libraries = ['lzma']) )
        else:
        ...
        if MS_WINDOWS:
            ...
            if (sysconfig.get_config_var('HAVE_SHM_OPEN') and
                ...
                if sysconfig.get_config_var('SHM_NEEDS_LIBRT'):
                    ...
                    libs.append('rt')
                self.add_ext( Extension('_posixshmem', posixshmem_srcs,
                                       define_macros={},
                                       libraries=libs,
                                       include_dirs=["Modules/_multiprocessing"]))

        self.add_ext ( Extension('_multiprocessing', multiprocessing_srcs,
                                include_dirs=["Modules/_multiprocessing"]))

    def add_simple_extensions(self):
        #
        ...
        # array objects
        self.add_ext( Extension('array', ['arraymodule.c']) )

        # Context Variables
        self.add_ext( Extension('_contextvars', ['_contextvarsmodule.c']) )

        shared_math = 'Modules/_math.o'
        # complex math library functions
        self.add_ext( Extension('cmath', ['cmathmodule.c'],
                               extra_objects=[shared_math],
                               depends=['_math.h', shared_math],
                               libraries=MATH_LIBS) )
        # math library functions, e.g. sin()
        self.add_ext( Extension('math',  ['mathmodule.c'],
                               extra_objects=[shared_math],
                               depends=['_math.h', shared_math],
                               libraries=MATH_LIBS) )
        ...
        time_lib = sysconfig.get_config_var('TIMEMODULE_LIB')
        time_libs = []
        if time_lib:
            time_libs.append(time_lib)
        self.add_ext( Extension('time', ['timemodule.c'],
                               libraries=time_libs) )

        # MATH_LIBS is needed by delta_new() that uses round()
        # and by accum() that uses modf().
        self.add_ext( Extension('_datetime', ['_datetimemodule.c'],
                               libraries=MATH_LIBS) )
        # random number generator implemented in C
        self.add_ext( Extension("_random", ["_randommodule.c"]) )
        # bisect
        self.add_ext( Extension("_bisect", ["_bisectmodule.c"]) )
        # heapq
        self.add_ext( Extension("_heapq", ["_heapqmodule.c"]) )
        # C-optimized pickle replacement
        self.add_ext( Extension("_pickle", ["_pickle.c"]) )
        # atexit
        self.add_ext( Extension("atexit", ["atexitmodule.c"]) )
        # _json speedups
        self.add_ext( Extension("_json", ["_json.c"],
                               # pycore_accu.h requires Py_BUILD_CORE_BUILTIN
                               extra_compile_args=['-DPy_BUILD_CORE_BUILTIN']) )
        # profiler (_lsprof is for cProfile.py)
        self.add_ext( Extension('_lsprof', ['_lsprof.c', 'rotatingtree.c']) )
        # static Unicode character database
        self.add_ext( Extension('unicodedata', ['unicodedata.c'],
                               depends=['unicodedata_db.h', 'unicodename_db.h']) )
        # _opcode module
        self.add_ext( Extension('_opcode', ['_opcode.c']) )
        # asyncio speedups
        self.add_ext( Extension("_asyncio", ["_asynciomodule.c"]) )
        # _abc speedups
        self.add_ext( Extension("_abc", ["_abc.c"]) )
        # _queue module
        self.add_ext( Extension("_queue", ["_queuemodule.c"]) )
        ...
        if (self.config_h_vars.get('FLOCK_NEEDS_LIBBSD', False)):
            ...
            fcntl_libs = ['bsd']
        self.add_ext( Extension('fcntl', ['fcntlmodule.c'], libraries=fcntl_libs) )
        # pwd(3)
        self.add_ext( Extension('pwd', ['pwdmodule.c']) )
        # grp(3)
        if not VXWORKS:
            self.add_ext( Extension('grp', ['grpmodule.c']) )
        # spwd, shadow passwords
        if (self.config_h_vars.get('HAVE_GETSPNAM', False) or
                self.config_h_vars.get('HAVE_GETSPENT', False)):
            self.add_ext( Extension('spwd', ['spwdmodule.c']) )
        else:
            self.missing.append('spwd')

        # select(2); not on ancient System V
        self.add_ext( Extension('select', ['selectmodule.c']) )

        # Fred Drake's interface to the Python parser
        self.add_ext( Extension('parser', ['parsermodule.c']) )

        # Memory-mapped files (also works on Win32).
        self.add_ext( Extension('mmap', ['mmapmodule.c']) )

        # Lance Ellinghaus's syslog module
        # syslog daemon interface
        self.add_ext( Extension('syslog', ['syslogmodule.c']) )

        # Python interface to subinterpreter C-API.
        self.add_ext(Extension('_xxsubinterpreters', ['_xxsubinterpretersmodule.c'],
        ...
        self.add_ext( Extension('audioop', ['audioop.c'],
                               libraries=MATH_LIBS) )

        # CSV files
        self.add_ext( Extension('_csv', ['_csv.c']) )

        # POSIX subprocess module helper.
        self.add_ext( Extension('_posixsubprocess', ['_posixsubprocess.c']) )

    def add_test_extensions(self):
        # Python C API test module
        self.add_ext( Extension('_testcapi', ['_testcapimodule.c'],
                               depends=['testcapi_long.h']) )
        # Python PEP-3118 (buffer protocol) test module
        self.add_ext( Extension('_testbuffer', ['_testbuffer.c']) )
        # Test loading multiple modules from one compiled file (http://bugs.python.org/issue16421)
        self.add_ext( Extension('_testimportmultiple', ['_testimportmultiple.c']) )
        # Test multi-phase extension module init (PEP 489)
        self.add_ext( Extension('_testmultiphase', ['_testmultiphase.c']) )
        # Fuzz tests.
        self.add_ext( Extension(
            '_xxtestfuzz',
    ...
    def detect_platform_specific_exts(self):
        ...
        if not MS_WINDOWS:
            if not VXWORKS:
                ...
                self.add_ext( Extension('termios', ['termios.c']) )
                # Jeremy Hylton's rlimit interface
            self.add_ext( Extension('resource', ['resource.c']) )
        else:
            self.missing.extend(['resource', 'termios'])

        # Platform-specific libraries
        if HOST_PLATFORM.startswith(('linux', 'freebsd', 'gnukfreebsd')):
            self.add_ext( Extension('ossaudiodev', ['ossaudiodev.c']) )
        else:
    ...
    def detect_uuid(self):
        ...
        if uuid_incs is not None:
            if self.compiler.find_library_file(self.lib_dirs, 'uuid'):
                ...
                uuid_libs = []
            self.add_ext(Extension('_uuid', ['_uuidmodule.c'],
                                   libraries=uuid_libs,
    ...
    def detect_modules(self):
        ...
        self.add_simple_extensions()
        if TEST_EXTENSIONS:
            self.add_test_extensions()
...
##         self.add_ext(ext)
    ...
    def _detect_tkinter_explicitly(self):
    ...
    def _detect_tkinter_macos(self):
    ...
    def detect_tkinter(self):
        ...
        # locations.
        if self._detect_tkinter_explicitly():
            return

        # Rather than complicate the code below, detecting and building
        # AquaTk is a separate method. Only one Tkinter will be built on
        # Darwin - either AquaTk, if it is found, or X11 based Tk.
        if MACOS and self._detect_tkinter_macos():
            return
    ...
    def detect_ctypes(self):
        ...
        if TEST_EXTENSIONS:
            # function my_sqrt() needs MATH_LIBS for sqrt()
            ext_test = Extension('_ctypes_test',
                         sources=['_ctypes/_ctypes_test.c'],
                         libraries=MATH_LIBS)
            self.add_ext(ext_test)
    ...
    def detect_openssl_hashlib(self):
        ...
        if config_vars.get("HAVE_X509_VERIFY_PARAM_SET1_HOST"):
            ...
            self.add_ext(ssl_ext)
        else:
            self.missing.append('_ssl')

        # _hashlib module
        hashlib_depends = ['hashlib.h',
                           # hashlib.h includes pythread.h
                           'pythread.h']
        hashlib_ext = Extension(
            '_hashlib', ['_hashopenssl.c'],
            depends=hashlib_depends,
            include_dirs=openssl_includes,
            library_dirs=openssl_libdirs,
            libraries=openssl_libs,
        )
        self.add_ext(hashlib_ext)

        # We always compile these even when OpenSSL is available (issue #14693).
        # It's harmless and the object code is tiny (40-50 KiB per module,
        # only loaded when actually used).
        self.add_ext( Extension('_sha256', ['sha256module.c'],
                               depends=hashlib_depends) )
        self.add_ext( Extension('_sha512', ['sha512module.c'],
                               depends=hashlib_depends) )
        self.add_ext( Extension('_md5', ['md5module.c'],
                               depends=hashlib_depends) )
        self.add_ext( Extension('_sha1', ['sha1module.c'],
                               depends=hashlib_depends) )

        blake2_deps = glob(os.path.join(self.srcdir,
                                        'Modules/_blake2/impl/*'))
        blake2_deps.extend(hashlib_depends)

        self.add_ext( Extension('_blake2',
        ...
        sha3_deps = glob(os.path.join(self.srcdir,
                                      'Modules/_sha3/kcp/*'))
        sha3_deps.extend(hashlib_depends)
        self.add_ext( Extension('_sha3',
                               ['_sha3/sha3module.c'],
                               depends=sha3_deps))
