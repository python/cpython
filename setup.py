...
class PyBuildExt(build_ext):
    ...
    def detect_modules(self):
        ...
        # that uses modf().
        exts.append(Extension("_datetime", ["_datetimemodule.c"], libraries=math_libs))
        ...
        search_for_ssl_incs_in = ["/usr/local/ssl/include", "/usr/contrib/ssl/include/"]
        ...
        #
        # Some downstream builds override pyexpat sources without recording
        # --with-system-expat in CONFIG_ARGS. If the bundled expat sources are
        # not present, fall back to system expat.
        expat_cfgargs = sysconfig.get_config_var("CONFIG_ARGS") or ""
        if "--with-system-expat" in expat_cfgargs or not os.path.exists(
            os.path.join(srcdir, "Modules", "expat")
        ):
            expat_inc = []
            define_macros = []
            extra_compile_args = []
    ...
    def configure_ctypes(self, ext):
        if not self.use_system_libffi:
            ...
            print(
                "warning: building with the bundled copy of libffi is"
            ...
            ffi_srcdir = os.path.abspath(
                os.path.join(srcdir, "Modules", "_ctypes", "libffi")
            )
            ...
            config_sources = [
                os.path.join(ffi_srcdir, fname)
                for fname in os.listdir(ffi_srcdir)
                if os.path.isfile(os.path.join(ffi_srcdir, fname))
            ]
            if self.force or newer_group(config_sources, ffi_configfile):
            ...
            include_dirs = [
                os.path.join(ffi_builddir, "include"),
                ffi_builddir,
            ...
            ext.sources.extend(
                os.path.join(ffi_srcdir, f) for f in fficonfig["ffi_sources"]
            )
            ext.include_dirs.extend(include_dirs)
    ...
    def detect_ctypes(self, inc_dirs, lib_dirs):
        ...
        ext_test = Extension(
            "_ctypes_test", sources=["_ctypes/_ctypes_test.c"], libraries=math_libs
        )
        ...
        if ffi_inc is not None:
            ...
            with open(ffi_h) as f:
                for line in f:
                    line = line.strip()
                    if line.startswith(("#define LIBFFI_H", "#define ffi_wrapper_h")):
                        break
                else:
                    ffi_inc = None
                    print(
