...
get_event_loop(int stacklevel)
{
    ...
    loop = _PyObject_CallMethodNoArgs(policy, _Py_ID(get_event_loop));
    Py_DECREF(policy);
    return loop;
}
...
future_init(FutureObj *fut, PyObject *loop)
{
    ...
    res = _PyObject_CallMethodNoArgs(fut->fut_loop, _Py_ID(get_debug));
    if (res == NULL) {
        return -1;
    }
...
FutureObj_repr(FutureObj *fut)
{
    ...
    PyObject *rinfo = _PyObject_CallMethodNoArgs((PyObject*)fut,
                                                   _Py_ID(_repr_info));
    if (rinfo == NULL) {
        return NULL;
    }
...
{
    PyObject *res = _PyObject_CallMethodOneArg(all_tasks,
                                                 _Py_ID(add), task);
    if (res == NULL) {
        return -1;
    }
...
task_step_impl(TaskObj *task, PyObject *exc)
{
    ...
    if (exc == NULL) {
        ...
        result = _PyObject_CallMethodOneArg(coro, _Py_ID(throw), exc);
    ...
    if (Future_CheckExact(result) || Task_CheckExact(result)) {
        ...
        if (task->task_must_cancel) {
            PyObject *r;
            int is_true;
            r = _PyObject_CallMethodOneArg(result, _Py_ID(cancel),
