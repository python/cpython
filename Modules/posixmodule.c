...
#if defined(HAVE_LINUX_LIMITS_H) && !defined(__FreeBSD_kernel__) &&            \
    !defined(__GNU__)
#define USE_XATTRS
 ...
 *
 * Due to compiler restrictions there is one valid use of
 * HAVE_<FUNCTION>_RUNTIME: if (HAVE_<FUNCTION>_RUNTIME) { ... }
 *
...
#define HAVE_FUTIMENS_RUNTIME                                                  \
  __builtin_available(macOS 10.13, iOS 11.0, tvOS 11.0, watchOS 4.0, *)
...
#define HAVE_PTSNAME_R_RUNTIME                                                 \
  __builtin_available(macOS 10.13.4, iOS 11.3, tvOS 11.3, watchOS 4.3, *)
...
#ifdef HAVE_FORK
static void run_at_forkers(PyObject *lst, int reverse) {
  Py_ssize_t i;
  PyObject *cpy;
...
static void reset_remotedebug_data(PyThreadState *tstate) {
  tstate->remote_debugger_support.debugger_pending_call = 0;
  memset(tstate->remote_debugger_support.debugger_script_path, 0,
         _Py_MAX_SCRIPT_PATH_SIZE);
}

static void reset_asyncio_state(_PyThreadStateImpl *tstate) {
...
static int register_at_forker(PyObject **lst, PyObject *func) {
...
#ifndef MS_WINDOWS
PyObject *_PyLong_FromUid(uid_t uid) {
  if (uid == (uid_t)-1)
    return PyLong_FromLong(-1);
  return PyLong_FromUnsignedLong(uid);
}

PyObject *_PyLong_FromGid(gid_t gid) {
  if (gid == (gid_t)-1)
    return PyLong_FromLong(-1);
  return PyLong_FromUnsignedLong(gid);
}

int _Py_Uid_Converter(PyObject *obj, uid_t *p) {
  uid_t uid;
  PyObject *index;
  int overflow;
  long result;
  ...
  if (index == NULL) {
    PyErr_Format(PyExc_TypeError, "uid should be integer, not %.200s",
  ...
  if (!overflow) {
    ...
    if (sizeof(uid_t) < sizeof(long) && (long)uid != result)
      goto underflow;
    goto success;
  }
...
underflow:
  PyErr_SetString(PyExc_OverflowError, "uid is less than minimum");
  goto fail;

overflow:
  PyErr_SetString(PyExc_OverflowError, "uid is greater than maximum");
  /* fallthrough */
...
int _Py_Gid_Converter(PyObject *obj, gid_t *p) {
  gid_t gid;
  PyObject *index;
  int overflow;
  long result;
  ...
  index = _PyNumber_Index(obj);
  if (index == NULL) {
    PyErr_Format(PyExc_TypeError, "gid should be integer, not %.200s",
                 _PyType_Name(Py_TYPE(obj)));
    return 0;
  }
  ...
  if (!overflow) {
    ...
    /* Ensure the value wasn't truncated. */
    if (sizeof(gid_t) < sizeof(long) && (long)gid != result)
      goto underflow;
    goto success;
  }
  ...
  /* Ensure the value wasn't truncated. */
  if (sizeof(gid_t) < sizeof(long) && (unsigned long)gid != uresult)
    goto overflow;
  /* fallthrough */
...
underflow:
  PyErr_SetString(PyExc_OverflowError, "gid is less than minimum");
  goto fail;

overflow:
  PyErr_SetString(PyExc_OverflowError, "gid is greater than maximum");
  /* fallthrough */
...
static PyObject *_PyLong_FromDev(dev_t dev) {
#ifdef NODEV
  if (dev == NODEV) {
...
static int _fd_converter(PyObject *o, int *p) {
  int overflow;
  long long_value;

  if (PyBool_Check(o)) {
    if (PyErr_WarnEx(PyExc_RuntimeWarning, "bool is used as a file descriptor",
                     1)) {
      return 0;
    }
  }
  ...
  if (overflow < 0 || long_value < INT_MIN) {
    PyErr_SetString(PyExc_OverflowError, "fd is less than minimum");
    return 0;
  }
...
static int dir_fd_converter(PyObject *o, void *p) {
  if (o == Py_None) {
...
static inline _posixstate *get_posix_state(PyObject *module) {
...
#define PATH_T_INITIALIZE(function_name, argument_name, nullable, nonstrict,   \
  ...
  {function_name,                                                              \
   ...
   NULL}
#ifdef MS_WINDOWS
#define PATH_T_INITIALIZE_P(function_name, argument_name, nullable, nonstrict, \
...
static void path_cleanup(path_t *path) {
...
static int path_converter(PyObject *o, void *p) {
...
#define FORMAT_EXCEPTION(exc, fmt)                                             \
  ...
  if (!is_index && !is_unicode && !is_bytes) {
    ...
    if (NULL == res) {
      ...
      PyErr_Format(PyExc_TypeError,
                   ...
                   "not %.200s",
                   _PyType_Name(Py_TYPE(o)), _PyType_Name(Py_TYPE(res)));
      Py_DECREF(res);
      goto error_exit;
    }
  ...
  error_format:
    PyErr_Format(PyExc_TypeError, "%s%s%s should be %s, not %.200s",
                 ...
                 path->function_name ? ": " : "",
                 path->argument_name ? path->argument_name : "path",
                 path->allow_fd && path->nullable
                     ? "string, bytes, os.PathLike, "
                       "integer or None"
...
static void argument_unavailable_error(const char *function_name,
                                       const char *argument_name) {
  PyErr_Format(PyExc_NotImplementedError, "%s%s%s unavailable on this platform",
               (function_name != NULL) ? function_name : "",
               (function_name != NULL) ? ": " : "", argument_name);
}

static int dir_fd_unavailable(PyObject *o, void *p) {
  int dir_fd;
...
static int fd_specified(const char *function_name, int fd) {
  if (fd == -1)
    return 0;
...
static int follow_symlinks_specified(const char *function_name,
                                     int follow_symlinks) {
  if (follow_symlinks)
    return 0;
...
static int path_and_dir_fd_invalid(const char *function_name, path_t *path,
                                   int dir_fd) {
...
static int dir_fd_and_fd_invalid(const char *function_name, int dir_fd,
                                 int fd) {
  if ((dir_fd != DEFAULT_DIR_FD) && (fd != -1)) {
    PyErr_Format(PyExc_ValueError, "%s: can't specify both dir_fd and fd",
                 function_name);
    return 1;
  }
  return 0;
...
static int fd_and_follow_symlinks_invalid(const char *function_name, int fd,
                                          int follow_symlinks) {
...
static int dir_fd_and_follow_symlinks_invalid(const char *function_name,
...
#if defined(HAVE_WAITID)
static int idtype_t_converter(PyObject *arg, void *addr) {
...
static int Py_off_t_converter(PyObject *arg, void *addr) {
...
static PyObject *PyLong_FromPy_off_t(Py_off_t offset) {
#ifdef HAVE_LARGEFILE_SUPPORT
...
int _Py_Sigset_Converter(PyObject *obj, void *addr) {
  ...
  while ((item = PyIter_Next(iterator)) != NULL) {
    ...
    if (signum <= 0 || signum >= Py_NSIG) {
      if (overflow || signum != -1 || !PyErr_Occurred()) {
        PyErr_Format(PyExc_ValueError, "signal number %ld out of range [1; %i]",
                     signum, Py_NSIG - 1);
      }
      goto error;
    }
...
/* Return a dictionary corresponding to the POSIX environment table */
    ...
    (defined(__APPLE__) && defined(Py_ENABLE_SHARED))
...
#elif !defined(_MSC_VER) &&                                                    \
    (!defined(__WATCOMC__) || defined(__QNX__) || defined(__VXWORKS__))
extern char **environ;
#endif /* !_MSC_VER */

static PyObject *convertenviron(void) {
  PyObject *d;
#ifdef MS_WINDOWS
  wchar_t **e;
#else
...
static PyObject *posix_error(void) { return PyErr_SetFromErrno(PyExc_OSError); }

#ifdef MS_WINDOWS
static PyObject *win32_error(const char *function, const char *filename) {
...
static PyObject *win32_error_object_err(const char *function,
                                        PyObject *filename, DWORD err) {
  /* XXX - see win32_error for comments on 'function' */
  if (filename)
    return PyErr_SetExcFromWindowsErrWithFilenameObject(PyExc_OSError, err,
                                                        filename);
  else
    return PyErr_SetFromWindowsErr(err);
}

static PyObject *win32_error_object(const char *function, PyObject *filename) {
  errno = GetLastError();
...
static PyObject *posix_path_object_error(PyObject *path) {
  return PyErr_SetFromErrnoWithFilenameObject(PyExc_OSError, path);
}

static PyObject *path_object_error(PyObject *path) {
#ifdef MS_WINDOWS
  return PyErr_SetExcFromWindowsErrWithFilenameObject(PyExc_OSError, 0, path);
#else
  return posix_path_object_error(path);
#endif
}

static PyObject *path_object_error2(PyObject *path, PyObject *path2) {
#ifdef MS_WINDOWS
  return PyErr_SetExcFromWindowsErrWithFilenameObjects(PyExc_OSError, 0, path,
                                                       path2);
#else
...
static PyObject *path_error(path_t *path) {
  return path_object_error(path->object);
}

static PyObject *posix_path_error(path_t *path) {
  return posix_path_object_error(path->object);
}

static PyObject *path_error2(path_t *path, path_t *path2) {
  return path_object_error2(path->object, path2->object);
}

/* POSIX generic methods */

static PyObject *posix_fildes_fd(int fd, int (*func)(int)) {
  int res;
  int async_err = 0;

  do {
    Py_BEGIN_ALLOW_THREADS _Py_BEGIN_SUPPRESS_IPH res = (*func)(fd);
    _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS
...
static BOOL __stdcall win32_wchdir(LPCWSTR path) {
...
static void find_data_to_file_info(WIN32_FIND_DATAW *pFileData,
...
static BOOL attributes_from_dir(LPCWSTR pszFile,
                                BY_HANDLE_FILE_INFORMATION *info,
                                ULONG *reparse_tag) {
  HANDLE hFindFile;
...
static void update_st_mode_from_path(const wchar_t *path, DWORD attr,
...
static int win32_xstat_slow_impl(const wchar_t *path,
                                 struct _Py_stat_struct *result,
                                 BOOL traverse) {
  HANDLE hFile;
  ...
  if (hFile == INVALID_HANDLE_VALUE) {
    ...
    case ERROR_SHARING_VIOLATION: /* It's a paging file. */
      ...
      if (fileInfo.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) {
        if (traverse || !IsReparseTagNameSurrogate(tagInfo.ReparseTag)) {
  ...
  if (hFile != INVALID_HANDLE_VALUE) {
    ...
    if (!traverse) {
      if (!GetFileInformationByHandleEx(hFile, FileAttributeTagInfo, &tagInfo,
                                        sizeof(tagInfo))) {
  ...
  _Py_attribute_data_to_stat(&fileInfo, tagInfo.ReparseTag, pBasicInfo, pIdInfo,
                             result);
...
static int win32_xstat_impl(const wchar_t *path, struct _Py_stat_struct *result,
                            BOOL traverse) {
  FILE_STAT_BASIC_INFORMATION statInfo;
  if (_Py_GetFileInformationByName(path, FileStatBasicByNameInfo, &statInfo,
                                   sizeof(statInfo))) {
...
static int win32_xstat(const wchar_t *path, struct _Py_stat_struct *result,
                       BOOL traverse) {
...
static int win32_lstat(const wchar_t *path, struct _Py_stat_struct *result) {
  return win32_xstat(path, result, FALSE);
}

static int win32_stat(const wchar_t *path, struct _Py_stat_struct *result) {
...
static PyStructSequence_Desc statvfs_result_desc = {
    ...
    statvfs_result_fields, 10};

#if defined(HAVE_WAITID)
PyDoc_STRVAR(waitid_result__doc__, "waitid_result: Result from waitid.\n\n\
This object may be accessed either as a tuple of\n\
...
static PyStructSequence_Desc waitid_result_desc = {
    ...
    waitid_result_fields, 5};
#endif

static PyObject *statresult_new(PyTypeObject *type, PyObject *args,
                                PyObject *kwds) {
...
static int _posix_clear(PyObject *module) {
...
static int _posix_traverse(PyObject *module, visitproc visit, void *arg) {
  _posixstate *state = get_posix_state(module);
...
static void _posix_free(void *module) { _posix_clear((PyObject *)module); }

#define SEC_TO_NS (1000000000LL)
static PyObject *stat_nanosecond_timestamp(_posixstate *state, time_t sec,
                                           unsigned long nsec) {
#if SIZEOF_TIME_T == 4
...
static int fill_time(_posixstate *state, PyObject *v, int s_index, int f_index,
...
#ifdef MS_WINDOWS
static PyObject *_pystat_l128_from_l64_l64(uint64_t low, uint64_t high) {
...
static PyObject *_pystat_fromstructstat(PyObject *module, STRUCT_STAT *st) {
  assert(!PyErr_Occurred());
...
static PyObject *posix_do_stat(PyObject *module, const char *function_name,
                               path_t *path, int dir_fd, int follow_symlinks) {
  STRUCT_STAT st;
  int result;
  ...
  Py_BEGIN_ALLOW_THREADS if (path->fd != -1) result = FSTAT(path->fd, &st);
#ifdef MS_WINDOWS
  else if (follow_symlinks) result = win32_stat(path->wide, &st);
  else result = win32_lstat(path->wide, &st);
#else
  else
#if defined(HAVE_LSTAT)
      if ((!follow_symlinks) && (dir_fd == DEFAULT_DIR_FD)) result =
          LSTAT(path->narrow, &st);
  else
#endif /* HAVE_LSTAT */
#ifdef HAVE_FSTATAT
      if ((dir_fd != DEFAULT_DIR_FD) || !follow_symlinks) {
...
converter!")
    ...
    def pre_render(self):
        ...
        if self.make_wide is None:
            self.c_default = 'PATH_T_INITIALIZE_P("{}", "{}", {}, {}, {},
{})'.format( self.function.name, self.name, strify(self.nullable),
                strify(self.nonstrict),
                strify(self.suppress_value_error),
                strify(self.allow_fd),
            )
        else:
            self.c_default = 'PATH_T_INITIALIZE("{}", "{}", {}, {}, {}, {},
{})'.format( self.function.name, self.name, strify(self.nullable),
                strify(self.nonstrict),
...
static PyObject *os_stat_impl(PyObject *module, path_t *path, int dir_fd,
                              int follow_symlinks)
...
static PyObject *os_lstat_impl(PyObject *module, path_t *path, int dir_fd)
...
static PyMemberDef pystatx_result_members[] = {
    ...
    MM(stx_attributes, Py_T_ULONGLONG, attributes,
       "Linux inode attribute bits"),
    MM(stx_attributes_mask, Py_T_ULONGLONG, attributes_mask,
       "Mask of supported bits in stx_attributes"),
    MM(stx_rdev_major, Py_T_UINT, rdev_major,
       "represented device major number"),
    MM(stx_rdev_minor, Py_T_UINT, rdev_minor,
...
#define STATX_GET_UINT(ATTR, MASK)                                             \
  static PyObject *pystatx_result_get_##ATTR(PyObject *op,                     \
...
static PyObject *pystatx_result_get_stx_mode(PyObject *op,
                                             void *Py_UNUSED(context)) {
  Py_statx_result *self = Py_statx_result_CAST(op);
...
#define STATX_GET_NSEC(ATTR, MEMBER, MASK)                                     \
  static PyObject *pystatx_result_get_##ATTR(PyObject *op, void *context) {    \
    Py_statx_result *self = Py_statx_result_CAST(op);                          \
    if (!(self->stx.stx_mask & MASK)) {                                        \
      Py_RETURN_NONE;                                                          \
    }                                                                          \
...
static PyObject *pystatx_result_repr(PyObject *op) {
...
static int pystatx_result_traverse(PyObject *self, visitproc visit, void *arg) {
  Py_VISIT(Py_TYPE(self));
  return 0;
}

static void pystatx_result_dealloc(PyObject *op) {
...
static PyObject *os_statx_impl(PyObject *module, path_t *path,
                               unsigned int mask, int flags, int dir_fd,
                               int follow_symlinks)
/*[clinic end generated code: output=e3765979ac6fe15b input=f0116380c5dc4f2f]*/
{
  ...
  int result;
  Py_BEGIN_ALLOW_THREADS if (path->fd != -1) {
...
static int os_access_impl(PyObject *module, path_t *path, int mode, int dir_fd,
...
#ifdef MS_WINDOWS
  Py_BEGIN_ALLOW_THREADS attr = GetFileAttributesW(path->wide);
  Py_END_ALLOW_THREADS
      ...
      return_value = (attr != INVALID_FILE_ATTRIBUTES) &&
                     (!(mode & 2) || !(attr & FILE_ATTRIBUTE_READONLY) ||
                      (attr & FILE_ATTRIBUTE_DIRECTORY));
#else

  Py_BEGIN_ALLOW_THREADS
#ifdef HAVE_FACCESSAT
      if ((dir_fd != DEFAULT_DIR_FD) || effective_ids || !follow_symlinks) {
...
static PyObject *os_ttyname_impl(PyObject *module, int fd)
...
static PyObject *os_ctermid_impl(PyObject *module)
...
static PyObject *os_chdir_impl(PyObject *module, path_t *path)
...
#else
#ifdef HAVE_FCHDIR
      if (path->fd != -1) result = fchdir(path->fd);
  else
#endif
...
static PyObject *os_fchdir_impl(PyObject *module, int fd)
...
#ifdef MS_WINDOWS
static int win32_lchmod(LPCWSTR path, int mode) {
...
static int win32_hchmod(HANDLE hfile, int mode) {
  ...
  return SetFileInformationByHandle(hfile, FileBasicInfo, &info, sizeof(info));
}

static int win32_fchmod(int fd, int mode) {
...
path-like object. On some platforms, path may also be specified as an open file
descriptor. If this functionality is unavailable, using it raises an exception.
...
a ``0o`` prefix in Python.
    ...
    *
    ...
    follow_symlinks: bool(c_default="CHMOD_DEFAULT_FOLLOW_SYMLINKS", \
                          py_default="(os.name != 'nt')") =
CHMOD_DEFAULT_FOLLOW_SYMLINKS If False, and the last element of the path is a
symbolic link, chmod will modify the symbolic link itself instead of the file
        the link points to.
...
static PyObject *os_chmod_impl(PyObject *module, path_t *path, int mode,
                               int dir_fd, int follow_symlinks)
/*[clinic end generated code: output=5cf6a94915cc7bff input=fcf115d174b9f3d8]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS if (path->fd != -1) {
    ...
    HANDLE hfile =
        CreateFileW(path->wide, FILE_READ_ATTRIBUTES | FILE_WRITE_ATTRIBUTES, 0,
  ...
  Py_END_ALLOW_THREADS if (!result) { return path_error(path); }
#else /* MS_WINDOWS */
  Py_BEGIN_ALLOW_THREADS
#ifdef HAVE_FCHMOD
      if (path->fd != -1) result = fchmod(path->fd, mode);
  else
#endif /* HAVE_CHMOD */
#ifdef HAVE_LCHMOD
      if ((!follow_symlinks) && (dir_fd == DEFAULT_DIR_FD)) result =
          lchmod(path->narrow, mode);
  else
#endif /* HAVE_LCHMOD */
#ifdef HAVE_FCHMODAT
      if ((dir_fd != DEFAULT_DIR_FD) || !follow_symlinks) {
    if (HAVE_FCHMODAT_RUNTIME) {
      ...
      fchmodat_nofollow_unsupported =
          result && ((errno == ENOTSUP) || (errno == EOPNOTSUPP)) &&
          !follow_symlinks;
    } else {
  ...
  Py_END_ALLOW_THREADS
    ...
    if (fchmodat_nofollow_unsupported) {
      if (dir_fd != DEFAULT_DIR_FD)
        dir_fd_and_follow_symlinks_invalid("chmod", dir_fd, follow_symlinks);
      else
...
static PyObject *os_fchmod_impl(PyObject *module, int fd, int mode)
...
#ifdef MS_WINDOWS
  res = 0;
  Py_BEGIN_ALLOW_THREADS res = win32_fchmod(fd, mode);
  Py_END_ALLOW_THREADS if (!res) { return PyErr_SetFromWindowsErr(0); }
#else  /* MS_WINDOWS */
  int async_err = 0;
  do {
    Py_BEGIN_ALLOW_THREADS res = fchmod(fd, mode);
    Py_END_ALLOW_THREADS
  } while (res != 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
  if (res != 0)
    return (!async_err) ? posix_error() : NULL;
...
static PyObject *os_lchmod_impl(PyObject *module, path_t *path, int mode)
...
#ifdef MS_WINDOWS
  Py_BEGIN_ALLOW_THREADS res = win32_lchmod(path->wide, mode);
  Py_END_ALLOW_THREADS if (!res) {
    path_error(path);
    return NULL;
  }
#else  /* MS_WINDOWS */
  Py_BEGIN_ALLOW_THREADS res = lchmod(path->narrow, mode);
  Py_END_ALLOW_THREADS if (res < 0) {
    path_error(path);
    return NULL;
  }
#endif /* MS_WINDOWS */
...
static PyObject *os_chflags_impl(PyObject *module, path_t *path,
                                 unsigned long flags, int follow_symlinks)
/*[clinic end generated code: output=85571c6737661ce9 input=0327e29feb876236]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS
#ifdef HAVE_LCHFLAGS
      if (!follow_symlinks) result = lchflags(path->narrow, flags);
  else
#endif
      result = chflags(path->narrow, flags);
  Py_END_ALLOW_THREADS

      if (result) return path_error(path);
...
static PyObject *os_lchflags_impl(PyObject *module, path_t *path,
                                  unsigned long flags)
/*[clinic end generated code: output=30ae958695c07316 input=f9f82ea8b585ca9d]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS res = lchflags(path->narrow, flags);
  Py_END_ALLOW_THREADS if (res < 0) { return path_error(path); }
  Py_RETURN_NONE;
}
#endif /* HAVE_LCHFLAGS */
...
static PyObject *os_chroot_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=de80befc763a4475 input=14822965652c3dc3]*/
{
  int res;
  Py_BEGIN_ALLOW_THREADS res = chroot(path->narrow);
  Py_END_ALLOW_THREADS if (res < 0) return path_error(path);
  Py_RETURN_NONE;
}
#endif /* HAVE_CHROOT */
...
static PyObject *os_fsync_impl(PyObject *module, int fd)
...
static PyObject *os_fdatasync_impl(PyObject *module, int fd)
...
static PyObject *os_chown_impl(PyObject *module, path_t *path, uid_t uid,
                               gid_t gid, int dir_fd, int follow_symlinks)
/*[clinic end generated code: output=4beadab0db5f70cd input=b08c5ec67996a97d]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS
#ifdef HAVE_FCHOWN
      if (path->fd != -1) result = fchown(path->fd, uid, gid);
  else
#endif
#ifdef HAVE_LCHOWN
      if ((!follow_symlinks) && (dir_fd == DEFAULT_DIR_FD)) result =
          lchown(path->narrow, uid, gid);
  else
#endif
#ifdef HAVE_FCHOWNAT
      if ((dir_fd != DEFAULT_DIR_FD) || (!follow_symlinks)) {
...
static PyObject *os_fchown_impl(PyObject *module, int fd, uid_t uid, gid_t gid)
/*[clinic end generated code: output=97d21cbd5a4350a6 input=3af544ba1b13a0d7]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS res = fchown(fd, uid, gid);
    Py_END_ALLOW_THREADS
  } while (res != 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
  if (res != 0)
    return (!async_err) ? posix_error() : NULL;
...
static PyObject *os_lchown_impl(PyObject *module, path_t *path, uid_t uid,
                                gid_t gid)
/*[clinic end generated code: output=25eaf6af412fdf2f input=b1c6014d563a7161]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS res = lchown(path->narrow, uid, gid);
  Py_END_ALLOW_THREADS if (res < 0) { return path_error(path); }
  Py_RETURN_NONE;
}
#endif /* HAVE_LCHOWN */

static PyObject *posix_getcwd(int use_bytes) {
#ifdef MS_WINDOWS
  wchar_t wbuf[MAXPATHLEN];
  wchar_t *wbuf2 = wbuf;
  DWORD len;

  Py_BEGIN_ALLOW_THREADS len =
      GetCurrentDirectoryW(Py_ARRAY_LENGTH(wbuf), wbuf);
...
static PyObject *os_getcwd_impl(PyObject *module)
...
static PyObject *os_getcwdb_impl(PyObject *module)
...
static PyObject *os_link_impl(PyObject *module, path_t *src, path_t *dst,
...
#ifdef MS_WINDOWS
  Py_BEGIN_ALLOW_THREADS result = CreateHardLinkW(dst->wide, src->wide, NULL);
  Py_END_ALLOW_THREADS

      if (!result) return path_error2(src, dst);
#else
  Py_BEGIN_ALLOW_THREADS
#ifdef HAVE_LINKAT
      if (HAVE_LINKAT_RUNTIME) {
...
static PyObject *_listdir_windows_no_opendir(path_t *path, PyObject *list) {
  PyObject *v;
  ...
  Py_BEGIN_ALLOW_THREADS hFindFile = FindFirstFileW(wnamebuf, &wFileData);
  Py_END_ALLOW_THREADS if (hFindFile == INVALID_HANDLE_VALUE) {
  ...
  do {
    ...
    Py_BEGIN_ALLOW_THREADS result = FindNextFileW(hFindFile, &wFileData);
    Py_END_ALLOW_THREADS
...
static PyObject *_posix_listdir(path_t *path, PyObject *list) {
  PyObject *v;
  DIR *dirp = NULL;
  struct dirent *ep;
  ...
  if (path->fd != -1) {
    if (HAVE_FDOPENDIR_RUNTIME) {
      ...
      Py_BEGIN_ALLOW_THREADS dirp = fdopendir(fd);
      Py_END_ALLOW_THREADS
    } else {
      PyErr_SetString(PyExc_TypeError, "listdir: path should be string, bytes, "
                                       "os.PathLike or None, not int");
      return NULL;
    }
  } else
#endif
  {
    ...
    Py_BEGIN_ALLOW_THREADS dirp = opendir(name);
    Py_END_ALLOW_THREADS
  }
...
#ifdef HAVE_FDOPENDIR
    if (fd != -1) {
      Py_BEGIN_ALLOW_THREADS close(fd);
      Py_END_ALLOW_THREADS
    }
#endif
    goto exit;
  ...
  for (;;) {
    errno = 0;
    Py_BEGIN_ALLOW_THREADS ep = readdir(dirp);
    Py_END_ALLOW_THREADS if (ep == NULL) {
      if (errno == 0) {
        break;
      } else {
        path_error(path);
...
path can be specified as either str, bytes, or a path-like object.  If path is
bytes, the filenames returned will also be bytes; in all other circumstances the
filenames returned will be str. If path is None, uses the path='.'.
On some platforms, path may also be specified as an open file descriptor;\
...
static PyObject *os_listdir_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=293045673fcd1a75 input=0bd1728387391b9a]*/
{
  if (PySys_Audit("os.listdir", "O", path->object ? path->object : Py_None) <
      0) {
    return NULL;
  }
#if defined(MS_WINDOWS) && !defined(HAVE_OPENDIR)
...
static PyObject *os_listdrives_impl(PyObject *module)
...
static PyObject *os_listvolumes_impl(PyObject *module)
...
static PyObject *os_listmounts_impl(PyObject *module, path_t *volume)
/*[clinic end generated code: output=06da49679de4512e input=a8a27178e3f67845]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS attributes = GetFileAttributesW(volume->wide);
  Py_END_ALLOW_THREADS if (attributes == INVALID_FILE_ATTRIBUTES &&
  ...
  while (1) {
    ...
    Py_BEGIN_ALLOW_THREADS success =
        GetVolumePathNamesForVolumeNameW(volume->wide, buffer, buflen, &buflen);
    Py_END_ALLOW_THREADS if (success) { break; }
...
static PyObject *os__path_isdevdrive_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=1f437ea6677433a2 input=ee83e4996a48e23d]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS if (!GetVolumePathNameW(path->wide, volume,
                                                 MAX_PATH)) {
    ...
    HANDLE hVolume = CreateFileW(
        volume, FILE_READ_ATTRIBUTES, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
    ...
    if (hVolume == INVALID_HANDLE_VALUE) {
      ...
      if (!DeviceIoControl(hVolume, FSCTL_QUERY_PERSISTENT_VOLUME_STATE,
                           &volumeState, sizeof(volumeState), &volumeState,
                           sizeof(volumeState), NULL, NULL)) {
        err = GetLastError();
      }
      CloseHandle(hVolume);
...
int _PyOS_getfullpathname(const wchar_t *path, wchar_t **abspath_p) {
  wchar_t woutbuf[MAX_PATH], *woutbufp = woutbuf;
  DWORD result;

  result = GetFullPathNameW(path, Py_ARRAY_LENGTH(woutbuf), woutbuf, NULL);
  if (!result) {
    return -1;
  }
...
static PyObject *os__getfullpathname_impl(PyObject *module, path_t *path)
...
static PyObject *os__getfinalpathname_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=621a3c79bc29ebfa input=2b6b6c7cbad5fb84]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS hFile = CreateFileW(
      path->wide, 0, /* desired access */
      0,             /* share mode */
  ...
  /* We have a good handle to the target, use it to determine the
     target path name. */
  while (1) {
    Py_BEGIN_ALLOW_THREADS result_length = GetFinalPathNameByHandleW(
        hFile, target_path, buf_size, VOLUME_NAME_DOS);
    Py_END_ALLOW_THREADS

        if (!result_length) {
      result = win32_error_object("GetFinalPathNameByHandleW", path->object);
      goto cleanup;
    }
...
static PyObject *os__findfirstfile_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=106dd3f0779c83dd input=0734dff70f60e1a8]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS hFindFile = FindFirstFileW(path->wide, &wFileData);
  Py_END_ALLOW_THREADS
...
static PyObject *os__getvolumepathname_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=804c63fd13a1330b input=722b40565fa21552]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS ret = GetVolumePathNameW(
      path->wide, mountpath, Py_SAFE_DOWNCAST(buflen, size_t, DWORD));
  Py_END_ALLOW_THREADS
...
static PyObject *os__path_splitroot_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=ab7f1a88b654581c input=42831e41f8458f6d]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS ret = PathCchSkipRoot(buffer, &end);
  Py_END_ALLOW_THREADS if (FAILED(ret)) {
    result = Py_BuildValue("sO", "", path->object);
  }
  else if (end != buffer) {
    size_t rootLen = (size_t)(end - buffer);
    result = Py_BuildValue("NN", PyUnicode_FromWideChar(path->wide, rootLen),
...
static inline BOOL _testInfo(DWORD attributes, DWORD reparseTag,
                             BOOL diskDevice, int testedType) {
  switch (testedType) {
  case PY_IFREG:
    return diskDevice && attributes && !(attributes & FILE_ATTRIBUTE_DIRECTORY);
  case PY_IFDIR:
  ...
  case PY_IFRRP:
    return (attributes & FILE_ATTRIBUTE_REPARSE_POINT) && reparseTag &&
           !IsReparseTagNameSurrogate(reparseTag);
  }
...
static BOOL _testFileTypeByHandle(HANDLE hfile, int testedType, BOOL diskOnly) {
...
static BOOL _testFileTypeByName(LPCWSTR path, int testedType) {
  assert(testedType == PY_IFREG || testedType == PY_IFDIR ||
         testedType == PY_IFLNK || testedType == PY_IFMNT ||
         testedType == PY_IFLRP || testedType == PY_IFRRP);
  ...
  if (_Py_GetFileInformationByName(path, FileStatBasicByNameInfo, &info,
    ...
    BOOL result =
        _testInfo(info.FileAttributes, info.ReparseTag, diskDevice, testedType);
  ...
  HANDLE hfile = CreateFileW(path, FILE_READ_ATTRIBUTES, 0, NULL, OPEN_EXISTING,
                             flags, NULL);
  if (hfile != INVALID_HANDLE_VALUE) {
...
static BOOL _testFileExistsByName(LPCWSTR path, BOOL followLinks) {
  FILE_STAT_BASIC_INFORMATION info;
  if (_Py_GetFileInformationByName(path, FileStatBasicByNameInfo, &info,
                                   sizeof(info))) {
...
static BOOL _testFileExists(path_t *path, BOOL followLinks) {
  BOOL result = FALSE;
  if (path->value_error) {
    return FALSE;
  }
...
static BOOL _testFileType(path_t *path, int testedType) {
  BOOL result = FALSE;
  if (path->value_error) {
    return FALSE;
  }
...
static int os__path_exists_impl(PyObject *module, path_t *path)
...
static int os__path_lexists_impl(PyObject *module, path_t *path)
...
static int os__path_isdir_impl(PyObject *module, path_t *path)
...
static int os__path_isfile_impl(PyObject *module, path_t *path)
...
static int os__path_islink_impl(PyObject *module, path_t *path)
...
static int os__path_isjunction_impl(PyObject *module, path_t *path)
...
static PyObject *os__path_splitroot_ex_impl(PyObject *module, path_t *path)
...
static PyObject *os__path_normpath_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=d353e7ed9410c044 input=3d4ac23b06332dcb]*/
{
  PyObject *result;
  Py_ssize_t norm_len;
  wchar_t *norm_path =
      _Py_normpath_and_size((wchar_t *)path->wide, path->length, &norm_len);
  if (!norm_len) {
...
static PyObject *os_mkdir_impl(PyObject *module, path_t *path, int mode,
                               int dir_fd)
/*[clinic end generated code: output=a70446903abe821f input=a61722e1576fab03]*/
{
  ...
  // but to silently create a directory with default ACL.
    ...
    if (!ConvertStringSecurityDescriptorToSecurityDescriptorW(
            ...
            SDDL_REVISION_1, &secAttr.lpSecurityDescriptor, &sdSize)) {
      error = GetLastError();
    }
  }
#endif
...
static PyObject *os_nice_impl(PyObject *module, int increment)
...
static PyObject *os_getpriority_impl(PyObject *module, int which, int who)
...
static PyObject *os_setpriority_impl(PyObject *module, int which, int who,
                                     int priority)
...
static PyObject *internal_rename(path_t *src, path_t *dst, int src_dir_fd,
                                 int dst_dir_fd, int is_replace) {
...
#ifdef MS_WINDOWS
  Py_BEGIN_ALLOW_THREADS result = MoveFileExW(src->wide, dst->wide, flags);
  Py_END_ALLOW_THREADS
...
static PyObject *os_rename_impl(PyObject *module, path_t *src, path_t *dst,
                                int src_dir_fd, int dst_dir_fd)
...
static PyObject *os_replace_impl(PyObject *module, path_t *src, path_t *dst,
                                 int src_dir_fd, int dst_dir_fd)
...
static PyObject *os_rmdir_impl(PyObject *module, path_t *path, int dir_fd)
...
static long os_system_impl(PyObject *module, const wchar_t *command)
/*[clinic end generated code: output=dd528cbd5943a679 input=303f5ce97df606b0]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS _Py_BEGIN_SUPPRESS_IPH result = _wsystem(command);
  _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS return result;
}
#else /* MS_WINDOWS */
/*[clinic input]
os.system -> long
...
static long os_system_impl(PyObject *module, PyObject *command)
/*[clinic end generated code: output=290fc437dd4f33a0 input=47c6f24b6dc92881]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS result = system(bytes);
...
static PyObject *os_umask_impl(PyObject *module, int mask)
...
BOOL WINAPI Py_DeleteFileW(LPCWSTR lpFileName) {
  ...
  if (GetFileAttributesExW(lpFileName, GetFileExInfoStandard, &info)) {
    ...
    /* Get WIN32_FIND_DATA structure for the path to determine if
       it is a symlink */
    if (is_directory && info.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) {
...
static PyObject *os_unlink_impl(PyObject *module, path_t *path, int dir_fd)
/*[clinic end generated code: output=621797807b9963b1 input=d7bcde2b1b2a2552]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS _Py_BEGIN_SUPPRESS_IPH
#ifdef MS_WINDOWS
      /* Windows, success=1, UNIX, success=0 */
...
static PyObject *os_remove_impl(PyObject *module, path_t *path, int dir_fd)
...
PyDoc_STRVAR(uname_result__doc__, "uname_result: Result from os.uname().\n\n\
This object may be accessed either as a tuple of\n\
...
static PyStructSequence_Desc uname_result_desc = {MODNAME
                                                  ".uname_result", /* name */
                                                  uname_result__doc__, /* doc */
                                                  uname_result_fields, 5};
...
static PyObject *os_uname_impl(PyObject *module)
/*[clinic end generated code: output=e6a49cf1a1508a19 input=e68bd246db3043ed]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS res = uname(&u);
...
static int utime_dir_fd(utime_t *ut, int dir_fd, const char *path,
                        int follow_symlinks) {
...
static int utime_fd(utime_t *ut, int fd) {
#ifdef HAVE_FUTIMENS
...
static int utime_nofollow_symlinks(utime_t *ut, const char *path) {
#ifdef HAVE_UTIMENSAT
  if (HAVE_UTIMENSAT_RUNTIME) {
    UTIME_TO_TIMESPEC;
...
static int utime_default(utime_t *ut, const char *path) {
#if defined(__APPLE__) && defined(HAVE_UTIMENSAT)
  if (HAVE_UTIMENSAT_RUNTIME) {
    UTIME_TO_TIMESPEC;
...
static int split_py_long_to_s_and_ns(PyObject *module, PyObject *py_long,
                                     time_t *s, long *ns) {
  int result = 0;
  PyObject *divmod;
...
static PyObject *os_utime_impl(PyObject *module, path_t *path, PyObject *times,
...
/*[clinic end generated code: output=cfcac69d027b82cf input=2fbd62a2f228f8f4]*/
{
  ...
  if (times != Py_None && ns) {
    PyErr_SetString(PyExc_ValueError, "utime: you may specify either 'times'"
                                      " or 'ns' but not both");
    return NULL;
  }
  ...
  if (times != Py_None) {
    ...
    if (!PyTuple_CheckExact(times) || (PyTuple_Size(times) != 2)) {
      PyErr_SetString(PyExc_TypeError, "utime: 'times' must be either"
                                       " a tuple of two numbers or None");
      return NULL;
    }
    utime.now = 0;
    if (_PyTime_ObjectToTimespec(PyTuple_GET_ITEM(times, 0), &a_sec, &a_nsec,
                                 _PyTime_ROUND_FLOOR) == -1 ||
        _PyTime_ObjectToTimespec(PyTuple_GET_ITEM(times, 1), &m_sec, &m_nsec,
                                 _PyTime_ROUND_FLOOR) == -1) {
      return NULL;
    }
    utime.atime_s = a_sec;
  ...
  Py_BEGIN_ALLOW_THREADS hFile =
      CreateFileW(path->wide, FILE_WRITE_ATTRIBUTES, 0, NULL, OPEN_EXISTING,
                  FILE_FLAG_BACKUP_SEMANTICS, NULL);
...
#ifdef UTIME_HAVE_NOFOLLOW_SYMLINKS
      if ((!follow_symlinks) && (dir_fd == DEFAULT_DIR_FD)) result =
          utime_nofollow_symlinks(&utime, path->narrow);
  else
#endif
...
#if defined(HAVE_FUTIMES) || defined(HAVE_FUTIMENS)
      if (path->fd != -1) result = utime_fd(&utime, path->fd);
  else
#endif
...
static PyObject *os__exit_impl(PyObject *module, int status)
...
static void free_string_array(EXECV_CHAR **array, Py_ssize_t count) {
  Py_ssize_t i;
  for (i = 0; i < count; i++)
    PyMem_Free(array[i]);
  PyMem_Free(array);
...
static int fsconvert_strdup(PyObject *o, EXECV_CHAR **out) {
  Py_ssize_t size;
  PyObject *ub;
  int result = 0;
#if defined(HAVE_WEXECV) || defined(HAVE_WSPAWNV)
...
static EXECV_CHAR **parse_envlist(PyObject *env, Py_ssize_t *envc_ptr) {
  ...
  if (!PyList_Check(keys) || !PyList_Check(vals)) {
    PyErr_Format(PyExc_TypeError, "env.keys() or env.values() is not a list");
    goto error;
  }
...
static EXECV_CHAR **parse_arglist(PyObject *argv, Py_ssize_t *argc) {
  int i;
...
static PyObject *os_execv_impl(PyObject *module, path_t *path, PyObject *argv)
...
static PyObject *os_execve_impl(PyObject *module, path_t *path, PyObject *argv,
                                PyObject *env)
/*[clinic end generated code: output=ff9fa8e4da8bde58 input=626804fa092606d9]*/
{
  ...
  if (!PyList_Check(argv) && !PyTuple_Check(argv)) {
    PyErr_SetString(PyExc_TypeError, "execve: argv must be a tuple or list");
    goto fail_0;
  }
  argc = PySequence_Size(argv);
  if (argc < 1) {
  ...
  _Py_BEGIN_SUPPRESS_IPH
#ifdef HAVE_FEXECVE
      if (path->fd > -1) fexecve(path->fd, argvlist, envlist);
  else
#endif
#ifdef HAVE_WEXECV
...
#if defined(HAVE_SCHED_SETPARAM) || defined(HAVE_SCHED_SETSCHEDULER) ||        \
    defined(POSIX_SPAWN_SETSCHEDULER) || defined(POSIX_SPAWN_SETSCHEDPARAM)
static int convert_sched_param(PyObject *module, PyObject *param,
                               struct sched_param *res);
#endif

static int parse_posix_spawn_flags(PyObject *module, const char *func_name,
                                   PyObject *setpgroup, int resetids,
                                   int setsid, PyObject *setsigmask,
...
static int parse_file_actions(PyObject *file_actions,
  ...
  seq =
      PySequence_Fast(file_actions, "file_actions must be a sequence or None");
  if (seq == NULL) {
    return -1;
  }
  ...
  for (Py_ssize_t i = 0; i < PySequence_Fast_GET_SIZE(seq); ++i) {
    ...
    case POSIX_SPAWN_OPEN: {
      ...
      PyObject *path;
      unsigned long mode;
      if (!PyArg_ParseTuple(file_action,
                            "OiO&ik"
                            ";A open file_action tuple must have 5 elements",
                            &tag_obj, &fd, PyUnicode_FSConverter, &path, &oflag,
                            &mode)) {
        goto fail;
      }
      ...
      errno = posix_spawn_file_actions_addopen(
          file_actionsp, fd, PyBytes_AS_STRING(path), oflag, (mode_t)mode);
      if (errno) {
        posix_error();
        Py_DECREF(path);
        goto fail;
    ...
    case POSIX_SPAWN_CLOSEFROM: {
      ...
      errno = posix_spawn_file_actions_addclosefrom_np(file_actionsp, fd);
      if (errno) {
        posix_error();
        goto fail;
      }
...
static PyObject *py_posix_spawn(int use_posix_spawnp, PyObject *module,
                                path_t *path, PyObject *argv, PyObject *env,
                                PyObject *file_actions, PyObject *setpgroup,
  ...
  if (!PyList_Check(argv) && !PyTuple_Check(argv)) {
    PyErr_Format(PyExc_TypeError, "%s: argv must be a tuple or list",
                 func_name);
    goto exit;
  }
  argc = PySequence_Size(argv);
  if (argc < 1) {
    PyErr_Format(PyExc_ValueError, "%s: argv must not be empty", func_name);
    goto exit;
  }
  ...
  if (scheduler && !PyTuple_Check(scheduler) && scheduler != Py_None) {
    PyErr_Format(PyExc_TypeError, "%s: scheduler must be a tuple or None",
                 func_name);
    goto exit;
  }
  ...
  if (!argvlist[0][0]) {
    PyErr_Format(PyExc_ValueError, "%s: argv first element cannot be empty",
                 func_name);
    goto exit;
  }
  ...
  _Py_BEGIN_SUPPRESS_IPH
      ...
      if (use_posix_spawnp) {
    err_code = posix_spawnp(&pid, path->narrow, file_actionsp, attrp, argvlist,
                            envlist);
  }
  else
#endif /* HAVE_POSIX_SPAWNP */
  {
    err_code = posix_spawn(&pid, path->narrow, file_actionsp, attrp, argvlist,
                           envlist);
  }
  _Py_END_SUPPRESS_IPH
...
will be activated. setsigmask: object(c_default='NULL') = () The sigmask to use
with the POSIX_SPAWN_SETSIGMASK flag. setsigdef: object(c_default='NULL') = ()
...
os_posix_spawn_impl(PyObject *module, path_t *path, PyObject *argv,
                    PyObject *env, PyObject *file_actions, PyObject *setpgroup,
                    int resetids, int setsid, PyObject *setsigmask,
                    PyObject *setsigdef, PyObject *scheduler)
/*[clinic end generated code: output=14a1098c566bc675 input=69e7c9ebbdcf94a5]*/
{
  return py_posix_spawn(0, module, path, argv, env, file_actions, setpgroup,
...
{
  return py_posix_spawn(1, module, path, argv, env, file_actions, setpgroup,
                        resetids, setsid, setsigmask, setsigdef, scheduler);
}
#endif /* HAVE_POSIX_SPAWNP */

#ifdef HAVE_RTPSPAWN
static intptr_t _rtp_spawn(int mode, const char *rtpFileName,
  ...
  /* Set priority=100 and uStackSize=16 MiB (0x1000000) for new processes.
     ...
     Python. */
  if (envp) {
    rtpid = rtpSpawn(rtpFileName, argv, envp, 100, 0x1000000, 0, VX_FP_TASK);
  } else {
    rtpid = rtpSpawn(rtpFileName, argv, (const char **)environ, 100, 0x1000000,
                     0, VX_FP_TASK);
  }
...
static PyObject *os_spawnv_impl(PyObject *module, int mode, path_t *path,
                                PyObject *argv)
/*[clinic end generated code: output=71cd037a9d96b816 input=43224242303291be]*/
{
  ...
  if (argc == 0) {
    PyErr_SetString(PyExc_ValueError, "spawnv() arg 2 cannot be empty");
    return NULL;
  }
  ...
  for (i = 0; i < argc; i++) {
    if (!fsconvert_strdup((*getitem)(argv, i), &argvlist[i])) {
  ...
  if (PySys_Audit("os.spawn", "iOOO", mode, path->object, argv, Py_None) < 0) {
    free_string_array(argvlist, argc);
    return NULL;
  }
...
static PyObject *os_spawnve_impl(PyObject *module, int mode, path_t *path,
...
/*[clinic end generated code: output=30fe85be56fe37ad input=3e40803ee7c4c586]*/
{
  ...
  if (argc == 0) {
    PyErr_SetString(PyExc_ValueError, "spawnve() arg 2 cannot be empty");
    goto fail_0;
  }
  if (!PyMapping_Check(env)) {
    PyErr_SetString(PyExc_TypeError,
  ...
  _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS
      ...
      if (spawnval == -1)(void) posix_error();
  else res = Py_BuildValue(_Py_PARSE_INTPTR, spawnval);
...
static int check_null_or_callable(PyObject *obj, const char *obj_name) {
  if (obj && !PyCallable_Check(obj)) {
    PyErr_Format(PyExc_TypeError, "'%s' must be callable, not %s", obj_name,
                 _PyType_Name(Py_TYPE(obj)));
    return -1;
  }
  return 0;
...
static PyObject *os_register_at_fork_impl(PyObject *module, PyObject *before,
                                          PyObject *after_in_child,
                                          PyObject *after_in_parent)
...
static int warn_about_fork_with_threads(
...
static Py_ssize_t get_number_of_os_threads(void) {
  ...
  if (task_for_pid(macos_self, getpid(), &macos_task) == KERN_SUCCESS) {
    ...
    if (task_threads(macos_task, &macos_threads, &macos_n_threads) ==
        KERN_SUCCESS) {
...
static PyObject *os_fork1_impl(PyObject *module)
...
static PyObject *os_fork_impl(PyObject *module)
...
static PyObject *os_sched_get_priority_max_impl(PyObject *module, int policy)
...
static PyObject *os_sched_get_priority_min_impl(PyObject *module, int policy)
...
static PyObject *os_sched_param_impl(PyTypeObject *type,
...
static PyObject *os_sched_param_reduce(PyObject *self,
...
static int convert_sched_param(PyObject *module, PyObject *param,
  ...
  if (!Py_IS_TYPE(param,
                  (PyTypeObject *)get_posix_state(module)->SchedParamType)) {
...
#endif /* defined(HAVE_SCHED_SETPARAM) || defined(HAVE_SCHED_SETSCHEDULER) ||  \
          defined(POSIX_SPAWN_SETSCHEDULER) ||                                 \
          defined(POSIX_SPAWN_SETSCHEDPARAM) */
...
static PyObject *os_sched_getparam_impl(PyObject *module, pid_t pid)
...
static PyObject *os_sched_setparam_impl(PyObject *module, pid_t pid,
                                        PyObject *param_obj)
...
static double os_sched_rr_get_interval_impl(PyObject *module, pid_t pid)
...
static PyObject *os_sched_yield_impl(PyObject *module)
/*[clinic end generated code: output=902323500f222cac input=e54d6f98189391d4]*/
{
  int result;
  Py_BEGIN_ALLOW_THREADS result = sched_yield();
  Py_END_ALLOW_THREADS if (result < 0) { return posix_error(); }
  Py_RETURN_NONE;
}
...
static PyObject *os_sched_setaffinity_impl(PyObject *module, pid_t pid,
                                           PyObject *mask)
...
Return the affinity of the process identified by pid (or the current process if
zero).

The affinity is returned as a set of CPU identifiers.
[clinic start generated code]*/

static PyObject *os_sched_getaffinity_impl(PyObject *module, pid_t pid)
...
static int os_posix_openpt_impl(PyObject *module, int oflag)
...
static PyObject *os_grantpt_impl(PyObject *module, int fd)
...
static PyObject *os_unlockpt_impl(PyObject *module, int fd)
...
static PyObject *py_ptsname(int fd) {
...
static PyObject *os_ptsname_impl(PyObject *module, int fd)
...
#define DEV_PTY_FILE "/dev/ptmx"
    ...
    defined(HAVE_LOGIN_TTY) || defined(HAVE_DEV_PTMX)
#ifdef HAVE_PTY_H
#include <pty.h>
...
static PyObject *os_openpty_impl(PyObject *module)
...
static PyObject *os_login_tty_impl(PyObject *module, int fd)
...
static PyObject *os_forkpty_impl(PyObject *module)
...
static PyObject *os_getegid_impl(PyObject *module)
...
static PyObject *os_geteuid_impl(PyObject *module)
...
static PyObject *os_getgid_impl(PyObject *module)
...
static PyObject *os_getpid_impl(PyObject *module)
...
static PyObject *os_getgrouplist_impl(PyObject *module, const char *user,
                                      int basegid)
...
static PyObject *os_getgroups_impl(PyObject *module)
...
static PyObject *os_initgroups_impl(PyObject *module, PyObject *oname, int gid)
...
static PyObject *os_initgroups_impl(PyObject *module, PyObject *oname,
                                    gid_t gid)
...
static PyObject *os_getpgid_impl(PyObject *module, pid_t pid)
...
static PyObject *os_getpgrp_impl(PyObject *module)
...
static PyObject *os_setpgrp_impl(PyObject *module)
...
static ULONG win32_getppid_fast(void) {
  NTSTATUS status;
  HMODULE ntdll;
  ...
  status = pNtQueryInformationProcess(
      GetCurrentProcess(), ProcessBasicInformation, &basic_information,
      sizeof(basic_information), NULL);
  ...
  // NtQueryInformationProcess. The check covers values which exceed the 32-bit
  // range (if running on x64) as well as zero and (ULONG) -1.
  ...
  // Now that we have reached this point, the
  // BasicInformation.InheritedFromUniqueProcessId structure member contains a
  // ULONG_PTR which represents the process ID of our parent process. This
  // process ID will be correctly returned even if the parent process has exited
  // or been terminated.
...
static PyObject *win32_getppid(void) {
  DWORD error;
  PyObject *result = NULL;
...
static PyObject *os_getppid_impl(PyObject *module)
...
static PyObject *os_getlogin_impl(PyObject *module)
...
static PyObject *os_getuid_impl(PyObject *module)
...
static PyObject *os_kill_impl(PyObject *module, pid_t pid, Py_ssize_t signal)
...
static PyObject *os_killpg_impl(PyObject *module, pid_t pgid, int signal)
...
static PyObject *os_plock_impl(PyObject *module, int op)
...
static PyObject *os_setuid_impl(PyObject *module, uid_t uid)
...
static PyObject *os_seteuid_impl(PyObject *module, uid_t euid)
...
static PyObject *os_setegid_impl(PyObject *module, gid_t egid)
...
static PyObject *os_setreuid_impl(PyObject *module, uid_t ruid, uid_t euid)
...
static PyObject *os_setregid_impl(PyObject *module, gid_t rgid, gid_t egid)
...
static PyObject *os_setgid_impl(PyObject *module, gid_t gid)
...
static PyObject *os_setgroups(PyObject *module, PyObject *groups)
/*[clinic end generated code: output=3fcb32aad58c5ecd input=fa742ca3daf85a7e]*/
{
  ...
  for (Py_ssize_t i = 0; i < len; i++) {
    ...
    if (!PyIndex_Check(elem)) {
      PyErr_SetString(PyExc_TypeError, "groups must be integers");
      Py_DECREF(elem);
      PyMem_Free(grouplist);
      return NULL;
    } else {
...
#if defined(HAVE_WAIT3) || defined(HAVE_WAIT4)
static PyObject *wait_helper(PyObject *module, pid_t pid, int status,
                             struct rusage *ru) {
  PyObject *result;
  PyObject *struct_rusage;
...
static PyObject *os_wait3_impl(PyObject *module, int options)
/*[clinic end generated code: output=92c3224e6f28217a input=8ac4c56956b61710]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS pid = wait3(&status, options, &ru);
    Py_END_ALLOW_THREADS
...
static PyObject *os_wait4_impl(PyObject *module, pid_t pid, int options)
/*[clinic end generated code: output=66195aa507b35f70 input=d11deed0750600ba]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS res = wait4(pid, &status, options, &ru);
    Py_END_ALLOW_THREADS
  } while (res < 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
  if (res < 0)
    return (!async_err) ? posix_error() : NULL;
...
static PyObject *os_waitid_impl(PyObject *module, idtype_t idtype, id_t id,
                                int options)
/*[clinic end generated code: output=5d2e1c0bde61f4d8 input=d8e7f76e052b7920]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS res = waitid(idtype, id, &si, options);
    Py_END_ALLOW_THREADS
  } while (res < 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
  if (res < 0)
    return (!async_err) ? posix_error() : NULL;
...
static PyObject *os_waitpid_impl(PyObject *module, pid_t pid, int options)
/*[clinic end generated code: output=5c37c06887a20270 input=0bf1666b8758fda3]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS res = waitpid(pid, &status, options);
    Py_END_ALLOW_THREADS
  } while (res < 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
  if (res < 0)
    return (!async_err) ? posix_error() : NULL;
...
static PyObject *os_waitpid_impl(PyObject *module, intptr_t pid, int options)
/*[clinic end generated code: output=be836b221271d538 input=40f2440c515410f8]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS _Py_BEGIN_SUPPRESS_IPH res =
        _cwait(&status, pid, options);
    _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS
  } while (res < 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
  if (res < 0)
    return (!async_err) ? posix_error() : NULL;
...
static PyObject *os_wait_impl(PyObject *module)
/*[clinic end generated code: output=6bc419ac32fb364b input=03b0182d4a4700ce]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS pid = wait(&status);
    Py_END_ALLOW_THREADS
  } while (pid < 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
  if (pid < 0)
    return (!async_err) ? posix_error() : NULL;
...
static PyObject *os_pidfd_open_impl(PyObject *module, pid_t pid,
                                    unsigned int flags)
...
static PyObject *os_setns_impl(PyObject *module, int fd, int nstype)
/*[clinic end generated code: output=5dbd055bfb66ecd0 input=42787871226bf3ee]*/
{
  int res;

  Py_BEGIN_ALLOW_THREADS res = setns(fd, nstype);
  Py_END_ALLOW_THREADS
...
static PyObject *os_unshare_impl(PyObject *module, int flags)
/*[clinic end generated code: output=1b3177906dd237ee input=9e065db3232b8b1b]*/
{
  int res;

  Py_BEGIN_ALLOW_THREADS res = unshare(flags);
  Py_END_ALLOW_THREADS
...
static PyObject *os_readlink_impl(PyObject *module, path_t *path, int dir_fd)
/*[clinic end generated code: output=d21b732a2e814030 input=113c87e0db1ecaf2]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS reparse_point_handle =
      CreateFileW(path->wide, 0, 0, 0, OPEN_EXISTING,
                  FILE_FLAG_OPEN_REPARSE_POINT | FILE_FLAG_BACKUP_SEMANTICS, 0);
  if (reparse_point_handle != INVALID_HANDLE_VALUE) {
    /* New call DeviceIoControl to read the reparse point */
    io_result = DeviceIoControl(
        reparse_point_handle, FSCTL_GET_REPARSE_POINT, 0, 0, /* in buffer */
        target_buffer, sizeof(target_buffer), &n_bytes_returned,
...
static int _is_absW(const WCHAR *path) {
  return path[0] == L'\\' || path[0] == L'/' || (path[0] && path[1] == L':');
}

/* join root and rest with a backslash - return 0 on success */
static int _joinW(WCHAR *dest_path, const WCHAR *root, const WCHAR *rest) {
  if (_is_absW(rest)) {
...
static int _check_dirW(LPCWSTR src, LPCWSTR dest) {
  WIN32_FILE_ATTRIBUTE_DATA src_info;
  WCHAR dest_parent[MAX_PATH];
  WCHAR src_resolved[MAX_PATH] = L"";

  /* dest_parent = os.path.dirname(dest) */
  if (wcscpy_s(dest_parent, MAX_PATH, dest) || _dirnameW(dest_parent)) {
    return 0;
  }
...
static PyObject *os_symlink_impl(PyObject *module, path_t *src, path_t *dst,
...
PyDoc_STRVAR(times_result__doc__, "times_result: Result from os.times().\n\n\
This object may be accessed either as a tuple of\n\
...
static PyStructSequence_Desc times_result_desc = {MODNAME
                                                  ".times_result", /* name */
                                                  times_result__doc__, /* doc */
                                                  times_result_fields, 5};

static PyObject *build_times_result(PyObject *module, double user,
                                    double system, double children_user,
                                    double children_system, double elapsed) {
...
static PyObject *os_times_impl(PyObject *module)
/*[clinic end generated code: output=35f640503557d32a input=8dbfe33a2dcc3df3]*/
{
  ...
  */
  return build_times_result(
      module,
      (double)(user.dwHighDateTime * 429.4967296 + user.dwLowDateTime * 1e-7),
...
static PyObject *build_itimerspec(const struct itimerspec *curr_value) {
  ...
  double _interval = CONVERT_SEC_AND_NSEC_TO_DOUBLE(
...
static PyObject *build_itimerspec_ns(const struct itimerspec *curr_value) {
    ...
    *
    flags: int = 0
        ...
        os.TFD_NONBLOCK
            ...
            If *TFD_NONBLOCK* is not set as a flag, read block until the timer
fires.
...
static PyObject *os_timerfd_create_impl(PyObject *module, int clockid,
                                        int flags)
/*[clinic end generated code: output=1caae80fb168004a input=64b7020c5ac0b8f4]*/

{
  int fd;
  Py_BEGIN_ALLOW_THREADS flags |=
      TFD_CLOEXEC; // PEP 446: always create non-inheritable FD
  fd = timerfd_create(clockid, flags);
  Py_END_ALLOW_THREADS if (fd == -1) {
    return PyErr_SetFromErrno(PyExc_OSError);
  }
  return PyLong_FromLong(fd);
}
...
static PyObject *os_timerfd_settime_impl(PyObject *module, int fd, int flags,
...
/*[clinic end generated code: output=df4c1bce6859224e input=81d2c0d7e936e8a7]*/
{
  ...
  if (_PyTime_FromSecondsDouble(initial_double, _PyTime_ROUND_FLOOR, &initial) <
      0) {
    return NULL;
  }
  ...
  Py_END_ALLOW_THREADS if (result == -1) {
    return PyErr_SetFromErrno(PyExc_OSError);
  }
...
static PyObject *os_timerfd_settime_ns_impl(PyObject *module, int fd, int flags,
                                            long long initial,
                                            long long interval)
...
Return a tuple of a timer file descriptor's (interval, next expiration) in float
seconds. [clinic start generated code]*/

static PyObject *os_timerfd_gettime_impl(PyObject *module, int fd)
/*[clinic end generated code: output=ec5a94a66cfe6ab4 input=05f7d568a4820dc6]*/
{
  struct itimerspec curr_value;
  int result;
  Py_BEGIN_ALLOW_THREADS result = timerfd_gettime(fd, &curr_value);
  Py_END_ALLOW_THREADS if (result == -1) {
    return PyErr_SetFromErrno(PyExc_OSError);
  }
...
Return a tuple of a timer file descriptor's (interval, next expiration) in
nanoseconds. [clinic start generated code]*/

static PyObject *os_timerfd_gettime_ns_impl(PyObject *module, int fd)
...
static PyObject *os_getsid_impl(PyObject *module, pid_t pid)
...
static PyObject *os_setsid_impl(PyObject *module)
...
static PyObject *os_setpgid_impl(PyObject *module, pid_t pid, pid_t pgrp)
...
static PyObject *os_tcgetpgrp_impl(PyObject *module, int fd)
...
static PyObject *os_tcsetpgrp_impl(PyObject *module, int fd, pid_t pgid)
...
static int os_open_impl(PyObject *module, path_t *path, int flags, int mode,
                        int dir_fd)
...
static PyObject *os_close_impl(PyObject *module, int fd)
/*[clinic end generated code: output=2fe4e93602822c14 input=2bc42451ca5c3223]*/
{
   ...
   * for more details.
   */
  Py_BEGIN_ALLOW_THREADS _Py_BEGIN_SUPPRESS_IPH res = close(fd);
  _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS if (res < 0) return posix_error();
  Py_RETURN_NONE;
}
...
static PyObject *os_closerange_impl(PyObject *module, int fd_low, int fd_high)
/*[clinic end generated code: output=0ce5c20fcda681c2 input=5855a3d053ebd4ec]*/
{
  Py_BEGIN_ALLOW_THREADS _Py_closerange(fd_low, fd_high - 1);
  Py_END_ALLOW_THREADS Py_RETURN_NONE;
}
...
static int os_dup_impl(PyObject *module, int fd)
...
static int os_dup2_impl(PyObject *module, int fd, int fd2, int inheritable)
/*[clinic end generated code: output=bc059d34a73404d1 input=c3cddda8922b038d]*/
{
  int res = 0;
#if defined(HAVE_DUP3) && !(defined(HAVE_FCNTL_H) && defined(F_DUP2FD_CLOEXEC))
   ...
   */
#ifdef MS_WINDOWS
  Py_BEGIN_ALLOW_THREADS _Py_BEGIN_SUPPRESS_IPH res = dup2(fd, fd2);
  _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS if (res < 0) {
    posix_error();
    return -1;
  }
  ...
  Py_BEGIN_ALLOW_THREADS if (!inheritable) res =
  ...
  if (!inheritable && dup3_works != 0) {
    Py_BEGIN_ALLOW_THREADS res = dup3(fd, fd2, O_CLOEXEC);
    Py_END_ALLOW_THREADS if (res < 0) {
      if (dup3_works == -1)
...
static PyObject *os_lockf_impl(PyObject *module, int fd, int command,
                               Py_off_t length)
/*[clinic end generated code: output=af7051f3e7c29651 input=65da41d2106e9b79]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS res = lockf(fd, command, length);
  Py_END_ALLOW_THREADS
...
static Py_off_t os_lseek_impl(PyObject *module, int fd, Py_off_t position,
                              int how)
...
#endif
  _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS if (result < 0) posix_error();
...
static PyObject *os_read_impl(PyObject *module, int fd, Py_ssize_t length)
...
static Py_ssize_t os_readinto_impl(PyObject *module, int fd, Py_buffer *buffer)
...
static int iov_setup(struct iovec **iov, Py_buffer **buf, PyObject *seq,
                     Py_ssize_t cnt, int type) {
  Py_ssize_t i, j;
...
static void iov_cleanup(struct iovec *iov, Py_buffer *buf, int cnt) {
  int i;
  PyMem_Free(iov);
  for (i = 0; i < cnt; i++) {
...
static Py_ssize_t os_readv_impl(PyObject *module, int fd, PyObject *buffers)
/*[clinic end generated code: output=792da062d3fcebdb input=e679eb5dbfa0357d]*/
{
  ...
  if (!PySequence_Check(buffers)) {
    PyErr_SetString(PyExc_TypeError, "readv() arg 2 must be a sequence");
    return -1;
  }
  ...
  do {
    Py_BEGIN_ALLOW_THREADS n = readv(fd, iov, cnt);
    Py_END_ALLOW_THREADS
...
static PyObject *os_pread_impl(PyObject *module, int fd, Py_ssize_t length,
                               Py_off_t offset)
/*[clinic end generated code: output=3f875c1eef82e32f input=5943beb009d3da04]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS _Py_BEGIN_SUPPRESS_IPH n =
        pread(fd, PyBytesWriter_GetData(writer), length, offset);
    _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS
  } while (n < 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
...
static Py_ssize_t os_preadv_impl(PyObject *module, int fd, PyObject *buffers,
                                 Py_off_t offset, int flags)
/*[clinic end generated code: output=26fc9c6e58e7ada5 input=34fb3b9ca06f7ba7]*/
{
  ...
  if (!PySequence_Check(buffers)) {
    PyErr_SetString(PyExc_TypeError, "preadv2() arg 2 must be a sequence");
    return -1;
  }
  ...
  do {
    Py_BEGIN_ALLOW_THREADS _Py_BEGIN_SUPPRESS_IPH n =
        preadv2(fd, iov, cnt, offset, flags);
    _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS
  } while (n < 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
#else
  do {
...
static Py_ssize_t os_write_impl(PyObject *module, int fd, Py_buffer *data)
...
static PyObject *os_sendfile_impl(PyObject *module, int out_fd, int in_fd,
                                  Py_off_t offset, Py_off_t sbytes,
                                  PyObject *headers, PyObject *trailers,
                                  int flags)
...
#endif
{
  ...
  if (headers != NULL) {
    if (!PySequence_Check(headers)) {
      PyErr_SetString(PyExc_TypeError, "sendfile() headers must be a sequence");
      return NULL;
    } else {
      ...
      if (i < 0)
        return NULL;
      if (i > INT_MAX) {
        PyErr_SetString(PyExc_OverflowError, "sendfile() header is too large");
        return NULL;
      }
      if (i > 0) {
        sf.hdr_cnt = (int)i;
        if (iov_setup(&(sf.headers), &hbuf, headers, sf.hdr_cnt, PyBUF_SIMPLE) <
            0)
          return NULL;
#ifdef __APPLE__
...
#define OFF_T_MAX 0x7fffffffffffffff
  ...
  if (trailers != NULL) {
    if (!PySequence_Check(trailers)) {
      ...
      if (i < 0)
        return NULL;
      if (i > INT_MAX) {
        PyErr_SetString(PyExc_OverflowError, "sendfile() trailer is too large");
        return NULL;
      }
      if (i > 0) {
        sf.trl_cnt = (int)i;
        if (iov_setup(&(sf.trailers), &tbuf, trailers, sf.trl_cnt,
                      PyBUF_SIMPLE) < 0)
          return NULL;
      }
    }
  ...
  if (offobj == Py_None) {
    do {
      Py_BEGIN_ALLOW_THREADS ret = sendfile(out_fd, in_fd, NULL, count);
      Py_END_ALLOW_THREADS
    } while (ret < 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
    if (ret < 0)
      return (!async_err) ? posix_error() : NULL;
  ...
  do {
    Py_BEGIN_ALLOW_THREADS ret = fstat(in_fd, &st);
    Py_END_ALLOW_THREADS
  } while (ret != 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
  if (ret < 0)
    return (!async_err) ? posix_error() : NULL;
  ...
  do {
    Py_BEGIN_ALLOW_THREADS ret = sendfile(out_fd, in_fd, &offset, count);
#if defined(__sun) && defined(__SVR4)
...
static PyObject *os__fcopyfile_impl(PyObject *module, int in_fd, int out_fd,
                                    int flags)
/*[clinic end generated code: output=c9d1a35a992e401b input=1e34638a86948795]*/
{
  int ret;

  Py_BEGIN_ALLOW_THREADS ret = fcopyfile(in_fd, out_fd, NULL, flags);
  Py_END_ALLOW_THREADS if (ret < 0) return posix_error();
  Py_RETURN_NONE;
}
#endif
...
static PyObject *os_fstat_impl(PyObject *module, int fd)
/*[clinic end generated code: output=efc038cb5f654492 input=27e0e0ebbe5600c9]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS res = FSTAT(fd, &st);
    Py_END_ALLOW_THREADS
  } while (res != 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
  if (res != 0) {
#ifdef MS_WINDOWS
...
static int os_isatty_impl(PyObject *module, int fd)
/*[clinic end generated code: output=6a48c8b4e644ca00 input=08ce94aa1eaf7b5e]*/
{
  int return_value;
  Py_BEGIN_ALLOW_THREADS _Py_BEGIN_SUPPRESS_IPH return_value = isatty(fd);
  _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS return return_value;
}
...
static PyObject *os_pipe_impl(PyObject *module)
/*[clinic end generated code: output=ff9b76255793b440 input=02535e8c8fa6c4d4]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS ok = CreatePipe(&read, &write, &attr, 0);
  if (ok) {
...
#ifdef HAVE_PIPE2
  Py_BEGIN_ALLOW_THREADS res = pipe2(fds, O_CLOEXEC);
  Py_END_ALLOW_THREADS

      if (res != 0 && errno == ENOSYS) {
#endif
    Py_BEGIN_ALLOW_THREADS res = pipe(fds);
    Py_END_ALLOW_THREADS
...
static PyObject *os_pipe2_impl(PyObject *module, int flags)
...
static Py_ssize_t os_writev_impl(PyObject *module, int fd, PyObject *buffers)
/*[clinic end generated code: output=56565cfac3aac15b input=5b8d17fe4189d2fe]*/
{
  ...
  if (!PySequence_Check(buffers)) {
    PyErr_SetString(PyExc_TypeError, "writev() arg 2 must be a sequence");
    return -1;
  }
  cnt = PySequence_Size(buffers);
  if (cnt < 0)
  ...
  do {
    Py_BEGIN_ALLOW_THREADS result = writev(fd, iov, cnt);
    Py_END_ALLOW_THREADS
...
static Py_ssize_t os_pwrite_impl(PyObject *module, int fd, Py_buffer *buffer,
                                 Py_off_t offset)
/*[clinic end generated code: output=c74da630758ee925 input=614acbc7e5a0339a]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS _Py_BEGIN_SUPPRESS_IPH size =
        pwrite(fd, buffer->buf, (size_t)buffer->len, offset);
    _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS
...
contents of first buffer is written before proceeding to second, and so on. The
operating system may set a limit (sysconf() value SC_IOV_MAX) on the number of
buffers that can be used. This function writes the contents of each object to
the file descriptor and returns the total number of bytes written.
...
static Py_ssize_t os_pwritev_impl(PyObject *module, int fd, PyObject *buffers,
                                  Py_off_t offset, int flags)
/*[clinic end generated code: output=e3dd3e9d11a6a5c7 input=7de72245873f56bf]*/
{
  ...
  if (!PySequence_Check(buffers)) {
    PyErr_SetString(PyExc_TypeError, "pwritev() arg 2 must be a sequence");
    return -1;
  }
...
#ifdef HAVE_PWRITEV2
  do {
    Py_BEGIN_ALLOW_THREADS _Py_BEGIN_SUPPRESS_IPH result =
        pwritev2(fd, iov, cnt, offset, flags);
    _Py_END_SUPPRESS_IPH Py_END_ALLOW_THREADS
  } while (result < 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
#else
...
static PyObject *os_copy_file_range_impl(PyObject *module, int src, int dst,
...
/*[clinic end generated code: output=1a91713a1d99fc7a input=08dacb760869b87c]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS ret =
        copy_file_range(src, p_offset_src, dst, p_offset_dst, count, flags);
    Py_END_ALLOW_THREADS
  } while (ret < 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
...
static PyObject *os_splice_impl(PyObject *module, int src, int dst,
                                Py_ssize_t count, PyObject *offset_src,
                                PyObject *offset_dst, unsigned int flags)
/*[clinic end generated code: output=d0386f25a8519dc5 input=034852a7b2e7af35]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS ret =
        splice(src, p_offset_src, dst, p_offset_dst, count, flags);
    Py_END_ALLOW_THREADS
  } while (ret < 0 && errno == EINTR && !(async_err = PyErr_CheckSignals()));
...
static PyObject *os_mkfifo_impl(PyObject *module, path_t *path, int mode,
                                int dir_fd)
...
static PyObject *os_mknod_impl(PyObject *module, path_t *path, int mode,
                               dev_t device, int dir_fd)
...
static PyObject *major_minor_conv(unsigned int value) {
#ifdef NODEV
...
static int major_minor_check(dev_t value) {
#ifdef NODEV
  if (value == NODEV) {
    return 1;
  }
...
static PyObject *os_major_impl(PyObject *module, dev_t device)
...
static PyObject *os_minor_impl(PyObject *module, dev_t device)
...
static dev_t os_makedev_impl(PyObject *module, dev_t major, dev_t minor)
...
static PyObject *os_ftruncate_impl(PyObject *module, int fd, Py_off_t length)
...
static PyObject *os_truncate_impl(PyObject *module, path_t *path,
                                  Py_off_t length)
...
#endif
  _Py_END_SUPPRESS_IPH
      Py_END_ALLOW_THREADS if (result < 0) return posix_path_error(path);
...
static PyObject *os_posix_fallocate_impl(PyObject *module, int fd,
                                         Py_off_t offset, Py_off_t length)
/*[clinic end generated code: output=73f107139564aa9d input=d7a2ef0ab2ca52fb]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS result = posix_fallocate(fd, offset, length);
    Py_END_ALLOW_THREADS
...
static PyObject *os_posix_fadvise_impl(PyObject *module, int fd,
                                       Py_off_t offset, Py_off_t length,
                                       int advice)
/*[clinic end generated code: output=412ef4aa70c98642 input=0fbe554edc2f04b5]*/
{
  ...
  do {
    Py_BEGIN_ALLOW_THREADS result = posix_fadvise(fd, offset, length, advice);
    Py_END_ALLOW_THREADS
  } while (result == EINTR && !(async_err = PyErr_CheckSignals()));
...
#ifdef MS_WINDOWS
static PyObject *win32_putenv(PyObject *name, PyObject *value) {
  /* Search from index 1 because on Windows starting '=' is allowed for
     defining hidden environment variables. */
...
static PyObject *os_putenv_impl(PyObject *module, PyObject *name,
                                PyObject *value)
...
static PyObject *os_unsetenv_impl(PyObject *module, PyObject *name)
...
static PyObject *os__clearenv_impl(PyObject *module)
...
static PyObject *os_strerror_impl(PyObject *module, int code)
/*[clinic end generated code: output=baebf09fa02a78f2 input=75a8673d97915a91]*/
{
  char *message = strerror(code);
  if (message == NULL) {
    PyErr_SetString(PyExc_ValueError, "strerror() argument out of range");
    return NULL;
  }
...
static int os_WCOREDUMP_impl(PyObject *module, int status)
...
static int os_WIFCONTINUED_impl(PyObject *module, int status)
...
static int os_WIFSTOPPED_impl(PyObject *module, int status)
...
static int os_WIFSIGNALED_impl(PyObject *module, int status)
...
static int os_WIFEXITED_impl(PyObject *module, int status)
...
static int os_WEXITSTATUS_impl(PyObject *module, int status)
...
static int os_WTERMSIG_impl(PyObject *module, int status)
...
static int os_WSTOPSIG_impl(PyObject *module, int status)
...
static PyObject *_pystatvfs_fromstructstatfs(PyObject *module,
                                             struct statfs st) {
  ...
  _Static_assert(sizeof(st.f_blocks) == sizeof(long long),
                 "assuming large file");
...
static PyObject *_pystatvfs_fromstructstatvfs(PyObject *module,
                                              struct statvfs st) {
  PyObject *StatVFSResultType = get_posix_state(module)->StatVFSResultType;
  PyObject *v = PyStructSequence_New((PyTypeObject *)StatVFSResultType);
  if (v == NULL)
...
static PyObject *os_fstatvfs_impl(PyObject *module, int fd)
/*[clinic end generated code: output=53547cf0cc55e6c5 input=d8122243ac50975e]*/
{
   ...
   */
  do {
    Py_BEGIN_ALLOW_THREADS result = fstatfs(fd, &st);
    Py_END_ALLOW_THREADS
  } while (result != 0 && errno == EINTR &&
           !(async_err = PyErr_CheckSignals()));
  if (result != 0)
  ...
  do {
    Py_BEGIN_ALLOW_THREADS result = fstatvfs(fd, &st);
    Py_END_ALLOW_THREADS
  } while (result != 0 && errno == EINTR &&
           !(async_err = PyErr_CheckSignals()));
  if (result != 0)
...
static PyObject *os_statvfs_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=87106dd1beb8556e input=3f5c35791c669bd9]*/
{
  ...
  else result = statfs(path->narrow, &st);
  Py_END_ALLOW_THREADS
...
static PyObject *os__getdiskusage_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=3bd3991f5e5c5dfb input=6af8d1b7781cc042]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS retval =
      GetDiskFreeSpaceExW(path->wide, &_, &total, &free);
  Py_END_ALLOW_THREADS if (retval == 0) {
    if (GetLastError() == ERROR_DIRECTORY) {
      ...
      if (_dirnameW(dir_path) != -1) {
        Py_BEGIN_ALLOW_THREADS retval =
            GetDiskFreeSpaceExW(dir_path, &_, &total, &free);
        Py_END_ALLOW_THREADS
      }
...
static int conv_confname(PyObject *module, PyObject *arg, int *valuep,
...
static long os_fpathconf_impl(PyObject *module, int fd, int name)
/*[clinic end generated code: output=d5b7042425fc3e21 input=023d44589c9ed6aa]*/
{
  ...
  if (fd < 0) {
    PyErr_SetString(PyExc_ValueError, "file descriptor cannot be negative");
...
static long os_pathconf_impl(PyObject *module, path_t *path, int name)
...
static PyObject *os_confstr_impl(PyObject *module, int name)
...
static long os_sysconf_impl(PyObject *module, int name)
...
static int setup_confname_table(struct constdef *table, size_t tablesize,
...
static PyObject *os_abort_impl(PyObject *module)
...
static HINSTANCE(CALLBACK *Py_ShellExecuteW)(HWND, LPCWSTR, LPCWSTR, LPCWSTR,
                                             LPCWSTR, INT);
static int check_ShellExecute(void) {
  HINSTANCE hShell32;
  ...
  if (-1 == has_ShellExecute) {
    ...
    if (hShell32) {
      *(FARPROC *)&Py_ShellExecuteW = GetProcAddress(hShell32, "ShellExecuteW");
...
static PyObject *
os_startfile_impl(PyObject *module, path_t *filepath, const wchar_t *operation,
                  const wchar_t *arguments, path_t *cwd, int show_cmd)
/*[clinic end generated code: output=1c6f2f3340e31ffa input=8248997b80669622]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS rc = Py_ShellExecuteW(
      (HWND)0, operation, filepath->wide, arguments, cwd->wide, show_cmd);
  Py_END_ALLOW_THREADS
...
static PyObject *os_getloadavg_impl(PyObject *module)
...
static PyObject *os_device_encoding_impl(PyObject *module, int fd)
...
static PyObject *os_setresuid_impl(PyObject *module, uid_t ruid, uid_t euid,
                                   uid_t suid)
...
static PyObject *os_setresgid_impl(PyObject *module, gid_t rgid, gid_t egid,
                                   gid_t sgid)
...
static PyObject *os_getresuid_impl(PyObject *module)
/*[clinic end generated code: output=8e0becff5dece5bf input=41ccfa8e1f6517ad]*/
{
  uid_t ruid, euid, suid;
  if (getresuid(&ruid, &euid, &suid) < 0)
    return posix_error();
  return Py_BuildValue("(NNN)", _PyLong_FromUid(ruid), _PyLong_FromUid(euid),
...
static PyObject *os_getresgid_impl(PyObject *module)
/*[clinic end generated code: output=2719c4bfcf27fb9f input=ad9adadc86fbdb17]*/
{
  gid_t rgid, egid, sgid;
  if (getresgid(&rgid, &egid, &sgid) < 0)
    return posix_error();
  return Py_BuildValue("(NNN)", _PyLong_FromGid(rgid), _PyLong_FromGid(egid),
...
static PyObject *os_getxattr_impl(PyObject *module, path_t *path,
                                  path_t *attribute, int follow_symlinks)
...
static PyObject *os_setxattr_impl(PyObject *module, path_t *path,
                                  path_t *attribute, Py_buffer *value,
                                  int flags, int follow_symlinks)
/*[clinic end generated code: output=98b83f63fdde26bb input=4098e6f68699f3d7]*/
{
  ...
  if (path->fd > -1)
    result =
        fsetxattr(path->fd, attribute->narrow, value->buf, value->len, flags);
  else if (follow_symlinks)
    result = setxattr(path->narrow, attribute->narrow, value->buf, value->len,
                      flags);
  else
    result = lsetxattr(path->narrow, attribute->narrow, value->buf, value->len,
                       flags);
  Py_END_ALLOW_THREADS;
...
static PyObject *os_removexattr_impl(PyObject *module, path_t *path,
                                     path_t *attribute, int follow_symlinks)
...
descriptor. if path is None, listxattr will examine the current directory. If
follow_symlinks is False, and the last element of the path is a symbolic link,
listxattr will examine the symbolic link itself instead of the file the link
points to. [clinic start generated code]*/

static PyObject *os_listxattr_impl(PyObject *module, path_t *path,
                                   int follow_symlinks)
  ...
  for (i = 0;; i++) {
    ...
    for (trace = start = buffer; trace != end; trace++) {
      if (!*trace) {
        int error;
        PyObject *attribute =
            PyUnicode_DecodeFSDefaultAndSize(start, trace - start);
        if (!attribute) {
...
static PyObject *os_urandom_impl(PyObject *module, Py_ssize_t size)
/*[clinic end generated code: output=42c5cca9d18068e9 input=58a0def87dbc2c22]*/
{
  if (size < 0) {
    return PyErr_Format(PyExc_ValueError, "negative argument not allowed");
  }
...
static PyObject *os_memfd_create_impl(PyObject *module, PyObject *name,
                                      unsigned int flags)
/*[clinic end generated code: output=6681ede983bdb9a6 input=cd0eb092cfac474b]*/
{
  int fd;
  const char *bytes = PyBytes_AS_STRING(name);
  Py_BEGIN_ALLOW_THREADS fd = memfd_create(bytes, flags);
  Py_END_ALLOW_THREADS if (fd == -1) {
    return PyErr_SetFromErrno(PyExc_OSError);
  }
  return PyLong_FromLong(fd);
...
static PyObject *os_eventfd_impl(PyObject *module, unsigned int initval,
                                 int flags)
...

{
  ...
  int fd;
  Py_BEGIN_ALLOW_THREADS fd = eventfd(initval, flags);
  Py_END_ALLOW_THREADS if (fd == -1) {
    return PyErr_SetFromErrno(PyExc_OSError);
  }
  return PyLong_FromLong(fd);
...
static PyObject *os_eventfd_read_impl(PyObject *module, int fd)
/*[clinic end generated code: output=8f2c7b59a3521fd1 input=110f8b57fa596afe]*/
{
  eventfd_t value;
  int result;
  Py_BEGIN_ALLOW_THREADS result = eventfd_read(fd, &value);
  Py_END_ALLOW_THREADS if (result == -1) {
    return PyErr_SetFromErrno(PyExc_OSError);
  }
  return PyLong_FromUnsignedLongLong(value);
...
static PyObject *os_eventfd_write_impl(PyObject *module, int fd,
                                       unsigned long long value)
/*[clinic end generated code: output=bebd9040bbf987f5 input=156de8555be5a949]*/
{
  int result;
  Py_BEGIN_ALLOW_THREADS result = eventfd_write(fd, value);
  Py_END_ALLOW_THREADS if (result == -1) {
    return PyErr_SetFromErrno(PyExc_OSError);
  }
  Py_RETURN_NONE;
...
static PyObject *os_get_terminal_size_impl(PyObject *module, int fd)
...
static PyObject *os_cpu_count_impl(PyObject *module)
/*[clinic end generated code: output=5fc29463c3936a9c input=ba2f6f8980a0e2eb]*/
{
  ...
  ncpu = _Py_popcount32(vxCpuEnabledGet());
    ...
    defined(__FreeBSD__) || defined(__NetBSD__) || defined(__APPLE__)
  ncpu = 0;
...
static int os_get_inheritable_impl(PyObject *module, int fd)
/*[clinic end generated code: output=0445e20e149aa5b8 input=89ac008dc9ab6b95]*/
{
  int return_value;
  _Py_BEGIN_SUPPRESS_IPH return_value = _Py_get_inheritable(fd);
  _Py_END_SUPPRESS_IPH return return_value;
}
...
static PyObject *os_set_inheritable_impl(PyObject *module, int fd,
                                         int inheritable)
/*[clinic end generated code: output=f1b1918a2f3c38c2 input=9ceaead87a1e2402]*/
{
  int result;

  _Py_BEGIN_SUPPRESS_IPH result = _Py_set_inheritable(fd, inheritable, NULL);
  _Py_END_SUPPRESS_IPH if (result < 0) return NULL;
  Py_RETURN_NONE;
}
...
static int os_get_handle_inheritable_impl(PyObject *module, intptr_t handle)
...
static PyObject *os_set_handle_inheritable_impl(PyObject *module,
                                                intptr_t handle,
                                                int inheritable)
...
static int os_get_blocking_impl(PyObject *module, int fd)
/*[clinic end generated code: output=336a12ad76a61482 input=f4afb59d51560179]*/
{
  int blocking;

  _Py_BEGIN_SUPPRESS_IPH blocking = _Py_get_blocking(fd);
  _Py_END_SUPPRESS_IPH return blocking;
}
...
static PyObject *os_set_blocking_impl(PyObject *module, int fd, int blocking)
/*[clinic end generated code: output=384eb43aa0762a9d input=7e9dfc9b14804dd4]*/
{
  int result;

  _Py_BEGIN_SUPPRESS_IPH result = _Py_set_blocking(fd, blocking);
  _Py_END_SUPPRESS_IPH if (result < 0) return NULL;
  Py_RETURN_NONE;
}
...
static void DirEntry_dealloc(PyObject *op) {
...
/* Forward reference */
static int DirEntry_test_mode(PyTypeObject *defining_class, DirEntry *self,
...
#ifdef HAVE_FSTATAT
    if (HAVE_FSTATAT_RUNTIME) {
      Py_BEGIN_ALLOW_THREADS result = fstatat(
          self->dir_fd, path, &st, follow_symlinks ? 0 : AT_SYMLINK_NOFOLLOW);
      Py_END_ALLOW_THREADS
    } else
...
#endif
  {
    Py_BEGIN_ALLOW_THREADS if (follow_symlinks) { result = STAT(path, &st); }
    else {
...
static PyObject *DirEntry_get_lstat(PyTypeObject *defining_class,
                                    DirEntry *self) {
  if (!self->lstat) {
...
static int os_DirEntry_is_dir_impl(DirEntry *self, PyTypeObject *defining_class,
                                   int follow_symlinks)
...
/*[clinic end generated code: output=156bb3a72162440e input=3ee7b872ae8649f0]*/
{
  ...
  return _pystat_l128_from_l64_l64(self->win32_file_index,
                                   self->win32_file_index_high);
#else /* POSIX */
...
static PyObject *DirEntry_repr(PyObject *op) {
...
static PyMethodDef DirEntry_methods[] = {
    OS_DIRENTRY_IS_DIR_METHODDEF OS_DIRENTRY_IS_FILE_METHODDEF
        OS_DIRENTRY_IS_SYMLINK_METHODDEF OS_DIRENTRY_IS_JUNCTION_METHODDEF
            OS_DIRENTRY_STAT_METHODDEF OS_DIRENTRY_INODE_METHODDEF
...
static wchar_t *join_path_filenameW(const wchar_t *path_wide,
...
static PyObject *DirEntry_from_find_data(PyObject *module, path_t *path,
  ...
  _Py_attribute_data_to_stat(&file_info, reparse_tag, NULL, NULL,
                             &entry->win32_lstat);
...
static char *join_path_filename(const char *path_narrow, const char *filename,
...
static PyObject *DirEntry_from_posix_info(PyObject *module, path_t *path,
...
static int ScandirIterator_is_closed(ScandirIterator *iterator) {
  return iterator->handle == INVALID_HANDLE_VALUE;
}

static void ScandirIterator_closedir(ScandirIterator *iterator) {
  ...
  Py_BEGIN_ALLOW_THREADS FindClose(handle);
  Py_END_ALLOW_THREADS
}

static PyObject *ScandirIterator_iternext(PyObject *op) {
  ...
  while (1) {
    if (!iterator->first_time) {
      Py_BEGIN_ALLOW_THREADS success =
          FindNextFileW(iterator->handle, file_data);
...
static void ScandirIterator_closedir(ScandirIterator *iterator) {
  ...
  Py_BEGIN_ALLOW_THREADS
#ifdef HAVE_FDOPENDIR
      if (iterator->path.fd != -1) rewinddir(dirp);
#endif
  closedir(dirp);
...
static PyObject *ScandirIterator_iternext(PyObject *op) {
  ...
  while (1) {
    errno = 0;
    Py_BEGIN_ALLOW_THREADS direntp = readdir(iterator->dirp);
    Py_END_ALLOW_THREADS
    ...
    if (!is_dot) {
      PyObject *module = PyType_GetModule(Py_TYPE(iterator));
      entry = DirEntry_from_posix_info(module, &iterator->path, direntp->d_name,
...
static PyObject *ScandirIterator_close(PyObject *op,
                                       PyObject *Py_UNUSED(dummy)) {
...
static PyObject *ScandirIterator_enter(PyObject *self,
                                       PyObject *Py_UNUSED(dummy)) {
  return Py_NewRef(self);
}

static PyObject *ScandirIterator_exit(PyObject *op, PyObject *Py_UNUSED(args)) {
  ScandirIterator *self = ScandirIterator_CAST(op);
  ScandirIterator_closedir(self);
  Py_RETURN_NONE;
}

static void ScandirIterator_finalize(PyObject *op) {
  ScandirIterator *iterator = ScandirIterator_CAST(op);
  ...
  if (!ScandirIterator_is_closed(iterator)) {
    ...
    if (PyErr_ResourceWarning(op, 1, "unclosed scandir iterator %R",
                              iterator)) {
...
static void ScandirIterator_dealloc(PyObject *op) {
...
static PyObject *os_scandir_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=6eb2668b675ca89e input=6bdd312708fc3bb0]*/
{
  ...
  if (PySys_Audit("os.scandir", "O", path->object ? path->object : Py_None) <
      0) {
    return NULL;
  }
  ...
  Py_BEGIN_ALLOW_THREADS iterator->handle =
      FindFirstFileW(path_strW, &iterator->file_data);
  Py_END_ALLOW_THREADS
  ...
  if (iterator->path.fd != -1) {
    if (HAVE_FDOPENDIR_RUNTIME) {
      ...
      Py_BEGIN_ALLOW_THREADS iterator->dirp = fdopendir(fd);
      Py_END_ALLOW_THREADS
    } else {
      PyErr_SetString(PyExc_TypeError, "scandir: path should be string, bytes, "
                                       "os.PathLike or None, not int");
      return NULL;
    }
  } else
#endif
  {
    ...
    Py_BEGIN_ALLOW_THREADS iterator->dirp = opendir(path_str);
    Py_END_ALLOW_THREADS
  }
...
PyObject *PyOS_FSPath(PyObject *path) {
  ...
  if (!(PyUnicode_Check(path_repr) || PyBytes_Check(path_repr))) {
    PyErr_Format(PyExc_TypeError,
                 "expected %.200s.__fspath__() to return str or bytes, "
                 "not %.200s",
                 _PyType_Name(Py_TYPE(path)), _PyType_Name(Py_TYPE(path_repr)));
    Py_DECREF(path_repr);
    return NULL;
  }
...
static PyObject *os_fspath_impl(PyObject *module, PyObject *path)
...
static PyObject *os_getrandom_impl(PyObject *module, Py_ssize_t size, int flags)
...
static PyObject *os__add_dll_directory_impl(PyObject *module, path_t *path)
/*[clinic end generated code: output=80b025daebb5d683 input=1de3e6c13a5808c8]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS if (!(cookie = AddDllDirectory(path->wide))) {
    err = GetLastError();
  }
  Py_END_ALLOW_THREADS

      if (err) {
    return win32_error_object_err("add_dll_directory", path->object, err);
  }
...
static PyObject *os__remove_dll_directory_impl(PyObject *module,
                                               PyObject *cookie)
/*[clinic end generated code: output=594350433ae535bc input=c1d16a7e7d9dc5dc]*/
{
  ...
  Py_BEGIN_ALLOW_THREADS if (!RemoveDllDirectory(cookieValue)) {
    err = GetLastError();
  }
  Py_END_ALLOW_THREADS

      if (err) {
    return win32_error_object_err("remove_dll_directory", NULL, err);
  }
...
static PyObject *os_waitstatus_to_exitcode_impl(PyObject *module,
                                                PyObject *status_obj)
/*[clinic end generated code: output=db50b1b0ba3c7153 input=7fe2d7fdaea3db42]*/
{
  ...
  if (WIFEXITED(wait_status)) {
    ...
    PyErr_Format(PyExc_ValueError, "process stopped by delivery of signal %i",
                 signum);
    return NULL;
  } else {
...
static PyObject *os__supports_virtual_terminal_impl(PyObject *module)
...
static PyObject *os__inputhook_impl(PyObject *module)
...
static PyObject *os__is_inputhook_installed_impl(PyObject *module)
...
static PyObject *os__create_environ_impl(PyObject *module)
...
static PyObject *os__emscripten_debugger_impl(PyObject *module)
...
static PyObject *os__emscripten_log_impl(PyObject *module, const char *arg)
...
static PyMethodDef posix_methods[] = {
    OS_STAT_METHODDEF OS_STATX_METHODDEF OS_ACCESS_METHODDEF OS_TTYNAME_METHODDEF OS_CHDIR_METHODDEF OS_CHFLAGS_METHODDEF
        OS_CHMOD_METHODDEF OS_FCHMOD_METHODDEF OS_LCHMOD_METHODDEF OS_CHOWN_METHODDEF OS_FCHOWN_METHODDEF OS_LCHOWN_METHODDEF
            OS_LCHFLAGS_METHODDEF OS_CHROOT_METHODDEF OS_CTERMID_METHODDEF OS_GETCWD_METHODDEF OS_GETCWDB_METHODDEF
                OS_LINK_METHODDEF OS_LISTDIR_METHODDEF OS_LISTDRIVES_METHODDEF OS_LISTMOUNTS_METHODDEF OS_LISTVOLUMES_METHODDEF
                    OS_LSTAT_METHODDEF OS_MKDIR_METHODDEF OS_NICE_METHODDEF OS_GETPRIORITY_METHODDEF OS_SETPRIORITY_METHODDEF
                        OS_POSIX_SPAWN_METHODDEF OS_POSIX_SPAWNP_METHODDEF OS_READLINK_METHODDEF OS_COPY_FILE_RANGE_METHODDEF OS_SPLICE_METHODDEF
                            OS_RENAME_METHODDEF OS_REPLACE_METHODDEF OS_RMDIR_METHODDEF OS_SYMLINK_METHODDEF OS_SYSTEM_METHODDEF
                                OS_UMASK_METHODDEF OS_UNAME_METHODDEF OS_UNLINK_METHODDEF OS_REMOVE_METHODDEF
                                    OS_UTIME_METHODDEF OS_TIMES_METHODDEF OS__EXIT_METHODDEF OS__FCOPYFILE_METHODDEF
                                        OS_EXECV_METHODDEF OS_EXECVE_METHODDEF OS_SPAWNV_METHODDEF OS_SPAWNVE_METHODDEF
                                            OS_FORK1_METHODDEF OS_FORK_METHODDEF
                                                OS_REGISTER_AT_FORK_METHODDEF OS_SCHED_GET_PRIORITY_MAX_METHODDEF OS_SCHED_GET_PRIORITY_MIN_METHODDEF OS_SCHED_GETPARAM_METHODDEF OS_SCHED_GETSCHEDULER_METHODDEF
                                                    OS_SCHED_RR_GET_INTERVAL_METHODDEF OS_SCHED_SETPARAM_METHODDEF OS_SCHED_SETSCHEDULER_METHODDEF
                                                        OS_SCHED_YIELD_METHODDEF OS_SCHED_SETAFFINITY_METHODDEF
                                                            OS_SCHED_GETAFFINITY_METHODDEF OS_POSIX_OPENPT_METHODDEF
                                                                OS_GRANTPT_METHODDEF OS_UNLOCKPT_METHODDEF OS_PTSNAME_METHODDEF
                                                                    OS_OPENPTY_METHODDEF OS_LOGIN_TTY_METHODDEF
                                                                        OS_FORKPTY_METHODDEF OS_GETEGID_METHODDEF
                                                                            OS_GETEUID_METHODDEF OS_GETGID_METHODDEF OS_GETGROUPLIST_METHODDEF
                                                                                OS_GETGROUPS_METHODDEF OS_GETPID_METHODDEF OS_GETPGRP_METHODDEF
                                                                                    OS_GETPPID_METHODDEF OS_GETUID_METHODDEF OS_GETLOGIN_METHODDEF
                                                                                        OS_KILL_METHODDEF OS_KILLPG_METHODDEF OS_PLOCK_METHODDEF OS_STARTFILE_METHODDEF OS_SETUID_METHODDEF OS_SETEUID_METHODDEF OS_SETREUID_METHODDEF OS_SETGID_METHODDEF OS_SETEGID_METHODDEF OS_SETREGID_METHODDEF OS_SETGROUPS_METHODDEF OS_INITGROUPS_METHODDEF OS_GETPGID_METHODDEF OS_SETPGRP_METHODDEF OS_WAIT_METHODDEF OS_WAIT3_METHODDEF OS_WAIT4_METHODDEF OS_WAITID_METHODDEF OS_WAITPID_METHODDEF OS_PIDFD_OPEN_METHODDEF OS_GETSID_METHODDEF OS_SETSID_METHODDEF OS_SETPGID_METHODDEF OS_TCGETPGRP_METHODDEF OS_TCSETPGRP_METHODDEF OS_OPEN_METHODDEF OS_CLOSE_METHODDEF OS_CLOSERANGE_METHODDEF OS_DEVICE_ENCODING_METHODDEF OS_DUP_METHODDEF OS_DUP2_METHODDEF OS_LOCKF_METHODDEF OS_LSEEK_METHODDEF OS_READ_METHODDEF OS_READINTO_METHODDEF OS_READV_METHODDEF OS_PREAD_METHODDEF OS_PREADV_METHODDEF OS_WRITE_METHODDEF OS_WRITEV_METHODDEF OS_PWRITE_METHODDEF OS_PWRITEV_METHODDEF OS_SENDFILE_METHODDEF OS_FSTAT_METHODDEF OS_ISATTY_METHODDEF OS_PIPE_METHODDEF OS_PIPE2_METHODDEF OS_MKFIFO_METHODDEF OS_MKNOD_METHODDEF OS_MAJOR_METHODDEF OS_MINOR_METHODDEF OS_MAKEDEV_METHODDEF OS_FTRUNCATE_METHODDEF OS_TRUNCATE_METHODDEF OS_POSIX_FALLOCATE_METHODDEF OS_POSIX_FADVISE_METHODDEF OS_PUTENV_METHODDEF OS_UNSETENV_METHODDEF OS__CLEARENV_METHODDEF OS_STRERROR_METHODDEF OS_FCHDIR_METHODDEF OS_FSYNC_METHODDEF OS_SYNC_METHODDEF OS_FDATASYNC_METHODDEF OS_WCOREDUMP_METHODDEF OS_WIFCONTINUED_METHODDEF OS_WIFSTOPPED_METHODDEF OS_WIFSIGNALED_METHODDEF OS_WIFEXITED_METHODDEF OS_WEXITSTATUS_METHODDEF OS_WTERMSIG_METHODDEF OS_WSTOPSIG_METHODDEF OS_FSTATVFS_METHODDEF OS_STATVFS_METHODDEF OS_CONFSTR_METHODDEF OS_SYSCONF_METHODDEF OS_FPATHCONF_METHODDEF OS_PATHCONF_METHODDEF OS_ABORT_METHODDEF OS__GETFULLPATHNAME_METHODDEF OS__GETDISKUSAGE_METHODDEF OS__GETFINALPATHNAME_METHODDEF
                                                                                            OS__FINDFIRSTFILE_METHODDEF OS__GETVOLUMEPATHNAME_METHODDEF OS__PATH_SPLITROOT_METHODDEF OS__PATH_SPLITROOT_EX_METHODDEF OS__PATH_NORMPATH_METHODDEF OS_GETLOADAVG_METHODDEF OS_URANDOM_METHODDEF OS_SETRESUID_METHODDEF OS_SETRESGID_METHODDEF
                                                                                                OS_GETRESUID_METHODDEF OS_GETRESGID_METHODDEF

                                                                                                    OS_GETXATTR_METHODDEF OS_SETXATTR_METHODDEF
                                                                                                                ...
                                                                                                                OS_GET_TERMINAL_SIZE_METHODDEF
                                                                                                                    OS_CPU_COUNT_METHODDEF
                                                                                                                        OS_GET_INHERITABLE_METHODDEF
                                                                                                                            OS_SET_INHERITABLE_METHODDEF OS_GET_HANDLE_INHERITABLE_METHODDEF OS_SET_HANDLE_INHERITABLE_METHODDEF OS_GET_BLOCKING_METHODDEF OS_SET_BLOCKING_METHODDEF OS_SCANDIR_METHODDEF OS_FSPATH_METHODDEF OS_GETRANDOM_METHODDEF OS_MEMFD_CREATE_METHODDEF OS_EVENTFD_METHODDEF OS_EVENTFD_READ_METHODDEF OS_EVENTFD_WRITE_METHODDEF OS__ADD_DLL_DIRECTORY_METHODDEF OS__REMOVE_DLL_DIRECTORY_METHODDEF OS_WAITSTATUS_TO_EXITCODE_METHODDEF OS_SETNS_METHODDEF OS_UNSHARE_METHODDEF OS_TIMERFD_CREATE_METHODDEF OS_TIMERFD_SETTIME_METHODDEF OS_TIMERFD_SETTIME_NS_METHODDEF
                                                                                                                                OS_TIMERFD_GETTIME_METHODDEF OS_TIMERFD_GETTIME_NS_METHODDEF

                                                                                                                                    OS__PATH_ISDEVDRIVE_METHODDEF OS__PATH_ISDIR_METHODDEF OS__PATH_ISFILE_METHODDEF
                                                                                                                                        OS__PATH_ISLINK_METHODDEF OS__PATH_ISJUNCTION_METHODDEF
                                                                                                                                            OS__PATH_EXISTS_METHODDEF OS__PATH_LEXISTS_METHODDEF

                                                                                                                                                OS__SUPPORTS_VIRTUAL_TERMINAL_METHODDEF
                                                                                                                                                    OS__INPUTHOOK_METHODDEF
                                                                                                                                                        OS__IS_INPUTHOOK_INSTALLED_METHODDEF OS__CREATE_ENVIRON_METHODDEF
...
#endif
#ifdef NGROUPS_MAX
  if (PyModule_AddIntMacro(m, NGROUPS_MAX))
    return -1;
#endif
#ifdef TMP_MAX
  if (PyModule_AddIntMacro(m, TMP_MAX))
    return -1;
#endif
#ifdef WCONTINUED
  if (PyModule_AddIntMacro(m, WCONTINUED))
    return -1;
#endif
#ifdef WNOHANG
  if (PyModule_AddIntMacro(m, WNOHANG))
    return -1;
#endif
#ifdef WUNTRACED
  if (PyModule_AddIntMacro(m, WUNTRACED))
    return -1;
#endif
#ifdef O_RDONLY
...
#endif
#ifdef O_NDELAY
  if (PyModule_AddIntMacro(m, O_NDELAY))
    return -1;
#endif
#ifdef O_NONBLOCK
...
#endif
#ifdef O_BINARY
  if (PyModule_AddIntMacro(m, O_BINARY))
    return -1;
#endif
#ifdef O_TEXT
  if (PyModule_AddIntMacro(m, O_TEXT))
    return -1;
#endif
#ifdef O_XATTR
  if (PyModule_AddIntMacro(m, O_XATTR))
    return -1;
#endif
#ifdef O_LARGEFILE
  if (PyModule_AddIntMacro(m, O_LARGEFILE))
    return -1;
#endif
#ifndef __GNU__
#ifdef O_SHLOCK
  if (PyModule_AddIntMacro(m, O_SHLOCK))
    return -1;
#endif
#ifdef O_EXLOCK
  if (PyModule_AddIntMacro(m, O_EXLOCK))
    return -1;
#endif
#endif
#ifdef O_EXEC
  if (PyModule_AddIntMacro(m, O_EXEC))
    return -1;
#endif
#ifdef O_SEARCH
  if (PyModule_AddIntMacro(m, O_SEARCH))
    return -1;
#endif
#ifdef O_PATH
  if (PyModule_AddIntMacro(m, O_PATH))
    return -1;
#endif
#ifdef O_TTY_INIT
  if (PyModule_AddIntMacro(m, O_TTY_INIT))
    return -1;
#endif
#ifdef O_TMPFILE
  if (PyModule_AddIntMacro(m, O_TMPFILE))
    return -1;
#endif
#ifdef PRIO_PROCESS
  if (PyModule_AddIntMacro(m, PRIO_PROCESS))
    return -1;
#endif
#ifdef PRIO_PGRP
  if (PyModule_AddIntMacro(m, PRIO_PGRP))
    return -1;
#endif
#ifdef PRIO_USER
  if (PyModule_AddIntMacro(m, PRIO_USER))
    return -1;
#endif
#ifdef PRIO_DARWIN_THREAD
  if (PyModule_AddIntMacro(m, PRIO_DARWIN_THREAD))
    return -1;
#endif
#ifdef PRIO_DARWIN_PROCESS
  if (PyModule_AddIntMacro(m, PRIO_DARWIN_PROCESS))
    return -1;
#endif
#ifdef PRIO_DARWIN_BG
  if (PyModule_AddIntMacro(m, PRIO_DARWIN_BG))
    return -1;
#endif
#ifdef PRIO_DARWIN_NONUI
  if (PyModule_AddIntMacro(m, PRIO_DARWIN_NONUI))
    return -1;
#endif
#ifdef O_CLOEXEC
  if (PyModule_AddIntMacro(m, O_CLOEXEC))
    return -1;
#endif
#ifdef O_ACCMODE
...
#endif
#ifdef O_EVTONLY
  if (PyModule_AddIntMacro(m, O_EVTONLY))
    return -1;
#endif
#ifdef O_FSYNC
  if (PyModule_AddIntMacro(m, O_FSYNC))
    return -1;
#endif
#ifdef O_SYMLINK
  if (PyModule_AddIntMacro(m, O_SYMLINK))
    return -1;
#endif

#ifdef SEEK_HOLE
  if (PyModule_AddIntMacro(m, SEEK_HOLE))
    return -1;
#endif
#ifdef SEEK_DATA
  if (PyModule_AddIntMacro(m, SEEK_DATA))
    return -1;
#endif
...
/* MS Windows */
  ...
  if (PyModule_AddIntMacro(m, O_NOINHERIT))
    return -1;
#endif
#ifdef _O_SHORT_LIVED
  /* Optimize for short life (keep in memory). */
  /* MS forgot to define this one with a non-underscore form too. */
  if (PyModule_AddIntConstant(m, "O_SHORT_LIVED", _O_SHORT_LIVED))
    return -1;
#endif
#ifdef O_TEMPORARY
  /* Automatically delete when last handle is closed. */
  if (PyModule_AddIntMacro(m, O_TEMPORARY))
    return -1;
#endif
#ifdef O_RANDOM
  /* Optimize for random access. */
  if (PyModule_AddIntMacro(m, O_RANDOM))
    return -1;
#endif
#ifdef O_SEQUENTIAL
  /* Optimize for sequential access. */
  if (PyModule_AddIntMacro(m, O_SEQUENTIAL))
    return -1;
#endif
...
/* GNU extensions. */
  ...
  if (PyModule_AddIntMacro(m, O_ASYNC))
    return -1;
#endif
#ifdef O_DIRECT
  /* Direct disk access. */
  if (PyModule_AddIntMacro(m, O_DIRECT))
    return -1;
#endif
#ifdef O_DIRECTORY
  /* Must be a directory.      */
...
#ifdef O_NOFOLLOW
  /* Do not follow links.      */
  if (PyModule_AddIntMacro(m, O_NOFOLLOW))
    return -1;
#endif
#ifdef O_NOFOLLOW_ANY
  if (PyModule_AddIntMacro(m, O_NOFOLLOW_ANY))
    return -1;
#endif
#ifdef O_NOLINKS
  /* Fails if link count of the named file is greater than 1 */
  if (PyModule_AddIntMacro(m, O_NOLINKS))
    return -1;
#endif
#ifdef O_NOATIME
  /* Do not update the access time. */
  if (PyModule_AddIntMacro(m, O_NOATIME))
    return -1;
#endif

  /* These come from sysexits.h */
#ifdef EX_OK
  if (PyModule_AddIntMacro(m, EX_OK))
    return -1;
#endif /* EX_OK */
#ifdef EX_USAGE
  if (PyModule_AddIntMacro(m, EX_USAGE))
    return -1;
#endif /* EX_USAGE */
#ifdef EX_DATAERR
  if (PyModule_AddIntMacro(m, EX_DATAERR))
    return -1;
#endif /* EX_DATAERR */
#ifdef EX_NOINPUT
  if (PyModule_AddIntMacro(m, EX_NOINPUT))
    return -1;
#endif /* EX_NOINPUT */
#ifdef EX_NOUSER
  if (PyModule_AddIntMacro(m, EX_NOUSER))
    return -1;
#endif /* EX_NOUSER */
#ifdef EX_NOHOST
  if (PyModule_AddIntMacro(m, EX_NOHOST))
    return -1;
#endif /* EX_NOHOST */
#ifdef EX_UNAVAILABLE
  if (PyModule_AddIntMacro(m, EX_UNAVAILABLE))
    return -1;
#endif /* EX_UNAVAILABLE */
#ifdef EX_SOFTWARE
  if (PyModule_AddIntMacro(m, EX_SOFTWARE))
    return -1;
#endif /* EX_SOFTWARE */
#ifdef EX_OSERR
  if (PyModule_AddIntMacro(m, EX_OSERR))
    return -1;
#endif /* EX_OSERR */
#ifdef EX_OSFILE
  if (PyModule_AddIntMacro(m, EX_OSFILE))
    return -1;
#endif /* EX_OSFILE */
#ifdef EX_CANTCREAT
  if (PyModule_AddIntMacro(m, EX_CANTCREAT))
    return -1;
#endif /* EX_CANTCREAT */
#ifdef EX_IOERR
  if (PyModule_AddIntMacro(m, EX_IOERR))
    return -1;
#endif /* EX_IOERR */
#ifdef EX_TEMPFAIL
  if (PyModule_AddIntMacro(m, EX_TEMPFAIL))
    return -1;
#endif /* EX_TEMPFAIL */
#ifdef EX_PROTOCOL
  if (PyModule_AddIntMacro(m, EX_PROTOCOL))
    return -1;
#endif /* EX_PROTOCOL */
#ifdef EX_NOPERM
  if (PyModule_AddIntMacro(m, EX_NOPERM))
    return -1;
#endif /* EX_NOPERM */
#ifdef EX_CONFIG
  if (PyModule_AddIntMacro(m, EX_CONFIG))
    return -1;
#endif /* EX_CONFIG */
#ifdef EX_NOTFOUND
  if (PyModule_AddIntMacro(m, EX_NOTFOUND))
    return -1;
#endif /* EX_NOTFOUND */

  /* statvfs */
#ifdef ST_RDONLY
  if (PyModule_AddIntMacro(m, ST_RDONLY))
    return -1;
#endif /* ST_RDONLY */
#ifdef ST_NOSUID
  if (PyModule_AddIntMacro(m, ST_NOSUID))
    return -1;
#endif /* ST_NOSUID */

  /* GNU extensions */
#ifdef ST_NODEV
  if (PyModule_AddIntMacro(m, ST_NODEV))
    return -1;
#endif /* ST_NODEV */
#ifdef ST_NOEXEC
  if (PyModule_AddIntMacro(m, ST_NOEXEC))
    return -1;
#endif /* ST_NOEXEC */
#ifdef ST_SYNCHRONOUS
  if (PyModule_AddIntMacro(m, ST_SYNCHRONOUS))
    return -1;
#endif /* ST_SYNCHRONOUS */
#ifdef ST_MANDLOCK
  if (PyModule_AddIntMacro(m, ST_MANDLOCK))
    return -1;
#endif /* ST_MANDLOCK */
#ifdef ST_WRITE
  if (PyModule_AddIntMacro(m, ST_WRITE))
    return -1;
#endif /* ST_WRITE */
#ifdef ST_APPEND
  if (PyModule_AddIntMacro(m, ST_APPEND))
    return -1;
#endif /* ST_APPEND */
#ifdef ST_NOATIME
  if (PyModule_AddIntMacro(m, ST_NOATIME))
    return -1;
#endif /* ST_NOATIME */
#ifdef ST_NODIRATIME
  if (PyModule_AddIntMacro(m, ST_NODIRATIME))
    return -1;
#endif /* ST_NODIRATIME */
#ifdef ST_RELATIME
  if (PyModule_AddIntMacro(m, ST_RELATIME))
    return -1;
#endif /* ST_RELATIME */

  /* FreeBSD sendfile() constants */
#ifdef SF_NODISKIO
  if (PyModule_AddIntMacro(m, SF_NODISKIO))
    return -1;
#endif
  /* is obsolete since the 11.x release */
#ifdef SF_MNOWAIT
  if (PyModule_AddIntMacro(m, SF_MNOWAIT))
    return -1;
#endif
#ifdef SF_SYNC
  if (PyModule_AddIntMacro(m, SF_SYNC))
    return -1;
#endif
#ifdef SF_NOCACHE
  if (PyModule_AddIntMacro(m, SF_NOCACHE))
    return -1;
#endif

#ifdef TFD_NONBLOCK
  if (PyModule_AddIntMacro(m, TFD_NONBLOCK))
    return -1;
#endif
#ifdef TFD_CLOEXEC
  if (PyModule_AddIntMacro(m, TFD_CLOEXEC))
    return -1;
#endif
#ifdef TFD_TIMER_ABSTIME
  if (PyModule_AddIntMacro(m, TFD_TIMER_ABSTIME))
    return -1;
#endif
#ifdef TFD_TIMER_CANCEL_ON_SET
  if (PyModule_AddIntMacro(m, TFD_TIMER_CANCEL_ON_SET))
    return -1;
#endif

  /* constants for posix_fadvise */
#ifdef POSIX_FADV_NORMAL
  if (PyModule_AddIntMacro(m, POSIX_FADV_NORMAL))
    return -1;
#endif
#ifdef POSIX_FADV_SEQUENTIAL
  if (PyModule_AddIntMacro(m, POSIX_FADV_SEQUENTIAL))
    return -1;
#endif
#ifdef POSIX_FADV_RANDOM
  if (PyModule_AddIntMacro(m, POSIX_FADV_RANDOM))
    return -1;
#endif
#ifdef POSIX_FADV_NOREUSE
  if (PyModule_AddIntMacro(m, POSIX_FADV_NOREUSE))
    return -1;
#endif
#ifdef POSIX_FADV_WILLNEED
  if (PyModule_AddIntMacro(m, POSIX_FADV_WILLNEED))
    return -1;
#endif
#ifdef POSIX_FADV_DONTNEED
  if (PyModule_AddIntMacro(m, POSIX_FADV_DONTNEED))
    return -1;
#endif

  /* constants for waitid */
#if defined(HAVE_SYS_WAIT_H) && defined(HAVE_WAITID)
  if (PyModule_AddIntMacro(m, P_PID))
    return -1;
  if (PyModule_AddIntMacro(m, P_PGID))
    return -1;
  if (PyModule_AddIntMacro(m, P_ALL))
    return -1;
#ifdef P_PIDFD
  if (PyModule_AddIntMacro(m, P_PIDFD))
    return -1;
#endif
#ifdef PIDFD_NONBLOCK
  if (PyModule_AddIntMacro(m, PIDFD_NONBLOCK))
    return -1;
#endif
#endif
#ifdef WEXITED
  if (PyModule_AddIntMacro(m, WEXITED))
    return -1;
#endif
#ifdef WNOWAIT
  if (PyModule_AddIntMacro(m, WNOWAIT))
    return -1;
#endif
#ifdef WSTOPPED
  if (PyModule_AddIntMacro(m, WSTOPPED))
    return -1;
#endif
#ifdef CLD_EXITED
  if (PyModule_AddIntMacro(m, CLD_EXITED))
    return -1;
#endif
#ifdef CLD_KILLED
  if (PyModule_AddIntMacro(m, CLD_KILLED))
    return -1;
#endif
#ifdef CLD_DUMPED
  if (PyModule_AddIntMacro(m, CLD_DUMPED))
    return -1;
#endif
#ifdef CLD_TRAPPED
  if (PyModule_AddIntMacro(m, CLD_TRAPPED))
    return -1;
#endif
#ifdef CLD_STOPPED
  if (PyModule_AddIntMacro(m, CLD_STOPPED))
    return -1;
#endif
#ifdef CLD_CONTINUED
  if (PyModule_AddIntMacro(m, CLD_CONTINUED))
    return -1;
#endif
...
#ifdef RWF_DSYNC
  if (PyModule_AddIntConstant(m, "RWF_DSYNC", RWF_DSYNC))
    return -1;
#endif
#ifdef RWF_HIPRI
  if (PyModule_AddIntConstant(m, "RWF_HIPRI", RWF_HIPRI))
    return -1;
#endif
#ifdef RWF_SYNC
  if (PyModule_AddIntConstant(m, "RWF_SYNC", RWF_SYNC))
    return -1;
#endif
#ifdef RWF_NOWAIT
  if (PyModule_AddIntConstant(m, "RWF_NOWAIT", RWF_NOWAIT))
    return -1;
#endif
#ifdef RWF_DONTCACHE
  if (PyModule_AddIntConstant(m, "RWF_DONTCACHE", RWF_DONTCACHE))
    return -1;
#endif
#ifdef RWF_ATOMIC
  if (PyModule_AddIntConstant(m, "RWF_ATOMIC", RWF_ATOMIC))
    return -1;
#endif
#ifdef RWF_APPEND
  if (PyModule_AddIntConstant(m, "RWF_APPEND", RWF_APPEND))
    return -1;
#endif

/* constants for splice */
#if defined(HAVE_SPLICE) && defined(__linux__)
  if (PyModule_AddIntConstant(m, "SPLICE_F_MOVE", SPLICE_F_MOVE))
    return -1;
  if (PyModule_AddIntConstant(m, "SPLICE_F_NONBLOCK", SPLICE_F_NONBLOCK))
    return -1;
  if (PyModule_AddIntConstant(m, "SPLICE_F_MORE", SPLICE_F_MORE))
    return -1;
#endif

/* constants for posix_spawn */
#ifdef HAVE_POSIX_SPAWN
  if (PyModule_AddIntConstant(m, "POSIX_SPAWN_OPEN", POSIX_SPAWN_OPEN))
    return -1;
  if (PyModule_AddIntConstant(m, "POSIX_SPAWN_CLOSE", POSIX_SPAWN_CLOSE))
    return -1;
  if (PyModule_AddIntConstant(m, "POSIX_SPAWN_DUP2", POSIX_SPAWN_DUP2))
    return -1;
#ifdef HAVE_POSIX_SPAWN_FILE_ACTIONS_ADDCLOSEFROM_NP
  if (PyModule_AddIntMacro(m, POSIX_SPAWN_CLOSEFROM))
    return -1;
#endif
#endif

#if defined(HAVE_SPAWNV) || defined(HAVE_RTPSPAWN)
  if (PyModule_AddIntConstant(m, "P_WAIT", _P_WAIT))
    return -1;
  if (PyModule_AddIntConstant(m, "P_NOWAIT", _P_NOWAIT))
    return -1;
  if (PyModule_AddIntConstant(m, "P_NOWAITO", _P_NOWAITO))
    return -1;
#endif
#ifdef HAVE_SPAWNV
  if (PyModule_AddIntConstant(m, "P_OVERLAY", _OLD_P_OVERLAY))
    return -1;
  if (PyModule_AddIntConstant(m, "P_DETACH", _P_DETACH))
    return -1;
#endif

#ifdef HAVE_SCHED_H
#ifdef SCHED_OTHER
  if (PyModule_AddIntMacro(m, SCHED_OTHER))
    return -1;
#endif
#ifdef SCHED_DEADLINE
  if (PyModule_AddIntMacro(m, SCHED_DEADLINE))
    return -1;
#endif
#ifdef SCHED_FIFO
  if (PyModule_AddIntMacro(m, SCHED_FIFO))
    return -1;
#endif
#ifdef SCHED_NORMAL
  if (PyModule_AddIntMacro(m, SCHED_NORMAL))
    return -1;
#endif
#ifdef SCHED_RR
  if (PyModule_AddIntMacro(m, SCHED_RR))
    return -1;
#endif
#ifdef SCHED_SPORADIC
  if (PyModule_AddIntMacro(m, SCHED_SPORADIC))
    return -1;
#endif
#ifdef SCHED_BATCH
  if (PyModule_AddIntMacro(m, SCHED_BATCH))
    return -1;
#endif
#ifdef SCHED_IDLE
  if (PyModule_AddIntMacro(m, SCHED_IDLE))
    return -1;
#endif
#ifdef SCHED_RESET_ON_FORK
  if (PyModule_AddIntMacro(m, SCHED_RESET_ON_FORK))
    return -1;
#endif
#ifdef SCHED_SYS
  if (PyModule_AddIntMacro(m, SCHED_SYS))
    return -1;
#endif
#ifdef SCHED_IA
  if (PyModule_AddIntMacro(m, SCHED_IA))
    return -1;
#endif
#ifdef SCHED_FSS
  if (PyModule_AddIntMacro(m, SCHED_FSS))
    return -1;
#endif
#ifdef SCHED_FX
  if (PyModule_AddIntConstant(m, "SCHED_FX", SCHED_FSS))
    return -1;
#endif
...
#ifdef CLONE_FS
  if (PyModule_AddIntMacro(m, CLONE_FS))
    return -1;
#endif
#ifdef CLONE_FILES
  if (PyModule_AddIntMacro(m, CLONE_FILES))
    return -1;
#endif
#ifdef CLONE_NEWNS
  if (PyModule_AddIntMacro(m, CLONE_NEWNS))
    return -1;
#endif
#ifdef CLONE_NEWCGROUP
  if (PyModule_AddIntMacro(m, CLONE_NEWCGROUP))
    return -1;
#endif
#ifdef CLONE_NEWUTS
  if (PyModule_AddIntMacro(m, CLONE_NEWUTS))
    return -1;
#endif
#ifdef CLONE_NEWIPC
  if (PyModule_AddIntMacro(m, CLONE_NEWIPC))
    return -1;
#endif
#ifdef CLONE_NEWUSER
  if (PyModule_AddIntMacro(m, CLONE_NEWUSER))
    return -1;
#endif
#ifdef CLONE_NEWPID
  if (PyModule_AddIntMacro(m, CLONE_NEWPID))
    return -1;
#endif
#ifdef CLONE_NEWNET
  if (PyModule_AddIntMacro(m, CLONE_NEWNET))
    return -1;
#endif
#ifdef CLONE_NEWTIME
  if (PyModule_AddIntMacro(m, CLONE_NEWTIME))
    return -1;
#endif
#ifdef CLONE_SYSVSEM
  if (PyModule_AddIntMacro(m, CLONE_SYSVSEM))
    return -1;
#endif
#ifdef CLONE_THREAD
  if (PyModule_AddIntMacro(m, CLONE_THREAD))
    return -1;
#endif
#ifdef CLONE_SIGHAND
  if (PyModule_AddIntMacro(m, CLONE_SIGHAND))
    return -1;
#endif
#ifdef CLONE_VM
  if (PyModule_AddIntMacro(m, CLONE_VM))
    return -1;
#endif
#endif
...
#ifdef USE_XATTRS
  if (PyModule_AddIntMacro(m, XATTR_CREATE))
    return -1;
  if (PyModule_AddIntMacro(m, XATTR_REPLACE))
    return -1;
  if (PyModule_AddIntMacro(m, XATTR_SIZE_MAX))
    return -1;
#endif

#if HAVE_DECL_RTLD_LAZY
  if (PyModule_AddIntMacro(m, RTLD_LAZY))
    return -1;
#endif
#if HAVE_DECL_RTLD_NOW
  if (PyModule_AddIntMacro(m, RTLD_NOW))
    return -1;
#endif
#if HAVE_DECL_RTLD_GLOBAL
  if (PyModule_AddIntMacro(m, RTLD_GLOBAL))
    return -1;
#endif
#if HAVE_DECL_RTLD_LOCAL
  if (PyModule_AddIntMacro(m, RTLD_LOCAL))
    return -1;
#endif
#if HAVE_DECL_RTLD_NODELETE
  if (PyModule_AddIntMacro(m, RTLD_NODELETE))
    return -1;
#endif
#if HAVE_DECL_RTLD_NOLOAD
  if (PyModule_AddIntMacro(m, RTLD_NOLOAD))
    return -1;
#endif
#if HAVE_DECL_RTLD_DEEPBIND
  if (PyModule_AddIntMacro(m, RTLD_DEEPBIND))
    return -1;
#endif
#if HAVE_DECL_RTLD_MEMBER
  if (PyModule_AddIntMacro(m, RTLD_MEMBER))
    return -1;
#endif

#ifdef HAVE_GETRANDOM_SYSCALL
  if (PyModule_AddIntMacro(m, GRND_RANDOM))
    return -1;
  if (PyModule_AddIntMacro(m, GRND_NONBLOCK))
    return -1;
#endif
#ifdef HAVE_MEMFD_CREATE
  if (PyModule_AddIntMacro(m, MFD_CLOEXEC))
    return -1;
  if (PyModule_AddIntMacro(m, MFD_ALLOW_SEALING))
    return -1;
#ifdef MFD_HUGETLB
  if (PyModule_AddIntMacro(m, MFD_HUGETLB))
    return -1;
#endif
#ifdef MFD_HUGE_SHIFT
  if (PyModule_AddIntMacro(m, MFD_HUGE_SHIFT))
    return -1;
#endif
#ifdef MFD_HUGE_MASK
  if (PyModule_AddIntMacro(m, MFD_HUGE_MASK))
    return -1;
#endif
#ifdef MFD_HUGE_64KB
  if (PyModule_AddIntMacro(m, MFD_HUGE_64KB))
    return -1;
#endif
#ifdef MFD_HUGE_512KB
  if (PyModule_AddIntMacro(m, MFD_HUGE_512KB))
    return -1;
#endif
#ifdef MFD_HUGE_1MB
  if (PyModule_AddIntMacro(m, MFD_HUGE_1MB))
    return -1;
#endif
#ifdef MFD_HUGE_2MB
  if (PyModule_AddIntMacro(m, MFD_HUGE_2MB))
    return -1;
#endif
#ifdef MFD_HUGE_8MB
  if (PyModule_AddIntMacro(m, MFD_HUGE_8MB))
    return -1;
#endif
#ifdef MFD_HUGE_16MB
  if (PyModule_AddIntMacro(m, MFD_HUGE_16MB))
    return -1;
#endif
#ifdef MFD_HUGE_32MB
  if (PyModule_AddIntMacro(m, MFD_HUGE_32MB))
    return -1;
#endif
#ifdef MFD_HUGE_256MB
  if (PyModule_AddIntMacro(m, MFD_HUGE_256MB))
    return -1;
#endif
#ifdef MFD_HUGE_512MB
  if (PyModule_AddIntMacro(m, MFD_HUGE_512MB))
    return -1;
#endif
#ifdef MFD_HUGE_1GB
  if (PyModule_AddIntMacro(m, MFD_HUGE_1GB))
    return -1;
#endif
#ifdef MFD_HUGE_2GB
  if (PyModule_AddIntMacro(m, MFD_HUGE_2GB))
    return -1;
#endif
#ifdef MFD_HUGE_16GB
  if (PyModule_AddIntMacro(m, MFD_HUGE_16GB))
    return -1;
#endif
#endif /* HAVE_MEMFD_CREATE */

#if defined(HAVE_EVENTFD) && defined(EFD_CLOEXEC)
  if (PyModule_AddIntMacro(m, EFD_CLOEXEC))
    return -1;
#ifdef EFD_NONBLOCK
  if (PyModule_AddIntMacro(m, EFD_NONBLOCK))
    return -1;
#endif
#ifdef EFD_SEMAPHORE
  if (PyModule_AddIntMacro(m, EFD_SEMAPHORE))
    return -1;
#endif
#endif /* HAVE_EVENTFD && EFD_CLOEXEC */

#ifdef NODEV
  if (PyModule_Add(m, "NODEV", _PyLong_FromDev(NODEV)))
    return -1;
#endif

#ifdef AT_NO_AUTOMOUNT
  if (PyModule_AddIntMacro(m, AT_NO_AUTOMOUNT))
    return -1;
#endif

#ifdef HAVE_STATX
  if (PyModule_AddIntMacro(m, STATX_TYPE))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_MODE))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_NLINK))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_UID))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_GID))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_ATIME))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_MTIME))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_CTIME))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_INO))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_SIZE))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_BLOCKS))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_BASIC_STATS))
    return -1;
  if (PyModule_AddIntMacro(m, STATX_BTIME))
    return -1;
#ifdef STATX_MNT_ID
  if (PyModule_AddIntMacro(m, STATX_MNT_ID))
    return -1;
#endif
#ifdef STATX_DIOALIGN
  if (PyModule_AddIntMacro(m, STATX_DIOALIGN))
    return -1;
#endif
#ifdef STATX_MNT_ID_UNIQUE
  if (PyModule_AddIntMacro(m, STATX_MNT_ID_UNIQUE))
    return -1;
#endif
#ifdef STATX_SUBVOL
  if (PyModule_AddIntMacro(m, STATX_SUBVOL))
    return -1;
#endif
#ifdef STATX_WRITE_ATOMIC
  if (PyModule_AddIntMacro(m, STATX_WRITE_ATOMIC))
    return -1;
#endif
#ifdef STATX_DIO_READ_ALIGN
  if (PyModule_AddIntMacro(m, STATX_DIO_READ_ALIGN))
    return -1;
#endif
  /* STATX_ALL intentionally omitted because it is deprecated */
  if (PyModule_AddIntMacro(m, AT_STATX_SYNC_AS_STAT))
    return -1;
  if (PyModule_AddIntMacro(m, AT_STATX_FORCE_SYNC))
    return -1;
  if (PyModule_AddIntMacro(m, AT_STATX_DONT_SYNC))
    return -1;
  /* STATX_ATTR_* constants are in the stat module */
#endif /* HAVE_STATX */

#if defined(__APPLE__)
  if (PyModule_AddIntConstant(m, "_COPYFILE_DATA", COPYFILE_DATA))
    return -1;
  if (PyModule_AddIntConstant(m, "_COPYFILE_STAT", COPYFILE_STAT))
    return -1;
  if (PyModule_AddIntConstant(m, "_COPYFILE_ACL", COPYFILE_ACL))
    return -1;
  if (PyModule_AddIntConstant(m, "_COPYFILE_XATTR", COPYFILE_XATTR))
    return -1;
#endif

#ifdef MS_WINDOWS
  if (PyModule_AddIntConstant(m, "_LOAD_LIBRARY_SEARCH_DEFAULT_DIRS",
                              LOAD_LIBRARY_SEARCH_DEFAULT_DIRS))
    return -1;
  if (PyModule_AddIntConstant(m, "_LOAD_LIBRARY_SEARCH_APPLICATION_DIR",
                              LOAD_LIBRARY_SEARCH_APPLICATION_DIR))
    return -1;
  if (PyModule_AddIntConstant(m, "_LOAD_LIBRARY_SEARCH_SYSTEM32",
                              LOAD_LIBRARY_SEARCH_SYSTEM32))
    return -1;
  if (PyModule_AddIntConstant(m, "_LOAD_LIBRARY_SEARCH_USER_DIRS",
                              LOAD_LIBRARY_SEARCH_USER_DIRS))
    return -1;
  if (PyModule_AddIntConstant(m, "_LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR",
...
static int posixmodule_exec(PyObject *m) {
  ...
  if (statx == NULL) {
    ...
    state->StatxResultType =
        PyType_FromModuleAndSpec(m, &pystatx_result_spec, NULL);
...
#if defined(HAVE_WAITID)
  state->WaitidResultType =
      (PyObject *)PyStructSequence_NewType(&waitid_result_desc);
  ...
  state->StatResultType =
      (PyObject *)PyStructSequence_NewType(&stat_result_desc);
  ...
  state->StatVFSResultType =
      (PyObject *)PyStructSequence_NewType(&statvfs_result_desc);
...
#if defined(HAVE_SCHED_SETPARAM) || defined(HAVE_SCHED_SETSCHEDULER) ||        \
    defined(POSIX_SPAWN_SETSCHEDULER) || defined(POSIX_SPAWN_SETSCHEDPARAM)
  state->SchedParamType =
      (PyObject *)PyStructSequence_NewType(&sched_param_desc);
  ...
  state->TerminalSizeType =
      (PyObject *)PyStructSequence_NewType(&TerminalSize_desc);
  ...
  PyObject *ScandirIteratorType =
      PyType_FromModuleAndSpec(m, &ScandirIteratorType_spec, NULL);
  ...
  state->TimesResultType =
      (PyObject *)PyStructSequence_NewType(&times_result_desc);
  if (PyModule_AddObjectRef(m, "times_result", state->TimesResultType) < 0) {
    return -1;
  }

  state->UnameResultType =
      (PyObject *)PyStructSequence_NewType(&uname_result_desc);
  ...
  for (const struct have_function *trace = have_functions; trace->label;
       trace++) {
    PyObject *unicode;
    if (trace->probe && !trace->probe())
      continue;
...
static struct PyModuleDef posixmodule = {
    PyModuleDef_HEAD_INIT,         .m_name = MODNAME,
    .m_doc = posix__doc__,         .m_size = sizeof(_posixstate),
    .m_methods = posix_methods,    .m_slots = posixmodile_slots,
    .m_traverse = _posix_traverse, .m_clear = _posix_clear,
    .m_free = _posix_free,
};

PyMODINIT_FUNC INITFUNC(void) { return PyModuleDef_Init(&posixmodule); }
