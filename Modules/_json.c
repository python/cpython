...
/* Convert the key to the string used in the output.  Return 1 and set
   *pkeystr on success, 0 if the key is skipped, and -1 on error. */
static int
encoder_coerce_key(PyEncoderObject *s, PyObject *key, PyObject **pkeystr)
{
    PyObject *keystr;
...
/* Return a new list of (key string, value) pairs, with the keys converted
   and the skipped keys omitted. */
static PyObject *
encoder_coerce_items(PyEncoderObject *s, PyObject *items)
{
    PyObject *coerced = PyList_New(0);
    if (coerced == NULL) {
        return NULL;
    }
    for (Py_ssize_t i = 0; i < PyList_GET_SIZE(items); i++) {
        PyObject *item = Py_NewRef(PyList_GET_ITEM(items, i));
        ...
        PyObject *keystr;
        int res = encoder_coerce_key(s, PyTuple_GET_ITEM(item, 0), &keystr);
        if (res <= 0) {
            Py_DECREF(item);
            if (res < 0) {
                goto bail;
            }
            continue;  /* the key is skipped */
        }
        PyObject *pair = PyTuple_Pack(2, keystr, PyTuple_GET_ITEM(item, 1));
        Py_DECREF(keystr);
        Py_DECREF(item);
        if (pair == NULL) {
            goto bail;
        }
        int rv = PyList_Append(coerced, pair);
        Py_DECREF(pair);
        if (rv < 0) {
            goto bail;
        }
                         ...
                         PyObject *item_separator)
{
    ...
    rv = encoder_coerce_key(s, key, &keystr);
    if (rv <= 0) {
        return rv;
    }
                       ...
                       Py_ssize_t indent_level, PyObject *indent_cache)
{
    ...
    if (s->sort_keys || !PyAnyDict_CheckExact(dct)) {
        ...
        if (s->sort_keys) {
            /* Keys are converted to strings before sorting, so that
               a dict with mixed key types can be sorted, and the keys
               which are skipped do not participate in the sorting. */
            Py_SETREF(items, encoder_coerce_items(s, items));
