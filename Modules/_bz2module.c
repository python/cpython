...
compress(BZ2Compressor *c, char *data, size_t len, int action)
{
    ...
    if (OBW(InitAndGrow)(&writer, -1, &c->bzs.next_out, &c->bzs.avail_out) < 0) {
        goto error;
    }
    c->bzs.next_in = data;
    c->bzs.avail_in = 0;
    ...
    for (;;) {
        ...
        if (c->bzs.avail_out == 0) {
            if (OBW(Grow)(&writer, &c->bzs.next_out, &c->bzs.avail_out) < 0) {
                goto error;
            }
        }
    ...
    result = OBW(Finish)(&writer, c->bzs.avail_out);
    if (result == NULL) {
        goto error;
    }
    return result;
...
decompress_buf(BZ2Decompressor *d, Py_ssize_t max_length)
{
    ...
    if (OBW(InitAndGrow)(&writer, max_length, &bzs->next_out, &bzs->avail_out) < 0) {
        goto error;
    }
    ...
    for (;;) {
        ...
        if (bzret == BZ_STREAM_END) {
            ...
            if (OBW(GetDataSize)(&writer, bzs->avail_out) == max_length)
                break;
            if (OBW(Grow)(&writer, &bzs->next_out, &bzs->avail_out) < 0) {
                goto error;
            }
        }
    }

    result = OBW(Finish)(&writer, bzs->avail_out);
    if (result == NULL) {
        goto error;
    }
    return result;
