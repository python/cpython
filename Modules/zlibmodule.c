...
/*[clinic end generated code: output=c4d2cfb4b0d7350b input=235497e482d40986]*/
{
    ...
    if (self->gz.name != NULL) {
        return_value->gz.name = (Bytef *)_PyMem_Strdup((char *)self->gz.name);
        if (return_value->gz.name == NULL) {
            PyErr_SetNone(PyExc_MemoryError);
            goto error;
        }
    }
...
"1 if decompressing a gzip archive and the header is fully read, "
"-1 if the header is partially read, otherwise 0.");
...
static PyMemberDef ZlibDecompressor_members[] = {
    ...
    {"gz_header_mtime", Py_T_ULONG, offsetof(ZlibDecompressor, gz.time), Py_READONLY,
     ZlibDecompressor_gz_header_mtime_doc},
    {"gz_header_done", Py_T_INT, offsetof(ZlibDecompressor, gz.done), Py_READONLY,
