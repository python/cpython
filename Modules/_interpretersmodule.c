...
static int
_make_call(PyThreadState *tstate, struct interp_call *call)
{
...
_run_in_interpreter(PyThreadState *tstate, PyInterpreterState *interp,
                     ...
                     PyObject *shareables, PyObject **p_excinfo)
{
    assert(!_PyErr_Occurred(tstate));
    ...
    if (script != NULL) {
        // Run the script.
        assert(call == NULL);
        res = _run_script(script, session.main_ns);
    }
    else {
        assert(call != NULL);
        res = _make_call(session.init_tstate, call);
    }
...
#define FUNCNAME MODULE_NAME_STR ".exec"
    ...
    int res = _run_in_interpreter(
                    tstate, interp, &xidata, NULL, shared, &excinfo);
    _PyXIData_Release(&xidata);
    if (res < 0) {
...
#define FUNCNAME MODULE_NAME_STR ".call"
    ...
    PyObject *excinfo = NULL;
    if (_run_in_interpreter(tstate, interp, NULL, &call, NULL, &excinfo) < 0) {
        if (excinfo == NULL) {
            assert(_PyErr_Occurred(tstate));
            goto finally;
        }
        assert(!_PyErr_Occurred(tstate));
        assert(call.result == NULL);
        res_and_exc = Py_BuildValue("OO", Py_None, excinfo);
        Py_DECREF(excinfo);
    }
    else {
        assert(excinfo == NULL);
