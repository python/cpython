...
void
_PyGC_Initialize(void)
{
    _PyRuntime.gc.enabled = 1; /* automatic collection enabled? */
    ...
    for (int i = 0; i < NUM_GENERATIONS; i++) {
        _PyRuntime.gc.generations[i] = generations[i];
    };
    _PyRuntime.gc.generation0 = GEN_HEAD(0);
}
