...
{
    static char *keywords[] = {"string", "filename", "mode", "oldparser", "vmparser", "ast", NULL};
    char *the_string;
    char *filename = "<string>";
    char *mode_str = "exec";
    int oldparser = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|ssppp", keywords,
            &the_string, &filename, &mode_str, &oldparser, &vmparser, &ast)) {
        return NULL;
    }
