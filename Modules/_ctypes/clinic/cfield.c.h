...
PyCField_new(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    static struct {
        ...
        .ob_item = { &_Py_ID(name), &_Py_ID(type), &_Py_ID(size), &_Py_ID(offset), &_Py_ID(index), &_Py_ID(bit_size), &_Py_ID(swapped_bytes), &_Py_ID(_ms), &_Py_ID(pack), &_Py_ID(padding), },
    };
    #undef NUM_KEYWORDS
    ...
    static const char * const _keywords[] = {"name", "type", "size", "offset", "index", "bit_size", "swapped_bytes", "_ms", "pack", "padding", NULL};
    ...
    {
        Py_ssize_t ival = -1;
        PyObject *iobj = _PyNumber_Index(fastargs[9]);
        if (iobj != NULL) {
            ival = PyLong_AsSsize_t(iobj);
            Py_DECREF(iobj);
        }
...
skip_optional_pos:
    return_value = PyCField_new_impl(type, name, proto, size, offset, index, bit_size_obj, swapped_bytes, _ms, pack_obj, padding);

exit:
    return return_value;
}
/*[clinic end generated code: output=d9326c74d88bf23f input=a9049054013a1b77]*/
