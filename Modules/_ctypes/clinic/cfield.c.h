...
PyCField_new(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    static struct {
        ...
        .ob_item = { &_Py_ID(name), &_Py_ID(type), &_Py_ID(size), &_Py_ID(offset), &_Py_ID(bit_size), &_Py_ID(swapped_bytes), &_Py_ID(_ms), },
    };
    #undef NUM_KEYWORDS
    ...
    fastargs = _PyArg_UnpackKeywords(_PyTuple_CAST(args)->ob_item, nargs, kwargs, NULL, &_parser, 4, 7, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
...
skip_optional_pos:
    return_value = PyCField_new_impl(type, name, proto, size, offset, bit_size_obj, swapped_bytes, _ms);

exit:
    return return_value;
}
/*[clinic end generated code: output=c391d6260d1a8db0 input=a9049054013a1b77]*/
