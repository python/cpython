...
PyCField_new(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    static struct {
        ...
        .ob_item = { &_Py_ID(name), &_Py_ID(type), &_Py_ID(size), &_Py_ID(offset), &_Py_ID(index), &_Py_ID(bit_size), &_Py_ID(swapped_bytes), &_Py_ID(_ms), &_Py_ID(pack), },
    };
    #undef NUM_KEYWORDS
    ...
    static const char * const _keywords[] = {"name", "type", "size", "offset", "index", "bit_size", "swapped_bytes", "_ms", "pack", NULL};
    ...
    fastargs = _PyArg_UnpackKeywords(_PyTuple_CAST(args)->ob_item, nargs, kwargs, NULL, &_parser, 5, 9, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    ...
    pack_obj = fastargs[8];
skip_optional_pos:
    return_value = PyCField_new_impl(type, name, proto, size, offset, index, bit_size_obj, swapped_bytes, _ms, pack_obj);

exit:
    return return_value;
}
/*[clinic end generated code: output=9576e54b11a846eb input=a9049054013a1b77]*/
