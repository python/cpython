...
*/
int
_copy_array_elements(ctypes_state *st, int arrays_seen, Py_ssize_t total_size,
                     PyObject *layout_fields, Py_ssize_t ffi_ofs,
                     StgInfo *baseinfo, StgInfo *stginfo)
{
...
PyCStructUnionType_update_stginfo(PyObject *type, PyObject *fields, int isStruct)
{
    ...
    if (_copy_array_elements(st, arrays_seen, total_size, layout_fields,
                             ffi_ofs, baseinfo, stginfo) < 0) {
        goto error;
    }
