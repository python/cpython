...
#define MAX_ELEMENTS 16
    ...
    if (arrays_seen && (size <= MAX_ELEMENTS)) {
         ...
         * for this struct/union. The second block takes up num_ffi_type_pointers
         * - len, so the sum of these is ffi_ofs + 1 + num_ffi_type_pointers as
         * allocated above. The last bit is the num_ffi_types structs.
         */
        ...
        if (num_ffi_types > 0) {
            ...
            for (i = 0; i < num_ffi_types; i++) {
                structs[i].type = FFI_TYPE_STRUCT;
            }
        }
