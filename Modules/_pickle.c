...
get_dotted_path(PyObject *obj, PyObject *name)
{
    ...
    dotted_path = PyUnicode_Split(name, PyUnicode_FromOrdinal('.'), -1);
    if (dotted_path == NULL)
        return NULL;
...
save_reduce(PicklerObject *self, PyObject *args, PyObject *obj)
{
    ...
    if (self->proto >= 2) {
        ...
        if (name != NULL && PyUnicode_Check(name)) {
            use_newobj_ex = _PyUnicode_EQ(name, _Py_ID(__newobj_ex__));
            if (!use_newobj_ex) {
                use_newobj = _PyUnicode_EQ(name, _Py_ID(__newobj__));
            }
        }
        Py_XDECREF(name);
    }
...
instantiate(PyObject *cls, PyObject *args)
{
    ...
    if (!PyTuple_GET_SIZE(args) && PyType_Check(cls)) {
        ...
        if (func == NULL) {
            return _PyObject_CallMethodOneArg(cls, _Py_ID(__new__), cls);
        }
        Py_DECREF(func);
    }
