...
_io__BufferedIOBase_readinto(PyObject *self, PyObject *arg)
{
    ...
    if (PyObject_GetBuffer(arg, &buffer, PyBUF_WRITABLE) < 0) {
        PyErr_Clear();
        _PyArg_BadArgument("readinto", 0, "argument", "read-write bytes-like object", arg);
        goto exit;
    }
    if (!PyBuffer_IsContiguous(&buffer, 'C')) {
        _PyArg_BadArgument("readinto", 0, "argument", "contiguous buffer", arg);
        goto exit;
    }
...
_io__BufferedIOBase_readinto1(PyObject *self, PyObject *arg)
{
    ...
    if (PyObject_GetBuffer(arg, &buffer, PyBUF_WRITABLE) < 0) {
        PyErr_Clear();
        _PyArg_BadArgument("readinto1", 0, "argument", "read-write bytes-like object", arg);
        goto exit;
    }
    if (!PyBuffer_IsContiguous(&buffer, 'C')) {
        _PyArg_BadArgument("readinto1", 0, "argument", "contiguous buffer", arg);
        goto exit;
    }
...
_io_BufferedWriter_write(buffered *self, PyObject *arg)
{
    ...
    if (!PyBuffer_IsContiguous(&buffer, 'C')) {
        _PyArg_BadArgument("write", 0, "argument", "contiguous buffer", arg);
        goto exit;
    }
...
/*[clinic end generated code: output=c5bdcc46d36124ff input=a9049054013a1b77]*/
