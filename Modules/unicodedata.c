...
{
    Py_ssize_t i, j;
    Py_UCS4 code, previous;
    unsigned char combining;

    for (i = start + 1; i < end; i++) {
        code = PyUnicode_READ(kind, data, i);
        combining = _getrecord_ex(code)->combining;
        j = i;

        while (j > start) {
            previous = PyUnicode_READ(kind, data, j - 1);
                                 ...
                                 Py_UCS4 *sortbuf)
{
    ...
    Py_ssize_t i, count, run_length, total;
    Py_UCS4 code;
    unsigned char combining;
    ...
    for (i = 0; i < Py_ARRAY_LENGTH(counts); i++) {
        count = counts[i];
        counts[i] = total;
        total += count;
    }
...
nfd_nfkd(PyObject *self, PyObject *input, int k)
{
    ...
    while (i < o) {
        ...
        needs_sort = 0;
        ch = PyUnicode_READ(result_kind, result_data, i);
