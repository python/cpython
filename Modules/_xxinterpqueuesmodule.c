...
clear_module_state(module_state *state)
{
    ...
    if (state->QueueIDType != NULL) {
        (void)_PyCrossInterpreterData_UnregisterClass(state->QueueIDType);
    }
    Py_CLEAR(state->QueueIDType);
...
channel_id_converter(PyObject *arg, void *ptr)
{
    ...
    assert(state != NULL);
    if (PyObject_TypeCheck(arg, state->QueueIDType)) {
        cid = ((queueid *)arg)->cid;
        end = ((queueid *)arg)->end;
    }
...
static int
newqueueid(PyTypeObject *cls, int64_t cid, int end, _channels *channels,
             int force, int resolve, queueid **res)
{
    *res = NULL;

    queueid *self = PyObject_New(queueid, cls);
    if (self == NULL) {
        return -1;
    }
    self->cid = cid;
...
static PyObject *
_queueid_new(PyObject *mod, PyTypeObject *cls,
               PyObject *args, PyObject *kwds)
{
    ...
    int err = newqueueid(cls, cid, end, _global_channels(),
                           force, resolve,
                           (queueid **)&cidobj);
...
static void
queueid_dealloc(PyObject *self)
{
    int64_t cid = ((queueid *)self)->cid;
    _channels *channels = ((queueid *)self)->channels;
...
static PyObject *
queueid_repr(PyObject *self)
{
    PyTypeObject *type = Py_TYPE(self);
    const char *name = _PyType_Name(type);

    queueid *cidobj = (queueid *)self;
    const char *fmt;
...
static PyObject *
queueid_richcompare(PyObject *self, PyObject *other, int op)
{
    PyObject *res = NULL;
    ...
    if (!PyObject_TypeCheck(self, state->QueueIDType)) {
    ...
    queueid *cidobj = (queueid *)self;
    int equal;
    if (PyObject_TypeCheck(other, state->QueueIDType)) {
        queueid *othercidobj = (queueid *)other;
...
static PyObject *
_queueid_from_xid(_PyCrossInterpreterData *data)
{
    struct _queueid_xid *xid = (struct _queueid_xid *)data->data;
    ...
    PyObject *cidobj = NULL;
    int err = newqueueid(state->QueueIDType, xid->cid, xid->end,
...
static int
_queueid_shared(PyThreadState *tstate, PyObject *obj,
                  _PyCrossInterpreterData *data)
{
    if (_PyCrossInterpreterData_InitWithSize(
            data, tstate->interp, sizeof(struct _queueid_xid), obj,
            _queueid_from_xid
            ) < 0)
    {
        return -1;
    ...
    struct _queueid_xid *xid = (struct _queueid_xid *)data->data;
    xid->cid = ((queueid *)obj)->cid;
    xid->end = ((queueid *)obj)->end;
    xid->resolve = ((queueid *)obj)->resolve;
    return 0;
}

static PyObject *
queueid_end(PyObject *self, void *end)
{
    int force = 1;
    queueid *cidobj = (queueid *)self;
    if (end != NULL) {
        PyObject *obj = NULL;
        int err = newqueueid(Py_TYPE(self), cidobj->cid, *(int *)end,
...
static int _queueid_end_send = CHANNEL_SEND;
static int _queueid_end_recv = CHANNEL_RECV;

static PyGetSetDef queueid_getsets[] = {
    {"end", (getter)queueid_end, NULL,
     PyDoc_STR("'send', 'recv', or 'both'")},
    {"send", (getter)queueid_end, NULL,
     PyDoc_STR("the 'send' end of the channel"), &_queueid_end_send},
    {"recv", (getter)queueid_end, NULL,
     PyDoc_STR("the 'recv' end of the channel"), &_queueid_end_recv},
    {NULL}
};

PyDoc_STRVAR(queueid_doc,
"A channel ID identifies a channel and may be used as an int.");

static PyType_Slot queueid_typeslots[] = {
    {Py_tp_dealloc, (destructor)queueid_dealloc},
    {Py_tp_doc, (void *)queueid_doc},
    {Py_tp_repr, (reprfunc)queueid_repr},
    {Py_tp_str, (reprfunc)queueid_str},
    {Py_tp_hash, queueid_hash},
    {Py_tp_richcompare, queueid_richcompare},
    {Py_tp_getset, queueid_getsets},
    // number slots
    {Py_nb_int, (unaryfunc)queueid_int},
    {Py_nb_index,  (unaryfunc)queueid_int},
    {0, NULL},
};

static PyType_Spec queueid_typespec = {
    .name = MODULE_NAME ".ChannelID",
    .basicsize = sizeof(queueid),
    .flags = (Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE |
              Py_TPFLAGS_DISALLOW_INSTANTIATION | Py_TPFLAGS_IMMUTABLETYPE),
    .slots = queueid_typeslots,
};
...
{
    queueid *cidobj = (queueid *)_queueid_from_xid(data);
    if (cidobj == NULL) {
        return NULL;
    }
                    ...
                    _PyCrossInterpreterData *data)
{
    ...
    int res = _queueid_shared(tstate, cidobj, data);
    Py_DECREF(cidobj);
    if (res < 0) {
        return -1;
    }
...
queuesmod_create(PyObject *self, PyObject *Py_UNUSED(ignored))
{
    ...
    PyObject *cidobj = NULL;
    int err = newqueueid(state->QueueIDType, cid, 0,
    ...
    assert(cidobj != NULL);
    assert(((queueid *)cidobj)->channels != NULL);
    return cidobj;
}
...
queuesmod_list_all(PyObject *self, PyObject *Py_UNUSED(ignored))
{
    ...
    for (int64_t i=0; i < count; cur++, i++) {
        PyObject *cidobj = NULL;
        int err = newqueueid(state->QueueIDType, *cur, 0,
                               &_globals.channels, 0, 0,
                               (queueid **)&cidobj);
...
static PyObject *
queuesmod_list_interpreters(PyObject *self, PyObject *args, PyObject *kwds)
{
...
PyDoc_STRVAR(queuesmod_list_interpreters_doc,
...
static PyObject *
queuesmod_send_buffer(PyObject *self, PyObject *args, PyObject *kwds)
{
    static char *kwlist[] = {"cid", "obj", "blocking", "timeout", NULL};
    struct channel_id_converter_data cid_data = {
        .module = self,
...
PyDoc_STRVAR(queuesmod_send_buffer_doc,
...
static PyObject *
queuesmod_close(PyObject *self, PyObject *args, PyObject *kwds)
{
...
PyDoc_STRVAR(queuesmod_close_doc,
...
static PyObject *
queuesmod_get_info(PyObject *self, PyObject *args, PyObject *kwds)
{
    static char *kwlist[] = {"cid", NULL};
    struct channel_id_converter_data cid_data = {
        .module = self,
...
PyDoc_STRVAR(queuesmod_get_info_doc,
"get_info(cid)\n\
\n\
Return details about the channel.");

static PyObject *
queuesmod__queue_id(PyObject *self, PyObject *args, PyObject *kwds)
{
    module_state *state = get_module_state(self);
    if (state == NULL) {
        return NULL;
    }
    PyTypeObject *cls = state->QueueIDType;

    PyObject *mod = get_module_from_owned_type(cls);
    assert(mod == self);
    Py_DECREF(mod);

    return _queueid_new(self, cls, args, kwds);
}
...
queuesmod__register_queue_type(PyObject *self, PyObject *args, PyObject *kwds)
{
    ...
    PyObject *recv;
    if (!PyArg_ParseTupleAndKeywords(args, kwds,
                                     "OO:_register_queue_type", kwlist,
                                     &send, &recv)) {
        return NULL;
    }
...
static PyMethodDef module_functions[] = {
    ...
    {"close",                      _PyCFunction_CAST(queuesmod_close),
     METH_VARARGS | METH_KEYWORDS, queuesmod_close_doc},
    {"release",                    _PyCFunction_CAST(queuesmod_release),
     METH_VARARGS | METH_KEYWORDS, queuesmod_release_doc},
    {"get_info",                   _PyCFunction_CAST(queuesmod_get_info),
     METH_VARARGS | METH_KEYWORDS, queuesmod_get_info_doc},
    {"_queue_id",                  _PyCFunction_CAST(queuesmod__queue_id),
...
module_exec(PyObject *mod)
{
    ...
    // ChannelID
    state->QueueIDType = add_new_type(
            mod, &queueid_typespec, _queueid_shared, xid_classes);
    if (state->QueueIDType == NULL) {
        goto error;
    }
