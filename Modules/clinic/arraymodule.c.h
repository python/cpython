...
array_array_fromstring(arrayobject *self, PyObject *arg)
{
    ...
    if (PyUnicode_Check(arg)) {
        ...
        if (!PyBuffer_IsContiguous(&buffer, 'C')) {
            _PyArg_BadArgument("fromstring", 0, "argument", "contiguous buffer", arg);
            goto exit;
        }
    }
...
array_array_frombytes(arrayobject *self, PyObject *arg)
{
    ...
    if (!PyBuffer_IsContiguous(&buffer, 'C')) {
        _PyArg_BadArgument("frombytes", 0, "argument", "contiguous buffer", arg);
        goto exit;
    }
...
array__array_reconstructor(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (!PyUnicode_Check(args[1])) {
        _PyArg_BadArgument("_array_reconstructor", 2, "argument 2", "a unicode character", args[1]);
        goto exit;
    }
...
/*[clinic end generated code: output=3509df49a81ae0c0 input=a9049054013a1b77]*/
