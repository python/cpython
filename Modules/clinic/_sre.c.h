...
_sre_compile(PyObject *module, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    if (!PyList_Check(args[2])) {
        _PyArg_BadArgument("compile", 3, "code", "list", args[2]);
        goto exit;
    }
    code = args[2];
    ...
    if (!PyDict_Check(args[4])) {
        _PyArg_BadArgument("compile", 5, "groupindex", "dict", args[4]);
        goto exit;
    }
    groupindex = args[4];
    if (!PyTuple_Check(args[5])) {
        _PyArg_BadArgument("compile", 6, "indexgroup", "tuple", args[5]);
        goto exit;
    }
    indexgroup = args[5];
...
/*[clinic end generated code: output=2ffd8eb155f22c72 input=a9049054013a1b77]*/
