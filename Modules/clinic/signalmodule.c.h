...
PyDoc_STRVAR(signal_SIG_DFL__doc__,
"SIG_DFL($module, signalnum, frame, /)\n"
"--\n"
"\n"
"Standard signal handler used to refer to the system default handler.");

#define SIGNAL_SIG_DFL_METHODDEF    \
    {"SIG_DFL", (PyCFunction)(void(*)(void))signal_SIG_DFL, METH_FASTCALL, signal_SIG_DFL__doc__},

static PyObject *
signal_SIG_DFL_impl(PyObject *module, int signalnum, PyObject *frame);

static PyObject *
signal_SIG_DFL(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    PyObject *return_value = NULL;
    int signalnum;
    PyObject *frame;

    if (!_PyArg_CheckPositional("SIG_DFL", nargs, 2, 2)) {
        goto exit;
    }
    signalnum = _PyLong_AsInt(args[0]);
    if (signalnum == -1 && PyErr_Occurred()) {
    ...
    frame = args[1];
    return_value = signal_SIG_DFL_impl(module, signalnum, frame);

exit:
    return return_value;
}

PyDoc_STRVAR(signal_SIG_IGN__doc__,
"SIG_IGN($module, signalnum, frame, /)\n"
"--\n"
"\n"
"Standard signal handler used to ignore the given signal.");

#define SIGNAL_SIG_IGN_METHODDEF    \
    {"SIG_IGN", (PyCFunction)(void(*)(void))signal_SIG_IGN, METH_FASTCALL, signal_SIG_IGN__doc__},

static PyObject *
signal_SIG_IGN_impl(PyObject *module, int signalnum, PyObject *frame);

static PyObject *
signal_SIG_IGN(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    PyObject *return_value = NULL;
    int signalnum;
    PyObject *frame;

    if (!_PyArg_CheckPositional("SIG_IGN", nargs, 2, 2)) {
        goto exit;
    }
    signalnum = _PyLong_AsInt(args[0]);
    if (signalnum == -1 && PyErr_Occurred()) {
    ...
    frame = args[1];
    return_value = signal_SIG_IGN_impl(module, signalnum, frame);
...
/*[clinic end generated code: output=66c16d84d35950bc input=a9049054013a1b77]*/
