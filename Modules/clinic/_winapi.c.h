...
_winapi_OpenThread(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (!_PyArg_CheckPositional("OpenThread", nargs, 3, 3)) {
        goto exit;
    }
    if (!_PyLong_UnsignedLong_Converter(args[0], &desired_access)) {
        goto exit;
    }
    inherit_handle = PyLong_AsInt(args[1]);
    if (inherit_handle == -1 && PyErr_Occurred()) {
    ...
    if (!_PyLong_UnsignedLong_Converter(args[2], &thread_id)) {
        goto exit;
    }
...
_winapi_CancelSynchronousIo(PyObject *module, PyObject *arg)
{
    ...
    thread = PyLong_AsVoidPtr(arg);
    if (!thread && PyErr_Occurred()) {
        goto exit;
    }
...
/*[clinic end generated code: output=dd093fb3083ee507 input=a9049054013a1b77]*/
