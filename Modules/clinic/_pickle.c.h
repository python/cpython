...
_pickle_Unpickler___init__(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (fastargs[2]) {
        if (!PyUnicode_Check(fastargs[2])) {
            _PyArg_BadArgument("Unpickler", 3, "encoding", "str", fastargs[2]);
            goto exit;
        }
    ...
    if (fastargs[3]) {
        if (!PyUnicode_Check(fastargs[3])) {
            _PyArg_BadArgument("Unpickler", 4, "errors", "str", fastargs[3]);
            goto exit;
        }
...
_pickle_load(PyObject *module, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    if (args[2]) {
        if (!PyUnicode_Check(args[2])) {
            _PyArg_BadArgument("load", 3, "encoding", "str", args[2]);
            goto exit;
        }
        Py_ssize_t encoding_length;
    ...
    if (args[3]) {
        if (!PyUnicode_Check(args[3])) {
            _PyArg_BadArgument("load", 4, "errors", "str", args[3]);
            goto exit;
        }
        Py_ssize_t errors_length;
...
_pickle_loads(PyObject *module, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    if (args[2]) {
        if (!PyUnicode_Check(args[2])) {
            _PyArg_BadArgument("loads", 3, "encoding", "str", args[2]);
            goto exit;
        }
        Py_ssize_t encoding_length;
        encoding = PyUnicode_AsUTF8AndSize(args[2], &encoding_length);
    ...
    if (args[3]) {
        if (!PyUnicode_Check(args[3])) {
            _PyArg_BadArgument("loads", 4, "errors", "str", args[3]);
            goto exit;
        }
        Py_ssize_t errors_length;
        errors = PyUnicode_AsUTF8AndSize(args[3], &errors_length);
...
/*[clinic end generated code: output=2dcf493ea059a277 input=a9049054013a1b77]*/
