...
_tkinter_tkapp_eval(TkappObject *self, PyObject *arg)
{
    ...
    script = PyUnicode_AsUTF8NoNUL(arg);
    if (script == NULL) {
        goto exit;
    }
...
_tkinter_tkapp_evalfile(TkappObject *self, PyObject *arg)
{
    ...
    fileName = PyUnicode_AsUTF8NoNUL(arg);
    if (fileName == NULL) {
        goto exit;
    }
...
_tkinter_create(PyObject *module, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (args[0] == Py_None) {
        ...
        screenName = PyUnicode_AsUTF8NoNUL(args[0]);
        if (screenName == NULL) {
            goto exit;
        }
    }
    ...
    baseName = PyUnicode_AsUTF8NoNUL(args[1]);
    if (baseName == NULL) {
        goto exit;
    }
    if (nargs < 3) {
    ...
    className = PyUnicode_AsUTF8NoNUL(args[2]);
    if (className == NULL) {
        goto exit;
    }
    if (nargs < 4) {
    ...
    if (args[7] == Py_None) {
        ...
        use = PyUnicode_AsUTF8NoNUL(args[7]);
        if (use == NULL) {
            goto exit;
        }
    }
...
/*[clinic end generated code: output=1bd2d8236b8e1752 input=a9049054013a1b77]*/
