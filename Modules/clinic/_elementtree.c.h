...
_elementtree_Element_append(ElementObject *self, PyObject *arg)
{
    ...
    if (!PyObject_TypeCheck(arg, &Element_Type)) {
        _PyArg_BadArgument("append", 0, NULL, (&Element_Type)->tp_name, arg);
        goto exit;
    }
    subelement = arg;
...
_elementtree_Element___deepcopy__(ElementObject *self, PyObject *arg)
{
    ...
    if (!PyDict_Check(arg)) {
        _PyArg_BadArgument("__deepcopy__", 0, NULL, "dict", arg);
        goto exit;
    }
    memo = arg;
...
_elementtree_Element_insert(ElementObject *self, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (!PyObject_TypeCheck(args[1], &Element_Type)) {
        _PyArg_BadArgument("insert", 2, NULL, (&Element_Type)->tp_name, args[1]);
        goto exit;
    }
    subelement = args[1];
...
_elementtree_Element_remove(ElementObject *self, PyObject *arg)
{
    ...
    if (!PyObject_TypeCheck(arg, &Element_Type)) {
        _PyArg_BadArgument("remove", 0, NULL, (&Element_Type)->tp_name, arg);
        goto exit;
    }
    subelement = arg;
...
_elementtree_XMLParser___init__(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (fastargs[1] == Py_None) {
        ...
        _PyArg_BadArgument("XMLParser", 2, "encoding", "str or None", fastargs[1]);
        goto exit;
    }
skip_optional_kwonly:
...
/*[clinic end generated code: output=7c113ba1f9ac82ff input=a9049054013a1b77]*/
