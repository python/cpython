...
sslmodule_init_errorcodes(PyObject *module)
{
    ...
    if (PyModule_Add(module, "err_codes_to_names", err_codes_to_names))
        return -1;
    err_names_to_codes = PyDict_New();
    if (PyModule_Add(module, "err_names_to_codes", err_names_to_codes))
        return -1;
    lib_codes_to_names = PyDict_New();
    if (PyModule_Add(module, "lib_codes_to_names", lib_codes_to_names))
        return -1;
    errcode = error_codes;
    while (errcode->mnemonic != NULL) {
        ...
        if (key == NULL
            || PyDict_SetItem(err_codes_to_names, key, mnemo) < 0
            || PyDict_SetItem(err_names_to_codes, mnemo, key) < 0)
        {
            Py_XDECREF(key);
            Py_DECREF(mnemo);
            return -1;
    ...
    while (libcode->library != NULL) {
        ...
        if (key == NULL || mnemo == NULL
            || PyDict_SetItem(lib_codes_to_names, key, mnemo))
        {
            Py_XDECREF(key);
            Py_XDECREF(mnemo);
            return -1;
