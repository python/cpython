...
_servername_callback(SSL *s, int *al, void *args)
{
    ...
    if (result == NULL) {
        PyErr_FormatUnraisable("Exception ignored in ssl servername "
                               "callback on calling set SNI callback %R",
                               sslctx->set_sni_cb);
        *al = SSL_AD_HANDSHAKE_FAILURE;
        ...
        if (result == Py_None) {
            ...
            *al = (int) PyLong_AsLong(result);
            if (PyErr_Occurred()) {
                PyErr_FormatUnraisable("Exception ignored in ssl servername "
                                       "callback on calling set SNI callback "
                                       "(result=%R)",
                                       result);
...
error:
    if (PyErr_Occurred()) {
        PyErr_FormatUnraisable("Exception ignored in ssl PSK client callback "
                               "on calling callback %R", callback);
    }
    PyGILState_Release(gstate);
    return 0;
}
