...
reader_parse_header(BinaryReader *reader, const uint8_t *data, size_t file_size)
{
    ...
    uint32_t thread_count, compression_type, profiling_mode;
    ...
    memcpy(&profiling_mode, &data[HDR_OFF_MODE], HDR_SIZE_MODE);
    ...
    profiling_mode = SWAP32_IF(reader->needs_swap, profiling_mode);
...
binary_reader_get_info(BinaryReader *reader)
{
    ...
    return Py_BuildValue(
        "{s:I, s:N, s:K, s:K, s:K, s:I, s:I, s:I, s:i, s:N, s:N, s:N, s:N, s:N}",
        "version", BINARY_FORMAT_VERSION,
