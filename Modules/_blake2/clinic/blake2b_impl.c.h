...
skip_optional_posonly:
    ...
    if (fastargs[2]) {
        ...
        if (!PyBuffer_IsContiguous(&key, 'C')) {
            _PyArg_BadArgument("blake2b", 3, "key", "contiguous buffer", fastargs[2]);
            goto exit;
        }
        if (!--noptargs) {
            goto skip_optional_kwonly;
    ...
    if (fastargs[3]) {
        ...
        if (!PyBuffer_IsContiguous(&salt, 'C')) {
            _PyArg_BadArgument("blake2b", 4, "salt", "contiguous buffer", fastargs[3]);
            goto exit;
        }
        if (!--noptargs) {
            goto skip_optional_kwonly;
    ...
    if (fastargs[4]) {
        ...
        if (!PyBuffer_IsContiguous(&person, 'C')) {
            _PyArg_BadArgument("blake2b", 5, "person", "contiguous buffer", fastargs[4]);
            goto exit;
        }
        if (!--noptargs) {
            goto skip_optional_kwonly;
...
/*[clinic end generated code: output=dda27ce5a2bfc110 input=a9049054013a1b77]*/
