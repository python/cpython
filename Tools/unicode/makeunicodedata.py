...
def maketables(trace=0):
    ...
    makegraphemedata(unicode, trace)
...
# --------------------------------------------------------------------
# grapheme cluster break data for pure Python implementation

def makegraphemedata(unicode, trace):

    FILE = "Lib/_py_grapheme_db.py"

    print("--- Preparing", FILE, "...")

    gcb_names = GRAPHEME_CLUSTER_NAMES
    incb_names = INDIC_CONJUNCT_BREAK_NAMES

    # Build per-codepoint GCB array, excluding LV/LVT (computed algorithmically)
    gcb_values = []
    for char in unicode.chars:
        gcb_name = unicode.grapheme_breaks[char] or 'Other'
        gcb_values.append(gcb_names.index(gcb_name))

    # Compress into ranges: (start, end, value), skipping LV/LVT and Other
    gcb_lv = gcb_names.index('LV')
    gcb_lvt = gcb_names.index('LVT')
    gcb_other = gcb_names.index('Other')

    gcb_ranges = _compress_ranges(
        gcb_values, skip_values={gcb_lv, gcb_lvt, gcb_other},
    )

    # Build per-codepoint ExtPict array
    ep_ranges = _compress_ranges(
        [int(v) for v in unicode.ext_picts], skip_values={0},
    )

    # Build per-codepoint InCB array
    incb_values = []
    for char in unicode.chars:
        record = unicode.table[char]
        if record:
            incb_values.append(incb_names.index(record.incb))
        else:
            incb_values.append(0)  # None

    incb_none = incb_names.index('None')
    incb_ranges = _compress_ranges(
        incb_values, skip_values={incb_none},
    )

    print(len(gcb_ranges), "GCB ranges")
    print(len(ep_ranges), "Extended_Pictographic ranges")
    print(len(incb_ranges), "InCB ranges")
    ...
    with open(FILE, "w") as fp:
        ...
        fprint("# This file was generated by %s %s." % (SCRIPT, VERSION))
        fprint("# Unicode version: %s." % UNIDATA_VERSION)
        fprint("#")
        fprint("# Property tables for the pure Python grapheme cluster break")
        fprint("# implementation in _py_grapheme.py.")
        fprint("#")
        fprint("# Each table is a tuple of (start, end, value) ranges sorted")
        fprint("# by start codepoint.  The default value for codepoints not")
        fprint("# covered is 0.")
        fprint()

        fprint("UNIDATA_VERSION = %r" % UNIDATA_VERSION)
        fprint()

        # GCB constants
        fprint("# Grapheme_Cluster_Break values")
        for i, name in enumerate(gcb_names):
            fprint("GCB_%s = %d" % (name, i))
        fprint()

        # InCB constants
        fprint("# Indic_Conjunct_Break values")
        for i, name in enumerate(incb_names):
            fprint("InCB_%s = %d" % (name, i))
        fprint()
        ...
        _write_ranges(fprint, "INCB_RANGES", incb_ranges)


def _compress_ranges(values, skip_values):
    """Compress a per-codepoint value array into sorted (start, end, value) ranges."""
    ranges = []
    n = len(values)
    i = 0
    while i < n:
        ...
        if v in skip_values:
            ...
            continue
        start = i
        while i < n and values[i] == v:
            i += 1
        ranges.append((start, i - 1, v))
    return ranges


def _write_ranges(fprint, name, ranges, has_value=True):
    """Write a range table as a Python tuple of tuples."""
    fprint("%s = (" % name)
    per_line = 4 if has_value else 5
    for i in range(0, len(ranges), per_line):
        chunk = ranges[i:i + per_line]
        if has_value:
            parts = ["(0x%04X,0x%04X,%d)" % (s, e, v) for s, e, v in chunk]
        else:
