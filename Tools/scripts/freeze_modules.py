...
def regen_gitignore(modules):
    frozenfiles = [
        '# These are auto-generated by Tools/scripts/freeze_modules.py.',
    ]
    for src in _iter_sources(modules):
        header = os.path.relpath(src.frozenfile, ROOT_DIR).replace('\\', '/')
        frozenfiles.append(header)

    print(f'# Updating {os.path.relpath(GITIGNORE)}')
    with updating_file_with_tmpfile(GITIGNORE) as (infile, outfile):
        lines = infile.readlines()
        lines = replace_block(
            lines,
            "# BEGIN: frozen files",
...
def regen_test_ctypes(modules):
    indent = '                '
    deflines = [
        f'{indent}# Auto-generated by Tools/scripts/freeze_modules.py',
    ]
    for mod in modules:
        # Get the size of the array in the header file.
        size = mod.get_array_size()
        if mod.ispkg:
            size *= -1
        deflines.append(f'{indent}("{mod.name}", {size}),')

    print(f'# Updating {os.path.relpath(TEST_CTYPES)}')
    with updating_file_with_tmpfile(TEST_CTYPES) as (infile, outfile):
        lines = infile.readlines()
        lines = replace_block(
            lines,
            '# BEGIN: frozen modules',
...
def main():
    ...
    regen_gitignore(modules)
    regen_makefile(modules)
    regen_pcbuild(modules)
    regen_test_ctypes(modules)
