...
def gen_doc_annotations(manifest, args, outfile):
    ...
    write("# Fields: rest_role, name, version_added")
    write()
    for item in manifest.select(REST_ROLES.keys(), include_abi_only=False):
        write(REST_ROLES[item.kind], item.name, item.added)
...
@generator("doc_rst", 'Doc/c-api/limited_api.inc')
def gen_doc_page(manifest, args, outfile):
    """Generate/check the ReST list for stable ABI documentation"""
    write = partial(print, file=outfile)
    write(".. File generated by Tools/stable_abi.py")
    write()
    for item in manifest.select(REST_LINK_ROLES.keys(), include_abi_only=False):
        write(f'* :c:{REST_LINK_ROLES[item.kind]}:`{item.name}`')
