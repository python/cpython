...
parser.add_argument(
    "-H", "--openssl-version", default=None,
    help="named OpenSSL version used to generate the C header guard\n"
...
def parse_err_h(args):
    ...
    lib2errnum = {}
    for match in file_search(args.err_h, pat):
...
def parse_extra_reasons(args):
    ...
    for match in file_search(args.errcodes, pat):
        reason, libname, errname, num = match.groups()
        yield reason, libname, errname, int(num)
