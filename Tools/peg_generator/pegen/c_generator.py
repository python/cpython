...
from pegen.parser_generator import NullableVisitor, ParserGenerator
...
class CParserGenerator(ParserGenerator, GrammarVisitor):
    def __init__(
        ...
        self.token_types = {name: type_ for type_, name in tokens.items()}
        self.exact_tokens = exact_tokens
        self._varname_counter = 0
        self.debug = debug
        self.skip_actions = skip_actions
        self.cleanup_statements: list[str] = []
        self.first_set_calculator: FirstSetCalculator | None = None
        self.nullable_visitor: NullableVisitor | None = None
    ...
    def generate(self, filename: str) -> None:
        self.collect_rules()
        self.first_set_calculator = FirstSetCalculator(self.all_rules)
        self.first_set_calculator.calculate()
        self.nullable_visitor = NullableVisitor(self.all_rules)
        for rule in self.all_rules.values():
            self.nullable_visitor.visit(rule)
    ...
    def _handle_default_rule_body(self, node: Rule, rhs: Rhs, result_type: str) -> None:
        ...
        dispatch = self._first_set_dispatch(rhs)
        ...
        with self.indent():
            ...
            self.print("int _mark = p->mark;")
            uses_locations = any(
                alt.action and "EXTRA" in alt.action for alt in rhs.alts
            )
            if uses_locations:
            ...
            if dispatch is not None:
                if not uses_locations:
                    ...
                    with self.indent():
                        ...
                        self.add_return("NULL")
                    self.print("}")
                self.print("int _current_token_type = p->tokens[_mark]->type;")
            self.visit(
                rhs,
                is_loop=False,
    ...
    def visit_Rhs(
        ...
        dispatch = None if is_loop else self._first_set_dispatch(node)
        if dispatch is None:
            for alt in node.alts:
                self.visit(alt, is_loop=is_loop, is_gather=is_gather, rulename=rulename)
            return

        all_token_types = set().union(*dispatch)
        has_overlaps = sum(len(token_types) for token_types in dispatch) != len(
            all_token_types
        )
        if has_overlaps:
            self._visit_rhs_with_first_set_mask(node, dispatch, is_gather, rulename)
            return

        labels = [self.unique_varname("first_set_alt") for _ in node.alts]
        end_label = self.unique_varname("first_set_done")
        # The second pass must try invalid rules in grammar order so that it
        # reports the same error as an unoptimized parser.
        self.print("if (!p->call_invalid_rules) {")
        with self.indent():
            self.print("switch (_current_token_type) {")
            with self.indent():
                for label, token_types in zip(labels, dispatch, strict=True):
                    for token_type in sorted(token_types):
                        self.print(f"case {token_type}:")
                    with self.indent():
                        self.print(f"goto {label};")
                self.print("default:")
                with self.indent():
                    self.print(f"goto {end_label};")
            self.print("}")
        self.print("}")

        for label, alt in zip(labels, node.alts, strict=True):
            self.print(f"{label}:")
            self.visit(alt, is_loop=False, is_gather=is_gather, rulename=rulename)
            self.print("if (!p->call_invalid_rules) {")
            with self.indent():
                self.print(f"goto {end_label};")
            self.print("}")
        self.print(f"{end_label}:")
        self.print(";")

    def _visit_rhs_with_first_set_mask(
        self,
        node: Rhs,
        dispatch: list[set[int]],
        is_gather: bool,
        rulename: str | None,
    ) -> None:
        token_masks: dict[int, int] = {}
        for alt_index, token_types in enumerate(dispatch):
            for token_type in token_types:
                token_masks[token_type] = token_masks.get(token_type, 0) | (1 << alt_index)

        masks: dict[int, list[int]] = {}
        for token_type, mask in token_masks.items():
            masks.setdefault(mask, []).append(token_type)

        self.print("unsigned long long _first_set_mask = ~0ULL;")
        self.print("if (!p->call_invalid_rules) {")
        with self.indent():
        ...
        for alt_index, alt in enumerate(node.alts):
            self.print(f"if (_first_set_mask & 0x{1 << alt_index:x}ULL)")
            self.visit(alt, is_loop=False, is_gather=is_gather, rulename=rulename)

    def _first_set_token_type(self, terminal: str) -> int | None:
        if terminal in self.token_types:
            return self.token_types[terminal]

        try:
            value = ast.literal_eval(terminal)
        except (SyntaxError, ValueError):
            return None

        if not isinstance(value, str) or not value:
            return None
        if re.match(r"[a-zA-Z_]\w*\Z", value):
            # Soft keyword spelling is checked by the parser; the lexer emits
            # NAME for all of them.
            if terminal.startswith('"') and terminal.endswith('"'):
                return self.token_types.get("NAME")
            return self.keywords.get(value)
        return self.exact_tokens.get(value)

    def _first_set_dispatch(self, node: Rhs) -> list[set[int]] | None:
        # Overlapping choices use one bit per alternative.
        if len(node.alts) < 2 or len(node.alts) > 64:
            return None
        assert self.first_set_calculator is not None
        assert self.nullable_visitor is not None

        dispatch: list[set[int]] = []
        for alt in node.alts:
            if self.nullable_visitor.visit(alt):
                return None
            token_types: set[int] = set()
            for terminal in self.first_set_calculator.visit(alt):
                token_type = self._first_set_token_type(terminal)
                if token_type is None:
                    return None
                token_types.add(token_type)
            if not token_types:
                return None
            dispatch.append(token_types)
        return dispatch
