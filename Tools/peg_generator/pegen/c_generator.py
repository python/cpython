...
class CParserGenerator(ParserGenerator, GrammarVisitor):
    def __init__(
        ...
        self.exact_tokens = exact_tokens
        self.operator_loops: dict[str, Rhs] = {}
    ...
    def generate(self, filename: str) -> None:
        self.collect_rules()
        self.operator_loops = self._find_operator_loops()
    ...
    def _set_up_rule_memoization(self, node: Rule, result_type: str) -> None:
        self.print("{")
        with self.indent():
            if node.name in self.operator_loops:
                self.print("if (!p->call_invalid_rules) {")
                with self.indent():
                    self.print(f"return {node.name}_operator_loop(p);")
                self.print("}")
            self.add_level()
    ...
    def _find_operator_loops(self) -> dict[str, Rhs]:
        """Validate marked rules under their explicit C-action contract.

        Compute a conservative fixed point of rules that consume on success.
        Unknown nodes, lookahead and optional items do not prove consumption.
        C actions can hide parser calls and cursor changes from this analysis;
        the opt-in contract is documented in InternalDocs/parser.md.
        """
        def reject(name: str, reason: str) -> NoReturn:
            raise ValueError(f"rule {name!r}: operator_loop {reason}")

        marked = {}
        for name, rule in self.all_rules.items():
            if "operator_loop" in rule.flags:
                marked[name] = rule
        if not marked:
            return {}
        ...
        def consumes(node: Any) -> bool:
            if isinstance(node, NameLeaf):
                return node.value not in self.all_rules or node.value in consuming
            if isinstance(node, StringLeaf):
                return True
            if isinstance(node, (NamedItem, Repeat1, Gather, Forced)):
                return consumes(node.item if isinstance(node, NamedItem) else node.node)
            if isinstance(node, Group):
                return consumes(node.rhs)
            if isinstance(node, Alt):
                return any(consumes(item) for item in node.items)
            if isinstance(node, Rhs):
                return bool(node.alts) and all(consumes(alt) for alt in node.alts)
            return False

        while True:
            added = {name for name, rule in self.all_rules.items()
                     if consumes(rule.rhs)} - consuming
            if not added:
                break
            consuming.update(added)

        # Include calls inside predicates and nullable prefixes, which must not
        # re-enter an operator rule before its final memo has been installed.
        def initial_calls(node: Any) -> set[str]:
            if isinstance(node, NameLeaf):
                return {node.value} if node.value in self.all_rules else set()
            if isinstance(node, NamedItem):
                return initial_calls(node.item)
            if isinstance(node, (Lookahead, Opt, Repeat0, Repeat1, Gather, Forced)):
                return initial_calls(node.node)
            if isinstance(node, Group):
                return initial_calls(node.rhs)
            if isinstance(node, Rhs):
                return set().union(*(initial_calls(alt) for alt in node.alts))
            if isinstance(node, Alt):
                result: set[str] = set()
                for item in node.items:
                    result.update(initial_calls(item))
                    if consumes(item):
                        break
                return result
            return set()

        graph = {name: initial_calls(rule.rhs)
                 for name, rule in self.all_rules.items()}
        operator_kinds = {kind.__name__ for kind in ast.operator.__subclasses__()}
        operator_kind = "(?:" + "|".join(sorted(operator_kinds)) + ")"
        candidates: dict[str, Rhs] = {}
        for name, rule in marked.items():
            if rule.type != "expr_ty":
                reject(name, "requires result type expr_ty")
            if not rule.left_recursive or not rule.leader:
                reject(name, "requires a direct left-recursion leader")
            if rule.flags != {"operator_loop"}:
                reject(name, "cannot be combined with other rule flags")
            if name.endswith("without_invalid"):
                reject(name, "does not support without_invalid rules")
            # These are precisely the alternatives visit_Alt disables on pass 1.
            alts = [alt for alt in rule.flatten().alts
                    if not (len(alt.items) == 1
                            and isinstance(alt.items[0].item, NameLeaf)
                            and str(alt.items[0]).startswith("invalid_")
                            and not alt.action)]
            if len(alts) < 2:
                reject(name, "requires recursive alternatives followed by one operand alternative")
            base = alts[-1]
            if (base.action or len(base.items) != 1
                    or not isinstance(base.items[0].item, NameLeaf)
                    or base.items[0].type):
                reject(name, "requires a final plain operand rule without an action or type cast")
            operand = base.items[0].item.value
            if operand not in self.all_rules or self.all_rules[operand].type != "expr_ty":
                reject(name, f"requires operand {operand!r} to be an expr_ty rule")
            if operand not in consuming:
                reject(name, f"cannot prove that operand {operand!r} consumes input")
            reached: set[str] = set()
            pending = [operand]
            while pending:
                current = pending.pop()
            ...
            if name in reached:
                reject(name, f"operand {operand!r} can re-enter the rule without consuming input")
            tokens: set[int] = set()
            for alt in alts[:-1]:
                if len(alt.items) != 3:
                    reject(name, "requires each recursive alternative to be left=self token right=operand")
                left, operator, right = alt.items
                if (not isinstance(left.item, NameLeaf) or left.item.value != name
                        or not isinstance(right.item, NameLeaf) or right.item.value != operand
                        or not isinstance(operator.item, StringLeaf)
                        or not left.name or not right.name or left.name == right.name
                        or left.name in operator_kinds or right.name in operator_kinds
                        or left.type or right.type or operator.name or operator.type):
                    reject(name, "requires distinct operand bindings, an unnamed exact token, and no casts")
                token = self.exact_tokens.get(ast.literal_eval(operator.item.value))
                if token is None:
                    reject(name, f"operator {operator.item.value} is not an exact token")
                if token in tokens:
                    reject(name, f"operator {operator.item.value} occurs in more than one alternative")
                tokens.add(token)
                # Only immutable AST construction, optionally with the existing
                # feature-version check. Custom actions need a separate proof.
                action = (re.escape(left.name) + r"\s*,\s*" + operator_kind
                          + r"\s*,\s*" + re.escape(right.name))
                action = r"_PyAST_BinOp\s*\(\s*" + action + r"\s*,\s*EXTRA\s*\)"
                versioned = (r'CHECK_VERSION\s*\(\s*expr_ty\s*,\s*[0-9]+\s*,\s*'
                             r'"(?:[^"\\]|\\.)*"\s*,\s*' + action + r"\s*\)")
                if not re.fullmatch(r"\s*(?:" + action + "|" + versioned + r")\s*", alt.action or ""):
                    reject(name, "requires a BinOp action on the bound operands, optionally CHECK_VERSION")
            candidates[name] = Rhs(alts)
        ...
        for name, rhs in candidates.items():
            ...
            if operand not in candidates and not self._should_memoize(rule):
                reject(name, f"requires operand {operand!r} to use (operator_loop), "
                       "or ordinary (memo)")
        # Validate the annotations even when the existing action-free debugging
        # mode requests ordinary PEG emission instead of AST folding.
        return {} if self.skip_actions else candidates

    def _handle_operator_loop(self, node: Rule, rhs: Rhs) -> None:
        self.print(f"static expr_ty\n{node.name}_operator_loop(Parser *p)")
        self.print("{")
        with self.indent():
            self.add_level()
            self._check_for_errors()
            self.print("expr_ty _res = NULL;")
            self.print(f"if (_PyPegen_is_memoized(p, {node.name}_type, &_res)) {{")
            with self.indent():
                self.add_return("_res")
            self.print("}")
            ...
            self.print("while (1) {")
            with self.indent():
                self.print("int _operator_mark = p->mark;")
                self.print("if (p->mark == p->fill && _PyPegen_fill_token(p) < 0) {")
                with self.indent():
                    self.print("p->error_indicator = 1;")
                    self.add_return("NULL")
                self.print("}")
                self.print("switch (p->tokens[p->mark]->type) {")
                with self.indent():
                    for alt in rhs.alts[:-1]:
                        ...
                        with self.indent():
                            self.print(f"expr_ty {left.name} = _res;")
                            self.print("p->mark++;")
                            self.print(f"expr_ty {right.name} = {operand}_rule(p);")
                            self._check_for_errors()
                            self.print(f"if ({right.name} == NULL) {{")
                            with self.indent():
                                self.print("p->mark = _operator_mark;")
                                self.print("goto done;")
                            self.print("}")
                            self._set_up_token_end_metadata_extraction()
                            self.emit_action(alt)
                            self.print("break;")
                        self.print("}")
                    self.print("default: goto done;")
                self.print("}")
            self.print("}")
        self.print("  done:")
        with self.indent():
            self.print(f"if (_PyPegen_insert_memo(p, _mark, {node.name}_type, _res) < 0) {{")
            with self.indent():
                self.print("p->error_indicator = 1;")
                self.add_return("NULL")
            self.print("}")
    ...
    def visit_Rule(self, node: Rule) -> None:
        ...
        for line in str(node).splitlines():
            self.print(f"// {line}")
        if node.name in self.operator_loops:
            self._handle_operator_loop(node, self.operator_loops[node.name])
