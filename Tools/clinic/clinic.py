...
class CConverter(metaclass=CConverterAutoRegister):
    ...
    def parse_arg(self, argname, argnum, displayname):
        if self.format_unit == 'O&':
            return """
            ...
            if self.subclass_of in type_checks:
                ...
                return """
                    if (!{typecheck}({argname})) {{{{
                        _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, "{typename}", {argname});
                        goto exit;
                    }}}}
                    {paramname} = {cast}{argname};
                    """.format(argname=argname, paramname=self.name,
                               argnum=argnum, displayname=displayname,
                               typecheck=typecheck, typename=typename, cast=cast)
            return """
                if (!PyObject_TypeCheck({argname}, {subclass_of})) {{{{
                    _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, ({subclass_of})->tp_name, {argname});
                    goto exit;
                }}}}
                {paramname} = {cast}{argname};
        ...
        if self.format_unit == 'c':
            return """
                if (PyBytes_Check({argname}) && PyBytes_GET_SIZE({argname}) == 1) {{{{
                    ...
                    _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, "a byte string of length 1", {argname});
                    goto exit;
                }}}}
                """.format(argname=argname, paramname=self.name, argnum=argnum,
                           displayname=displayname)
        ...
        if self.format_unit == 'i':
            ...
            return """
                if (!PyUnicode_Check({argname})) {{{{
                    _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, "a unicode character", {argname});
                    goto exit;
                }}}}
        ...
        if self.format_unit == 'k':
            return """
                if (!PyLong_Check({argname})) {{{{
                    _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, "int", {argname});
                    goto exit;
                }}}}
...
class str_converter(CConverter):
    ...
    def parse_arg(self, argname, argnum, displayname):
        if self.format_unit == 's':
            return """
                if (!PyUnicode_Check({argname})) {{{{
                    _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, "str", {argname});
                    goto exit;
                }}}}
        ...
        if self.format_unit == 'z':
            return """
                if ({argname} == Py_None) {{{{
                    ...
                    _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, "str or None", {argname});
                    goto exit;
                }}}}
                """.format(argname=argname, paramname=self.name, argnum=argnum,
                           displayname=displayname)
        ...
        if self.format_unit == 'S':
            return """
                if (!PyBytes_Check({argname})) {{{{
                    _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, "bytes", {argname});
                    goto exit;
                }}}}
        ...
        if self.format_unit == 'Y':
            return """
                if (!PyByteArray_Check({argname})) {{{{
                    _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, "bytearray", {argname});
                    goto exit;
                }}}}
                {paramname} = ({type}){argname};
                """.format(argname=argname, paramname=self.name, argnum=argnum,
        ...
        if self.format_unit == 'y*':
            return """
                ...
                if (!PyBuffer_IsContiguous(&{paramname}, 'C')) {{{{
                    _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, "contiguous buffer", {argname});
                    goto exit;
                }}}}
                """.format(argname=argname, paramname=self.name, argnum=argnum,
                           displayname=displayname)
            ...
            return """
                if (PyObject_GetBuffer({argname}, &{paramname}, PyBUF_WRITABLE) < 0) {{{{
                    PyErr_Clear();
                    _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, "read-write bytes-like object", {argname});
                    goto exit;
                }}}}
                if (!PyBuffer_IsContiguous(&{paramname}, 'C')) {{{{
                    _PyArg_BadArgument("{{name}}", {argnum}, {displayname}, "contiguous buffer", {argname});
