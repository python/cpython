...
#scp "%PYTHON_SOURCE%python.bat" "%SSH_SERVER%:%REMOTE_PYTHON_DIR%python.bat"
#scp -r "%PYTHON_SOURCE%PCBuild" "%SSH_SERVER%:%REMOTE_PYTHON_DIR%PCBuild"
#scp -r "%PYTHON_SOURCE%Lib" "%SSH_SERVER%:%REMOTE_PYTHON_DIR%Lib"
if EXIST %REMOTE_PYTHON_DIR% (rd %REMOTE_PYTHON_SHARE% /s/q)
python PC/layout -vv %dparam% -s %PYTHON_SOURCE% -b "%PYTHON_SOURCE%\PCBuild\arm32" -t "%PYTHON_SHARE%\temp" --copy "%PYTHON_SHARE%" --preset-iot --include-tests --include-venv
if NOT EXIST %REMOTE_PYTHON_DIR%\PCbuild (md %REMOTE_PYTHON_DIR%\PCbuild)
copy %PYTHON_SOURCE%\PCBuild\*.bat %REMOTE_PYTHON_DIR%\PCbuild
copy %PYTHON_SOURCE%\PCBuild\*.py %REMOTE_PYTHON_DIR%\PCbuild
