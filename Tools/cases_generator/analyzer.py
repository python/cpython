...
class Instruction:
    ...
    def _compute_properties(self) -> Properties:
        return Properties(
            ...
            any(part.properties.eval_breaker for part in self.uops),
...
def analyze_stack_items(items: list[parser.StackEffect]) -> list[StackEffect]:
...
def analyze_stack(op: parser.InstDef) -> StackEffect:
    inputs: list[parser.StackEffect] = [i for i in op.inputs if isinstance(i, parser.StackEffect)]
    return StackEffect(
        analyze_stack_items(inputs),
        analyze_stack_items(op.outputs),
    )
...
def always_exits(op: parser.InstDef):
    ...
    for tkn in op.tokens:
        if tkn.kind == "LBRACE":
            ...
            depth -= 1
        elif tkn.kind == "KEYWORD" and depth == 0 and tkn.text in EXITS:
            return True
        elif tkn.kind == "IDENTIFIER" and depth == 0 and tkn.text in EXITS:
            return True
    return False
...
def compute_properties(op: parser.InstDef) -> Properties:
    return Properties(
        ...
        variable_used(op, "CHECK_EVAL_BREAKER"),
