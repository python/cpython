...
def check_escaping_calls(instr: parser.CodeDef, escapes: dict[lexer.Token, EscapingCall]) -> None:
    ...
    for tkn in tkn_iter:
        if tkn.kind == lx.IF:
            next(tkn_iter)
            in_if = 1
        if tkn.kind == lx.IDENTIFIER and tkn.text in ("DEOPT_IF", "ERROR_IF", "EXIT_IF"):
            next(tkn_iter)
            in_if = 1
...
def compute_properties(op: parser.CodeDef) -> Properties:
    ...
    pure = False if isinstance(op, parser.LabelDef) else lx.ANN_PURE in op.annotations
    no_save_ip = False if isinstance(op, parser.LabelDef) else lx.ANN_NO_SAVE_IP in op.annotations
    return Properties(
        escaping_calls=escaping_calls,
        escapes=escapes,
