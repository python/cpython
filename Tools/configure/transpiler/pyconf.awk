...
function pyconf_ax_c_float_words_bigendian(on_big, on_little, on_unknown,    src, result) {
	src = "#include <stdio.h>\nint main(void) {\n  double d = 1.0;\n  unsigned char *p = (unsigned char *)&d;\n  if (p[0] == 0x3f) { printf(\"big\\n\"); }\n  else if (p[7] == 0x3f) { printf(\"little\\n\"); }\n  else { printf(\"unknown\\n\"); }\n  return 0;\n}\n"
	result = _pyconf_run_test(_pyconf_confdefs() "\n" src, "")
	# Store result for caller to dispatch callbacks
	_pyconf_retval = result
}

function pyconf_check_c_bigendian(    source, result) {
