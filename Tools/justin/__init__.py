...
class Engine:
    ...
    def dump(self) -> str:
        ...
        for opname, stencil in sorted(self._stencils_built.items()) + [("trampoline", self._trampoline_built)]:
            ...
            lines.append(f"}};")
            lines.append(f"")
        lines.append(f"static const Stencil * const stencils[256] = {{")
        ...
        for kind in sorted(kinds):
            if kind.startswith("HOLE_"):
                value = "(uintptr_t)0xBAD0BAD0BAD0BAD0"
            else:
        ...
        header.append(f"")
        header.append(f"PyAPI_FUNC(unsigned char *)_PyJustin_CompileTrace(int size, _Py_CODEUNIT **trace);")
        header.append(f"")
