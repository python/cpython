...
``options`` object returned by :meth:`~argparse.ArgumentParser.parse_args` based on user-supplied
command-line values.  When :meth:`~argparse.ArgumentParser.parse_args` returns from parsing this command
...
(If you like, you can pass a custom argument list to :meth:`~argparse.ArgumentParser.parse_args`, but
that's rarely necessary: by default it uses ``sys.argv[1:]``.)

:meth:`~argparse.ArgumentParser.parse_args` returns two values:
...
call to :meth:`~argparse.ArgumentParser.parse_args`, ``options.filename`` is ``"foo.txt"``.
...
If :meth:`~argparse.ArgumentParser.parse_args` encounters any errors in the argument list, it calls the
...
``parser``
   ...
   ``parser.largs``
      ...
      become ``args``, the second return value of :meth:`~argparse.ArgumentParser.parse_args`.)
