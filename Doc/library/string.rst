...
keyword.  If it is a number, it refers to a positional argument, and if it is a keyword,
...
Some examples::
   ...
   "Harold is a clever {0!s}"       # Calls str() on the argument first
...
The *width* option is a decimal integer defining the minimum total field width,
...
The *type* option can have one of the following values for :class:`float`
...
* *idpattern* -- This is the regular expression describing the pattern for
  ...
  .. note::
     ...
     with some non-ASCII characters.  That is why we use the local ``a`` flag
     here.
