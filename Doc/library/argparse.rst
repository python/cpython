...
* ``'append'`` - This stores a list, and appends each argument value to the
  list. This means that if the default value is non-empty, the default elements
  will be present in the parsed value for the option, with any values from the
  command line appended after those default values. It is useful to allow
  an option to be specified multiple times. Example usage::
