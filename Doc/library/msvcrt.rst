...
.. function:: set_error_mode(mode)
   ...
   that might end the program. *mode* must be one of the :const:`!OUT_\*` or
   :const:`REPORT_ERRMODE` constants listed below. Returns the old setting or -1
   if an error occurs. Only available in
...
.. function:: CrtSetReportMode(type, mode)
   ...
   generated by ``_CrtDbgReport`` in the MS VC++ runtime. *type* must be one of
   the :const:`!CRT_\*` constants listed below. *mode* must be one of the
