...
.. data:: month_abbr
   ...
   locale where ``JANUARY`` is ``1`` and ``DECEMBER`` is ``12``..  This follows
   normal convention of January being month number 1, so it has a length of 13
   and  ``month_abbr[0]`` is the empty string.
