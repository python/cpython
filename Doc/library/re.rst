...
.. function:: prefixmatch(pattern, string, flags=0)
   ...
   Use the :func:`~re.match` spelling when your code needs to support older
   Python versions.

   .. versionchanged:: 3.12
      The :func:`~re.prefixmatch` spelling was added as a more explicit name
      for :func:`~re.match`. Use it to avoid confusion as the industry norm in
      other languages and regular expression implementations is to use the term
      *match* to refer to the behavior of Python's :func:`~re.search`.  See
...
.. method:: Pattern.prefixmatch(string[, pos[, endpos]])
   ...
   Use the :meth:`~Pattern.match` spelling when your code needs to support
   older Python versions.

   .. versionchanged:: 3.12
      The :func:`~Pattern.prefixmatch` spelling was added as a more explicit
      name for :func:`~Pattern.match`. Use it to avoid confusion as the
      industry norm in other languages and regular expression implementations
      is to use the term *match* to refer to the behavior of Python's
...
favor of the longer :func:`~re.prefixmatch` spelling as of Python 3.12?
...
ambiguious to those reading or writing code who are not familiar with the
Python API's divergence from what has become the software industry norm.
...
a seed of doubt about the intended behavior.
