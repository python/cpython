...
.. cmdoption:: -X
   ...
   * ``-X importtime`` to show how long each import takes. It can be used to
     optimize application's startup time. Cumulative time (including nested
     imports) and self time (exluding nested imports) are shown.
