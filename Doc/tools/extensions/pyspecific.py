...
class DeprecatedRemoved(Directive):
    ...
    def run(self):
        ...
        env = self.state.document.settings.env
        # 当ValueError: invalid literal for int() with base 10: 'beta28'，说明不是完全由数字组成的字符，可能包含有beta ， alpha等词。
        current_version = tuple(int(e)
                                for e in env.config.version.split('.')[:-1])
...
=======
        ...
        text = label.format(
            deprecated=self.arguments[0], removed=self.arguments[1])
>>>>>>> main
        if len(self.arguments) == 3:
            inodes, messages = self.state.inline_text(self.arguments[2],
                                                      self.lineno+1)
            para = nodes.paragraph(
                self.arguments[2], '', *inodes, translatable=False)
            node.append(para)
        else:
            messages = []
        if self.content:
        ...
        if len(node):
            if isinstance(node[0], nodes.paragraph) and node[0].rawsource:
                ...
                node[0].replace_self(nodes.paragraph(
                    '', '', content, translatable=False))
...
=======
    ...
    app.add_object_type('pdbcommand', 'pdbcmd',
                        '%s (pdb command)', parse_pdb_command)
>>>>>>> main
