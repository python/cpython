...
``es#`` (:class:`str`) [*encoding*, char \*buffer, int or :c:type:`Py_ssize_t` buffer_length]
   This variant on ``s#`` is used for encoding Unicode into a character buffer.
...
``et#`` (:class:`str`, :class:`bytes` or :class:`bytearray`) [*encoding*, char \*buffer, int or :c:type:`Py_ssize_t` buffer_length]
