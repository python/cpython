...
* Added C API function :c:func:`PyTraceback_GetLineNumber` to get the lineno
  of a traceback object.
  (Contributed by Kumar Aditya in :gh:`95238`)
...
* The ``tb_lineno`` field of ``PyTracebackObject`` is now computed lazily,
  C extensions should use the new :c:func:`PyTraceback_GetLineNumber` instead
  or ``PyObject_GetAttrString(tb, "tb_lineno")``.
