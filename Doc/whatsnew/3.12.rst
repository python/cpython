...
* The default :mod:`multiprocessing` start method will change to a safer one on
  Linux, BSDs, and other non-macOS POSIX where ``'fork'`` is currently the
  default per :gh:`84559`.  Adding a runtime warning about this was deemed too
  disruptive as the majority of code is expected not to care.  Use the
