...
* Virtual opcodes are now exposed in :mod:`dis`. :data:`~dis.HAVE_ARGUMENT`
  is still relevant to real opcodes, but it is not useful for virtual opcodes.
  It is renamed to :data:`~dis._HAVE_ARGUMENT`, and you should use the
  new :data:`~dis.hasarg` collection instead.
