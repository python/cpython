...
* The :c:type:`PyFrameObject` structure members have been removed from the
  ...
  * ``f_state``: use `c:func:`PyFrame_GetState`.
  ...
  * ``f_valuestack``: removed.
  ...
  Tools that need to distinguish between ``yield`` and ``return`` or
  whether a "call" event is starting or resuming a generator should call
  :c:func:`PyFrame_GetState`, rather than trying to infer the state from
