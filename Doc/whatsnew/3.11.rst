...
* Python 3.11 is between 10-60% faster than Python 3.10.
  ...
  See :ref:`whatsnew311-faster-cpython` for more details.
...
Previous versions of the interpreter would just point to the line, making it
...
The built-in types :exc:`ExceptionGroup` and :exc:`BaseExceptionGroup`
...
(Contributed by Irit Katriel in :issue:`45292`. PEP is written by
...
PEP is written by Zac Hatfield-Dodds.)
...
will select the "best" distribution with the tag ``3.11``.

When using the legacy ``-<major>``, ``-<major>.<minor>``,
``-<major>-<bitness>`` or ``-<major>.<minor>-<bitness>`` arguments,
all existing behavior should be preserved from past versions,
...
:pep:`484` previously introduced:data:`~typing.TypeVar`, enabling the creation
of generics parameterized with a single type. :pep:`646` adds
...
parameterized with the array *shape*. Static type checkers can now catch shape-related bugs in code that uses these libraries.

See pep:`646` for more details.

(Contributed by Matthew Rahtz in issue:`43224`, with contributions by
Serhiy Storchaka and Jelle Zijlstra. PEP is written by Mark Mendoza, Matthew
...
straightforward way to mark whether individual items in a:class:`~typing.TypedDict` must be present. Previously, this was only possible
using inheritance.
...
(Contributed by James Hilton-Balfe in :issue:`46534`. PEP is written by
...
(Contributed by Jelle Zijlstra in :issue:`47088`. PEP is written by Pradeep
...
(Contributed by Jelle Zijlstra in :gh:`91860`. PEP is written by
...
* Added:meth:`object.__getstate__`, which provides the default
  ...
  and :mod:`pickle`\ing instances of subclasses of builtin types:class:`bytearray`, :class:`set`, :class:`frozenset`,
...
* Added a :option:`-P` command line option
  ...
  which disables the automatic prepending to :data:`sys.path`
...
* The output of the :option:`--help` option now fits in 50 lines/80 columns.
  ...
  and :option:`-X` options are now available using the respective
...
* Converting between :class:`int` and :class:`str` in bases other than 2
  ...
  This limit can be configured or disabled by an environment variable, command
