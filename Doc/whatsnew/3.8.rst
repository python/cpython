...
like f-strings or conditional expressions, it is easy to go over-board
with code that is challenging to decipher::

  # Cryptic assignment expression embedded in a conditional expression
  binomial_coefficient = [(c := c * (n - k + 1) // k if k else 1) for k in range(n+1)]

  # Visually confusing assignment expression embedded in an f-string
