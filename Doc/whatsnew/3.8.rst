...
* In order to support the :ref:`vectorcall protocol <vectorcall>`,
  the type slot ``tp_print`` is replaced by
  :c:member:`~PyTypeObject.tp_vectorcall_offset`
  and a new slot :c:member:`~PyTypeObject.tp_vectorcall` is added.
  Two new flags :const:`Py_TPFLAGS_METHOD_DESCRIPTOR`
  and :const:`_Py_TPFLAGS_HAVE_VECTORCALL` are added.
