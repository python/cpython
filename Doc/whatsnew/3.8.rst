...
* The new function :c:func:`!PyCode_NewWithPosOnlyArgs` allows to create
  ...
  parameter for indicating the number of positional-only parameters.
...
* :class:`types.CodeType` has a new parameter in the second position of the
  constructor (*posonlyargcount*) to support positional-only parameters defined
  in :pep:`570`. The first argument (*argcount*) now represents the total
  number of positional parameters (including positional-only parameters). The new
