...
For the vast mojority of cases, there should be no side effect. However, this
might cause some types to become immortal. To correctly port these types into
3.8, please apply the following changes:

* Remove any manual incref on the itself after allocating an instance. This
  usually happens after calling :c:func:`PyObject_New`,
