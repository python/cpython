...
* Special methods ``__divmod__`` and ``__rdivmod__`` should now return a tuple
  of size 2.  :func:`divmod` and :c:func`PyNumber_Divmod` will raise now an
  error if the result is not a 2-tuple.
