...
* The three-argument form of the :func:`pow` function now permits the second
  argument (the exponent) to be negative in the case where the first argument
  is relatively prime to the modulus. It then computes a modular
  inverse to the first argument if the exponent is ``-1``, and a suitable
  power of that inverse for other negative exponents.
