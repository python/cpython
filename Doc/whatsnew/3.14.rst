...
* :class:`types.UnionType` and :class:`typing.Union` are now aliases for each other,
  ...
  - Unions created using the old syntax are no longer cached. Previously, running
    ...
    unions, but unions used in annotations are no longer evaluated by default in Python 3.14
    (because of :pep:`649`), the new union object is itself much smaller, and removing the
    cache also saves some space, so it is unlikely that this change will cause a significant
