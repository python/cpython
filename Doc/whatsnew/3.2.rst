...
* Whenever a two-digit year is used in a time tuple, the interpretation has been
  ...
  :attr:`time.accept2dyear` is a falsey value, the :func:`time.asctime` function will
...
* The :attr:`sqlite3.Connection.in_transit` attribute is a truthy value if there is an
...
* JSON decoding performance is improved and memory consumption is reduced
  ...
  now uses the C speedups when the ``sort_keys`` argument is a truthy value.
