...
* Future versions of Python will support nested sets and set operations in
  regular expressions, but this can break backward compatibility since that
  syntax can be treated differently in older versions of Python.  For avoiding
  a breakage a :exc:`FutureWarning` will be raised in the meanwhile in
  ambiguous cases.  That incluse sets starting with a literal ``'['`` or
  containing literal character sequences ``'--'``, ``'&&'``, ``'~~'``, and
  ``'||'``.  For avoiding a warning escape them with a backslash.
