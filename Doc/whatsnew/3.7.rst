...
* Function :c:func:`PySlice_GetIndicesEx` is considered not safe for
  ...
  implement the :meth:`__index__` method, the sequence can be resized
  during executing the :meth:`!__index__` method, and resulting values can
  be computed with wrong sequence length.  For avoiding possible issues
  use new functions :c:func:`PySlice_Unpack` and
