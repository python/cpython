...
they return nanoseconds as integers.
...
width when read an audio file in modules :mod:`aifc`, :mod:`wave` and
:mod:`sunau`.  (Contributed by Serhiy Storchaka in :issue:`32056`.)
...
:func:`asyncio.get_event_loop`, which will *create* a new event loop if none
is running.
...
(Contributed by Srinivas Reddy Thatiparthy in :issue:`32418`.)
...
can be used to decouple server instantiaion and serving.  The new
...
TCP sockets on Linux are now created with ``TCP_NODELAY`` flag set by default.
...
The :meth:`loop.create_unix_connection() <asyncio.AbstractEventLoop.create_unix_connection>`
...
:func:`socket.socketpair` should be used instead.
(Contributed by Victor Stinner in :issue:`32154`.)
...
The :meth:`ReadTransport.resume_reading() <asyncio.ReadTransport.resume_reading>`
...
.. _whatsnew37-asyncio-optimizations:
...
The :func:`~atexit.atexit` callbacks are now correctly tracked and called for
subinterpreters.
(Contributed by Marcel Plch in :issue:`31901`.)

The traceback for :exc:`SystemExit` raised in an ``atexit`` callback
is no longer displayed.
(Contributed by Serhiy Storchaka in :issue:`28994`.)


bdb
---

Pdb and other debuggers dependent on :mod:`bdb` will correctly step over
(next command) native coroutines.
(Contributed by Pablo Galindo. in :issue:`32650`.)
...
The :func:`~calendar.weekday` function now accepts any value for its *year*
argument (previously only values from ``1`` to ``9999`` were accepted).
(Contributed by Mark Gollahon in :issue:`28281`.)
...
The :func:`collections.namedtuple` factory has been reimplemented to rely
on precomputed methods to substantially reduce the use of :func:`exec`.
As a result, the *verbose* parameter and the *_source* attribute of the
named tuple are not longer supported.  The benefits include:
1) having a smaller memory footprint for applications using multiple named
tuples, 2) faster creation of the named tuple class (approx 4x to 6x depending
on how it is measured), and 3) minor speed-ups for instance creation.
...
``executor.submit()`` now raises a :exc:`RuntimeError` if called during
...
:meth:`ConfigParser.read() <configparser.ConfigParser.read>` method now
accepts bytes objects.
(Contributed by Vincent Michel. in :issue:`31307`.)
...
The new :func:`~contextlib.asynccontextmanager` and
:class:`~contextlib.AbstractAsyncContextManager` have been added to
complement their synchronous counterparts.  (Contributed
by Jelle Zijlstra in :issue:`29679` and :issue:`30241`.)

The new :class:`contextlib.AsyncExitStack` class has been added as an
asynchronous equivalent to :class:`contextlib.ExitStack`.  (Contributed by
...
Added support for the Blowfish hashing method.
...
(Contributed by Licht Takeuchi. in :issue:`32255`.)
...
(Contributed by Serhiy Storchaka in :issue:`32922`.)


decimal
-------

The :mod:`decimal` now uses :ref:`context variables <whatsnew37-pep567>` to
store the decimal context.
...
coroutine objects.
(Contributed by George Collins based on diagnosis by Luciano Ramalho
in :issue:`31183`.)
...
(Contributed by Guilherme Polo and Cheryl Sabella in :issue:`1612262`.)
...
The :func:`locale.format` function is deprecated.
...
The :mod:`macpath` is now deprecated and will be removed in Python
3.8.
...
callbacks to be executed on a process fork.
...
The new :func:`socket.close` function closes a passed socket file descriptor.
...
(Contributed by Victor Stinner in :issue:`28240`.)
...
:class:`~types.MethodWrapperType`, and :class:`~types.MethodDescriptorType`
classes are now available.
(Contributed by Manuel Krebber and Guido van Rossum in :issue:`29377`.)

The new :class:`~types.ClassMethodDescriptorType` is used to describe
class and static methods of builtin types.
(Contributed by Serhiy Storchaka in :issue:`32265`.)
...
``"default::DeprecationWarning:__main__"`` entry, so deprecation warnings
are once again shown by default in single-file scripts and at the
interactive prompt.  See :ref:`whatsnew37-pep565` for details.
...
now adjusted to behave like ``str[start:end]``.
...
Various optimizations have reduced Python startup time by 5% on Linux and
up to 30% on macOS.
(Contributed by Victor Stinner, INADA Naoki in :issue:`29585`.)
...
used functions.
See :ref:`optimizations in asyncio <whatsnew37-asyncio-optimizations>` for
details.
...
Fast implementation from standard C library is now used for the
:func:`math.erf` and :func:`math.erfc` functions.
...
* Trace hooks may now opt out of receiving the ``line`` and ``opcode`` events
  from the interpreter by setting the corresponding new ``f_trace_lines`` and
  ``f_trace_opcodes`` attributes to :const:`False` on the frame
...
* :func:`os.listdir` and :func:`os.scandir` now emit file name as bytes when
  called with a bytes-like argument.
  (Contributed by Serhiy Storchaka in :issue:`30879`.)
