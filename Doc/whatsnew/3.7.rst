...
* OpenSSL 0.9.8 and 1.0.1 are no longer supported, which means building Python
  3.7 on platforms using these versions will require custom build options that
  link to a more recent OpenSSL. This affects projects testing on Ubuntu 14.04
  (the "dist: trusty" build environment on Travis CI), where the simplest
  resolution may be to update to using Ubuntu 16.04 (the "dist: xenial" build
  environment). For cases where that isn't an option, see
  :source:`Tools/ssl/multissltests.py` for examples of building custom
  versions of SSL/TLS libraries, and linking Python against them.
