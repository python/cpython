...
* Added Thread Specific Storage (TSS) API which would supersede use of the
  existing Thread Local Storage (TLS) API within the CPython interpreter, while
  deprecating the existing API.  The change is motivated primarily by the fact
  that the old API uses int to represent TLS keys across all platforms, which
  is neither POSIX-compliant, nor portable in any practical sense.  For more
  information, see :pep:`539`.  (Contributed by Erik M. Bray and Masayuki
  Yamamoto in :issue:`25658`.)
