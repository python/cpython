...
* The :mod:`logging` module's :class:`FileHandler` class
  ...
  is a truthy value, opening of the log file is deferred until the first
  :meth:`emit` call is made.  (Contributed by Vinay Sajip.)

  :class:`TimedRotatingFileHandler` also has a *utc* constructor
  parameter.  If the argument is a truthy value, UTC time will be used
...
* The :func:`os.walk` function now has a ``followlinks`` parameter. If
  ...
  parameter's default value is a falsey value.  Note that the function can fall
...
* A new variable in the :mod:`sys` module, :attr:`float_info`, is an
  ...
  If this variable is a truthy value, the compiled files are not written.  The
  ...
  attribute is a truthy value if Python
  was executed in verbose mode, :attr:`debug` is a truthy value in debugging mode, etc.
