...
* Add *recurse_symlinks* keyword-only parameter to
...
* Add the *follow_symlinks* keyword-only parameter to
...
* :meth:`!wm_attributes` can now return attributes as a :class:`dict`,
  by using the new optional keyword argument *return_python_dict*.
  (Contributed by Serhiy Storchaka in :gh:`43457`.)

* :meth:`!Text.count` can now return a simple :class:`int`
  when the new optional keyword argument *return_ints* is used.
