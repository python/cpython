...
* Guard elimination -- through a combination of constant and type information,
  ...
  As a proof of concept, we managed to eliminate over 70%
  of integer type checks in our own benchmarks.
...
The second is machine code. When the ``--enable-experimental-jit``
option is used, the optimized Tier 2 IR is translated to machine
...
directly inspired by the paper "Copy-and-Patch Compilation"
