...
* When :func:`.TaskGroup.create_task` is called on an inactive
...
* The :class:`~io.IOBase` finalizer now logs any errors in
...
* On Windows, :func:`~os.path.isabs` no longer considers paths
  starting with exactly one (back)slash (``\`` or ``/``) to be absolute.
...
* The :mod:`subprocess` module now uses the :func:`~os.posix_spawn` function in
  ...
  On Linux, this should perform similar to the existing
