  ...
  .. code-block:: shell-session
    ...
    Traceback (most recent call last):
      ...
      File "/home/random.py", line 1, in <module>
        import random; print(random.randint(5))
                            ^^^^^^^^^^^^^^
    AttributeError: module 'random' has no attribute 'randint' (consider renaming '/home/random.py' since it has the same name as the standard library module named 'random')

  Similarly, if a script has the same name as a third party
  module it attempts to import, we also display a more helpful
  error message:
