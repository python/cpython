...
``end_offset`` attributes.  The intepreter now highlights the
full error range of the expression that consistutes the syntax error itself,
instead of just where the problem is detected. In this way,
instead of displaying (before Python 3.10):
...
Python 3.10 now displays the exception as:
