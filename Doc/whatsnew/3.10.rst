...
a data object (sequences, mappings, primitive data types as well as class
instances) and *case statements* of patterns with associated actions. Pattern
matching enables programs to extract information from complex data types,
...
operates by::
    ...
    1. taking a data object ``subject`` with type and shape
    2. substituting the ``subject`` into a ``match`` statement
    3. comparing the match subject with each pattern in a ``case`` statement
       from top to bottom until a match is confirmed. If an exact match is not
       confirmed, the last case, a wildcard ``_``, will be used as the matching
       case.
    4. executing the actions associated with the pattern of the confirmed
       match
...
Elixir. With structural pattern matching, the approach is "declarative" and
...
true value for Python lies in its handling of data's type and shape.
...
pattern has the ability to capture class attributes into variables::
    ...
    def location(point):
        match point:
            ...
            case Point():
                print("The point is located somewhere else on the graph.")
            case _:
                print("Not a point")
