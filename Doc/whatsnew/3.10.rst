...
* The use of :meth:`~importlib.abc.Loader.module_repr` by the `__repr__` method
  of :class:`types.ModuleType` now triggers an :exc:`ImportWarning`. Current
  plans are to remove usage by the import system of `module_repr()` in Python
  3.12 instead of starting to raise :exc:`DeprecationWarning` as its use is
  already optional.
