...
Add *pipesize* and *exec_raise* parameters to the :class:`subprocess.Popen`
constructor:

* *pipesize* can be used to change the size of the pipe when :data:`PIPE` is
  used for *stdin*, *stdout* or *stderr*.
  (Contributed by Ruben Vorderman in :issue:`41586`.)
...
* If *exec_raise* is false, :exc:`OSError` exception is catched when creating
  ...
  exception ``errno`` attribute.
  (Contributed by Victor Stinner in :issue:`42648`.)
