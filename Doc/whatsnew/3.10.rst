...
* The import system now uses the ``__spec__`` attribute on modules over
  :meth:`~importlib.abc.Loader.module_repr` by the ``__repr__()`` method.
  Removal of the use of ``module_repr()`` method is scheduled for Python 3.12.
