...
whether a Boolean value is a truthy value or false.  A conditional expression lets you
...
when the condition is a falsey value.
...
returning the default value of ``None``.  ``None`` is a falsey value, so the exception
...
* Two new built-in functions, :func:`any` and :func:`all`, evaluate whether an
  ...
  if any value returned by the iterator is a truthy value; otherwise it will return
...
* A new warning, :class:`UnicodeWarning`, is triggered when  you attempt to
  ...
  using the default ASCII encoding.   The result of the comparison is a falsey value::
